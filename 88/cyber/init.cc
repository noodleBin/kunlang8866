...
using century::cyber::scheduler::Scheduler;
...
void InitLogger(const char* binary_name) {
  ...
  if (slash) {
    ::century::cyber::binary::SetName(slash + 1);
  } else {
    ::century::cyber::binary::SetName(binary_name);
  }
  ...
  // Init async logger
  async_logger = new ::century::cyber::logger::AsyncLogger(
...
bool Init(const char* binary_name) {
  ...
  if (global_data->IsMockTimeMode()) {
    ...
    auto cb =
        [](const std::shared_ptr<const century::cyber::proto::Clock>& msg) {
          if (msg->has_clock()) {
            Clock::Instance()->SetNow(Time(msg->clock()));
          }
        };
    clock_node->CreateReader<century::cyber::proto::Clock>(kClockChannel, cb);
  }
  return true;
}
