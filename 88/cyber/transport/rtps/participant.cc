...
#include "cyber/transport/rtps/participant_config.h"
...
void Participant::CreateFastRtpsParticipant(
  ...
  try {
    domain_id = GetRtpsDomainId(global_conf);
  ...
  RETURN_IF(!locator.set_IP4_address(GetRtpsMulticastAddress(global_conf)));
