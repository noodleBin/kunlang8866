...
static constexpr char kDefaultSchedName[] = "CYBER_DEFAULT";

using ::century::cyber::base::AtomicHashMap;
using ::century::cyber::proto::ClockMode;
using ::century::cyber::proto::CyberConfig;
using ::century::cyber::proto::RunMode;
enum SimulationState {
  NO_SIMULATION = 0,
  RECORDER_PLAYBACK_WITH_SIM_CONTROL = 1,
  WITH_CARLA_CO_SIMULATION = 2,
};
class GlobalData {
 public:
  ~GlobalData();
  ...
  void SetSimulationState(const SimulationState simulation_state);
 ...
 private:
  ...
  // sched policy info
  std::string sched_name_ = kDefaultSchedName;
