...
void CyberTopologyMessage::TopologyChanged(
  ...
  if (::century::cyber::proto::OperateType::OPT_JOIN ==
      changeMsg.operate_type()) {
    bool isWriter = true;
    if (::century::cyber::proto::RoleType::ROLE_READER == changeMsg.role_type())
      isWriter = false;
    ...
    if (iter != all_channels_map_.cend() &&
      ...
      if (::century::cyber::proto::RoleType::ROLE_WRITER ==
          changeMsg.role_type()) {
...
void CyberTopologyMessage::AddReaderWriter(
    const century::cyber::proto::RoleAttributes& role, bool isWriter) {
  ...
  if (!GeneralChannelMessage::IsErrorCode(channel_msg)) {
    if (isWriter) {
      if (msgTypeName != century::cyber::message::MessageType<
