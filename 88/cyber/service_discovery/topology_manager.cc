...
#include "cyber/transport/rtps/participant_ports.h"
...
bool TopologyManager::CreateParticipant() {
  ...
  const int port = transport::GetTopologyParticipantPort(
      common::GlobalData::Instance()->Config());
  participant_ = std::make_shared<transport::Participant>(
      participant_name, port, participant_listener_);
  return true;
}
