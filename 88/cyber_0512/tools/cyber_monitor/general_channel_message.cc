...
using century::cyber::record::kGB;
using century::cyber::record::kKB;
using century::cyber::record::kMB;
}  // namespace
...
double GeneralChannelMessage::frame_ratio(void) {
  ...
  auto time_now = century::cyber::Time::MonoTime();
...
GeneralChannelMessage* GeneralChannelMessage::OpenChannel(
  ...
  channel_node_ = century::cyber::CreateNode(node_name_);
  ...
  channel_reader_ =
      channel_node_->CreateReader<century::cyber::message::RawMessage>(
...
void GeneralChannelMessage::RenderDebugString(const Screen* s, int key,
  ...
  if (has_message_come()) {
    if (raw_msg_class_ == nullptr) {
      auto rawFactory = century::cyber::message::ProtobufFactory::Instance();
