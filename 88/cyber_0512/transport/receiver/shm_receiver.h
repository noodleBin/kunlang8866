...
class ShmReceiver : public Receiver<M>,
                    public std::enable_shared_from_this<ShmReceiver<M>> {
 public:
...
void ShmReceiver<M>::Enable() {
  ...
  std::weak_ptr<ShmReceiver<M>> weak_self = this->shared_from_this();
  dispatcher_->AddListener<M>(
      this->attr_, [weak_self](const std::shared_ptr<M>& msg,
                               const MessageInfo& msg_info) {
        auto self = weak_self.lock();
        if (!self) {
          return;
        }
        self->OnNewMessage(msg, msg_info);
      });
  this->enabled_ = true;
}
...
void ShmReceiver<M>::Enable(const RoleAttributes& opposite_attr) {
  ...
  dispatcher_->AddListener<M>(
      ...
      [weak_self](const std::shared_ptr<M>& msg,
                  const MessageInfo& msg_info) {
        auto self = weak_self.lock();
        if (!self) {
          return;
