...
bool Reader<MessageT>::Init() {
  ...
  if (reader_func_ != nullptr) {
    ...
    if (common::CallbackTraceEnabled()) {
      callback_trace_detail =
          std::string("received_msg=") +
          common::DemangleTypeName(message::MessageType<MessageT>().c_str());
    }
    func = [this, callback_trace_detail](const std::shared_ptr<MessageT>& msg) {
      this->Enqueue(msg);
      if (!common::CallbackTraceShouldTrace(
              "reader", this->role_attr_.channel_name())) {
        this->reader_func_(msg);
        return;
      }
      common::CallbackTrace trace(
          "reader", this->role_attr_.channel_name().c_str(),
          callback_trace_detail.c_str());
      this->reader_func_(msg);
    };
  } else {
  ...
  if (receiver_->attributes().has_socket_addr()) {
    this->role_attr_.mutable_socket_addr()->CopyFrom(
        receiver_->attributes().socket_addr());
  }
  channel_manager_ =
