...
```bash
 rostopic echo /century/sensor/velodyne64/VelodyneScanUnified
```
...
```bash
# add write permission to output path(e.g. /century/modules/calibration/data/mkz8)
sudo chmod a+w /century/modules/calibration/data/mkz8 -R
```
...
```bash
tee: /century/data/log/***.out: permission denied
```

Enter the following command to add `write` permission:

```bash
sudo chmod a+w /century/data/log
```
