...
Century is built within Century development Docker container following
...
- Make sure you're using `<century_host_ip>:8888` instead of `localhost:8888` if
  you are accessing the Dreamview UI through another host.

---

## Dreamview does not open up if I install more than 1 version of Century?
...
library. After that you need to run a `./century.sh build_fe` to compile.
...
**Century 2.5 or after**: simply turning on the SimControl switch as seen in the
image below:

![](images/sim_control_2.5.png)

**Century 2.0 or older**: you would need to enable the sim control manually,
...
3. Restart century using our bootstrap script
...
Dreamland is Century's web-based simulation platform. Based on an enormous amount
of driving scenario data and large-scale cloud computing capacity, Century
...
[our Simulation website](http://century.auto/platform/simulation.html) and join
now!
