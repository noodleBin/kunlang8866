...
Inside the proto data definition, the planning output includes the total planned time and length, as well as the actual trajectory for control to execute, and is defined in `repeated century.common.TrajectoryPointtrajectory_point`.  
...
**EM Planner** does the actual planning and derives from the **Planner** class. Both the Em Planner that is used in the Century 2.0 release, and the previously released **RTK Planner** derive from the Planner class.
