...
Although other Linux distributions may be OK, we have only tested Century on
...
The Century runtime in the vehicle requires NVIDIA GPU Driver.
...
Century 6.0+ requires Docker 19.03+ to work properly. Just follow the
...
There is also a
[dedicated bash script](../../docker/setup_host/install_docker.sh) Century
provides to ease Docker installation, which works both for X86_64 and AArch64
platforms.

## Installing NVIDIA Container Toolkit

The NVIDIA Container Toolkit for Docker is required to run Century's CUDA based
Docker images.
...
"Git Clone Century Repo" section now.
