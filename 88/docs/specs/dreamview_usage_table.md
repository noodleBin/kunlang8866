...
Depending on the mode chosen from the mode selector, the corresponding modules and commands, defined in [hmi.conf](https://github.com/CenturyAuto/century/blob/master/modules/dreamview/conf/hmi.conf), will be presented in the **Module Controller**, and **Quick Start**, respectively.

Note: navigation mode is for the purpose of the low-cost feature introduced in Century 2.5. Under this mode, Baidu (or Google) Map presents the absolute position of the ego-vehicle, while the main view has all objects and map elements presented in relative positions to the ego-vehicle.
...
* **Console**: monitor messages from the Century platform
### Module Controller
...
### Data Recorder
A panel to report issues to drive event topic ("/century/drive_event") to rosbag.
...
To view the monitor:
1. Build Century and run Dreamview on your web browser
