...
After finding a path in QP-Spline-Path, Century converts all obstacles on the path and the ADV (autonomous driving vehicle) into an path-time (S-T) graph, which represents that the station changes over time along the path. The speed optimization task is to find a path on the S-T graph that is collision-free and comfortable. 

Century uses splines to represent speed profiles, which are lists of S-T points in S-T graph. Century leverages Quadratic programming to find the best profile. The standard form of QP problem is defined as: 
```
$$
...
Century first defines $cost_1$ to make the trajectory smooth: 
```
$$
...
Then Century defines $cost_2$ as the difference between the final S-T trajectory and the cruise S-T trajectory (with given speed limits — m points):
```
$$
cost_2 = \sum_{i=1}^{n}\sum_{j=1}^{m}\Big(f_i(t_j)- s_j\Big)^2
$$
```
Similarly, Century defines $cost_3$ that is the difference between the first S-T path and the follow S-T path (o points):
```
$$
...
Given the assumption that the first point is ($t0$, $s0$), and $s0$ is on the planned path $f_i(t)$, $f'i(t)$, and $f_i(t)''$ (position, velocity, acceleration).  Century converts those constraint into QP equality constraints:
```
$$
A_{eq}x = b_{eq}
$$
...
This constraint is designed to smooth the spline joint.  Given the assumption that two segments, $seg_k$ and $seg_{k+1}$, are connected, and the accumulated **s** of segment $seg_k$ is $s_k$,  Century calculates the constraint equation as: 
```
$$
f_k(t_k) = f_{k+1} (t_0)
$$
...
$$
```
Century first finds the lower boundary $l_{lb,j}$ at those points ($s_j$, $l_j$) and  $j\in[0, m]$ based on the road width and surrounding obstacles. Then it calculates the inequality constraints as:
```
$$
\begin{vmatrix} 
...
Similarly, for upper boundary $l_{ub,j}$, Century calculates the inequality constraints as: 
```
$$
\begin{vmatrix} 
 1 & t_0 & t_0^2 & t_0^3 & t_0^4&t_0^5 \\
...
Century establishes a speed limit boundary as well.
