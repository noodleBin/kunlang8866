## Century传感器单元（ASU）安装指南

Century传感器单元（ASU）被设计为和工业级PC（IPC）协同工作以实现在Century自动驾驶平台上的传感器数据融合、车辆控制和网络访问。
...
Century传感器单元（ASU）目前只提供给Century合作伙伴和特定的开发者。关于使用和获取ASU的问题可通过Century官方渠道获得更多信息。
