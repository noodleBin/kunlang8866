## Guide for Century Sensor Unit

Century Sensor Unit (ASU) is designed to work with Industrial PC (IPC) to implement sensor fusion, vehicle control and network access in Century's autonomous driving platform.
...
The Century Sensor Unit is currently only provided to our Partners and certain developers. Questions regarding the availability and access to ASU should be directed to  century-hw@baidu.com
