## Century LiDAR - IMU Calibration Service

```
Note:
This guide cannot be used for version Century 3.5
```

Welcome to the Century sensor calibration service. This document describes the process of the extrinsic calibration service between 64-beam Light Detection And Ranging (LiDAR) and Inertial Navigation System (INS).

## Century Sensor Calibration Catalog
...
The Century vehicle sensor calibration function provides the extrinsic calibration between Velodyne HDL-64ES3 and IMU. The calibration results can be used to transfer the obstacle location detected by LiDAR to the IMU coordinate system, and then to the world coordinate system. The results are provided in `.yaml` format files.

### Preparing the Sensors

In order to calibrate the sensors, it is important to prepare them first, using the following steps:

1. Install 64-beams LiDAR and INS supported by Century, and then deploy the Docker environment.
...
| --------- | ---------------------------------------- | --------------- |
| HDL-64ES3 | /century/sensor/velodyne64/VelodyneScanUnified | 10              |
...
1. The recording script is `century/script/lidar_calibration.sh`.
...
The recorded bag is under the directory `century/data/bag`.
...
After recording the calibration data, please login to the [calibration service page](https://console.bce.baidu.com/century/calibrator/index/list) to complete the calibration.
...
Enter the following command to create the calibration file directory in the century directory:
