...
This document provides the details about how traffic light perception functions in Century 2.0.
...
Century has determined that using a single camera, which has a constant field of vision, cannot see traffic lights everywhere. This limitation is due to the following factors:

- The perception range should be above 100 meters
- The height of the traffic lights or the width of crossing varies widely

Consequently, Century 2.0 uses two cameras to enlarge its perception range:
...
The module decides which camera to use adaptively based on the light projection. Although there are only two cameras on the Century car, the algorithm can handle multiple cameras.
...
This section describes the input and the output of the Pre-process module. The input is obtained by subscribing to topic names from Century or directly reading them from locally stored files, and the output is fed to the successive Process module.

#### Input

- Images from different cameras, acquired by subscribing to the topic name:

    - `/century/sensor/camera/traffic/image_long`
    - `/century/sensor/camera/traffic/image_short`
...
Century needs to select the proper lights according to the detection score, and the input lights' position and shape. If the CNN network cannot find any lights in the ROI, the status from the input lights is marked as unknown and the two remaining steps (Recognizer and Reviser) are skipped.
