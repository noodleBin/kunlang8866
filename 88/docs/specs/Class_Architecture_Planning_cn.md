...
在proto数据的定义中，输出数据包括总时间、总长度和确切的路径信息，输出数据由控制单元解析执行，输出数据结构定义在`repeated century.common.TrajectoryPointtrajectory_point`。
...
**EM Planne**r执行具体的Planning任务，继承自**Planner**类。Century2.0中的**EM Planner**类和之前发布的**RTK Planner**类都继承自Planner类。
