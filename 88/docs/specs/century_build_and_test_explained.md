# Century Build and Test Explained

This document focuses on how to build and test Century in different ways.

## Pre-requisites

You are supposed to have followed
[Century Software Installation Guide](../quickstart/century_software_installation_guide.md).
Make sure you are in Century development Docker container before you proceed.
...
about Bazel, the better you know about Century Build.

## Building Century

### Subcommands in `century.sh`

Let's first take a look at the usage of `century.sh`.

![century.sh help](images/century_sh_help.jpg)
...
are all variants for their basic form: `century.sh build`.

### Building Century using `century.sh build`

You can use `./century.sh build` to build individual modules, or the whole Century
project.
...
![tip_icon](images/tip_icon.png) For simplicty, Century 6.0 introduces the notion
of `bash century.sh build [module]` to replace the notion of `build_cyber`,
`build_planning`, etc in previous Century releases.

### Variants of `century.sh build`

There are mainly four variants of `century.sh build`, namely, `build_dbg`,
...
When you type `bash century.sh build_dbg planning`, you are running
...
```bash
bash century.sh build_opt cyber
```
...
- `USE_GPU_HOST` is an environment variable determined by
  `docker/scripts/dev_start.sh` to pass to Century Docker container, which
...
- `USE_GPU_TARGET` is an environment variable determined by
  `scripts/century.bashrc` inside Docker container to indicate whether the
  container is GPU capable.
- `USE_GPU` is a variable indicating whether to perform CPU build or GPU build.

When you type `bash century.sh build --config=cpu` or
`century.sh build --config=gpu`, the
[build script](../../scripts/century_build.sh)
...
Century container, whereas running GPU build in a CPU-only container will fail.
...
## Running Unit Tests: `century.sh test`
...
`century.sh lint` command.

## Running Code Coverage using `century.sh coverage`
...
all the discussions about `century.sh test` applies to `century.sh coverage`.

## Build/Test/Coverage: An Insider's View

Under the hood, `century.sh build/test/coverage` makes use of
[scripts/century_build.sh](../../scripts/century_build.sh),
[scripts/century_test.sh](../../scripts/century_test.sh),
and
[scripts/century_coverage.sh](../../scripts/century_coverage.sh)
respectively.
