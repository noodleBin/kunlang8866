# Century Cyber RT Developer Tools

Century Cyber RT framework comes with a collection of useful tools for daily development, including one visualization tool cyber_visualizer and two command line tools cyber_monitor and cyber_recorder.

*Note: century docker environment is required to use the tools, please follow century wiki to enter docker*

All the tools from Century Cyber RT rely on Century Cyber RT library, so you must source the setup.bash file for environment setup before using any Century Cyber RT tools, shown as below:

```bash
username@computername:~$: source /your-path-to-century-install-dir/cyber/setup.bash
```
...
`cyber_visualizer` is a visualization tool for displaying the channel data in Century Cyber RT.
...
- When data flow through channels in Century Cyber RT, the list of all channels are displayed under `ChannelNames` as seen in the figure below. For example, you can use the record tool(cyber_recorder) of Century Cyber RT to replay data from another terminal, then `cyber_visualizer` will receive information of all active channels(from replay data) and display it.
...
The command line tool `cyber_monitor` provides a clear view of the list of real time channel information Century Cyber RT in the terminal.
...
`cyber_recorder` is a record/playback tool provided by Century Cyber RT. It provides many useful functions, including recording a record file, playing back a record file, splitting a record file, checking the information of record file and etc.
...
```bash
$ source /your-path-to-century-install-dir/cyber/setup.bash
$ cyber_recorder
...
channel_number: 16
channel_info:   /century/localization/pose                             2000 messages : century.localization.LocalizationEstimate
                /tf                                                   4000 messages : century.transform.TransformStampeds
                /century/control                                       2000 messages : century.control.ControlCommand
                /century/sensor/gnss/odometry                          2000 messages : century.localization.Gps
                /century/canbus/chassis                                2000 messages : century.canbus.Chassis
                /century/sensor/gnss/imu                               1999 messages : century.drivers.gnss.Imu
                /century/sensor/gnss/rtk_obs                             41 messages : century.drivers.gnss.EpochObservation
                /century/sensor/gnss/ins_stat                            20 messages : century.drivers.gnss.InsStat
                /century/sensor/gnss/best_pose                           20 messages : century.drivers.gnss.GnssBestPose
                /century/perception/obstacles                           400 messages : century.perception.PerceptionObstacles
                /century/prediction                                     400 messages : century.prediction.PredictionObstacles
                /century/sensor/conti_radar                             270 messages : century.drivers.ContiRadar
                /century/planning                                       200 messages : century.planning.ADCTrajectory
                /century/monitor/static_info                              1 messages : century.data.StaticInfo
                /century/sensor/gnss/rtk_eph                             25 messages : century.drivers.gnss.GnssEphemeris
                /century/monitor                                          3 messages : century.common.monitor.MonitorMessage
...
`rosbag_to_record` is a tool which can convert rosbag to recorder file provided by Century Cyber RT. Now the tool support following channel:
...
We can convert [Century2.5 demo bag](https://github.com/CenturyAuto/century/releases/download/v2.5.0/demo_2.5.bag) to record file.
