...
- 控制评测需要检查的数据至少包含以下channel信息，才能正确的进行评测：
    ...
    | 模块 | channel名称 | 检查项目 |
    |---|---|---|
    | Canbus | `/century/canbus/chassis` | 确保能正常输出底盘数据 |
    | Control | `/century/control` | 确保能正常输出控制信息数据，该数据需要启动自动驾驶后才能获得 |
    | Planning | `/century/planning` | 确保能正常输出规划信息 |
    | Localization | `/century/localization/pose` | 确保能正常输出位置数据 |
    | GPS | `century/sensor/gnss/best_pose` | 确保`sol_type`为`NARROW_INT` |
...
5） `Configuration File`即为`vehicle_param.pb.txt`（车辆配置文件），该文件在`century/modules/calition/data/dev_kit`文件夹内，将该文件夹下的`vehicle_param.pb.txt`拷贝至BOS对应的车辆控制评测文件夹下，例如上图`Vehicle2`文件夹内；
...
首先进入[Century云服务任务页面](http://bce.century.auto/login)，选择使用百度账号登录，在左侧框中选择`Century Fuel-->任务`，点击`新建任务`，然后在下拉框内选择`控制评测`，然后输入相应要评测的数据路径，在`输入数据路径`中填写到根目录，在本示例中填写为`task001`。如下图所示：
