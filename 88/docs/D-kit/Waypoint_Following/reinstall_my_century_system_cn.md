# 循迹搭建--Century系统重装
- [循迹搭建--Century系统重装](#循迹搭建--Century系统重装)
  - [概览](#概览)
  - [工控机系统重装](#工控机系统重装)
    - [工控机软件系统安装](#工控机软件系统安装)
      - [安装Linux系统](#安装Linux系统)
      ...
      - [安装docker软件](#安装docker软件)
        - [安装docker](#安装docker)
        - [检查docker](#检查docker)
      - [下载Century源代码](#下载Century源代码)
      - [设置Century编译环境](#设置Century编译环境)
      - [编译Century源代码](#编译Century源代码)
...
本手册旨在帮助用户在开发套件上重新安装Ubuntu操作系统、配置软件，并启动使用Century自动驾驶平台。

## 工控机系统安装

在本小结需要完成工控机的软件安装，包括Ubuntu Linux安装、Century软件系统安装等。

### 工控机软件系统安装

![tip_icon](images/tip_icon.png) 在本小结中，工控机软件系统安装包括计算机操作系统的安装，硬件驱动的安装，应用软件的安装和Century软件系统的安装。

#### 安装Linux系统

Century软件系统依赖于Linux操作系统而运行，而Linux操作系统种类繁多，且又分为服务器版本和桌面版本，这里我们选择当下比较流行的Ubuntu桌面操作系统的64位版本。安装Ubuntu Linux的操作系统的步骤如下：
...
![warning_icon](images/warning_icon.png)**WARNING**：在整个Century系统的安装和操作的过程中，全程禁用root账户，皆用普通账户进行操作，切记！
...
开发套件上运行Century需要Century Kernel，请按照如下步骤下载、安装预编译的内核：

a.从[linux-kernel-5.4.115-century-rt](https://century-system.cdn.bcebos.com/release_installer/linux-kernel-5.4.115-century-rt.zip)下载发布的Century Kernel的预编译的安装包linux-kernel-5.4.115-century-rt.zip压缩文件。

b.安装包下载完成后，解压后安装:

```
unzip linux-kernel-5.4.115-century-rt.zip
cd linux-kernel-5.4.115-century-rt
sudo dpkg -i ../*.deb
```
c.安装完成后，使用`reboot`命令重新启动计算机。进入系统后，在终端中输入`uname -r`，若输出“5.4.115-century-rt”字样，则表示此时系统是以century-kernel引导的。 注意：从此以后，每次开机都需要以century-kernel来引导系统。
...
下载century-kernel官网上的脚本[install-nvidia.sh](https://github.com/CenturyAuto/century-kernel/blob/master/linux/install-nvidia.sh)至当前用户的`home`目录下，输入以下命令完成显卡驱动内核模块的安装：
...
在后续启动century的canbus模块时，需要先在docker外运行start.sh脚本。
...
使用century官网上的[install_docker.sh](https://github.com/CenturyAuto/century/blob/v6.0_edu/docker/setup_host/install_docker.sh)来安装docker。工控机在联网情况下在终端中输入以下命令来完成安装：
...
git init
git clone -b r5.5.0 https://gitee.com/CenturyAuto/century.git
```

代码下载的时间视网速的快慢而有所区别，请耐心等待；下载完成后的代码在~/century目录下，若您需要和Century代码保持完全同步，请接着执行以下命令：

```
git remote set-url origin https://github.com/CenturyAuto/century.git
git pull
```
...
```
cd ~
echo "export CENTURY_HOME=$(pwd)" >> ~/.bashrc
source ~/.bashrc
```
...
第一次进入docker时或者image镜像有更新时会自动下载century所需的image镜像文件，下载镜像文件的过程会很长，请耐心等待。这个过程完成后，请输入以下命令以进入docker环境中：
```
bash docker/scripts/dev_into.sh
```

b.编译century，在终端输入以下命令，等待编译完成，整个编译过程大约耗时38分钟：

```
bash century.sh build_opt_gpu
```
...
如果成功在浏览器中看到回放画面，则表明您的century系统已经部署成功！

## NEXT

现在，您已经完成Century系统安装，接下来可以开始[循迹搭建--车辆集成](vehicle_integration_cn.md)

## 常见问题

a.显卡的驱动没有安装或者没有安装成功

请参考显卡的驱动安装的部分重新安装，注意需要在century内核中安装GPU的驱动并且安装后需要重新启动计算机。

b.docker进不去, no matching entries in passwd file

安装好ubuntu Linux，century-kernel，docker后，执行
 ...
 如果没有解决，那么有可能是/century/scripts/docker_adduser.sh没有权限，需要`sudo chmod a+rx /century/scripts/docker_adduser.sh`增加权限。（ 我们曾遇到一种情况就是/century/scripts/目录下有一个docker_adduser.sh~文件，导致报错，所以需要检查是否存在副本文件或者交换文件之类的，需要删除掉 ）。
...
d.`century`代码没有`git remote -v`、`git branch -a`和`git log`等相关的`git`信息。

造成以上问题的原因是用户错误地使用了`git init`等相关的命令而导致的`century`代码的`git`信息丢失。依次输入以下命令即可恢复`century`代码的`git`信息，切记在操作之前备份自己的`century`代码。
```
cd ~/century
git init
git remote add origin https://gitee.com/CenturyAuto/century.git
git clean -fd
