...
- [循迹搭建--车辆循迹演示](#循迹搭建--车辆循迹演示)
  ...
  - [常见问题](#常见问题)
    ...
    - [5. century系统第一次搭建完毕，测试发转角车辆实际转角不对](#5-century系统第一次搭建完毕测试发转角车辆实际转角不对)
...
- 在`/century/modules/calibration/data/`目录下列出了Century支持车型的配置参数。
- 在`/century/modules/calibration/data/`文件夹下，我们为开发套件提供了默认的参数，请务必按照前述**前提条件**的步骤进行配置，否则会导致循迹启动失败。在完成前提条件准备工作后，下面开始启动循迹操作。
...
在浏览器中打开`(http://localhost:8888)`，在`--setup mode--`内，选择模式为`Rtk`， 在`--vehicle--`选择车型，根据车辆铭牌信息选择对应的车型(见下表)，如使用Century D-KIT Lite，则在`--vehicle--`选择车型`Dev Kit`。
...
|channel_name | 检查项目 |
|---|---|
|`/century/canbus/chassis`|确保能正常输出数据|
|`/century/canbus/chassis_detail`|确保能正常输出数据|
|`/century/sensor/gnss/best_pose` | 确保能正常输出数据、`sol_type:` 选项显示为`NARROW_INT`|
|`/century/localization/pose`| 确保能正常输出数据 |
...
- 结束录制循迹轨迹后，录制的循迹数据在`century/data/log/garage.csv`中，文件内包含了车辆的轨迹、速度、加速度、曲率、档位、油门、刹车、转向等信息。
...
- 录制的`Record数据包`存储在`/century/data/bag`目录中，每一次的录制数据包会存储在以时间戳命名的文件夹内，（**注意**：数据包会在`century/data/bag/`文件夹下生成2个`年月日时分秒相似`的文件夹，分别以`年-月-日-时-分-秒`和`年-月-日-时-分-秒_s`命名，开发者可以不用理会`_s`后缀的文件夹，只需要使用`年-月-日-时-分-秒`文件夹内录制的标定数据即可）。
...
硬件连接：确保CAN硬件连接线CAN0和车辆线控底盘连接正常，进入`century docker`环境，输入以下命令：
...
3）如果（2）已经正确，但问题还存在，查看`century/data/log/canbus.INFO`是否有报错信息；

4）如果（3）尝试了，仍然没有解决，用CAN调试工具，看底盘是否有信号上报，以及century执行的时候，是否有命令下发。
...
### 3. Teleop进入不了自动驾驶模式 
 ...
 打开`/century/module/canbus/conf/canbus.conf`：
...
2）查看canbus配置文件`/century/module/canbus/conf/canbus.conf`内是否打开chassis_detail，如没有打开，把`--noenable_chassis_detail_pub`修改为`enable_chassis_detail_pub`，重启`canbus`后生效。

3）是否正确启动CAN驱动，查看启动canbus前是否在`docker`外是否启动`SocketCAN`启动脚本`start.sh`。

4）查看硬件CAN线接口有没有松动。

### 5. century系统第一次搭建完毕，测试发转角车辆实际转角不对

例如下发转向角10%，但是转动角度远超过转向最大角度的10%，可以在cyber_monitor中查看 /century/canbus/chassis及时观察底盘信号，century默认是Lincoln车型，如果使用者第一次搭建好century，还没有选择车型那么默认会按照`mkz_example`车型的最大转向角乘以百分比去执行。解决方法如下：检查modules/canbus/conf/canbus_conf.pb.txt中的配置，brand设置为正确的车型（例如`CH`），重启canbus，再尝试。如何仍然未解决，请运行bootstrap.sh脚本，在dreamview中选择对应的车型，选择`Dev Kit`开发套件，然后再执行前述步骤检查转角是否正确。
