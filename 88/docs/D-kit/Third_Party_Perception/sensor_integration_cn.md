...
### 摄像头的启动
 ...
 - 运行`git clone https://github.com/CenturyAuto/century-contrib.git`命令将century-contrib仓库克隆至本地，然后将century-contrib文件夹下面的smartereye文件夹整体拷贝到/century目录下。
 
 - 启动docker环境，在docker环境中输入以下命令：

 ```
 sudo bash /century/smartereye/install.sh /century/third_party/camera_library/smartereye
 ```

 将smartereye的共享库拷贝至century的对应的位置。删除`/century/smartereye`文件夹。

 - 将`/century/modules/dreamview/conf/hmi_modes/mkz_standard_debug.pb.txt`文件中的第四行的内容`dag_files: "/century/modules/drivers/camera/dag/camera.dag"`修改为`dag_files: "/century/modules/drivers/smartereye/dag/smartereye.dag"`并保存；将`/century/modules/common/data/global_flagfile.txt`文件中的末尾添加下面一行：`--image_short_topic=/century/sensor/smartereye/image/compressed`并保存。

 - 确保century被正确编译。
 
 - 启动DreamView： `bash /century/scripts/bootstrap.sh`，并在浏览器中输入DreamView地址：`http://localhost:8888/`
...
![camera_integration_dreamview2](images/camera_integration_dreamview2.png)
 ...
 - 在正确完成上述步骤后，在`docker`环境内使用`cyber_monitor`工具，查看`/century/sensor/smartereye/image`、`/century/sensor/smartereye/image/compressed`是否有数据输出，且帧率是否稳定在15帧左右。

```
//输入如下命令启动cyber_monitor
budaoshi@in_dev_docker:/century$ cyber_monitor
```