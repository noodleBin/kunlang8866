...
#### 5. 启动Lidar感知
在dreamview中启动`lidar perception`模块，使用`cyber_monitor`查看`/century/perception/obstacles`是否正常输出，并在dreamview上查看障碍物信息：
...
如果在dreamview上能看到障碍物并且`/century/perception/obstacles`有障碍物信息，则开环测试通过。
...
## 常见问题
#### 1. 感知的输出帧率达不到10帧
* 建议使用 `bash century.sh build_opt_gpu` 编译Century工程

#### 2. 感知的障碍物位置与实际障碍物误差较大
* 确认localization信号状态，保证`/century/sensor/gnss/best_pose`中`sol_type` 选项显示为`NARROW_INT`
* 保证各个传感器外参文件准确
