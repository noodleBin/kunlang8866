...
- 定位模块启动后，需要接收定位数据，需要等待约1分钟左右。打开新的终端，并使用`bash docker/scripts/dev_into.sh`命令进入docker环境，在新终端中输入`cyber_monitor`命令查看`tf`、`tf_static`、`/century/localization/pose`数据，这三个数据在cyber_monitor中均显示为绿色代表定位模块启动成功
...
- 单激光雷达用户用户，请检查如下channel是否正常输出
  ...
  |序号 | channel | 帧率 | 
  |---|---|---|
  |  1 | `/century/sensor/lidar16/PointCloud2` | 10Hz |
  |  2 | `/century/sensor/lidar16/Scan` | 10Hz |
  |  3 | `/century/sensor/lidar16/compensator/PointCloud2` | 10Hz |
...
- 三激光雷达用户用户，请检查如下channel是否正常输出
  ...
  |序号 | channel | 帧率 | 
  |---|---|---|
  |  1 | `/century/sensor/lidar16/back/PointCloud2` | 10Hz |
  |  2 | `/century/sensor/lidar16/left/PointCloud2` | 10Hz |
  |  3 | `/century/sensor/lidar16/right/PointCloud2` | 10Hz |
  |  4 | `/century/sensor/lidar16/fusion/PointCloud2` | 10HZ |
  |  5 | `/century/sensor/lidar16/compensator/PointCloud2` | 10Hz |
...
|`/century/sensor/lidar16/PointCloud2` | 确保能正常输出数据|
|`/century/sensor/lidar16/Scan`| 确保能正常输出数据|
| `/century/sensor/lidar16/compensator/PointCloud2`  | 确保能正常输出数据 |
|`/tf`|确保能正常输出数据|
|`/tf_static`|确保能正常输出数据|
...
| `/century/localization/pose`| 确保能正常输出数据 | 
|`/century/sensor/gnss/best_pose` | 确保能正常输出数据、`sol_type:` 选项显示为`NARROW_INT`   |
| `/century/sensor/lidar16/back/PointCloud2` |确保能正常输出数据|
| `/century/sensor/lidar16/left/PointCloud2` |确保能正常输出数据|
| `/century/sensor/lidar16/right/PointCloud2` |确保能正常输出数据|
| `/century/sensor/lidar16/fusion/PointCloud2` |确保能正常输出数据|
| `/century/sensor/lidar16/compensator/PointCloud2` |确保能正常输出数据|
|`/tf`|确保能正常输出数据|
|`/tf_static`|确保能正常输出数据|
|`/century/canbus/chassis`|确保能正常输出数据|
...
#### 6. 启动Lidar感知
确认各模块正常启动且channel输出正常后，在DreamView上启动`Lidar Perception`，使用`cyber_monitor`查看`/century/perception/obstacles`是否正常输出
![lidar_adaption_dreamview_closeloop](images/lidar_adaption_dreamview_closeloop.png)


#### 7. 验证Lidar感知
在DreamView上启动Perception模块，使用`cyber_monitor`查看`/century/perception/obstacles`是否正常输出，查看车前方10米处运动的人或者自行车（自行车上要有人），在DreamView上查看障碍物颜色以及位置速度信息（自行车青蓝色，行人黄色，车辆绿色），如下图所示：

![lidar_adaptation_dreamview2](images/lidar_adaptation_dreamview2.png)

`/century/perception/obstacles`的数据如下图所示：
...
确保在DreamView上能看到障碍物且`/century/perception/obstacles`有障碍物信息。
