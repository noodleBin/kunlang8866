...
## 百度云对象存储BOS注册
注册方法参考[百度云对象存储BOS注册与基本使用向导](../../Century_Fuel/apply_bos_account_cmd) 
## 开通云服务账号
...
- 保存生成的预处理文件。切换到`century/output/sensor_calibration/camera_to_lidar/extracted_data/`目录，保存该目录下的`camera_to_lidar-xxx`文件(`xxx`为数据包录制的时间)，这里和下文以`camera_to_lidar-2021-01-12-13-49`为例
