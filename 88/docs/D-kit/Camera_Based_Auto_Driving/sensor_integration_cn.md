...
### 摄像头规则文件的配置
 ...
 - 根据自身情况，修改rules文件，摄像头的rules文件示例如下所示(这里只包含了两个摄像头的规则文件，如需用到3个摄像头，可参照格式自行添加)：
	```
	SUBSYSTEM=="video4linux", SUBSYSTEMS=="usb", KERNELS=="2-1:1.0", ATTR{index}=="0", MODE="0666", SYMLINK+="camera/front_6mm", OWNER="century", GROUP="century"
	SUBSYSTEM=="video4linux", SUBSYSTEMS=="usb", KERNELS=="2-2:1.0", ATTR{index}=="0", MODE="0666", SYMLINK+="camera/front_12mm", OWNER="century", GROUP="century"
	```
其中，第一条代表连接到USB端口号为`2-1:1.0`的摄像头对应的软链接文件为`camera/front_6mm`；第二条代表连接到USB端口号为`2-2:1.0`的摄像头对应的软链接文件为`camera/front_12mm`。
 ...
 - 在docker环境外，执行如下命令，使配置的规则文件在本地系统生效:
	...
	```
	bash ~/century/docker/setup_host/setup_host.sh  
	sudo reboot  //重启工控机
	```
...
#### 5. 验证camera是否工作正常
 ...
 - 在`docker`环境内使用`cyber_monitor`工具，查看`/century/sensor/camera/front_6mm/image`、`/century/sensor/camera/front_12mm/image`是否有数据输出，且帧率是否稳定在15帧左右(关于cyber_monitor更详细使用，请参考[CyberRT_Developer_Tools](../../cyber/CyberRT_Developer_Tools.md))。
