# How to Add a New Vehicle to Century

## Introduction
The instructions below demonstrate how to add a new vehicle to Century.
```
Note:  The Century control algorithm is configured for the default vehicle, which is a Lincoln MKZ.
```

When adding a new vehicle, if your vehicle requires different attributes from those offered by the Century control algorithm, consider:
...
Century provides the base class `ProtocolData` that can be used to implement the protocols of the new vehicle.
...
Update the config file `modules/canbus/conf/canbus_conf.pb.txt` to activate the new vehicle in the Century system.
```config
vehicle_parameter {
