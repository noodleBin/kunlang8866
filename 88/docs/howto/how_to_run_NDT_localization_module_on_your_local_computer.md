...
- Download localization data from
  [Century Data Open Platform](http://data.century.auto/?name=sensor%20data&data_key=multisensor&data_type=1&locale=en-us&lang=en)（US
  only)
...
`/century/modules/localization/conf/localization.conf` to overwrite the default
value.
...
Add config of map path in /century/modules/localization/conf/localization.conf
...
For different LIDAR sensor, Century may has different LIDAR topic name. So set
...
`/century/modules/localization/dag/dag_streaming_msf_visualizer.dag`
...
run the script in century directory
...
localization map, which will be stored in the century/data/map_visual directory.
