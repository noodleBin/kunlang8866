...
CenturyAuto uses ROS1 as the underlying network which means that CenturyAuto borrows the Master-Nodes framework from ROS1. Since xmlRPC from ROS1 is really old \(compared
...
In Baidu CenturyAuto, three stages of development have already been described

1. Dreamviewer Offline Simulation Engine & CenturyAuto core software module
...
## ROS underlying Subscription and Publication mechanism and CenturyAuto modules structure


#### ROS underlying Subscription and Publication mechanism

So how does ROS1 based system communicate with each other and how does CenturyAuto make use of it? ROS has [tutorials](http://wiki.ros.org/ROS/Tutorials), and I will explain it
quickly before we analyze CenturyAuto modules structure.
...
cb here is a callback executed when Linux kernel IO is ready. With these signatures bearing in mind, we can quickly analyze CenturyAuto
module structures before diving deep into core modules implementation.

#### centuryauto modules structure

I have conducted full research about it but I cannot show you all of them. CenturyAuto modules/common/ provide basic micros to control ros::spin for each
module and /modules/common/adaptor contains the most information on how a topic is registered. Every module will be registered from the [point](https://github.com/yiakwy/century/blob/master/modules/common/adapters/adapter_manager.cc#L50)
...
Each module starts by firing "Init" interface and register callbacks. If you want to step by step debug CenturyAuto in gdb, make sure you have added breakpoints in those back. This also
...
3. Probability Fusion\(New in Century 1.5!\):
   - As far as I can understand, fusion system in CenturyAuto
