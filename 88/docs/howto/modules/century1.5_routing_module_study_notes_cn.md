...
基于请求，输出两点之间导航信息。
## 输入：
 * 用户的起始和终止位置。/century/routing_request[pb_msgs/RoutingRequest]。
 * HD map。
 * /century/monitor[pb_msgs/MonitorMessage]。

## 输出：
 * 请求的响应：/century/routing_response[pb_msgs/RoutingResponse],包含了最终的路由信息。
 * /century/monitor[pb_msgs/MonitorMessage]。
