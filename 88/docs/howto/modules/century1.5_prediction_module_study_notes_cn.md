...
## 输入：
 * 车辆位置信息：/century/localization/pose（[pb_msgs/LocalizationEstimate]）。
 * 障碍物信息：/century/perception/obstacles（[pb_msgs/PerceptionObstacles]）。

## 输出：
障碍物的运动轨迹。/century/prediction[pb_msgs/PredictionObstacle]。
