...
### 输入：
 * Imu数据：/century/sensor/gnss/corrected_imu。
 * Gps数据：/century/sensor/gnss/odometry。

### 输出：
 * 定位信息：/century/localization/pose。
 * 监视信息：/century/localization/。

## 代码分析：
主要文件：localization.cc,localization.h,localization_base.h。
### Localization类:
 * 继承于CenturyApp类，有三个数据，localization_具体的定位算法实例指针，localization_factory_存储定位算法类，config_存储配置文件信息。
