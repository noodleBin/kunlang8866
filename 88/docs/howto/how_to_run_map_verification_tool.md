...
The Map Data Verification tool is designed to help Century developers detect any issues in their map data collection process before the data is used for HD Map creation. The benefit of using this tool is to ensure that all issues are detected prior to map creation and also ensures that data can be efficiently recollected based on the suggestions in the tool.

## Running the tool

In order to run your data on this tool, please follow the steps below:

1. Build Century as recommended in the [Build Guide](../quickstart/century_software_installation_guide.md) until the `./century.sh build` step.
2. Once instde dev docker and after running `./century.sh build` please go to the folder `modules/tools/map_datachecker/`
