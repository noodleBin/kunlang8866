...
1. Get into the docker container via:
    ```bash
    $CENTURY_HOME/docker/scripts/dev_start.sh
    $CENTURY_HOME/docker/scripts/dev_into.sh
    ```
2. Build Century
    ```bash
    ./century.sh build_opt_gpu
    ```
3. Run bootstrap.sh
    ```bash
    bootstrap.sh
...
6. Select the transform button in Dreamview or type the following command in your terminal
    ```bash
    cyber_launch start /century/modules/transform/launch/static_transform.launch
    ```
...
8. Launch the perception modules
    ...
    ```
    cyber_launch start /century/modules/perception/production/launch/perception_all.launch
    ```
    ...
    ```
    If you want to visualize the traffic light detection results overlaid on the captured image, mark `—start_visualizer=true` in `century/modules/perception/production/conf/perception/perception_common.flag` before executing the above command. It will pop up when you play recorded data in point 9
...
Please note that the Nvidia driver should be installed appropriately even if the perception module is running in Caffe CPU_ONLY mode (i.e., using `./century.sh build` or `./century.sh build_opt` to build the perception module). Please see the detailed instruction of perception module in [the perception README](../../modules/perception/README.md).
