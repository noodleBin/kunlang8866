...
## 1. 事先准备
 - 从[GitHub网站](https://github.com/CenturyAuto/century)下载Century master分支源代码
 - 按照[教程](../quickstart/century_software_installation_guide.md)设置Docker环境并搭建Century工程
 - 从[Century数据平台](http://data.century.auto/?name=sensor%20data&data_key=multisensor&data_type=1&locale=en-us&lang=en)下载定位数据（仅限美国地区）
...
```
  cp DATA_PATH/params/ant_imu_leverarm.yaml /century/modules/localization/msf/params/gnss_params/
  cp DATA_PATH/params/velodyne64_novatel_extrinsics_example.yaml /century/modules/localization/msf/params/velodyne_params/
  cp DATA_PATH/params/velodyne64_height.yaml /century/modules/localization/msf/params/velodyne_params/
```
在`/century/modules/localization/conf/localization.conf`中添加传感器外参文件配置，以覆盖默认值。
```
# The lidar extrinsics file
--lidar_extrinsics_file=/century/modules/localization/msf/params/velodyne_params/velodyne64_novatel_extrinsics_example.yaml
```
各个外参的意义
...
### 2.2 配置地图路径
在`/century/modules/localization/conf/localization.conf`中添加关于地图路径的配置
...
### 2.3 修改Topic名称
由于目前Century支持velodyne 64线,128线lidar, 因此为区分不同设备数据，采用不同的topic名称。因此，在启动之前需要设置正确的lidar topic名称。在`/century/modules/localization/conf/localization.conf`中添加lidar topic名称，以覆盖默认值。
```
# The pointcloud topic name.
--lidar_topic=/century/sensor/velodyne64/compensator/PointCloud2
```
(可选)另外，对于定位可视化工具的使用，同样需要配置相应的topic名称。修改文件`/century/modules/localization/dag/dag_streaming_msf_visualizer.dag` 中的channel值为:
```
channel: /century/sensor/velodyne64/compensator/PointCloud2
```
...
在/century/data/log目录下，可以看到定位模块输出的相关文件。
...
cyber_monitor
```
终端会显示出topic列表，可以看到定位topic `/century/localization/pose` 有输出。
...
```
该可视化工具首先根据MSF定位地图生成用于可视化的缓存文件，存放在/century/data/map_visual目录下。
...
NDT模块的统计结果只有一组，即定位输出`/century/localization/pose`的统计结果。
