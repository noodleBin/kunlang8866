...
## Introduction
GPS receiver is a device that receives information from GPS satellites and then calculates the device's geographical position, velocity and precise time. The device usually includes a receiver, an IMU (depends on the model), an interface to a wheel encoder, and a fusion engine that combines information from those sensors. The Default GPS receiver used in Century is Novatel cards. The instruction demonstrates how to add and use a new GPS Receiver.
...
private:
    ...
    ::century::drivers::gnss::Gnss _gnss;
    ::century::drivers::gnss::Imu _imu;
    ::century::drivers::gnss::Ins _ins;
};
