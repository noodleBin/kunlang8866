...
本文档描述了在 Century 项目中准备 Bazel 的依赖缓存目录
...
为便利国内开发者，对 Century 中使用到的各 Bazel 版本，Century 均提供**与之对应**的
Bazel 隐式依赖压缩包文件。请注意，对不同版本的 Bazel 来说，其隐式依赖项不尽相同
。当 Bazel 版本更新后，请务必按照本文档描述的方法，**重新**执行一遍。

接下来，我们先熟悉把 Century 提供的 Bazel 隐式依赖压缩包解压到依赖缓存目录的流程
。

## 如何利用 Century 提供的 Bazel 隐式依赖压缩包

1. 在 Century 容器中执行`bazel version`确定 Bazel 版本：
...
3. 将压缩包解压到由环境变量`${CENTURY_BAZEL_DIST_DIR}`指代的 Bazel 依赖缓存目录
...
如果您需要的 Bazel 版本 Century 未能提供，除了
在[GitHub Issues 页](https://github.com/CenturyAuto/century/issues) 提交 Issue 外
...
# 确保${CENTURY_BAZEL_DIST_DIR} 变量已定义，且对应的目录存在
source ${CENTURY_ROOT_DIR}/cyber/setup.bash
[[ -d "${CENTURY_BAZEL_DIST_DIR}" ]] || mkdir -p "${CENTURY_BAZEL_DIST_DIR}"
...
然后，你就可以在 Century 容器中顺利执行`./century.sh build`，`./century.sh test` 等
命令而无需担心依赖项拉不下来的问题了。
