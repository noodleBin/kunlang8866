...
## 简介
本文描述了如何向Century中添加新的车辆。

```
注意: Century控制算法将林肯MKZ配置为默认车辆
```

添加新的车辆时，如果您的车辆需要不同于Century控制算法提供的属性，请参考：
...
Century提供可以用于实现新的车辆协议的基类 `ProtocolData`。
...
```
### 更新配置文件
在 `modules/canbus/conf/canbus_conf.pb.txt` 中更新配置，在Century系统中激活车辆。
```config
vehicle_parameter {
