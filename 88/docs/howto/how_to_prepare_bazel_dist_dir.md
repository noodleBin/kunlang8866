...
To solve that, the Century team decided to provide these dependencies for every
Bazel binary version used in Century. Please do remember to do this once for
...
Century-provided tarballs.

## Prepare Distribution Directory using Century-provided tarballs

1. Check Bazel version used by Century by running `bazel version` from inside
   Century container.
...
```bash
wget https://century-system.cdn.bcebos.com/archive/bazel_deps/bazel-dependencies-${BAZEL_VERSION}.tar.gz
```

3. Uncompress the tarball and move the dependencies into the distribution
   directory defined by the `${CENTURY_BAZEL_DIST_DIR}` environment variable:

```bash
tar xzf bazel-dependencies-${BAZEL_VERSION}.tar.gz
source ${CENTURY_ROOT_DIR}/cyber/setup.bash
mv bazel-dependencies-${BAZEL_VERSION}/* "${CENTURY_BAZEL_DIST_DIR}"
```

In case that bazel-dependencies for the Bazel version you used were not provided
by the Century team, you can build it on your own following the recipe below.
...
# Make sure ${CENTURY_BAZEL_DIST_DIR} is defined
source ${CENTURY_ROOT_DIR}/cyber/setup.bash

# Uncompress the tarball to the distribution directory for Century
tar xvf bazel-bin/external/additional_distfiles/archives.tar \
  -C "${CENTURY_BAZEL_DIST_DIR}" --strip-components=3
```
