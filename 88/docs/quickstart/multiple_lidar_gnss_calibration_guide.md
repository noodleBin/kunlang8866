...
```
NOTE: Supports upto Century 3.0. Century 3.5 is not supported yet.
```
...
1. Download the [calibration tool](https://github.com/CenturyAuto/century/releases/download/v2.5.0/multi_lidar_gnss_calibrator_and_doc.zip), and extract files to `$CENTURY_HOME/modules/calibration`. CENTURY_HOME is the root directory of century repository.
2. Choose a calibration place according to the calibration guide provided in Century 1.5.
3. Make sure the GNSS is in a good status. To verify this, use `rostopic echo /century/sensor/gnss/best_pose` and check the number after keywords `latitude_std_dev`, `longitude_std_dev` and `height_std_dev`. The smaller the deviation, the better the calibration quality. *** We strongly recommend calibrating the sensors when deviations are smaller than 0.02.*** 

## Using the Calibration Tool

### Record Calibration Data

When the LiDARs and GNSS are ready, use `/century/modules/calibration/multi_lidar_gnss/record.sh` to record calibration data. Note that, this script is only for recording  velodyne HDL64 and VLP16. For other purpose, some modification of this script is needed or just use rosbag record to do the same thing. Usually, 2 minites length of data is sufficient. After data capture, run `/century/modules/calibration/multi_lidar_gnss/calibrate.sh` to calibrate sensors. The script is composed of the following two steps.

### Export Data

Once the calibration bag is recorded, use `/century/modules/calibration/exporter/export_msgs --config /century/modules/calibration/exporter/conf/export_config.yaml` to get sensor data. The only input of the exporter is a YAML configuration file as follow.

```bash
bag_path: "/century/data/bag/calibration/" # The path where the calibration bag is placed.
dump_dir: "/century/data/bag/calibration/export/" # The path where the sensor data will be placed using exporter
topics:
    - /century/sensor/gnss/odometry: # Odometry topic name
        type: CenturyOdometry        # Odometry type
    - /century/sensor/velodyne16/PointCloud2: # vlp16 topic name
        type: PointCloud2                    # vlp16 type
    - /century/sensor/velodyne64/PointCloud2: # hdl64 topic name
...
```bash
    - TOPIC_NAME: # topic name
        type: PointCloud2
```
Till now, we only support `CenturyOdometry` and `PointCloud2`.  

### Run the Calibration Tool
If all sensor data are exported, run `/century/modules/calibration/lidar_gnss_calibrator/multi_lidar_gnss_calibrator --config /century/modules/calibration/lidar_gnss_calibrator/conf/multi_lidar_gnss_calibrator_config.yaml` will get the results. The input of the tool is a YAML configuration file as follow.
```bash
# multi-LiDAR-GNSS calibration configurations
data:
    odometry: "/century/data/bag/calibration/export/multi_lidar_gnss/_century_sensor_gnss_odometry/odometry"
    lidars: 
        - velodyne16: 
            path: "/century/data/bag/calibration/export/multi_lidar_gnss/_century_sensor_velodyne16_PointCloud2/"
        - velodyne64: 
            path: "/century/data/bag/calibration/export/multi_lidar_gnss/_century_sensor_velodyne64_PointCloud2/"
    result: "/century/data/bag/calibration/export/multi_lidar_gnss/result/"
calibration:
    init_extrinsics:
        velodyne16:
            translation:    
...
```
The two YAML files are extrinsics. To validate the results, use `pcl_viewer *_result.pcd` to check the registration quality. If the sensors are well calibrated, a large amount of details can be identified from the point cloud. For more details, please refer to the calibration guide in Century 1.5. 
