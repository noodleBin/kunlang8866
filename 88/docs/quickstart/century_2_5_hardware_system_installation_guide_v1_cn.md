...
本篇指南提供了所有 **Century项目**需要的的安装硬件部分和软件系统教程。系统安装信息包括下载和安装Century Linux内核的过程。
...
**Century项目**旨在为汽车和自动驾驶行业的合作伙伴提供开放，完整和可靠的软件平台。该项目的目的是使这些企业能够开发基于Century软件栈的自动驾驶系统。

## 文档说明

以下文档适用于Century 2.5:

- ***<u>[Century Hardware and System Installation Guide]</u>***  ─ 提供用于安装车辆的硬件部件和系统软件的教程:
...
- ***<u>[Century Quick Start Guide]</u>*** ─ 文档和产品蓝图提供了完整的端到端教程。本文还提供了一些其它链接用于将一辆普通汽车改装成一辆自动驾驶车辆。
...
车上运行Century需要 [Century Kernel](https://github.com/CenturyAuto/century-kernel). 强烈建议安装预编译内核。
...
4. 根据[ESDCAN-README.md](https://github.com/CenturyAuto/century-kernel/blob/master/linux/ESDCAN-README.md)编译ESD CAN驱动器源代码
...
cd century-kernel
```
2. 根据 [ESDCAN-README.md](https://github.com/CenturyAuto/century-kernel/blob/master/linux/ESDCAN-README.md)添加 ESD CAN 驱动源代码。
...
车辆中的Century运行需要[NVIDIA GPU 驱动](http://www.nvidia.com/download/driverResults.aspx/114708/en-us)。您必须安装具有特定选项的NVIDIA GPU驱动程序。
...
按照以下步骤将ProPak6 GNSS接收器和IMU连接到Century系统：
...
5. 通过串口连接进行LiDAR配置
   ...
   默认情况下，HDL-64E S3的网络IP地址设置为192.168.0.1。 但是，当我们配置Century时，我们应该将网络IP地址改为192.168.20.13。 可以使用终端应用程序Terminalite 3.2，进入网络设置命令。可以按照以下步骤配置HDL-64E S3的IP地址：
...
Century 2.5的地图制作服务已经向公众开放。为了获得地图创建所需的数据，需要在车辆上安装附加的VLP-16激光雷达。LiDAR的目的是收集HDL-64 S3 激光雷达FOV上方的物体的点云信息，例如交通信号灯和标志。它需要一个定制的机架安装VLP-16激光雷达在车辆顶部。下图显示了可以选择的配置之一。![VLP_16_installation](images/VLP16_install_configuration.jpg)
...
Century参考设计建议使用三个不同焦距的摄像头：两个6毫米镜头，一个25毫米镜头。这三个摄像头的摆放位置非常灵活，它们可以放置在LiDAR的旁边或前车窗的里。摄像机的安装可以为系统的实际设计量身定做。
...
2.   将板载计算机系统6108GC放在后备箱内（推荐）。
     ...
     例如，Century 2.5使用4x4螺钉将6108GC螺栓固定在后备箱的箱板上。 ![IPC-bolt_down-936x720](images/Mount_ipc_on_carpet.jpg)
...
# 建立网络
本节提供了一种建立网络的建议。
运行Century软件的IPC必须访问互联网获取实时运动学（RTK）数据，以便精确定位。移动设备还需要连接到IPC来运行Century软件。
...
- 在Apple iPad Pro上建立Wi-Fi连接，以访问HMI并控制IPC上运行的Century ADS。

# 下一步

完成硬件部分的安装之后，可以参考快速入门的教程 [Century Quick Start](https://github.com/CenturyAuto/century/blob/master/docs/quickstart/century_2_5_quick_start_cn.md)完成软件部分的安装。
