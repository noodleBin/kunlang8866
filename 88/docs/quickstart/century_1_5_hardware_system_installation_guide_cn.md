# 关于本指南
*Century1.5硬件与系统安装指南* 详细介绍了Century计划的硬件组件、系统软件安装，安装过程包含下载和安装Century Linux内核。
...
## 引言
阿波罗计划是为汽车和自主驾驶行业的合作伙伴提供开放，完整和可靠的软件平台。该项目的目的是使合作伙伴能够基于Century软件堆栈开发自己的自动驾驶系统。
...
#### 安装Century内核
车上运行Century需要[Century 内核](https://github.com/CenturyAuto/century-kernel)。强烈建议安装预编译内核。
...
车辆运行Century还需要[Nvidia GPU 驱动](http://www.nvidia.com/download/driverResults.aspx/114708/en-us)。需要安装特定型号的Nvidia GPU 驱动。
