...
本文档介绍了在Ubuntu 18.04.5 LTS（Bionic Beaver）（建议用于Century 6.0的Ubuntu版本）上安装Century软件所需的步骤。
...
运行以下命令克隆[Century的GitHub仓库](https://github.com/CenturyAuto/century.git)。
...
对于中国用户, 如果从GitHub克隆仓库有困难，可参考[国内环境下如何克隆Century仓库](../howto/how_to_clone_century_repo_from_china_cn.md)。

（可选）为方便起见，可以在Century的根目录运行以下命令来设置指向该目录的环境变量`CENTURY_ROOT_DIR`：

```bash
echo "export CENTURY_ROOT_DIR=$(pwd)" >> ~/.bashrc  && source ~/.bashrc
```

![tip](images/tip_icon.png) 在下面的章节中，我们会把Century的根目录称为`$CENTURY_ROOT_DIR`。

## 启动Docker容器

在`${CENTURY_ROOT_DIR}`目录, 输入：

```bash
bash docker/scripts/dev_start.sh
```

来启动Century的Docker开发容器。
...
来进行优化模式的构建。可以参考[Century构建和测试说明](../specs/century_build_and_test_explained.md)来全面了解Century的构建和测试。

## 启动并运行Century

请参考[如何启动并运行Century](../howto/how_to_launch_and_run_century.md)中的[运行Century](../howto/how_to_launch_and_run_century.md#run-century)部分。
