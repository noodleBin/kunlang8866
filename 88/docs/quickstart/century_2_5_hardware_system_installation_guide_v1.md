# Century 2.5 Hardware and System Installation Guide
...
The *Century 2.5 Hardware and System Installation Guide* provides the instructions to install all of the hardware components and system software for the **Century Project**. The system installation information included pertains to the procedures to download and install the Century Linux Kernel.
...
The **Century Project** is an initiative that provides an open, complete, and reliable software platform for Century partners in the automotive and autonomous driving industries. The aim of this project is to enable these entities to develop their own self-driving systems based on Century software stack.

## Documentation

The following set of documentation describes Century 2.5:

- ***<u>[Century Hardware and System Installation Guide]</u>***  ─ Provides the instructions to install the hardware components and the system software for the vehicle:
    ...
    - **Software**:
      - Ubuntu Linux
      - Century Linux Kernel
      - NVIDIA GPU Driver

- ***<u>[Century Quick Start Guide]</u>*** ─ A combination of tutorial and roadmap that provides the complete set of end-to-end instructions. The Quick Start Guide also provides links to additional documents that describe the conversion of a regular car to an autonomous-driving vehicle.
...
4. Install the software on the IPC (some Linux experience is required):
    ...
    b. Install the Century Linux kernel.
...
#### Installing the Century Kernel

The Century runtime in the vehicle requires the [Century Kernel](https://github.com/CenturyAuto/century-kernel). It is strongly recommended to install the pre-built kernel.

##### Use the pre-built Century Kernel.
...
```
https://github.com/CenturyAuto/century-kernel/releases
```
2. Install the kernel after having downloading the release package:
```
tar zxvf linux-4.4.32-century-1.5.0.tar.gz
cd install
sudo bash install_kernel.sh
```
3. Reboot your system using the `reboot` command.

4. Build the ESD CAN driver source code, according to [ESDCAN-README.md](https://github.com/CenturyAuto/century-kernel/blob/master/linux/ESDCAN-README.md)
...
```
git clone https://github.com/CenturyAuto/century-kernel.git
cd century-kernel
```
2. Add the ESD CAN driver source code according to [ESDCAN-README.md](https://github.com/CenturyAuto/century-kernel/blob/master/linux/ESDCAN-README.md)

3. Build the kernel using the following command.
```
bash build.sh
```
4. Install the kernel using the steps for a pre-built Century Kernel as described in the previous section.

#### Installing NVIDIA GPU Driver

The Century runtime in the vehicle requires the [NVIDIA GPU Driver](http://www.nvidia.com/download/driverResults.aspx/114708/en-us). You must install the NVIDIA GPU driver with specific options.
...
Follow these steps to connect the ProPak6 GNSS Receiver and the IMU to the Century system:
...
The Century reference design recommends using three cameras with different focal lengths: two with 6 mm lens and one with 25 mm lens. The location of these cameras are very flexible, they can be placed on the side of LiDAR or inside of the front windshield. The mounting of the cameras can be tailored to the actual design of the system. 
...
2.   Place the onboard computer system, the 6108GC, inside the trunk (recommended).
     ...
     For example, Century 2.5 uses 4x4 self-tapping screws to bolt the 6108GC to the carpeted floor of the trunk. ![IPC-bolt_down-936x720](images/Mount_ipc_on_carpet.jpg)
...
The IPC that is running the Century software must access the Internet to acquire the Real Time Kinematic (RTK) data for accurate localization. A mobile device also needs to connect to the IPC to run the Century software.
...
2.   Establish a Wi-Fi connection on the Apple iPad Pro to access the HMI and control the Century ADS that is running on the IPC.

# Time Sync Script Setup [Optional]

In order to, sync the computer time to the NTP server on the internet, you could use the [Time Sync script](https://github.com/CenturyAuto/century/blob/master/scripts/time_sync.sh)


# Next Steps

After you complete the hardware installation in the vehicle, see the [Century Quick Start](https://github.com/CenturyAuto/century/blob/master/docs/quickstart/century_2_5_quick_start.md) for the steps to complete the software installation.
