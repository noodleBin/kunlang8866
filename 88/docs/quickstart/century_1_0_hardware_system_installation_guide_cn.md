...
本篇指南提供了所有安装硬件部分和 Century 项目所需的教程。系统安装信息包括下载和安
装 Century Linux 内核的过程。
...
**Century**项目旨在为汽车和自动驾驶行业的合作伙伴提供开放，完整和可靠的软件平台。
该项目的目的是使这些企业能够开发基于 Century 软件栈的自动驾驶系统。

## 文档说明

以下文档适用于 Century 1.0:

- **_<u>[Century Hardware and System Installation Guide]</u>_** ─ 提供用于安装车
  辆的硬件部件和系统软件的教程:
...
- **_<u>[Century Quick Start Guide]</u>_** ─ 文档和蓝图的组合提供了完整的端到端教
...
4. 在 IPC 安装软件（需要部分 Linux 经验）： a. 安装 Ubuntu Linux. b. 安装 Century
   Linux kernel.
...
车上运行 Century 需要
[Century Kernel](https://github.com/CenturyAuto/century-kernel). 强烈建议安装预编译
内核。
...
3. 使用 `reboot`命令重启系统；
4. 根
   据[ESDCAN-README.md](https://github.com/CenturyAuto/century-kernel/blob/master/linux/ESDCAN-README.md)编
   译 ESD CAN 驱动器源代码
...
2. 根据
   [ESDCAN-README.md](https://github.com/CenturyAuto/century-kernel/blob/master/linux/ESDCAN-README.md)添
   加 ESD CAN 驱动源代码。
...
按照以下步骤将 ProPak6 GNSS 接收器和 IMU 连接到 Century 系统：
...
例如，Century 1.0 使用 4x4 自攻螺钉将 5059GC 固定在后备箱里。
...
运行 Century 软件的 IPC 必须访问互联网获取实时运动学（RTK）数据，以便精确定位。移
动设备还需要连接到 IPC 来运行 Century 软件。
...
2.  在 Apple iPad Pro 上建立 Wi-Fi 连接，以访问 HMI 并控制 IPC 上运行的 Century
    ADS。

# 下一步

完成硬件部分的安装之后，可以参考快速入门的教程
[Century Quick Start](century_1_0_quick_start.md) 完成软件部分的安装。
