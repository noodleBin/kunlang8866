...
class RobosenseDriver : public lidar::LidarDriver {
 ...
 protected:
  ...
  void PacketCallback(const ::robosense::lidar::Packet &pkt);
  void PublishScan(uint32_t frame_seq,
                   const std::shared_ptr<RobosenseScan> &scan_msg);
  void ProcessCloud();
 ...
 private:
  ...
  std::shared_ptr<cyber::Writer<PointCloudPacked>> pointpackedcloud_writer_ =
      nullptr;
  ...
  std::map<uint32_t, std::shared_ptr<RobosenseScan>> scan_msgs_;
};
}  // namespace robosense
}  // namespace drivers
}  // namespace century
