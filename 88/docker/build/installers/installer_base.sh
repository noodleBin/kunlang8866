...
export CENTURY_DIST="${CENTURY_DIST:-stable}"

export PKGS_DIR="/opt/apollo/pkgs"
export SYSROOT_DIR="/opt/apollo/sysroot"

export CENTURY_PROFILE="/etc/profile.d/century.sh"
export CENTURY_LD_FILE="/etc/ld.so.conf.d/century.conf"
...
function century_environ_setup() {
    ...
    if [ ! -f "${CENTURY_LD_FILE}" ]; then
        echo "${SYSROOT_DIR}/lib" | tee -a "${CENTURY_LD_FILE}"
    fi
    if [ ! -f "${CENTURY_PROFILE}" ]; then
        cp -f /opt/apollo/rcfiles/century.sh.sample "${CENTURY_PROFILE}"
        echo "add_to_path ${SYSROOT_DIR}/bin" >> "${CENTURY_PROFILE}"
    fi
    if [ ! -f "${DOWNLOAD_LOG}" ]; then
        echo "##==== Summary: Century Package Downloads ====##" > "${DOWNLOAD_LOG}"
        echo -e "Package\tSHA256\tDOWNLOADLINK" | tee -a "${DOWNLOAD_LOG}"
    fi
}

century_environ_setup
