...
function remove_container_if_exists() {
    ...
    if docker ps -a --format '{{.Names}}' | grep -q "${container}"; then
        info "Removing existing Century container: ${container}"
...
function postrun_start_user() {
    ...
    if [ "${USER}" != "root" ]; then
        docker exec -u root "${container}" \
            bash -c '/century/scripts/docker_start_user.sh'
    fi
}

function stop_all_century_containers() {
    local force="$1"
    local running_containers
    running_containers="$(docker ps -a --format '{{.Names}}')"
    for container in ${running_containers[*]}; do
        if [[ "${container}" =~ century_.*_${USER} ]]; then
...
function check_agreement() {
    local agreement_record="${HOME}/.century_agreement.txt"
    if [[ -e "${agreement_record}" ]]; then
        return 0
    fi
    local agreement_file
    agreement_file="${CENTURY_ROOT_DIR}/scripts/AGREEMENT.txt"
...
export -f stop_all_century_containers remove_container_if_exists
