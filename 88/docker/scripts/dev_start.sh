...
CACHE_ROOT_DIR="${CENTURY_ROOT_DIR}/.cache"

DOCKER_REPO="apolloauto/apollo"
DEV_CONTAINER="century_dev_${USER}"
...
OPTIONS:
    ...
    -d, --dist             Specify Century distribution(stable/testing)
    --shm-size <bytes>     Size of /dev/shm . Passed directly to "docker run"
    -y                     Agree to Century License Agreement non-interactively.
    stop                   Stop all running Century containers.
EOF
}
    ...
    while [ $# -gt 0 ]; do
        ...
        case "${opt}" in
            ...
            -y)
                ...
                ;;
            stop)
                info "Now, stop all Century containers created by ${USER} ..."
...
function determine_dev_image() {
    ...
    if [[ -z "${version}" ]]; then
        if [[ "${TARGET_ARCH}" == "x86_64" ]]; then
            ...
            error "Logic can't reach here! Please report this issue to Century@GitHub."
            exit 3
        fi
    fi
...
function check_host_environment() {
    if [[ "${HOST_OS}" != "Linux" ]]; then
        warning "Running Century dev container on ${HOST_OS} is UNTESTED, exiting..."
        exit 1
    fi
}
...
function setup_devices_and_mount_local_volumes() {
    ...
    GLOBAL_CACHE_BASE="/home/${USER}/century_global_cache"
    mkdir -p "${GLOBAL_CACHE_BASE}/repos"
    mkdir -p "${GLOBAL_CACHE_BASE}/bazel_root"

    [ -d "${CACHE_ROOT_DIR}" ] || mkdir -p "${CACHE_ROOT_DIR}"

    source "${CENTURY_ROOT_DIR}/scripts/century_base.sh"
    setup_device

    local volumes="-v ${CENTURY_ROOT_DIR}:/century"

    volumes="${volumes} -v ${GLOBAL_CACHE_BASE}/repos:/century/.cache/repos"
    volumes="${volumes} -v ${GLOBAL_CACHE_BASE}/bazel_root:/century/.cache/bazel"
    info "Global Cache Mounted: ${GLOBAL_CACHE_BASE} -> /century/.cache"

    local teleop="${CENTURY_ROOT_DIR}/../century-teleop"
    if [ -d "${teleop}" ]; then
        volumes="-v ${teleop}:/century/modules/teleop ${volumes}"
    fi
...
function docker_pull() {
    ...
    if [[ "$img" != registry.baidubce.com/* ]]; then
        img="registry.baidubce.com/${img}"
    fi
...
function restart_map_volume_if_needed() {
    ...
    local map_path="/century/modules/map/data/${map_name}"
...
function mount_other_volumes() {
    ...
    local audio_path="/century/modules/audio/data/"
    ...
    local yolov4_path="/century/modules/perception/camera/lib/obstacle/detector/yolov4/model/"
    ...
    local faster_rcnn_path="/century/modules/perception/production/data/perception/camera/models/traffic_light_detection/faster_rcnn_model"
    ...
    if [[ "${TARGET_ARCH}" == "x86_64" ]]; then
        ...
        local smoke_path="/century/modules/perception/production/data/perception/camera/models/yolo_obstacle_detector/smoke_libtorch_model"
...
function main() {
    ...
    info "Remove existing Century Development container ..."
...
function update_docker_image() {
    if docker images --format "{{.Repository}}:{{.Tag}}" | grep -q ${CENTURY}; then
	    echo "The docker image already exists on the native."
    else
	    mkdir  /home/$USER/docker_image
	    cd     /home/$USER/docker_image
	    git clone git@192.168.10.57:autonomous_driving/century_image.git
	    if [ ! -f /home/$USER/docker_image/century_image/centuryV6.tar ]; then
	      git clone git@192.168.10.57:autonomous_driving/century_image.git
	    fi
        cd /home/$USER/docker_image/century_image/
        docker import centuryV6.tar century:century
    fi
}

function rename_docker_user() {
    docker exec -u root -it ${DEV_CONTAINER} /bin/bash -c "sed -i 's#admin#${USER}#g' /etc/shadow-"
    docker exec -u root -it ${DEV_CONTAINER} /bin/bash -c "sed -i 's#admin#${USER}#g' /etc/subgid"
    docker exec -u root -it ${DEV_CONTAINER} /bin/bash -c "sed -i 's#admin#${USER}#g' /etc/subuid"
    docker exec -u root -it ${DEV_CONTAINER} /bin/bash -c "sed -i 's#admin#${USER}#g' /etc/passwd"
    docker exec -u root -it ${DEV_CONTAINER} /bin/bash -c "sed -i 's#admin#${USER}#g' /etc/group"
    docker exec -u root -it ${DEV_CONTAINER} /bin/bash -c "sed -i 's#admin#${USER}#g' /etc/group-"
    docker exec -u root -it ${DEV_CONTAINER} /bin/bash -c "sed -i 's#admin#${USER}#g' /etc/shadow"
    docker exec -u root -it ${DEV_CONTAINER} /bin/bash -c "mv /home/admin/ /home/${USER}/"
}
