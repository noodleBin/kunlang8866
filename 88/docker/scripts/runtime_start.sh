...
DOCKER_REPO="centuryauto/century"
RUNTIME_CONTAINER="century_runtime_${USER}"
...
OPTIONS:
    ...
    -s, --standalone       Run standalone container with all volumes and century itself included.
...
function check_host_environment() {
    if [[ "${HOST_OS}" != "Linux" ]]; then
        warning "Linux ONLY support for Century Runtime Docker!"
        exit 1
    fi
    if [[ "${HOST_ARCH}" != "x86_64" ]]; then
        warning "Century Runtime Docker supports x86_64 ONLY!"
        exit 2
    fi
}
...
function setup_devices_and_mount_local_volumes() {
    ...
    if $RUNTIME_STANDALONE; then
        volumes="-v ${CENTURY_ROOT_DIR}/data:/century/data \
                 -v ${CENTURY_ROOT_DIR}/modules/calibration/data:/century/modules/calibration/data \
                 -v ${CENTURY_ROOT_DIR}/modules/map/data:/century/modules/map/data \
                 -v ${CENTURY_ROOT_DIR}/output:/century/output"
    else
        volumes="-v ${CENTURY_ROOT_DIR}:/century"
    fi
...
function restart_map_volume_if_needed() {
    local map_name="$1"
    local map_version="$2"
    local map_volume="century_map_volume-${map_name}_${USER}"
...
function mount_other_volumes() {
    ...
    # AUDIO
    local audio_volume="century_audio_volume_${USER}"
    ...
    # YOLOV4
    local yolov4_volume="century_yolov4_volume_${USER}"
    ...
    # FASTER_RCNN
    local faster_rcnn_volume="century_faster_rcnn_volume_${USER}"
    ...
    # SMOKE
    if [[ "${TARGET_ARCH}" == "x86_64" ]]; then
        local smoke_volume="century_smoke_volume_${USER}"
...
function main() {
    ...
    $RUNTIME_STANDALONE && RUNTIME_CONTAINER="century_runtime_standalone_$USER"

    info "Check and remove existing Century Runtime container ..."
    ...
    $RUNTIME_STANDALONE && docker exec -u root "${RUNTIME_CONTAINER}" bash -c "chown -R ${docker_uid}:${docker_gid} /century"

    ok "Congratulations! You have successfully finished setting up Century Runtime Environment."
