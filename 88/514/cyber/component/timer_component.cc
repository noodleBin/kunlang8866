...
bool TimerComponent::Process() {
  ...
  common::CallbackTrace trace(
      "timer_component", node_->Name(),
      std::string("module=") + common::DemangleTypeName(typeid(*this).name()));
  return Proc();
}
...
bool TimerComponent::Initialize(const TimerComponentConfig& config) {
  ...
  auto func = [self]() {
    ...
    common::CallbackTrace trace(
        "timer_component", self->node_->Name(),
        std::string("module=") +
            common::DemangleTypeName(typeid(*self).name()));
    self->Proc();
  };
