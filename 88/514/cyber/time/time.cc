...
namespace {
constexpr uint16_t kMaxTimeStringLenth = 128;
}  // namespace
...
uint64_t Time::ToMicrosecond() const {
  return static_cast<uint64_t>(nanoseconds_ / 1e3);
}

uint64_t Time::ToMillisecond() const {
  return static_cast<uint64_t>(nanoseconds_ / 1e6);
}
...
#else
  char date_time[kMaxTimeStringLenth];
...
std::vector<uint64_t> Time::ToVector() const {
  auto nano = std::chrono::nanoseconds(nanoseconds_);
  system_clock::time_point tp(nano);
  auto time = system_clock::to_time_t(tp);
  struct tm stm;
...
std::string Time::ToStringSimple() const {
  auto nano = std::chrono::nanoseconds(nanoseconds_);
  system_clock::time_point tp(nano);
  auto time = system_clock::to_time_t(tp);
  struct tm stm;
  ...
  std::stringstream ss;
#if __GNUC__ >= 5
  ss << std::put_time(ret, "%Y%m%d%H%M%S");
#else
  char date_time[kMaxTimeStringLenth];
  strftime(date_time, sizeof(date_time), "%Y%m%d%H%M%S", ret);
  ss << std::string(date_time);
#endif
  return ss.str();
}
...
void Time::SleepFor(const Duration& duration) {
  auto nano = duration.ToNanosecond();
