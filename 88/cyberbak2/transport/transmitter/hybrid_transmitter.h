...
#include "cyber/transport/transmitter/tcp_transmitter.h"
#include "cyber/transport/transmitter/transmitter.h"
#include "cyber/proto/rtps_whitelist_conf.pb.h"
...
using century::cyber::common::WorkRoot;
...
class HybridTransmitter : public Transmitter<M> {
 ...
 private:
  ...
  std::set<std::string> rtps_whitelist_;
};
...
void HybridTransmitter<M>::Enable(const RoleAttributes& opposite_attr) {
  ...
  // Check if the transmitter for this relation exists
  // It may not exist if the channel is not in RTPS whitelist (for DIFF_HOST)
...
bool HybridTransmitter<M>::Transmit(const MessagePtr& msg,
  ...
  for (auto& item : transmitters_) {
    auto recv_iter = receivers_.find(item.first);
    if (recv_iter == receivers_.end() || recv_iter->second.empty()) {
      continue;
    }
...
void HybridTransmitter<M>::ObtainConfig() {
  ...
  const char* whitelist_enabled = ::getenv("CYBER_RTPS_WHITELIST_ENABLED");
  bool enable_whitelist = (whitelist_enabled != nullptr);
  if (enable_whitelist) {
    century::cyber::proto::RtpsWhitelistConf whitelist_conf;
    auto whitelist_file = GetAbsolutePath(WorkRoot(), "/century/cyber/conf/white_list.conf");
    if (PathExists(whitelist_file) && GetProtoFromFile(whitelist_file, &whitelist_conf)) {
      for (const auto& channel : whitelist_conf.whitelist_channels()) {
        rtps_whitelist_.insert(channel);
      }
    }
  }
  ...
  century::cyber::proto::CyberConfig cfg;
  ...
  for (auto& mode : modes) {
    switch (mode) {
      ...
      case OptionalMode::TCP:
        transmitters_[mode] = std::make_shared<TcpTransmitter<M>>(this->attr_);
        break;
      default:
        const std::string& channel_name = this->attr_.channel_name();
        if (!rtps_whitelist_.empty() && rtps_whitelist_.find(channel_name) == rtps_whitelist_.end()) {
          AWARN << "Channel [" << channel_name
                << "] is blacklist from RTPS, only using INTRA/SHM.";
          continue;
        }
