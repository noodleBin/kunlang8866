...
To make life easier, Century maintains Cyber Docker images and a number of
...
**Note: Century team also maintains ARM64 Cyber Docker images which was tested on
...
A Docker container named `century_cyber_$USER` will be started.
...
```bash
./century.sh build cyber
./century.sh test cyber
```

Or run with `--config=opt` for an optimized build/test.

```bash
./century.sh build --config=opt cyber
./century.sh test --config=opt cyber
```
