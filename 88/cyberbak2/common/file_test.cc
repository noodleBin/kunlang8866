...
TEST(FileTest, proto_set_get_test) {
  century::cyber::proto::UnitTest message;
  message.set_class_name("FileTest");
  century::cyber::proto::UnitTest read_message;
...
TEST(FileTest, Glob) {
  ...
  // Match one.
  EXPECT_THAT(Glob("/century"), testing::ElementsAre(std::string("/century")));
  EXPECT_THAT(Glob("/apol?o"), testing::ElementsAre(std::string("/century")));
  // Match multiple.
  EXPECT_THAT(
      Glob("/apol?o/modules/p*"),
      testing::AllOf(
          testing::Contains(std::string("/century/modules/perception")),
          testing::Contains(std::string("/century/modules/planning")),
          testing::Contains(std::string("/century/modules/prediction"))));
}
