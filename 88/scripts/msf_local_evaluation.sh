...
GNSS_LOC_TOPIC="/century/localization/msf_gnss"
LIDAR_LOC_TOPIC="/century/localization/msf_lidar"
FUSION_LOC_TOPIC="/century/localization/pose"
ODOMETRY_LOC_TOPIC="/century/sensor/gnss/odometry"
CLOUD_TOPIC="/century/sensor/velodyne64/compensator/PointCloud2"
...
function data_exporter() {
  local BAG_FILE=$1
  local OUT_FOLDER=$2
  $CENTURY_BIN_PREFIX/modules/localization/msf/local_tool/data_extraction/cyber_record_parser \
    --bag_file $BAG_FILE \
...
function compare_poses() {
  local IN_FOLDER=$1
  $CENTURY_BIN_PREFIX/modules/localization/msf/local_tool/data_extraction/compare_poses \
    --in_folder $IN_FOLDER \
...
python ${CENTURY_ROOT_DIR}/modules/tools/localization/evaluate_compare.py compare_fusion_odometry_all.txt

echo ""
echo "Lidar localization result:"
python ${CENTURY_ROOT_DIR}/modules/tools/localization/evaluate_compare.py compare_lidar_odometry_all.txt

if [ $# -eq 2 ]; then
  echo ""
  echo "GNSS localization result:"
  python ${CENTURY_ROOT_DIR}/modules/tools/localization/evaluate_compare.py compare_gnss_odometry_all.txt distance_only
fi
