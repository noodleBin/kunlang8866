...
function buildify() {
  ...
  build_dirs=$(printf "${CENTURY_ROOT_DIR}/%s " $build_dirs)
  ...
  if [ -f "${CENTURY_ROOT_DIR}/BUILD" ]; then
    ${buildifier_cmd} "${CENTURY_ROOT_DIR}/BUILD"
  fi
  if [ -f "${CENTURY_ROOT_DIR}/WORKSPACE.in" ]; then
    ${buildifier_cmd} "${CENTURY_ROOT_DIR}/WORKSPACE.in"
  fi
}
