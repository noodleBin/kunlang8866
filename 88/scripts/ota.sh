...
CENTURY_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CACHE_DIR="${CENTURY_ROOT}/.cache"

function update() {
  UPDATE_TAG=$(python ${CENTURY_ROOT}/modules/tools/ota/query_client.py)
  if [ "$?" != "0" ]; then
    echo $UPDATE_TAG
    exit 1
  fi
  ...
  cp ${CENTURY_ROOT}/scripts/ota.sh "${CACHE_DIR}"
  ssh $DOCKER_USER@localhost bash ${CACHE_DIR}/ota.sh download $UPDATE_TAG
  python ${CENTURY_ROOT}/modules/tools/ota/verify_client.py
  if [ "$?" != "0" ]; then
    exit 1
  fi

  [ -e "${CACHE_DIR}/century_release" ] && rm -rf "${CACHE_DIR}/century_release"
  tar xzf ${CACHE_DIR}/century_release.tar.gz -C ${CACHE_DIR}
  NEW_TAG="${UPDATE_TAG}-local"

  ssh $DOCKER_USER@localhost bash ${CACHE_DIR}/ota.sh setup $NEW_TAG
  python ${CENTURY_ROOT}/modules/tools/ota/update_client.py ${UPDATE_TAG}
}

function clean() {
  rm -rf ${CACHE_DIR}/century_update
  rm -rf ${CACHE_DIR}/century_release.tar.gz
  rm -rf ${CACHE_DIR}/sec_century_release.tar.gz
...
function setup() {
  docker exec test_container cp -Lr /root/mnt/century_release/century /
...
function download() {
  ...
  docker exec test_container cp /root/sec_century_release.tar.gz /root/mnt
}
