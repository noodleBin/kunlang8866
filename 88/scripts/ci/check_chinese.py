...
def scan_files(file_list, root_dir="."):
    ...
    exclude_file_names = {'skill.md'}
    exclude_file_extensions = {'.plan'}
    exclude_path_patterns = ['docs/specs/*']
    ...
    for file_path in file_list:
        try:
            ...
            if any(part in exclude_dirs for part in path_parts) or any(part.startswith('.') for part in path_parts):
                continue
            if any(fnmatch.fnmatch(rel_path, pattern) for pattern in exclude_path_patterns):
                continue

            file_name = os.path.basename(rel_path).lower()
            if file_name in exclude_file_names:
                continue
            if any(file_name.endswith(ext) for ext in exclude_file_extensions):
                continue

            if fnmatch.fnmatch(file_name, exclude_file_pattern):
                continue
