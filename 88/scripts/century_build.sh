...
source "${TOP_DIR}/scripts/century_base.sh"
...
function _determine_modules_disabled() {
  required_version=9.0.0
  current_version=$(g++ -dumpfullversion -dumpversion 2>/dev/null || g++ -dumpversion)

  if ! version_compare "$current_version" "$required_version"; then
    DISABLED_TARGETS="${DISABLED_TARGETS} except //modules/perception/..."
    DISABLED_TARGETS="${DISABLED_TARGETS} except //modules/prediction/..."
    DISABLED_TARGETS="${DISABLED_TARGETS} except //modules/task_manager/..."
    DISABLED_TARGETS="${DISABLED_TARGETS} except //modules/transform/..."
  fi

  if [ ! -f /.dockerenv ]; then
    # DISABLED_TARGETS="${DISABLED_TARGETS} except //modules/perception/..."
    # DISABLED_TARGETS="${DISABLED_TARGETS} except //modules/prediction/..."
    # DISABLED_TARGETS="${DISABLED_TARGETS} except //modules/localization/..."
    # DISABLED_TARGETS="${DISABLED_TARGETS} except //modules/task_manager/..."
    DISABLED_TARGETS="${DISABLED_TARGETS} except //modules/tools/..."
    DISABLED_TARGETS="${DISABLED_TARGETS} except //modules/storytelling/..."
    DISABLED_TARGETS="${DISABLED_TARGETS} except //modules/guardian/..."
    DISABLED_TARGETS="${DISABLED_TARGETS} except //modules/contrib/..."
    DISABLED_TARGETS="${DISABLED_TARGETS} except //modules/audio/..."
    DISABLED_TARGETS="${DISABLED_TARGETS} except //modules/calibration/..."
    DISABLED_TARGETS="${DISABLED_TARGETS} except //modules/third_party_perception/..."
    DISABLED_TARGETS="${DISABLED_TARGETS} except //modules/v2x/..."
  fi
}
...
function determine_build_targets() {
  ...
  for component in $@; do
    ...
    if [ "${component}" = "cyber" ]; then
      ...
      error "Directory <CENTURY_ROOT_DIR>/modules/${component} not found. Exiting ..."
      exit 1
    fi
...
function run_bazel_build() {
  ...
  if [[ ! "$(uname -m)" =~ "x86_64" ]]; then
    job_args="${job_args} --copt=-fPIC --host_copt=-fPIC"
  fi
