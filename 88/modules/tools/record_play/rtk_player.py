...
class RtkPlayer(object):
    ...
    def __init__(self, record_file, node, speedmultiplier, completepath,
        ...
        self.planning_pub = node.create_writer('/century/planning',
        ...
        proto_utils.get_pb_from_text_file(
            "/century/modules/common/data/vehicle_param.pb.txt", vehicle_config)
...
def main():
    ...
    Logger.config(
        log_file=os.path.join(CENTURY_ROOT, 'data/log/rtk_player.log'),
        use_stdout=True,
        log_level=logging.DEBUG)

    record_file = os.path.join(CENTURY_ROOT, 'data/log/garage.csv')

    player = RtkPlayer(record_file, node, args['speedmulti'],
                       args['complete'].lower(), args['replan'].lower())
    atexit.register(player.shutdown)

    node.create_reader('/century/canbus/chassis', chassis_pb2.Chassis,
                       player.chassis_callback)

    node.create_reader('/century/localization/pose',
                       localization_pb2.LocalizationEstimate,
                       player.localization_callback)

    node.create_reader('/century/control/pad', pad_msg_pb2.PadMessage,
