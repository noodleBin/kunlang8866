...
#include "cyber/common/scheduler_trace.h"
...
static void fill_lane_msg(const base::LaneLineCubicCurve &curve_coord,
                          century::perception::LaneMarker *lane_marker) {
...
static bool LoadExtrinsics(const std::string &yaml_file,
  ...
  if (!century::cyber::common::PathExists(yaml_file)) {
...
bool CameraObstacleDetectionComponent::Init() {
  ...
  camera_debug_writer_ =
      node_->CreateWriter<century::perception::camera::CameraDebug>(
  ...
  ACHECK(visualize_.InitAllInfoSingleCamera(
...
void CameraObstacleDetectionComponent::OnReceiveImage(
    const std::shared_ptr<century::drivers::Image> &message,
  ...
  // protobuf msg
  std::shared_ptr<century::perception::PerceptionObstacles> out_message(
      new (std::nothrow) century::perception::PerceptionObstacles);
  century::common::ErrorCode error_code = century::common::OK;
...
int CameraObstacleDetectionComponent::InitConfig() {
  ...
  century::perception::onboard::FusionCameraDetection
...
int CameraObstacleDetectionComponent::InitCameraListeners() {
  for (size_t i = 0; i < camera_names_.size(); ++i) {
    ...
    typedef std::shared_ptr<century::drivers::Image> ImageMsgType;
...
int CameraObstacleDetectionComponent::InitMotionService() {
  const std::string &channel_name_local = "/century/perception/motion_service";
...
int CameraObstacleDetectionComponent::InternalProc(
    const std::shared_ptr<century::drivers::Image const> &in_message,
    const std::string &camera_name, century::common::ErrorCode *error_code,
    SensorFrameMessage *prefused_message,
    century::perception::PerceptionObstacles *out_message) {
  ...
  if (!camera2world_trans_wrapper_map_[camera_name]->GetSensor2worldTrans(
    ...
    const std::string err_str =
        absl::StrCat("failed to get camera to world pose, ts: ", msg_timestamp,
                     " camera_name: ", camera_name);
    AERROR << err_str;
    *error_code = century::common::ErrorCode::PERCEPTION_ERROR_TF;
  ...
  if (!camera_obstacle_pipeline_->Perception(camera_perception_options_,
    ...
    *error_code = century::common::ErrorCode::PERCEPTION_ERROR_PROCESS;
    prefused_message->error_code_ = *error_code;
    return cyber::FAIL;
  }
  ...
  if (output_final_obstacles_ &&
    ...
    *error_code = century::common::ErrorCode::PERCEPTION_ERROR_UNKNOWN;
    prefused_message->error_code_ = *error_code;
    return cyber::FAIL;
  }
  *error_code = century::common::ErrorCode::OK;
  prefused_message->error_code_ = *error_code;
  ...
  if (output_camera_debug_msg_) {
    // camera debug msg
    std::shared_ptr<century::perception::camera::CameraDebug> camera_debug_msg(
        new (std::nothrow) century::perception::camera::CameraDebug);
...
int CameraObstacleDetectionComponent::MakeProtobufMsg(
    ...
    const century::common::ErrorCode error_code,
    century::perception::PerceptionObstacles *obstacles) {
  double publish_time = Clock::NowInSeconds();
  century::common::Header *header = obstacles->mutable_header();
  ...
  for (const auto &obj : objects) {
    century::perception::PerceptionObstacle *obstacle =
  ...
  century::perception::LaneMarkers *lane_markers =
      obstacles->mutable_lane_marker();
  century::perception::LaneMarker *lane_marker_l0 =
      lane_markers->mutable_left_lane_marker();
  century::perception::LaneMarker *lane_marker_r0 =
      lane_markers->mutable_right_lane_marker();
  century::perception::LaneMarker *lane_marker_l1 =
      lane_markers->add_next_left_lane_marker();
  century::perception::LaneMarker *lane_marker_r1 =
...
int CameraObstacleDetectionComponent::ConvertObjectToPb(
    ...
    century::perception::PerceptionObstacle *pb_msg) {
  ...
  century::common::Point3D *obj_center = pb_msg->mutable_position();
  obj_center->set_x(object_ptr->center(0));
  obj_center->set_y(object_ptr->center(1));
  obj_center->set_z(object_ptr->center(2));

  century::common::Point3D *obj_velocity = pb_msg->mutable_velocity();
  ...
  for (int i = 0; i < polygon_point_size; ++i) {
    ...
    century::common::Point3D *p = pb_msg->add_polygon_point();
    p->set_x(pt.x);
    p->set_y(pt.y);
    p->set_z(pt.z);
  }
  ...
  century::common::Point3D *obj_anchor_point = pb_msg->mutable_anchor_point();
  ...
  century::perception::BBox2D *obj_bbox2d = pb_msg->mutable_bbox2d();
  ...
  if (object_ptr->type == base::ObjectType::VEHICLE) {
    century::perception::LightStatus *light_status =
...
int CameraObstacleDetectionComponent::ConvertObjectToCameraObstacle(
    const base::ObjectPtr &object_ptr,
    century::perception::camera::CameraObstacle *camera_obstacle) {
  ...
  camera_obstacle->set_type(
      static_cast<century::perception::camera::CameraObstacle::CameraType>(
          object_ptr->type));
  ...
  type_score_msg
      << century::perception::camera::CameraObstacle::CameraType_Name(
...
int CameraObstacleDetectionComponent::ConvertLaneToCameraLaneline(
    ...
    century::perception::camera::CameraLaneLine *camera_laneline) {
  if (camera_laneline == nullptr) {
    AERROR << "camera_laneline is not available";
    return false;
  }
  // fill the lane line attribute
  century::perception::camera::LaneLineType line_type =
      static_cast<century::perception::camera::LaneLineType>(lane_line.type);
  camera_laneline->set_type(line_type);
  century::perception::camera::LaneLinePositionType pos_type =
      static_cast<century::perception::camera::LaneLinePositionType>(
          lane_line.pos_type);
  ...
  century::perception::camera::LaneLineUseType use_type =
      static_cast<century::perception::camera::LaneLineUseType>(
          lane_line.use_type);
  ...
  for (size_t i = 0; i < lane_line.curve_image_point_set.size(); i++) {
    ...
    century::common::Point2D *lane_point_2d =
  ...
  for (size_t i = 0; i < lane_line.curve_camera_point_set.size(); i++) {
    ...
    century::common::Point3D *lane_point_3d =
  ...
  for (size_t i = 0; i < lane_line.image_end_point_set.size(); i++) {
    ...
    century::perception::camera::EndPoints *lane_end_points =
...
int CameraObstacleDetectionComponent::MakeCameraDebugMsg(
    ...
    century::perception::camera::CameraDebug *camera_debug_msg) {
  ...
  // Fill header info.
  century::common::Header *header = camera_debug_msg->mutable_header();
