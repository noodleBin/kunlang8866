...
class TrafficLightsPerceptionComponent : public century::cyber::Component<> {
 public:
 ...
 private:
  ...
  int InitCameraFrame();
  int InitV2XListener();
  void OnReceiveImage(const std::shared_ptr<century::drivers::Image> image,
                      const std::string& camera_name);
  void OnReceiveV2XMsg(
      const std::shared_ptr<century::v2x::IntersectionTrafficLightData>
          v2x_msg);
  ...
  void TransRect2Box(const base::RectI& rect,
                     century::perception::TrafficLightBox* box);
 ...
 private:
  ...
  century::perception::map::HDMapInput* hd_map_ = nullptr;
  ...
  std::vector<century::hdmap::Signal> last_signals_;
  // image info.
  ...
  ::google::protobuf::RepeatedPtrField<century::hdmap::Curve> stoplines_;
  // msg channel name
  ...
  std::shared_ptr<
      ...
      writer_;
  // traffic lights
  century::perception::base::TLColor detected_trafficlight_color_;
  double cnt_r_;
  double cnt_g_;
  double cnt_y_;
  double cnt_u_;
  ...
  boost::circular_buffer<century::v2x::IntersectionTrafficLightData>
      v2x_msg_buffer_;
};
