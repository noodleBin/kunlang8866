...
bool DetectionComponent::Proc(
  ...
  AINFO << std::setprecision(16)
        ...
        << ", header timestamp: " << message->header().timestamp_sec()
...
bool DetectionComponent::InitAlgorithmPlugin() {
  ...
  AERROR << "InitAlgorithmPlugin sensor_info_ " << sensor_info_.name;
...
bool DetectionComponent::InternalProc(
  ...
  if (!lidar2world_trans_.GetSensor2worldTrans(lidar_query_tf_timestamp, &pose,
                                               &pose_novatel)) {
    out_message->error_code_ = century::common::ErrorCode::PERCEPTION_ERROR_TF;
  ...
  if (ret.error_code != lidar::LidarErrorCode::Succeed) {
    out_message->error_code_ =
        century::common::ErrorCode::PERCEPTION_ERROR_PROCESS;
