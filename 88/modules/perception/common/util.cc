...
using century::cyber::common::GetEnv;
...
std::string GetCommonConfigFile(const std::string &config_file) {
  ...
  if (!century::cyber::common::PathIsAbsolute(config_file)) {
    // If config_file is relative, then prepend "modules/" to it.
    relative_config_path = "modules/perception/data/conf/" + config_file;
  }
  if (!century::cyber::common::GetFilePathWithEnv(
          relative_config_path, "century_CONF_PATH", &actual_config_path)) {
    AERROR << "Failed to get config path of " << config_file << " please check "
           << "if config exists or `century_CONF_PATH` environment variable has "
           << "been set correctly.";
  }
...
std::string GetModelPath(const std::string &model_name) {
  ...
  if (!century::cyber::common::GetFilePathWithEnv(
          model_name, "century_MODEL_PATH", &model_path)) {
    AERROR << "Failed to get model path of " << model_name << " please check "
           << "if model has been installed or `century_MODEL_PATH` environment "
...
std::string GetConfigPath(const std::string &config_path) {
  ...
  if (!century::cyber::common::PathIsAbsolute(config_path)) {
...
std::string GetConfigFile(const std::string &config_path,
  ...
  std::string file_path =
      century::cyber::common::GetAbsolutePath(absolute_config_path, config_file);
  return file_path;
}
