...
PangolinViewer::PangolinViewer(const int& width, const int& height,
    ...
    : window_width_(width),
      ...
      grid_viz_(grid_viz),
      ego_vehicle_size_(vehicle_length, vehicle_width, vehicle_height) {
...
void PangolinViewer::UploadToGPU() {
  ...
  for (auto& p : points_local_) {
    ...
    if (p.intensity > 4.5f) {
      // ray source marker - bright white
      buf.emplace_back(1.0f);
      buf.emplace_back(1.0f);
      buf.emplace_back(1.0f);
    } else if (p.intensity > 1.5f) {
      // freespace mask points
      buf.emplace_back(0.0f);
      buf.emplace_back(0.6f);
      buf.emplace_back(1.0f);
    } else if (p.intensity > 0.5f) {
      // raw/non-ground points
      buf.emplace_back(0.0f);
      buf.emplace_back(1.0f);
      buf.emplace_back(0.0f);
    } else {
...
void PangolinViewer::CalculatePolygonToEgoDistances(
  ...
  const float half_length = ego_vehicle_size_.x() * 0.5f;
  const float half_width = ego_vehicle_size_.y() * 0.5f;
...
void PangolinViewer::DrawEgoBBOX(const Eigen::Quaternionf& q) {
  BBox box;
  box.center << 0, 0, ego_vehicle_size_.z() * 0.5f;
  box.size << ego_vehicle_size_.x(), ego_vehicle_size_.y(),
      ego_vehicle_size_.z();
  box.q = q;
  box.id = -1;
  DrawBBox(box);
...
void PangolinViewer::DrawBBox(const BBox& bb) {
  ...
  constexpr int kBoxEdgeCount = 12;
  const int edges[kBoxEdgeCount][2] = {
      {0, 1}, {0, 2}, {0, 4}, {7, 5}, {7, 6}, {7, 3},
      {1, 3}, {1, 5}, {2, 3}, {2, 6}, {4, 5}, {4, 6}};
  float color[3] = {1.0f, 0.0f, 0.0f};
  glColor3f(color[0], color[1], color[2]);
  glBegin(GL_LINES);
  for (int edge_idx = 0; edge_idx < kBoxEdgeCount; ++edge_idx) {
    auto point_a = wc[edges[edge_idx][0]];
    auto point_b = wc[edges[edge_idx][1]];
...
void PangolinViewer::DrawPolygons(PolygonVector& polygons) {
  for (const auto& polygon : polygons) {
    ...
    const float tag = polygon[0].intensity;
    const bool is_right_corridor = tag > 3.5f;
    const bool is_left_corridor = !is_right_corridor && tag > 2.5f;
    const bool is_freespace_polygon =
        !is_left_corridor && !is_right_corridor && tag > 1.5f;
    if (is_left_corridor) {
      glColor3f(0.0f, 1.0f, 1.0f);  // cyan
    } else if (is_right_corridor) {
      glColor3f(1.0f, 0.0f, 1.0f);  // magenta
    } else if (is_freespace_polygon) {
      glColor3f(1.0f, 1.0f, 0.0f);  // yellow
    } else {
      glColor3f(1.0f, 0.0f, 0.0f);  // red
    }
    glLineWidth((is_left_corridor || is_right_corridor || is_freespace_polygon)
                    ? 2.5f
                    : 2.0f);

    glBegin((is_left_corridor || is_right_corridor) ? GL_LINE_STRIP
                                                    : GL_LINE_LOOP);
    for (const auto& point : polygon) {
      glVertex3f(point.x, point.y, point.z);
    }
    ...
    if (!is_freespace_polygon && !is_left_corridor && !is_right_corridor) {
      pangolin::GlFont& font = pangolin::default_font();
      glColor3f(1.0f, 1.0f, 1.0f);
      const std::string text_type = kObjectTypeIntToStringMap.at(0);
      font.Text(text_type).Draw(polygon[0].x, polygon[0].y, polygon[0].z);
    }
  }
}
...
void PangolinViewer::DrawMap(MapInfo& maps_info) {
  for (const auto& polygon : maps_info) {
    ...
    const float tag = polygon[0].intensity;
    const bool is_road_polygon = tag > 9.5f && tag < 10.5f;
    const bool is_railway_polygon = tag > 12.5f && tag < 13.5f;

    if (is_road_polygon) {
      glColor3f(0.0f, 0.2f, 1.0f);
      glLineWidth(2.0f);
    } else if (is_railway_polygon) {
      glColor3f(1.0f, 0.0f, 0.0f);
      glLineWidth(3.0f);
    } else {
      glColor3f(0.0f, 0.0f, 1.0f);
