...
inline int64_t GetGridKey(int32_t x, int32_t y) {
...
bool PointCloudPreprocessor::Init(
  ...
  filter_outlier_points_ = config_.filter_outlier_points();
  outlier_search_radius_ = config_.outlier_search_radius();
  outlier_min_neighbors_ = config_.outlier_min_neighbors();
...
inline void PointCloudPreprocessor::TransformPoint(
...
void PointCloudPreprocessor::FilterOutlierCandidates(
    const std::vector<CloudCandidate>& candidates,
    std::vector<uint8_t>* keep) const noexcept {
  keep->assign(candidates.size(), 1);
  if (!filter_outlier_points_ || outlier_search_radius_ <= 0.0f ||
      outlier_min_neighbors_ <= 0 ||
      candidates.size() <= static_cast<size_t>(outlier_min_neighbors_)) {
    return;
  }

  const float cell_size = outlier_search_radius_;
  const float radius_square = outlier_search_radius_ * outlier_search_radius_;
  std::unordered_map<int64_t, std::vector<size_t>> grid;
  grid.reserve(candidates.size());

  for (size_t i = 0; i < candidates.size(); ++i) {
    const auto& pt = candidates[i].point;
    const int32_t cell_x = static_cast<int32_t>(std::floor(pt.x / cell_size));
    const int32_t cell_y = static_cast<int32_t>(std::floor(pt.y / cell_size));
    grid[GetGridKey(cell_x, cell_y)].emplace_back(i);
  }

  for (size_t i = 0; i < candidates.size(); ++i) {
    const auto& query = candidates[i].point;
    const int32_t cell_x =
        static_cast<int32_t>(std::floor(query.x / cell_size));
    const int32_t cell_y =
        static_cast<int32_t>(std::floor(query.y / cell_size));
    int neighbor_count = 0;

    for (int32_t dx = -1; dx <= 1; ++dx) {
      for (int32_t dy = -1; dy <= 1; ++dy) {
        const auto iter = grid.find(GetGridKey(cell_x + dx, cell_y + dy));
        if (iter == grid.end()) {
          continue;
        }
        for (const size_t candidate_index : iter->second) {
          if (candidate_index == i) {
            continue;
          }
          const auto& neighbor = candidates[candidate_index].point;
          const float diff_x = query.x - neighbor.x;
          const float diff_y = query.y - neighbor.y;
          const float diff_z = query.z - neighbor.z;
          const float distance_square =
              diff_x * diff_x + diff_y * diff_y + diff_z * diff_z;
          if (distance_square <= radius_square &&
              ++neighbor_count >= outlier_min_neighbors_) {
            break;
          }
        }
        if (neighbor_count >= outlier_min_neighbors_) {
          break;
        }
      }
      if (neighbor_count >= outlier_min_neighbors_) {
...
void PointCloudPreprocessor::FilterPointCloud(LidarFrame* frame) noexcept {
  ...
  candidates.reserve(non_ground_indices.size());
  ...
  for (size_t i = 0; i < non_ground_indices.size(); ++i) {
    ...
    base::PointF pt;
    pt.x = (*incloud)[pt_index].x;
    pt.y = (*incloud)[pt_index].y;
    pt.z = (*incloud)[pt_index].z;
    pt.intensity = (*incloud)[pt_index].intensity;
    ...
    candidate.point = pt;
    candidate.timestamp = static_cast<double>(timestamp);
    candidate.origin_index = pt_index;
    candidate.cloud_index = i;
  }

  std::vector<uint8_t> keep;
  FilterOutlierCandidates(candidates, &keep);
  ...
  for (size_t i = 0; i < candidates.size(); ++i) {
    ...
    const auto& pt = candidate.point;

    frame->cloud->push_back(pt, candidate.timestamp,
                            std::numeric_limits<float>::max(),
                            candidate.cloud_index, 0);
    origin_points_indices_.emplace_back(candidate.origin_index);
