...
static __global__ void decode_kernel(unsigned int num, const half* reg, const half* height, const half* dim, const half* rot,
  ...
  if (xs < param.post_center_range_start.x ||
      xs > param.post_center_range_end.x) {
    return;
  }
  if (ys < param.post_center_range_start.y ||
      ys > param.post_center_range_end.y) {
    return;
  }
  ...
  if (zs < param.post_center_range_start.z ||
      zs > param.post_center_range_end.z) {
    return;
  }
 ...
 public:
  ...
  virtual ~TransBBoxImplement() {
    for (size_t i = 0; i < bindings_.size(); ++i) {
      checkRuntime(cudaFree(bindings_[i]));
    }
  ...
  void create_binding_memory() {
    ...
    for (size_t i = 0; i < sizeof(bindings) / sizeof(bindings[0]); ++i) {
      if (engine_->is_input(bindings[i])) {
        continue;
      }
  ...
  virtual std::vector<BoundingBox> forward(const nvtype::half* transfusion_feature, float confidence_threshold, void* stream,
    ...
    if (enable_debug_) {
      century::perception::lidar::BevDebugUtils dbg;
      dbg.SetOutputDir("/century/data/bevfusion_debug/");
      dbg.SetEnabled(true);
      // Head outputs are 200 transformer queries. mmdet3d's argsort over
      // heatmap and TRT TopK use different tie-break rules, so the query
      // ordering can differ even when the selected positions coincide. A
      // direct index-wise compare would report huge false errors on reg/
      // height/dim/rot while the model is actually aligned. Compare by
      // matching each python query to its nearest c++ query in BEV (x,y).
      dbg.CompareHeadByQueryMatch(bindings_[0], bindings_[1], bindings_[2],
                                  bindings_[3], bindings_[4], bindings_[5],
                                  _stream);
    }
