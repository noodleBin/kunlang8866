...
__global__ void predictKernel(int N, int H, int W, int C_reg, int C_height, int C_dim, int C_rot,
                              int C_vel, int C_hm, int task_index, const half* reg, const half* height,
                              const half* dim, const half* rot, const half* vel, const half* hm,
                              unsigned int* detection_num, float* detections,
                              const float* post_center_range, float out_size_factor,
                              const float* voxel_size, const float* pc_range,
                              const float* score_threshold, const int* task_stride) {
  ...
  for (int n = 0; n < N; n++) {
    ...
    if (score < score_threshold[label + task_stride[task_index]]) continue;
    ...
    auto rs = atan2(__half2float(rot[n * C_rot * HW + h * W + w]),
                    __half2float(rot[n * C_rot * HW + HW + h * W + w]));

    *(float3*)(&detections[n * MAX_DET_NUM * DET_CHANNEL + DET_CHANNEL * curDet + 0]) =
        make_float3(xs, ys, zs);
    ...
    *(float3*)(&detections[n * MAX_DET_NUM * DET_CHANNEL + DET_CHANNEL * curDet + 6]) =
        make_float3(rs, label, score);
  }
}
...
__device__ inline bool intersection(const float2 p1, const float2 p0, const float2 q1,
...
__device__ inline void rotate_around_center(const float2& center, const float angle_cos,
...
__device__ inline bool devIoU(float const* const box_a, float const* const box_b,
...
__global__ void nms_cuda(const int n_boxes, const int task_index, const int cls_number,
                         const float* iou_threshold, const int* task_stride,
  ...
  if (tid < col_size) {
    block_boxes[tid * 8 + 0] =
        dev_boxes[(NMS_THREADS_PER_BLOCK * col_start + tid) * DET_CHANNEL + 0];
    block_boxes[tid * 8 + 1] =
        dev_boxes[(NMS_THREADS_PER_BLOCK * col_start + tid) * DET_CHANNEL + 1];
    block_boxes[tid * 8 + 2] =
        dev_boxes[(NMS_THREADS_PER_BLOCK * col_start + tid) * DET_CHANNEL + 2];
    block_boxes[tid * 8 + 3] =
        dev_boxes[(NMS_THREADS_PER_BLOCK * col_start + tid) * DET_CHANNEL + 3];
    block_boxes[tid * 8 + 4] =
        dev_boxes[(NMS_THREADS_PER_BLOCK * col_start + tid) * DET_CHANNEL + 4];
    block_boxes[tid * 8 + 5] =
        dev_boxes[(NMS_THREADS_PER_BLOCK * col_start + tid) * DET_CHANNEL + 5];
    block_boxes[tid * 8 + 6] =
        dev_boxes[(NMS_THREADS_PER_BLOCK * col_start + tid) * DET_CHANNEL + 6];
    block_boxes[tid * 8 + 7] =
        dev_boxes[(NMS_THREADS_PER_BLOCK * col_start + tid) * DET_CHANNEL + 7];
  ...
  if (tid < row_size) {
    ...
    // label type same as type decode, center point copy int type to float, not the binary representation of int
    int cur_box_cls_id = static_cast<int>(cur_box[7]);
    int i = 0;
    uint64_t t = 0;
    int start = 0;
    ...
    for (i = start; i < col_size; i++) {
      int block_boxes_cls_id = static_cast<int>(block_boxes[i * 8 + 7]) + task_stride[task_index];
      if (devIoU(cur_box, block_boxes + i * 8, iou_threshold[cur_box_cls_id * cls_number + block_boxes_cls_id])) {
        t |= 1ULL << i;
      }
    }
...
int postprocess_launch(int N, int H, int W, int C_reg, int C_height, int C_dim, int C_rot,
                       ...
                       const int* task_stride, cudaStream_t stream) {
  dim3 threads(32);
  dim3 blocks(H * W + threads.x - 1 / threads.x);

  predictKernel<<<blocks, threads, 0, stream>>>(N, H, W, C_reg, C_height, C_dim, C_rot, C_vel, C_hm,
                                                task_index, reg, height, dim, rot, vel, hm, detection_num,
                                                detections, post_center_range, out_size_factor,
                                                voxel_size, pc_range, score_threshold, task_stride);
...
int nms_launch(unsigned int boxes_num, const int task_index, const int cls_number,
  ...
  nms_cuda<<<blocks, threads, 0, stream>>>(boxes_num, task_index, cls_number, nms_iou_threshold, task_stride, boxes_sorted, mask);
  cudaStreamSynchronize(stream);
...
__global__ void permute_cuda(const float* boxes_sorted, float* permute_boxes_sorted,
                             int boxes_num) {
  const int tid = threadIdx.x;
  if (tid < boxes_num) {
    ...
    permute_boxes_sorted[tid * DET_CHANNEL + 7] = boxes_sorted[tid * DET_CHANNEL + 7];
  }
}
