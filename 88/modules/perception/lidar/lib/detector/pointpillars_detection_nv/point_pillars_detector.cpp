...
using century::cyber::common::PathExists;
...
std::string ResolvePointPillarsX86Plan(const std::string& engine_dir) {
  const std::string plan_40x = engine_dir + "pointpillar_x86_40x.plan";
  const std::string plan_50x = engine_dir + "pointpillar_x86_50x.plan";
  const bool has_plan_40x = PathExists(plan_40x);
  const bool has_plan_50x = PathExists(plan_50x);

  cudaDeviceProp prop;
  if (cudaGetDeviceProperties(&prop, FLAGS_gpu_id) != cudaSuccess) {
    AERROR << "Failed to query gpu " << FLAGS_gpu_id
           << ", can not select x86 PointPillars engine automatically.";
    if (has_plan_50x && !has_plan_40x) {
      AWARN << "Fallback to the only available x86 engine: " << plan_50x;
      return plan_50x;
    }
    if (has_plan_40x && !has_plan_50x) {
      AWARN << "Fallback to the only available x86 engine: " << plan_40x;
      return plan_40x;
    }
    return "";
  }

  const bool is_50_series_gpu = prop.major >= 12;
  if (is_50_series_gpu && has_plan_50x) {
    return plan_50x;
  }
  if (!is_50_series_gpu && has_plan_40x) {
    return plan_40x;
  }

  if (is_50_series_gpu && has_plan_40x) {
    AWARN << "50 series engine is missing, fallback to 40 series plan: "
          << plan_40x;
    return plan_40x;
  }
  if (!is_50_series_gpu && has_plan_50x) {
    AWARN << "40 series engine is missing, fallback to 50 series plan: "
          << plan_50x;
    return plan_50x;
  }

  AERROR << "No available PointPillars x86 engine found under: " << engine_dir;
  return "";
}
...
#else
  param.lidar_model = ResolvePointPillarsX86Plan(FLAGS_pointpillars_engine_path);
  if (param.lidar_model.empty()) {
    AERROR << "Failed to resolve x86 PointPillars engine under: "
           << FLAGS_pointpillars_engine_path;
    return nullptr;
  }
#endif
