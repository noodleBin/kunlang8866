...
static __global__ void generateVoxels_random_kernel(
    const float *points, size_t points_size, float intensity_threshold, float min_x_range,
    float max_x_range, float min_y_range, float max_y_range, float min_z_range,
    float max_z_range, float pillar_x_size, float pillar_y_size,
    float pillar_z_size, int grid_y_size, int grid_x_size, unsigned int *mask,
    float *voxels) {
...
cudaError_t generateVoxels_random_launch(
  ...
  generateVoxels_random_kernel<<<blocks, threads, 0, stream>>>(
      points, points_size, intensity_threshold, min_x_range, max_x_range, 
      min_y_range, max_y_range, min_z_range, max_z_range, pillar_x_size, 
      pillar_y_size, pillar_z_size, grid_y_size, grid_x_size, mask, voxels);
...
static __global__ void generateFeatures_kernel(
    ...
    float range_min_x, float range_min_y, float range_min_z, half *features) {
  ...
  if (threadIdx.x < WARPS_PER_BLOCK) {
    pointsNumSM[threadIdx.x] =
        voxel_num[blockIdx.x * WARPS_PER_BLOCK + threadIdx.x];
    idxsSM[threadIdx.x] =
        ((uint4 *)voxel_idxs)[blockIdx.x * WARPS_PER_BLOCK + threadIdx.x];
...
nvtype::Int3 VoxelizationParameter::compute_grid_size(
  ...
  nvtype::Int3 size;
  size.x =
      static_cast<int>(std::round((max_range.x - min_range.x) / voxel_size.x));
  size.y =
      static_cast<int>(std::round((max_range.y - min_range.y) / voxel_size.y));
  size.z =
      static_cast<int>(std::round((max_range.z - min_range.z) / voxel_size.z));
  return size;
}

cudaError_t generateFeatures_launch(
    float *voxel_features, unsigned int *voxel_num, unsigned int *voxel_idxs,
    unsigned int *params, unsigned int max_voxels, float voxel_x, float voxel_y,
    float voxel_z, float range_min_x, float range_min_y, float range_min_z,
    nvtype::half *features, cudaStream_t stream) {
  ...
  generateFeatures_kernel<<<blocks, threads, 0, stream>>>(
      voxel_features, voxel_num, voxel_idxs, params, voxel_x, voxel_y, voxel_z,
      range_min_x, range_min_y, range_min_z, (half *)features);
...
class VoxelizationImplement : public Voxelization {
 public:
  ...
  bool init(VoxelizationParameter param) {
    ...
    mask_size_ = param_.grid_size.z * param_.grid_size.y * param_.grid_size.x *
                 sizeof(unsigned int);
    voxels_size_ = param_.grid_size.z * param_.grid_size.y *
                   param_.grid_size.x * param_.max_points_per_voxel * 4 *
                   sizeof(float);
    voxel_features_size_ =
        param_.max_voxels * param_.max_points_per_voxel * 4 * sizeof(float);
    voxel_num_size_ = param_.max_voxels * sizeof(unsigned int);
    voxel_idxs_size_ = param_.max_voxels * 4 * sizeof(unsigned int);
    features_input_size_ = param_.max_voxels * param_.max_points_per_voxel *
    ...
    checkRuntime(
        cudaMalloc((void **)&voxelsList_, param_.max_points * sizeof(int)));
  ...
  virtual void forward(const float *_points, int num_points,
                       void *stream) override {
    cudaStream_t _stream = reinterpret_cast<cudaStream_t>(stream);

    checkRuntime(
        cudaMemsetAsync(params_input_, 0, sizeof(unsigned int), _stream));

    checkRuntime(generateVoxels_random_launch(
        _points, num_points, param_.intensity_threshold, param_.min_range.x, 
        param_.max_range.x, param_.min_range.y, param_.max_range.y, 
        param_.min_range.z, param_.max_range.z, param_.voxel_size.x, 
        param_.voxel_size.y, param_.voxel_size.z, param_.grid_size.y, 
        param_.grid_size.x, mask_, voxels_, _stream));

    checkRuntime(generateBaseFeatures_launch(
        mask_, voxels_, param_.grid_size.y, param_.grid_size.x, params_input_,
        voxel_features_, voxel_num_, voxel_idxs_, _stream));

    checkRuntime(generateFeatures_launch(
        voxel_features_, voxel_num_, voxel_idxs_, params_input_,
        param_.max_voxels, param_.voxel_size.x, param_.voxel_size.y,
        param_.voxel_size.z, param_.min_range.x, param_.min_range.y,
        param_.min_range.z, features_input_, _stream));
  }
