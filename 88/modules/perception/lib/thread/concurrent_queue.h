...
class FixedSizeConQueue : public ConcurrentQueue<Data> {
 public:
  ...
  virtual void PushBack(const Data &data) {
  ...
  virtual void Pop(Data *data) {
    ...
    while (this->queue_.empty() && !this->stop_) {
  ...
  virtual bool TryPeek(Data *data) {
    MutexLock lock(&this->mutex_);
