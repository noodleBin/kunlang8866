...
class MlfEngine : public BaseMultiTargetTracker {
 ...
 protected:
  ...
  void DumpMatchResult(const std::vector<TrackedObjectPtr>& objects, 
   ...
   * @param debug_info
   */
  void SetDebugInfo(const std::vector<std::shared_ptr<base::Object>>& debug_info);
  ...
  //      const std::vector<MlfTrackDataPtr>& tracks);
    ...
    /**
   * @brief process special area
   *
   */
  void ProcessSpecialArea(LidarFrame* frame);
 ...
 protected:
  ...
  bool print_debug_log_ = true;
  double delay_output_ = 0.3;
  size_t pub_track_times_ = 1;
  bool dump_match_result_ = false;
  bool dump_polygon_point_ = false;
  std::vector<SpecialAreaConfig> special_area_configs_;
};
