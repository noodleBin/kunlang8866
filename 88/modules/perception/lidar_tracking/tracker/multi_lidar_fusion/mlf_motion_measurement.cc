...
void MlfMotionMeasurement::MeasurementSelection(
    ...
    bool condition) {
  if((new_object->object_ptr) && (new_object->object_ptr->lidar_supplement.cloud.empty())) {
...
void MlfMotionMeasurement::MeasurementRefine(
    ...
    if (cur_vel.head<2>().norm() > velocity_threshold) {
        if(track_data->GetLatestObject().second) {
           AINFO << static_cast<int>(track_data->GetLatestObject().second->type) <<" "
                 << track_data->track_id_ <<" speed is " << cur_vel.head<2>().norm() << std::endl;
    ...
    if (history_objects.size() < get_history_size) {
        if (latest_object->output_velocity.head<2>().norm() < STATIC_VELOCITY
            ...
            //bugfix: believe current velocity
            //new_object->selected_measured_velocity =
