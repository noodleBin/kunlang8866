  ...
  // Check whether track is static or not
    ...
    if (type_select == base::ObjectType::PEDESTRIAN ||
        ...
        velocity_noise_level_2 = speed < (cyc_refine_speed_ / 2.0);
              ...
              (type_select == base::ObjectType::FORKLIFT_STACKER) ||
              (type_select == base::ObjectType::STACKER) ||
              (type_select == base::ObjectType::WHEELCRANE)) {
    ...
    if(type_select == base::ObjectType::UNKNOWN) {
      // unknown type object not propose theta
      velocity_heading_angle = 0.0;
    }
...
bool MlfMotionRefiner::CheckStaticViaVelocityConviance(
  const std::map<double, TrackedObjectPtr>& history_objects,
  const TrackedObjectConstPtr& new_object) const {
    const int min_history_size = 4;
    if(history_objects.size() < min_history_size) {
      return false;
    }
    Eigen::Vector2d velocity_mean = Eigen::Vector2d::Zero();
    for(const auto& history_object : history_objects) {
      const TrackedObjectPtr& history_object_ptr = history_object.second; 
      velocity_mean += history_object_ptr->output_velocity.head<2>();
    }
    velocity_mean = velocity_mean / history_objects.size();

    const double low_velocity_threshold = 0.1;
    if(velocity_mean.norm() < low_velocity_threshold) {
      ADEBUG << "track_id: " << new_object->track_id
             << " is static because of velocity mean.";
      return true;
    }

    if((velocity_mean.norm() < low_velocity_threshold * 2) && 
       (new_object->output_velocity.head<2>().norm()  < low_velocity_threshold * 4) && 
       (new_object->output_velocity.head<2>().norm()  > velocity_mean.norm() * 4)) {
      ADEBUG << "track_id: " << new_object->track_id
             << " is static because of velocity convience.";
      return true;
    }
    return false;
  }
