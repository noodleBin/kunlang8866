...
using PointFCloud = century::perception::base::AttributePointCloud<base::PointF>;
...
void TrackedObject::AttachObject(base::ObjectPtr obj_ptr,
  ...
  if (obj_ptr) {
    ...
    // xiaxinrong begin
    if(cloud.empty()){
      anchor_point = pose * obj_ptr->anchor_point;
      barycenter = anchor_point;
    }
    polygon_points.reserve(object_ptr->polygon.size());
    for (size_t i = 0; i < object_ptr->polygon.size(); ++i) {

      polygon_points.push_back(pose * Eigen::Vector3d(object_ptr->polygon.at(i).x, 
                               object_ptr->polygon.at(i).y, object_ptr->polygon.at(i).z));
    }
    //end

    // object-detection center
    float detection_center_x = obj_ptr->center(0);
    float detection_center_y = obj_ptr->center(1);
    float detection_center_z = obj_ptr->center(2);
    float detection_size_x = obj_ptr->size(0);
    float detection_size_y = obj_ptr->size(1);
    float detection_size_z = obj_ptr->size(2);
    float detection_theta = obj_ptr->theta;
    ...
    output_size = size;
    stable_direction = Eigen::Vector3d::Zero();
...
void TrackedObject::ToObject(base::ObjectPtr obj) const {
  ...
  */
 obj->polygon.clear();
 obj->polygon.reserve(polygon_points.size());
 for(const auto& point : polygon_points) {
   century::perception::base::PointD point_3d{point(0),point(1),point(2),0.0};
   obj->polygon.push_back(point_3d);
 }
