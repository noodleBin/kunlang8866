...
using century::hdmap::ElectricFenceInfoConstPtr;
using century::hdmap::JunctionInfoConstPtr;
using century::hdmap::LineBoundary;
using century::hdmap::RoadRoiPtr;
using century::hdmap::SignalInfoConstPtr;
...
bool BuildRailwayPolygonFromElectricFence(
    const ElectricFenceInfoConstPtr& electric_fence,
    base::PointCloud<base::PointD>* polygon) {
  if (nullptr == electric_fence || nullptr == polygon) {
    return false;
  }
  polygon->clear();
  const auto& points = electric_fence->polygon().points();
  if (points.size() < 3) {
    return false;
  }
  auto* polygon_points = polygon->mutable_points();
  polygon_points->reserve(points.size());
  for (const auto& point : points) {
    if (!polygon_points->empty() && polygon_points->back().x == point.x() &&
        polygon_points->back().y == point.y()) {
      continue;
    }
    polygon_points->emplace_back(
        base::PointD{point.x(), point.y(), 0.0, 0.0});
  }
  polygon->resize(polygon_points->size());
...
bool HDMapInput::InitHDMap() {
  hdmap_.reset(new century::hdmap::HDMap());
  ...
  std::string danger_junction_file;
  std::string config_file;
  if (!model_config->get_value("danger_junction_file",
                               &danger_junction_file)) {
    AERROR << "danger_junction_file not found.";
  } else {
    config_file = century::cyber::common::GetAbsolutePath(danger_junction_file, "map_input.conf");
  }
  ACHECK(century::cyber::common::GetProtoFromFile(config_file, &hdmap_input_config_));



  // TO DO: Decide which map to use
  // Option 1: Use global hdmap_ = century::hdmap::HDMapUtil::BaseMapPtr();
  ...
  if (!century::cyber::common::PathExists(hdmap_file_)) {
...
bool HDMapInput::GetRoiHDMapStruct(
  ...
  if (nullptr == hdmap_struct_ptr) {
    return false;
  }

  if (junctions_vec.empty()) {
    hdmap_->GetJunctions(point, distance, &junctions_vec);
  }

  junctions_vec.erase(
      std::remove_if(junctions_vec.begin(), junctions_vec.end(),
      [this](const auto& junction) {
        for (auto danger_junction_type : this->hdmap_input_config_.danger_junction_name()) {
          if (danger_junction_type == junction->junction().type()) {
            return false;
          }
        }
        return true;
  ...
  hdmap_struct_ptr->railway_boundary_polygons.clear();
  hdmap_struct_ptr->electric_fence_polygons.clear();
  ...
  std::vector<ElectricFenceInfoConstPtr> electric_fences_vec;
  if (0 == hdmap_->GetElectricFences(point, distance, &electric_fences_vec)) {
    std::unordered_set<std::string> railway_fence_ids;
    std::unordered_set<std::string> all_fence_ids;
    for (const auto& electric_fence : electric_fences_vec) {
      if (nullptr == electric_fence) {
        continue;
      }
      const auto& fence_id = electric_fence->id().id();
      // Populate railway_boundary_polygons (RAILWAY type only, for algorithm use).
      if (electric_fence->electric_fence().area_type() ==
          century::hdmap::ElectricFence::RAILWAY) {
        if (fence_id.empty() || railway_fence_ids.insert(fence_id).second) {
          base::PointCloud<base::PointD> railway_polygon;
          if (BuildRailwayPolygonFromElectricFence(electric_fence,
                                                   &railway_polygon)) {
            hdmap_struct_ptr->railway_boundary_polygons.emplace_back(
                std::move(railway_polygon));
          }
        }
      }
      // Populate electric_fence_polygons (all area_types, for visualization).
      if (fence_id.empty() || all_fence_ids.insert(fence_id).second) {
        base::PointCloud<base::PointD> fence_polygon;
        if (BuildRailwayPolygonFromElectricFence(electric_fence,
                                                 &fence_polygon)) {
          hdmap_struct_ptr->electric_fence_polygons.emplace_back(
              std::move(fence_polygon));
        }
      }
    }
  } else {
    AERROR << "Failed to get nearby electric fences, point: "
           << point.DebugString();
  }
  return true;
}
...
void HDMapInput::MergeBoundaryJunction(
    const std::vector<century::hdmap::RoadRoiPtr>& boundary,
    const std::vector<century::hdmap::JunctionInfoConstPtr>& junctions,
  ...
  for (int step = 0, i = 0; i < polygon_size; ++i) {
    ...
    const std::vector<century::common::PointENU>& left_line_points =
    ...
    const std::vector<century::common::PointENU>& right_line_points =
...
bool HDMapInput::GetSignalsFromHDMap(
    ...
    std::vector<century::hdmap::Signal>* signals) {
  century::common::PointENU point;
