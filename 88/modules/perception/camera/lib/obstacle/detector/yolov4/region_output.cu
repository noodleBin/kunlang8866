...
__host__ __device__ float jaccard_overlap_gpu(const float *bbox1,
                                              const float *bbox2) {
  if (bbox1[2] < bbox2[0] || bbox1[0] > bbox2[2] || bbox1[3] < bbox2[1] ||
      bbox1[1] > bbox2[3]) {
...
__global__ void get_object_kernel(
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < (n);
    ...
    if (with_ratios) {
      // Indices 0-3 represent: cos2, left, visa, visb
    ...
    if (0 < num_areas) {
      ...
      for (int area_id = 1; area_id < num_areas; ++area_id) {
        if (area_id_data[offset_area_id + max_area_id] <
            area_id_data[offset_area_id + area_id]) {
          max_area_id = area_id;
        }
      }
...
void apply_nms_gpu(const float *bbox_data, const float *conf_data,
  ...
  for (auto i : origin_indices) {
    if (confidence_threshold < conf_data[i]) {
      idx.emplace_back(i);
      confidences.emplace_back(conf_data[i]);
    }
  }
  ...
  for (size_t i = 0; i < selected_indices.size(); ++i) {
    indices->emplace_back(idx[selected_indices[i]]);
  }
}
...
void apply_nms(const bool *overlapped, const int num,
  ...
  indices->clear();
  while (0 != index_vec.size()) {
    int best_idx = index_vec.front();
    indices->emplace_back(best_idx);
...
void get_objects_gpu(const YoloBlobs &yolo_blobs, const cudaStream_t &stream,
  ...
  if (nullptr != yolo_blobs.det2_obj_blob) {
    multi_scale = true;
  }
  ...
  height_vec.emplace_back(yolo_blobs.det1_obj_blob->shape(1));
  width_vec.emplace_back(yolo_blobs.det1_obj_blob->shape(2));
  if (multi_scale) {
    height_vec.emplace_back(yolo_blobs.det2_obj_blob->shape(1));
    height_vec.emplace_back(yolo_blobs.det3_obj_blob->shape(1));
    width_vec.emplace_back(yolo_blobs.det2_obj_blob->shape(2));
    width_vec.emplace_back(yolo_blobs.det3_obj_blob->shape(2));
  }
  ...
  for (auto it = indices.begin(); it != indices.end(); ++it) {
    ...
    if (conf_scores.end() == conf_scores.find(label)) {
      continue;
    }
    ...
    for (size_t j = 0; j < indice.size(); ++j) {
      ...
      if (model_param.confidence_threshold() > scores[idx]) {
        continue;
      }
      ...
      build_object_from_detection(idx, bbox, types, num_classes, conf_scores,
                                   model_param, obj);
...
void get_intersect_bbox(const NormalizedBBox &bbox1,
  ...
  if (bbox1.xmax < bbox2.xmin || bbox1.xmin > bbox2.xmax ||
      bbox1.ymax < bbox2.ymin || bbox1.ymin > bbox2.ymax) {
...
void get_max_score_index(const std::vector<float> &scores,
  ...
  if (-1 < top_k && static_cast<int>(score_index_vec->size()) > top_k) {
...
void apply_boxvoting_fast(std::vector<NormalizedBBox> *bboxes,
  ...
  for (size_t count = 0; count < indices->size(); ++count) {
    ...
    for (size_t i = 1; i < indices->size(); ++i) {
      ...
      if ((*scores)[max_box_idx] < (*scores)[idx]) {
        max_box_idx = idx;
      }
    }
  ...
  while (0 != score_index_vec.size()) {
    ...
    for (size_t k = 0; k < indices->size(); ++k) {
      if (keep) {
        ...
        keep = adaptive_threshold >= overlap;
      } else {
        break;
      }
    }
    ...
    if (keep && 1 > eta && 0.5 < adaptive_threshold) {
...
void filter_bbox(const MinDims &min_dims,
  ...
  while (objects->size() > total_obj_idx) {
    const auto &obj = (*objects)[total_obj_idx];
    if (min_dims.min_2d_height <=
            (obj->camera_supplement.box.ymax -
             obj->camera_supplement.box.ymin) &&
        (0 >= min_dims.min_3d_height ||
         min_dims.min_3d_height <= obj->size[2]) &&
        (0 >= min_dims.min_3d_width ||
         min_dims.min_3d_width <= obj->size[1]) &&
        (0 >= min_dims.min_3d_length ||
         min_dims.min_3d_length <= obj->size[0])) {
...
void build_object_from_detection(
    int idx, const float *bbox, const std::vector<base::ObjectSubType> &types,
    int num_classes,
    const std::map<base::ObjectSubType, std::vector<float>> &conf_scores,
    const yolo::ModelParam &model_param, base::ObjectPtr obj) {
  float total = 1e-5;
  for (int k = 0; k < num_classes; ++k) {
    const auto &vis_type_k = types[k];
    const auto &obj_type_k = base::kSubType2TypeMap.at(vis_type_k);
    const auto &conf_score = conf_scores.at(vis_type_k)[idx];
  ...
  for (int k = 0; k < static_cast<int>(obj->type_probs.size()); ++k) {
  ...
  fill_area_id(0 < model_param.num_areas(), obj, bbox + 30);
}
...
int get_area_id(float visible_ratios[4]) {
  ...
  int max_face = 0;
  for (int i = 1; i < 4; ++i) {
    if (visible_ratios[max_face] < visible_ratios[i]) {
      max_face = i;
    }
  }
  ...
  if (eps > left_ratio && eps > right_ratio) {
