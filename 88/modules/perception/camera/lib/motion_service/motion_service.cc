...
bool MotionService::Init() {
  ...
  century::perception::onboard::MotionService motion_service_param;
...
void MotionService::PublishEvent(const double timestamp) {
  // protobuf msg
  std::shared_ptr<century::perception::Motion_Service> motion_service_msg(
      new (std::nothrow) century::perception::Motion_Service);
  century::common::Header *header = motion_service_msg->mutable_header();
  ...
  for (int k = static_cast<int>(motion_buffer.size()) - 1; k >= 0; k--) {
    century::perception::VehicleStatus *v_status_msg =
...
void MotionService::ConvertVehicleMotionToMsgOut(
    base::VehicleStatus vs, century::perception::VehicleStatus *v_status_msg) {
  ...
  century::perception::MotionType *motion_msg = v_status_msg->mutable_motion();
