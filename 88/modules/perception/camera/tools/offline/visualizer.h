...
class Visualizer {
 ...
 public:
  ...
  std::string type_to_string(const century::perception::base::ObjectType type);
  std::string sub_type_to_string(
      const century::perception::base::ObjectSubType type);
  ...
  void PlotObstacles(const base::MotionBufferPtr &motion_buffer,
                     const cv::Scalar &color, const base::ObjectPtr &object,
                     const Eigen::Vector2d &c_2D_l,
                     const Eigen::Affine3d &world2camera, double theta);
  void Draw3DBoxEdges(cv::Mat &image_2d, const std::vector<cv::Point> &p_proj,
                      const cv::Scalar &color_cipv, int line_thickness);
  void ShowInlierPoints(const std::string &camera_name, cv::Mat &image_2d,
                        const base::LaneLine &object,
                        const cv::Scalar &lane_color);
  void DrawLaneByCubicCurve(const std::string &camera_name, cv::Mat &image_2d,
                            const base::LaneLine &object,
                            const cv::Scalar &lane_color);

  void DrawLineAndVP(const std::string &camera_name, cv::Mat &image_2d,
                     const CameraFrame &frame);

  void DrawTrackedObjects(const std::string &camera_name, cv::Mat &image_2D,
                          const CameraFrame &frame,
                          const Eigen::Affine3d &world2camera,
                          const Eigen::Matrix3d &intrinsic,
                          const base::MotionBufferPtr &motion_buffer);
  void DrawVirtualEgoLane(const std::string &camera_name, cv::Mat &image_2D,
                          const base::MotionBufferPtr &motion_buffer,
                          const CameraFrame &frame);
 ...
 private:
  ...
  double angle_adjust_step_ = 0.05;

  static constexpr double kFrameTimeThreshold = 0.02;
  static constexpr double kVanishingPointX = 1024.0;
  static constexpr double kFocalLengthRatio = 0.25;
  static constexpr int kRangeCircleStart = 20;
  static constexpr int kRangeCircleEnd = 300;
  static constexpr int kRangeCircleStepSmall = 20;
  static constexpr int kRangeCircleStepLarge = 50;
  static constexpr float kLaneDistanceThreshold = 3.0f;
 ...
 private:
  bool HandleBasicKeys(int key);
  bool HandleDirectionKeys(const std::string &camera_name, int key);
  bool HandleSpecialKeys(const std::string &camera_name, int key);
  void UpdateHelpString();
  void UpdateCalibrationAngles(const std::string &camera_name, int key);

  // Helper functions for drawing
  void DrawWorldBoundingBox(const Eigen::Vector2d &p1,
                            const Eigen::Vector2d &p2,
                            const Eigen::Vector2d &p3,
                            const Eigen::Vector2d &p4, const cv::Scalar &color,
                            int thickness, bool is_cipv = false);
};
