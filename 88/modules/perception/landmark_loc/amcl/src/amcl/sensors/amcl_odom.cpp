...
bool AMCLOdom::UpdateAction(pf_t *pf, AMCLSensorData *data) {
  if(ODOM_MODEL_DIFF != this->model_type) {
    return false;
  } 
  ...
  const double max_yaw_noise = 24.0;
  for (int i = 0; i < set->sample_count; ++i) {
    ...
    if(i % special_yaw_interval == 0) {
      sample->pose.v[2] += delta_rot1_hat + delta_rot2_hat;
    } else {
      sample->pose.v[2] = ndata->pose.v[2] + (drand48() - 0.5) * M_PI / max_yaw_noise;
    }
