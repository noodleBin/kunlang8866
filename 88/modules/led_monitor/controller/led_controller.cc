...
bool LedController::Process(const std::shared_ptr<planning::ADCTrajectory>& msg) {
  ...
  // check if the display type is valid or not, if invalid we will play the default program.
  if (valid_display_types_.end() == valid_display_types_.find(static_cast<int>(program))) {
    AERROR << "Invalid display type: " << static_cast<int>(program) << ", play default program.";
    program = planning::DisplayType::DEFAULT_DIS;
  }
