...
void HesaiDriver::InitConfigValues() {
  // Load driver runtime parameters once from config before worker threads
  // start accessing them.
  packet_buffer_capacity_multiplier_ =
      static_cast<int>(conf_.packet_buffer_capacity_multiplier());
  packet_recv_warn_threshold_ms_ =
      static_cast<int64_t>(conf_.packet_recv_warn_threshold_ms());
  scan_buffer_size_ = static_cast<int>(conf_.scan_buffer_size());
}
...
bool HesaiDriver::Init() {
  ...
  if (conf_.model() == HESAI40P) {
    pkt_buffer_capacity_ =
        HESAI40_MAX_PACKETS * packet_buffer_capacity_multiplier_;
  } else if (conf_.model() == HESAI64) {
    pkt_buffer_capacity_ =
        HESAI64_MAX_PACKETS * packet_buffer_capacity_multiplier_;
  } else {
  ...
  while (running_) {
    ...
    auto duration =
        std::chrono::duration_cast<std::chrono::milliseconds>(end - start)
            .count();
    if (duration > packet_recv_warn_threshold_ms_) {
...
void HesaiDriver::ProcessGps(const HesaiPacket& pkt) {
  ...
  auto now = century::cyber::Time().Now().ToNanosecond();
...
void HesaiDriver::ProcessThread() {
  ...
  while (running_) {
    ...
    if (is_end && scan_buffer_[index_]->firing_pkts_size() > 0) {
      is_end = false;
      auto now = century::cyber::Time().Now();
