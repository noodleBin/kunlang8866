...
int NvBufSurf::NvAllocate(NvCommonAllocateParams *allocateParams,
                          uint32_t numBuffers, int *fd) {
  int ret = 0;
  NvBufSurfaceAllocateParams input_params = {{0}};

  if (numBuffers < 1 || allocateParams == NULL) return -1;
...
int NvBufSurf::NvTransform(NvCommonTransformParams *transformParams, int src_fd,
                           int dst_fd) {
  int ret = 0;
  if (transformParams == NULL) return -1;
...
int NvBufSurf::NvTransformAsync(NvCommonTransformParams *transformParams,
  ...
  NvBufSurfaceFromFd(src_fd, (void **)(&nvbuf_surf_src));
  NvBufSurfaceFromFd(dst_fd, (void **)(&nvbuf_surf_dst));
  ret = NvBufSurfTransformAsync(nvbuf_surf_src, nvbuf_surf_dst,
                                &transform_params, sync_obj);
