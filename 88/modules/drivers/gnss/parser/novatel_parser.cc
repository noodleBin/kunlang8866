...
class NovatelParser : public Parser {
 ...
 private:
  ...
  Parser::MessageType DispatchMessage(uint8_t* message,
  ...
  bool HandleInsPvax(const novatel::InsPvaX* pvax, uint16_t gps_week,
                     uint32_t gps_millisecs);
  bool HandleInsx(const novatel::InsPvaX* pvax, uint16_t gps_week,
                  uint32_t gps_millisecs);
  ...
  ::century::drivers::gnss::Gnss gnss_;
  ::century::drivers::gnss::GnssBestPose bestpos_;
  ::century::drivers::gnss::Imu imu_;
  ::century::drivers::gnss::Ins ins_;
  ::century::drivers::gnss::Insx insx_;
  ::century::drivers::gnss::InsStat ins_stat_;
  ::century::drivers::gnss::GnssEphemeris gnss_ephemeris_;
  ::century::drivers::gnss::EpochObservation gnss_observation_;
  ::century::drivers::gnss::Heading heading_;
};
...
Parser::MessageType NovatelParser::DispatchMessage(
    uint8_t* message, novatel::MessageId message_id, uint16_t message_length,
    uint16_t gps_week, uint32_t gps_millisecs, MessagePtr* message_ptr) {
  switch (message_id) {
    ...
    case novatel::INSPVAX:
      ...
      if (HandleInsx(reinterpret_cast<novatel::InsPvaX*>(message), gps_week,
                     gps_millisecs)) {
        *message_ptr = &insx_;
        return MessageType::INSX;
      }
...
Parser::MessageType NovatelParser::PrepareMessage(MessagePtr* message_ptr) {
  ...
  uint16_t gps_week;
  uint32_t gps_millisecs;
  if (novatel::SYNC_2_LONG_HEADER == buffer_[2]) {
  ...
  if (novatel::INSPVAX == message_id) {
    AINFO << "novatel::INSPVAX";
  }
  return DispatchMessage(message, message_id, message_length, gps_week,
                         gps_millisecs, message_ptr);
}

bool NovatelParser::HandleGnssBestpos(const novatel::BestPos* pos,
                                      uint16_t gps_week,
                                      uint32_t gps_millisecs) {
  bestpos_.set_sol_status(static_cast<century::drivers::gnss::SolutionStatus>(
      pos->solution_status));
  bestpos_.set_sol_type(
      static_cast<century::drivers::gnss::SolutionType>(pos->position_type));
  ...
  bestpos_.set_datum_id(
      static_cast<century::drivers::gnss::DatumId>(pos->datum_id));
  ...
  if (pos->solution_status == novatel::SolutionStatus::SOL_COMPUTED) {
    ...
    switch (pos->position_type) {
      ...
      case novatel::SolutionType::INS_RTKFLOAT:
        gnss_.set_type(century::drivers::gnss::Gnss::RTK_FLOAT);
        break;
      ...
      case novatel::SolutionType::INS_RTKFIXED:
        gnss_.set_type(century::drivers::gnss::Gnss::RTK_INTEGER);
        break;
      ...
      case novatel::SolutionType::INS_PPP:
        gnss_.set_type(century::drivers::gnss::Gnss::PPP);
        break;
      case novatel::SolutionType::PROPOGATED:
        gnss_.set_type(century::drivers::gnss::Gnss::PROPAGATED);
        break;
      default:
        gnss_.set_type(century::drivers::gnss::Gnss::INVALID);
    }
  } else {
    gnss_.set_type(century::drivers::gnss::Gnss::INVALID);
...
bool NovatelParser::HandleCorrImuData(const novatel::CorrImuData* imu) {
  rfu_to_flu(imu->x_velocity_change * imu_measurement_hz_,
             ...
             insx_.mutable_linear_acceleration());
  rfu_to_flu(imu->x_angle_change * imu_measurement_hz_,
             ...
             insx_.mutable_angular_velocity());
  ...
  // insx
  // insx_.mutable_linear_acceleration()->CopyFrom(ins_.linear_acceleration());
  // insx_.mutable_angular_velocity()->CopyFrom(ins_.angular_velocity());
  // if (insx_.measurement_time() != seconds) {
  //   insx_.set_measurement_time(seconds);
  //   return false;
  // }

  // double unix_sec = century::drivers::util::gps2unix(seconds);
  // if (insx_.measurement_time() != unix_sec) {
  //   insx_.set_measurement_time(unix_sec);
  //   return false;
  // }

  // insx_.mutable_header()->set_timestamp_sec(cyber::Time::Now().ToSecond());
  return true;
}
...
bool NovatelParser::HandleInsPva(const novatel::InsPva* pva) {
  ...
  switch (pva->status) {
    ...
    case novatel::InsStatus::SOLUTION_GOOD:
      ins_.set_type(century::drivers::gnss::Ins::GOOD);
      break;
    case novatel::InsStatus::ALIGNING:
    case novatel::InsStatus::HIGH_VARIANCE:
    case novatel::InsStatus::SOLUTION_FREE:
      ins_.set_type(century::drivers::gnss::Ins::CONVERGING);
      break;
    default:
      ins_.set_type(century::drivers::gnss::Ins::INVALID);
  }
...
bool NovatelParser::HandleInsx(const novatel::InsPvaX* pvax, uint16_t gps_week,
                               uint32_t gps_millisecs) {
  // if (ins_status_ != pvax->status) {
  //   ins_status_ = pvax->status;
  //   AINFO << "INS status: " << static_cast<int>(ins_status_);
  // }

  insx_.set_ins_status(pvax->ins_status);
  insx_.set_pos_type(pvax->pos_type);
  insx_.mutable_position()->set_lon(pvax->longitude);
  insx_.mutable_position()->set_lat(pvax->latitude);
  double height = pvax->height + pvax->undulation;
  insx_.mutable_position()->set_height(height);
  insx_.mutable_euler_angles()->set_x(pvax->roll);
  insx_.mutable_euler_angles()->set_y(-pvax->pitch);
  insx_.mutable_euler_angles()->set_z(90 - pvax->azimuth);
  insx_.mutable_linear_velocity()->set_x(pvax->east_velocity);
  insx_.mutable_linear_velocity()->set_y(pvax->north_velocity);
  insx_.mutable_linear_velocity()->set_z(pvax->up_velocity);

  insx_.mutable_position_std()->set_x(pvax->longitude_std);
  insx_.mutable_position_std()->set_y(pvax->latitude_std);
  insx_.mutable_position_std()->set_z(pvax->height_std);

  insx_.mutable_euler_angles_std()->set_x(pvax->roll_std);
  insx_.mutable_euler_angles_std()->set_y(pvax->pitch_std);
  insx_.mutable_euler_angles_std()->set_z(pvax->azimuth_std);

  insx_.mutable_linear_velocity_std()->set_x(pvax->east_velocity_std);
  insx_.mutable_linear_velocity_std()->set_y(pvax->north_velocity_std);
  insx_.mutable_linear_velocity_std()->set_z(pvax->up_velocity_std);
  ...
  insx_.mutable_header()->set_timestamp_sec(timestamp);

  // check time
  double diff_thresh = 0.02;  // default: 20ms
  double time_diff = timestamp - unix_sec;
  if (std::fabs(time_diff) > diff_thresh) {
    AWARN << "[insx] msg time_diff too large: " << std::to_string(time_diff);
    AWARN << "[insx] timestamp: " << std::to_string(timestamp)
          << " unix_sec: " << std::to_string(unix_sec);
  }
...
bool NovatelParser::HandleGpsEph(const novatel::GPS_Ephemeris* gps_emph) {
  gnss_ephemeris_.set_gnss_type(century::drivers::gnss::GnssType::GPS_SYS);

  century::drivers::gnss::KepplerOrbit* keppler_orbit =
      gnss_ephemeris_.mutable_keppler_orbit();

  keppler_orbit->set_gnss_type(century::drivers::gnss::GnssType::GPS_SYS);
  keppler_orbit->set_gnss_time_type(
      century::drivers::gnss::GnssTimeType::GPS_TIME);
...
bool NovatelParser::HandleBdsEph(const novatel::BDS_Ephemeris* bds_emph) {
  gnss_ephemeris_.set_gnss_type(century::drivers::gnss::GnssType::BDS_SYS);

  century::drivers::gnss::KepplerOrbit* keppler_orbit =
      gnss_ephemeris_.mutable_keppler_orbit();

  keppler_orbit->set_gnss_type(century::drivers::gnss::GnssType::BDS_SYS);
  keppler_orbit->set_gnss_time_type(
      century::drivers::gnss::GnssTimeType::BDS_TIME);
...
bool NovatelParser::HandleGloEph(const novatel::GLO_Ephemeris* glo_emph) {
  gnss_ephemeris_.set_gnss_type(century::drivers::gnss::GnssType::GLO_SYS);

  century::drivers::gnss::GlonassOrbit* glonass_orbit =
      gnss_ephemeris_.mutable_glonass_orbit();
  glonass_orbit->set_gnss_type(century::drivers::gnss::GnssType::GLO_SYS);
  glonass_orbit->set_gnss_time_type(
      century::drivers::gnss::GnssTimeType::GLO_TIME);
...
void NovatelParser::SetObservationTime() {
  ...
  gnss_observation_.set_gnss_time_type(century::drivers::gnss::GPS_TIME);
    ...
    switch (status) {
      case 1:  // observation data
        ...
        for (int i = 0; i < raw_.obs.n; ++i) {
          ...
          century::drivers::gnss::GnssType gnss_type;
          ...
          for (j = 0; j < NFREQ + NEXOBS; ++j) {
            ...
            century::drivers::gnss::GnssBandID baud_id;
            ...
            if (raw_.obs.data[i].code[i] == CODE_L1C) {
              band_obs->set_pseudo_type(
                  century::drivers::gnss::PseudoType::CORSE_CODE);
            } else if (raw_.obs.data[i].code[i] == CODE_L1P) {
              band_obs->set_pseudo_type(
                  century::drivers::gnss::PseudoType::PRECISION_CODE);
            } else {
