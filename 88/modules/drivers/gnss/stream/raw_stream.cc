...
void switch_stream_status(const century::drivers::gnss::Stream::Status &status,
                          StreamStatus_Type *report_status_type) {
  switch (status) {
    case century::drivers::gnss::Stream::Status::CONNECTED:
      *report_status_type = StreamStatus::CONNECTED;
      break;

    case century::drivers::gnss::Stream::Status::DISCONNECTED:
      *report_status_type = StreamStatus::DISCONNECTED;
      break;

    case century::drivers::gnss::Stream::Status::ERROR:
    default:
      *report_status_type = StreamStatus::DISCONNECTED;
      break;
  }
...
Stream *create_stream(const config::Stream &sd) {
  switch (sd.type_case()) {
    ...
    case config::Stream::kNtrip:
      ...
      AINFO << "ntrip user: " << sd.ntrip().user();
