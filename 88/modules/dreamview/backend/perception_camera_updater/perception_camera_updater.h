...
class PerceptionCameraUpdater {
 ...
 private:
  ...
  void InitReaders();
  void OnImage(const std::shared_ptr<century::drivers::CompressedImage> &image);
  void OnLocalization(
      const std::shared_ptr<century::localization::LocalizationEstimate> &loc);
  ...
  century::transform::Buffer *tf_buffer_ = century::transform::Buffer::Instance();
  ...
  std::deque<std::shared_ptr<century::localization::LocalizationEstimate>>
      localization_queue_;
