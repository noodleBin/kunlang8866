...
using century::cyber::blocker::BlockerManager;
...
class SimulationWorldServiceTest : public ::testing::Test {
 public:
  static void SetUpTestCase() {
    ...
    control_writer_ = node->CreateWriter<century::control::ControlCommand>(
  ...
  virtual void SetUp() {
    ...
    century::common::VehicleConfigHelper::Init();
 ...
 protected:
  ...
  static std::shared_ptr<cyber::Writer<century::control::ControlCommand>>
      control_writer_;
};

std::shared_ptr<cyber::Writer<century::control::ControlCommand>>
...
TEST_F(SimulationWorldServiceTest, UpdateLocalization) {
  ...
  auto heading = century::common::math::QuaternionToHeading(
  ...
  EXPECT_DOUBLE_EQ(
      century::common::math::QuaternionToHeading(0.0, 0.0, 0.0, 0.0),
      car.heading());
}
...
TEST_F(SimulationWorldServiceTest, UpdatePerceptionObstacles) {
  ...
  obstacle1->set_id(1);
  century::common::Point3D* point1 = obstacle1->add_polygon_point();
  point1->set_x(0.0);
  point1->set_y(0.0);
  century::common::Point3D* point2 = obstacle1->add_polygon_point();
  point2->set_x(0.0);
  point2->set_y(1.0);
  century::common::Point3D* point3 = obstacle1->add_polygon_point();
  point3->set_x(-1.0);
  point3->set_y(0.0);
  century::common::Point3D* point4 = obstacle1->add_polygon_point();
  point4->set_x(-1.0);
  point4->set_y(0.0);
  obstacle1->set_timestamp(1489794020.123);
  obstacle1->set_type(century::perception::PerceptionObstacle_Type_UNKNOWN);
  PerceptionObstacle* obstacle2 = obstacles.add_perception_obstacle();
  obstacle2->set_id(2);
  century::common::Point3D* point = obstacle2->mutable_position();
  point->set_x(1.0);
  point->set_y(2.0);
  ...
  obstacle2->set_type(century::perception::PerceptionObstacle_Type_VEHICLE);
...
TEST_F(SimulationWorldServiceTest, UpdateDecision) {
  ...
  century::planning::MainDecision* main_decision =
      decision_res.mutable_main_decision();
  main_decision->add_target_lane()->set_speed_limit(35);
  century::planning::MainStop* main_stop = main_decision->mutable_stop();
  ...
  main_stop->set_reason_code(
      century::planning::StopReasonCode::STOP_REASON_CROSSWALK);

  century::planning::ObjectDecisions* obj_decisions =
      decision_res.mutable_object_decision();
  // The 1st obstacle is from perception and has 1 decisions: nudge.
  century::planning::ObjectDecision* obj_decision1 =
  ...
  century::planning::ObjectDecision* obj_decision2 =
      obj_decisions->add_decision();
  obj_decision2->set_perception_id(2);
  century::planning::ObjectYield* yield =
      obj_decision2->add_object_decision()->mutable_yield();
  century::common::PointENU* fence_point = yield->mutable_fence_point();
...
TEST_F(SimulationWorldServiceTest, UpdateControlCommandWithSimpleLonLat) {
  ...
  // SimulationWorld object.
  century::control::ControlCommand control_command;
...
TEST_F(SimulationWorldServiceTest, DownsampleSpeedPointsByInterval) {
  century::planning_internal::STGraphDebug graph;
...
TEST_F(SimulationWorldServiceTest, UpdateLatency) {
  std::shared_ptr<century::control::ControlCommand> control_command =
      std::make_shared<century::control::ControlCommand>();
