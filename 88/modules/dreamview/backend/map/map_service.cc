...
void MapService::CollectMapElementIds(const PointENU &point, double radius,
  ...
  std::vector<ParkingLotInfoConstPtr> parking_lot;
  if (SimMap()->GetParkingLots(point, radius, &parking_lot) != 0) {
    AERROR << "Fail to get parking lot from sim_map.";
  }
  AINFO << "parking_lot size : " << parking_lot.size();
  ExtractIds(parking_lot, ids->mutable_parking_lot());

  std::vector<ElectricFenceInfoConstPtr> electric_fence;
  if (SimMap()->GetElectricFences(point, radius, &electric_fence) != 0){
    AERROR << "Fail to get electric fence from sim_map.";
  }
  AINFO << "electric_fence size : " << electric_fence.size();
  ExtractIds(electric_fence, ids->mutable_electric_fence());
...
Map MapService::RetrieveMapElements(const MapElementIds &ids) const {
  ...
  for (const auto &id : ids.parking_lot()) {
    map_id.set_id(id);
    auto element = SimMap()->GetParkingLotById(map_id);
    if (element) {
      *result.add_parking_lot() = element->parking_lot();
    }
  }

  for (const auto &id : ids.electric_fence()){
    map_id.set_id(id);
    auto element = SimMap()->GetElectricFenceById(map_id);
    if (element) {
      *result.add_electric_fence() = element->electric_fence();
...
bool MapService::ConstructLaneWayPointWithHeading(
  ...
  if (GetNearestLaneWithHeading(x, y, &lane, &s, &l, heading)) {
    ...
    if (CheckRoutingPointLaneType(lane)) {
      laneWayPoint->set_id(lane->id().id());
      // AINFO << "GetNearestLaneWithHeading false";
      // return false;
    }
  }
  ...
  laneWayPoint->set_heading(heading);
  auto *pose = laneWayPoint->mutable_pose();
  pose->set_x(x);
  pose->set_y(y);
...
bool MapService::CheckRoutingPointLaneType(LaneInfoConstPtr lane) const {
  if (lane->lane().type() != Lane::CITY_DRIVING) {
    AERROR
        ...
        << century::hdmap::Lane::LaneType_Name(lane->lane().type());
    //all lane type can selcet in dreamview
    return true;
  }
  return true;
}

bool MapService::GetStartPoint(century::common::PointENU *start_point) const {
