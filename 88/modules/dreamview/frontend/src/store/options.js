...
export const MONITOR_MENU = Object.freeze({
  ...
  EDIT_SCENARIO: 'showEditScenario',
  DEMONSTRATE: 'showDestination'
});
...
export default class Options {
    ...
    constructor() {
      ...
      this.scenarioEditOverlayOptions = new Set([
        'showBypassDialog',
        'showSelectRoutePath',
      ]);
      this.mainSideBarOptions = [
        ...
        'showMenu',
        'showSimulateEditingBar',
        'showDataRecorder',
      ];
      this.scenarioEditOptions = [
        'showSimulateEditingBar',
        'showEditScenario',
        'enableRoutingEditing',
        'enableObstacleEditing',
        'addObstaclePathPoint',
        'addObstacleTriggerPoint',
      ];
      this.secondarySideBarOptions = [];
    ...
    @computed get showGeo() {
      return this.showSimulateEditingBar
    ...
    @computed get monitorName() {
      ...
      } if (this.showEditScenario) {
        return MONITOR_MENU.EDIT_SCENARIO;
      } if (this.showDestination) {
        return MONITOR_MENU.DEMONSTRATE;
      }
      return null;
    }
    ...
    @action toggle(option, isCustomized) {
      ...
      const shouldResetScenarioEditOptions =
        this[option]
        && !this.scenarioEditOptions.includes(option)
        && !this.scenarioEditOverlayOptions.has(option);

      if (shouldResetScenarioEditOptions) {
        for (const other of this.scenarioEditOptions) {
          this[other] = false;
        }
      }
      if (this.scenarioEditOptions.includes(option)) {
        if (option !== 'showSimulateEditingBar') {
          for (const other of this.scenarioEditOptions) {
            if (other !== 'showSimulateEditingBar' && other !== 'showEditScenario' && other !== option) {
              this[other] = false;
            }
          }
        }
      ...
      if (this[option] && this.mainSideBarOptions.includes(option)) {
        for (const other of this.mainSideBarOptions) {
          ...
          if (option === 'showSimulateEditingBar') {
            this.showMeasuringDistance = false;
          }
        }
      }
      ...
      if (option === 'showMeasuringDistance' && this.showMeasuringDistance) {
        STORE.startMeasuringDistance();
      } else if ((option === 'showMeasuringDistance'
        || option === 'showSimulateEditingBar')
        && !this.showMeasuringDistance) {
        STORE.endMeasuringDistance(this.cameraAngle);
      }
      STORE.updateCurSorStyle(this.showMeasuringDistance);
    }
