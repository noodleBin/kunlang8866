...
export const DEFAULT_SPEED_COLOR = 0xFFB6C1;
export const DEFAULT_STATIC_COLOR = 0xFFEA00;
export const ObstacleColorMapping = {
  ...
  CIPV: 0xFF9966,
  WIDE40FOOT: 0xFF7F50,
  NARROW20FOOT: 0xFF4500,
  CONE: 0xCD5C5C,
  STACKER: 0x4169E1,
  FORKLIFT_STACKER: 0x008B8B,
  WHEELCRANE: 0x00FFCC,
};
...
export const ARROW_SIZE = Object.freeze({
  LENGTH: 3,
  LINEWIDTH: 1.5,
  CONELENGTH: 0.5,
  CONEWIDTH: 0.5,
});
export const DEFAULT_PEDESTRIAN_SIZE = Object.freeze({
  X: 0.8,
  Y: 0.8,
  Z: 2.0,
});
export const DEFAULT_BICYCLE_SIZE = Object.freeze({
  X: 0.8,
  Y: 2.2,
  Z: 2.0,
});
export const DEFAULT_VEHICLE_SIZE = Object.freeze({
  X: 1.8,
  Y: 4.0,
  Z: 2.0,
});
export const DEFAULT_UNKNOWN_SIZE = Object.freeze({
  X: 1.0,
  Y: 2.0,
  Z: 1.0,
});
export const DEFAULT_STACKER_SIZE = Object.freeze({
  X: 6.2,
  Y: 11.3,
  Z: 4.5,
});
export const DEFAULT_FORKLIFTSTACKER_SIZE = Object.freeze({
  X: 4.1,
  Y: 6.5,
  Z: 10.9,
});

export const DEFAULT_WHEELCRANE_SIZE = Object.freeze({
  X: 10.79,
  Y: 2.98,
  Z: 4.0,
});
...
export default class PerceptionObstacles {
  ...
  updateObjects(world, coordinates, scene, isBirdView) {
    ...
    adc.length = world.autoDrivingCar.length;
    ...
    for (let i = 0; i < objects.length; i++) {
      ...
      if (STORE.options['showAebObstacle'].toString() !== obstacle.isAebType) {
        continue;
      }
  ...
  hideUnusedObjects() {
    const showObstacle = STORE.options.showEditScenario;
    hideArrayObjects(this.arrows, !showObstacle ? this.arrowIdx : 0);
    hideArrayObjects(this.solidCubes, !showObstacle ? this.cubeIdx : 0);
    hideArrayObjects(this.dashedCubes, !showObstacle ? this.cubeIdx : 0);
    hideArrayObjects(this.extrusionSolidFaces, !showObstacle ? this.extrusionFaceIdx : 0);
    hideArrayObjects(this.extrusionDashedFaces, !showObstacle ? this.extrusionFaceIdx : 0);
    hideArrayObjects(this.icons, !showObstacle ? this.iconIdx : 0);
    hideArrayObjects(this.trafficCones, !showObstacle ? this.trafficConeIdx : 0);
    hideArrayObjects(this.v2xCubes, !showObstacle ? this.v2xCubeIdx : 0);
    hideArrayObjects(this.v2xSolidFaces, !showObstacle ? this.v2xSolidFaceIdx : 0);
  }
  ...
  getSignedDistanceToRect(ax, ay, cx, cy) {
    const hx = 7.5;
    const hy = 1.6;

    const dx = Math.abs(ax - cx) - hx;
    const dy = Math.abs(ay - cy) - hy;

    const externalDistance = Math.sqrt(Math.max(dx, 0) ** 2 + Math.max(dy, 0) ** 2);

    const internalDistance = Math.min(Math.max(dx, dy), 0);
  ...
  updateTexts(adc, obstacle, obstaclePosition, scene, isBirdView, isV2X) {
    ...
    const color = obstacle.speed > 0 ? DEFAULT_SPEED_COLOR : DEFAULT_STATIC_COLOR;
    let lineCount = 0;
    if (STORE.options.showObstaclesInfo) {
      // const distance = adc.distanceTo(obstaclePosition).toFixed(1);
      // const distanceM = (distance - adc.length / 2 - obstacle.length / 2).toFixed(1);
      const distance = this.getSignedDistanceToRect(
        adc.x, adc.y, obstaclePosition.x, obstaclePosition.y
      ).toFixed(1);
      const distanceM = (distance - obstacle.length / 2).toFixed(1);
      const speed = obstacle.speed.toFixed(1);
      this.drawTexts(`(${distanceM}m, ${speed}m/s)`, initPosition, scene, color);
      lineCount++;
    }
    if (STORE.options.showObstaclesId) {
      ...
      this.drawTexts(obstacle.id, textPosition, scene, color);
      lineCount++;
    }
    if (STORE.options.showPredictionPriority) {
      ...
      if (priority && priority !== 'NORMAL') {
        ...
        this.drawTexts(priority, textPosition, scene, color);
        lineCount++;
      }
    }
    if (STORE.options.showPredictionInteractiveTag) {
      ...
      if (interactiveTag && interactiveTag !== 'NONINTERACTION') {
        ...
        this.drawTexts(interactiveTag, textPosition, scene, color);
        lineCount++;
      }
    }
    if (isV2X) {
  ...
  drawTexts(content, position, scene, color = 0xFFEA00) {
    ...
    if (text) {
      if (STORE.options.showEditScenario) {
        text.visible = false;
      }
