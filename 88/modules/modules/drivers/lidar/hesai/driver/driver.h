...
class HesaiDriver : public century::drivers::lidar::LidarDriver {
 public:
  HesaiDriver(const std::shared_ptr<::century::cyber::Node>& node,
              const ::century::drivers::lidar::config& conf)
      : node_(node), conf_(conf.hesai()) {}
  HesaiDriver(const std::shared_ptr<::century::cyber::Node>& node,
              const ::century::drivers::hesai::Config& conf)
 ...
 private:
  ...
  std::shared_ptr<::century::cyber::Writer<HesaiScan>> scan_writer_ = nullptr;
  ...
  int packet_buffer_capacity_multiplier_ = 10;
  int64_t packet_recv_warn_threshold_ms_ = 50;
