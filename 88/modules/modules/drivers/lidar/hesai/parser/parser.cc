...
using century::drivers::PointCloud;

Parser::Parser(const std::shared_ptr<::century::cyber::Node>& node,
               const Config& conf)
    : node_(node), conf_(conf) {
  InitConfigValues();
}

void Parser::InitConfigValues() {
  // Load parser runtime parameters once from config so packet processing can
  // use member variables directly.
  tz_second_ = conf_.time_zone() * 3600;
  start_angle_ = static_cast<int>(conf_.start_angle() * 100);
  azimuth_gap_threshold_ = static_cast<int>(conf_.azimuth_gap_threshold());
  packet_time_diff_warn_threshold_sec_ =
      conf_.packet_time_diff_warn_threshold_sec();
  max_distance_ = conf_.max_distance();
  pool_size_ = static_cast<int>(conf_.raw_pointcloud_pool_size());
  raw_pointcloud_reserve_size_ =
      static_cast<int>(conf_.raw_pointcloud_reserve_size());
  if (conf_.has_min_packets()) {
    min_packets_ = conf_.min_packets();
  } else if (conf_.model() == HESAI64) {
    min_packets_ = HESAI64_MIN_PACKETS;
  } else {
    min_packets_ = HESAI40_MIN_PACKETS;
  }
  if (conf_.has_max_packets()) {
    max_packets_ = conf_.max_packets();
  } else if (conf_.model() == HESAI64) {
    max_packets_ = HESAI64_MAX_PACKETS;
  } else {
    max_packets_ = HESAI40_MAX_PACKETS;
  }

  if (conf_.has_min_distance()) {
    min_distance_ = conf_.min_distance();
  } else if (conf_.model() == HESAI40P) {
    min_distance_ = 0.5;
  } else {
    min_distance_ = 0.1;
  }
...
bool Parser::Init() {
  ...
  //     return false;
  //   }
  //   point_cloud->mutable_point()->Reserve(raw_pointcloud_reserve_size_);
  // }
  ...
  for (int i = 0; i < pool_size_; i++) {
    ...
    raw_pointcloud_pool_[i]->mutable_point()->Reserve(
        raw_pointcloud_reserve_size_);
  }
...
void Parser::ResetRawPointCloud() {
  ...
  // raw_pointcloud_out_->mutable_point()->Reserve(raw_pointcloud_reserve_size_);
...
bool Parser::LoadCalibration(const char* path_file) {
  ...
  std::string content;
  if (!century::cyber::common::GetContent(path, &content)) {
...
void Parser::CheckPktTime(double time_sec) {
  double now = century::cyber::Time().Now().ToSecond();
  double diff = std::abs(now - time_sec);
  if (diff > packet_time_diff_warn_threshold_sec_) {
