...
class Parser {
 public:
  Parser(const std::shared_ptr<::century::cyber::Node>& node,
         const Config& conf);
  virtual ~Parser();
 ...
 protected:
  ...
  Config conf_;
  std::shared_ptr<::century::cyber::Writer<PointCloud>> raw_pointcloud_writer_;
  int pool_size_ = 8;
  int raw_pointcloud_reserve_size_ = 70000;
  int pool_index_ = 0;
