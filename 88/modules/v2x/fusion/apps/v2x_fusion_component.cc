...
void V2XFusionComponent::SerializeMsg(
  ...
  century::common::Header* header = output_msg->mutable_header();
  header->set_timestamp_sec(century::cyber::Time::Now().ToSecond());
  ...
  output_msg->set_error_code(century::common::ErrorCode::PERCEPTION_ERROR_NONE);
}
