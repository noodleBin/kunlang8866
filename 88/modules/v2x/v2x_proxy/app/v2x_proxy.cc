...
namespace century {
namespace v2x {
using ::century::canbus::ChassisDetail;
using ::century::v2x::CarStatus;
using ::century::v2x::StatusResponse;
using ::century::v2x::obu::ObuRsi;
using ::century::v2x::obu::ObuTrafficLight;
...
V2xProxy::V2xProxy(std::shared_ptr<::century::hdmap::HDMap> hdmap)
    : node_(::century::cyber::CreateNode("v2x_proxy")), exit_(false) {
  if (node_ == nullptr) {
    AFATAL << "Create v2x proxy node failed.";
    exit(1);
  }
  internal_ = std::make_shared<InternalData>();
  hdmap_ = std::make_shared<::century::hdmap::HDMap>();
  const auto hdmap_file = century::hdmap::BaseMapFile();
  if (0 != hdmap_->LoadMapFromFile(hdmap_file)) {
    AERROR << "Failed to load hadmap file: " << hdmap_file;
    return;
  }
  ::century::cyber::TimerOption v2x_car_status_timer_option;
  ...
  v2x_car_status_timer_.reset(
      new ::century::cyber::Timer(v2x_car_status_timer_option));
  ...
  obs_thread_.reset(new std::thread([this]() {
    while (!exit_.load()) {
      std::shared_ptr<::century::v2x::V2XObstacles> obs = nullptr;
...
void V2xProxy::RecvOsPlanning() {
  auto adc_trajectory = std::make_shared<::century::planning::ADCTrajectory>();
  auto res_light =
      std::make_shared<::century::perception::TrafficLightDetection>();
  ...
  std::shared_ptr<OSLight> last_os_light = nullptr;
  {
    ...
    auto now_us = ::century::cyber::Time::MonoTime().ToMicrosecond();
...
void V2xProxy::RecvTrafficlight() {
  ...
  // save for hmi
  std::lock_guard<std::mutex> lock(lock_last_os_light_);
  ts_last_os_light_ = ::century::cyber::Time::MonoTime().ToMicrosecond();
  last_os_light_ = os_light;
}

double cal_distance(const ::century::common::PointENU &p1,
                    const ::century::common::PointENU &p2) {
  double x = p1.x() - p2.x();
  double y = p1.y() - p2.y();
...
void V2xProxy::OnV2xCarStatusTimer() {
  // get loc
  auto localization =
      std::make_shared<::century::localization::LocalizationEstimate>();
  ...
  ::century::common::PointENU car_position;
  car_position.set_x(localization->pose().position().x());
  car_position.set_y(localization->pose().position().y());
  std::shared_ptr<::century::v2x::CarStatus> v2x_car_status = nullptr;
  double heading = 0.0;
