...
ErrorStateKalmanFilter::ErrorStateKalmanFilter(const YAML::Node& node) {
  ...
  RZeroVel_.block<3, 3>(0, 0) =
      K_ZERO_VEL_MEASUREMENT_NOISE * Eigen::Matrix3d::Identity();
  AINFO << "---------RZeroVel_----------- \n"
        << std::fixed << std::scientific << RZeroVel_;

  // e. process equation:
  F_.block<3, 3>(K_INDEX_ERROR_POS, K_INDEX_ERROR_VEL) =
      Eigen::Matrix3d::Identity();
  F_.block<3, 3>(K_INDEX_ERROR_ORI, K_INDEX_ERROR_BIAS_GYRO) =
      -Eigen::Matrix3d::Identity();

  B_.block<3, 3>(K_INDEX_ERROR_ORI, K_INDEX_NOISE_GYRO) =
      Eigen::Matrix3d::Identity();
  B_.block<3, 3>(K_INDEX_ERROR_BIAS_ACCEL, K_INDEX_NOISE_BIAS_ACCEL) =
      Eigen::Matrix3d::Identity();
  B_.block<3, 3>(K_INDEX_ERROR_BIAS_GYRO, K_INDEX_NOISE_BIAS_GYRO) =
      Eigen::Matrix3d::Identity();
...
bool ErrorStateKalmanFilter::ApplyMeasurement(
  ...
  switch (measurement.measurement_type) {
    ...
    case MeasurementType::BODY_VEL:
      ...
      break;
    case MeasurementType::ZERO_ENU_VEL:
      ApplyMeasurementZeroVel(node_curr, Y, G, K);
      break;
...
bool ErrorStateKalmanFilter::ApplyMeasurementZeroVel(
    StateNodeBuffer::iterator& node_curr, Eigen::VectorXd& Y,
    Eigen::MatrixXd& G, Eigen::MatrixXd& K) {
  MatrixGVel GVel;
  MatrixCVel CVel;
  GVel.setZero();
  GVel.block<3, 3>(0, K_INDEX_ERROR_VEL) = Eigen::Matrix3d::Identity();
  G = GVel;
  CVel.setZero();
  CVel.block<3, 3>(0, 0) = Eigen::Matrix3d::Identity();

  Eigen::Vector3d v_enu_obs = node_curr->odometry.vel;
  AINFO << "zero_enu_vel_obs: " << v_enu_obs.transpose();
...
void ErrorStateKalmanFilter::GetCovariance(Cov& cov) {
  ...
  // d. gyro. bias:
  cov.gyro_bias.x = P_(K_INDEX_ERROR_BIAS_GYRO + OFFSET_X,
                       K_INDEX_ERROR_BIAS_GYRO + OFFSET_X);
  cov.gyro_bias.y = P_(K_INDEX_ERROR_BIAS_GYRO + OFFSET_Y,
                       K_INDEX_ERROR_BIAS_GYRO + OFFSET_Y);
  cov.gyro_bias.z = P_(K_INDEX_ERROR_BIAS_GYRO + OFFSET_Z,
                       K_INDEX_ERROR_BIAS_GYRO + OFFSET_Z);

  // e. accel bias:
  cov.accel_bias.x = P_(K_INDEX_ERROR_BIAS_ACCEL + OFFSET_X,
                        K_INDEX_ERROR_BIAS_ACCEL + OFFSET_X);
  cov.accel_bias.y = P_(K_INDEX_ERROR_BIAS_ACCEL + OFFSET_Y,
                        K_INDEX_ERROR_BIAS_ACCEL + OFFSET_Y);
  cov.accel_bias.z = P_(K_INDEX_ERROR_BIAS_ACCEL + OFFSET_Z,
                        K_INDEX_ERROR_BIAS_ACCEL + OFFSET_Z);
}
...
void ErrorStateKalmanFilter::EliminateError(
  ...
  node_curr->odometry.gyro_bias -=
      node_curr->state.X.block<3, 1>(K_INDEX_ERROR_BIAS_GYRO, 0);
  node_curr->odometry.accl_bias -=
      node_curr->state.X.block<3, 1>(K_INDEX_ERROR_BIAS_ACCEL, 0);
  // d. gyro bias:
  if (IsCovStable(K_INDEX_ERROR_BIAS_GYRO)) {
    node_curr->odometry.gyro_bias +=
        node_curr->state.X.block<3, 1>(K_INDEX_ERROR_BIAS_GYRO, 0);
  }
  // e. accel bias:
  if (IsCovStable(K_INDEX_ERROR_BIAS_ACCEL)) {
    node_curr->odometry.accl_bias +=
        node_curr->state.X.block<3, 1>(K_INDEX_ERROR_BIAS_ACCEL, 0);
  }
}
...
void ErrorStateKalmanFilter::ResetCovariance(void) {
  ...
  P_.block<3, 3>(K_INDEX_ERROR_BIAS_GYRO, K_INDEX_ERROR_BIAS_GYRO) =
      cov_.prior.epslion * Eigen::Matrix3d::Identity();
  P_.block<3, 3>(K_INDEX_ERROR_BIAS_ACCEL, K_INDEX_ERROR_BIAS_ACCEL) =
