...
bool MsfLocComponent::Init() {
  ...
  data_compare_csv_file_ << "timestamp_s" << "," << "ins_delta_x" << ","
                         ...
                         << "fusion_delta_pitch"
                         << ","
                         << "fusion_delta_yaw" << "," << "fusion_x" << ","
                         << "fusion_y"
                         << ","
                         << "fusion_z"
                         << "," << "fusion_roll" << "," << "fusion_pitch"
                         << ","
                         << "fusion_yaw" << std::endl;
...
void MsfLocComponent::InsLocCallback(
  ...
  ADEBUG << "Ins Loc msg:" << std::fixed << msg->status_type() << " "
...
void MsfLocComponent::InsPvaCallback(
  ...
  double timestamp;
  mmath::SE3 Tx_Mp_ins;
  Eigen::Vector3d vel_ins, fixed_vel_ins;  // enu coordinates
  Eigen::Vector3d ang_vel_ins;
  ...
  mmath::SO3 rot = mmath::SO3::fromEulerYPR(
      Eigen::Vector3d(msg->euler_angles().z(), msg->euler_angles().y(),
                      msg->euler_angles().x()) *
          mmath::kDegToRad +
      Eigen::Vector3d(gamma_.value(), 0.0, 0.0));
  Tx_Mp_ins = mmath::SE3(rot, pos);

  // vel
  vel_ins << msg->linear_velocity().x(), msg->linear_velocity().y(),
      msg->linear_velocity().z();
  ang_vel_ins << msg->angular_velocity().x(), msg->angular_velocity().y(),
      msg->angular_velocity().z();

  if (!eskf_initialized_) {
    if (!FixENUVel(vel_ins, &fixed_vel_ins)) {
      return;
    }
    init_pose_ = Tx_Mp_ins.getTransformationMatrix();
    init_vel_ = fixed_vel_ins;
  ...
  if (eskf_initialized_ && config_.use_ins_velocity) {
    ...
    task_pool_->Commit(std::move(add_ins_meas_velocity_task),
      ...
      if (config_.use_gnss_zero_velocity_update &&
          std::abs(vel_ins.x()) < GNSS_ZERO_VEL_THRESH &&
          std::abs(vel_ins.y()) < GNSS_ZERO_VEL_THRESH &&
          std::abs(ang_vel_ins.z()) < GNSS_ZERO_YAWRATE_THRESH) {
        KalmanFilter::Measurement zero_vel_meas;
        zero_vel_meas.time = meas.time;
        zero_vel_meas.v_enu.setZero();
        zero_vel_meas.measurement_type = KalmanFilter::ZERO_ENU_VEL;

      auto add_zero_vel_task = std::make_shared<Task>(zero_vel_meas.time);
      AINFO << "add_zero_vel_task time: "
            << std::to_string(add_zero_vel_task->GetTimeStamp());
      add_zero_vel_task->SetWorkItem(
          [=]() { eskf_->AddMeasurement(zero_vel_meas); });
      task_pool_->Commit(std::move(add_zero_vel_task), Task::Ins_Meas_Velocity);
    }
  }
...
void MsfLocComponent::PublishLocalizationEstimate() {
  if (cyber_likely(eskf_initialized_)) {
    ...
    mmath::SE3 Tx_Mp_ins(odom_pose_);
    mmath::SE3 Tx_Mp_veh = Tx_Mp_ins * config_.Tx_ins_veh;
    ...
    ADEBUG << "Msf Loc msg---->" << std::fixed << msf_loc_status_ << " "
      ...
      if (end - start > PRINT_DATA_INTERVAL) {
        ...
        data_compare_csv_file_
            ...
            << diff_ins_pre_to_curr.getSO3().getEulerYPR().x() *
                   mmath::kRadToDeg
            << "," << ins_loc_data_->map_veh_pose.getTranslation().x() << ","
            << ins_loc_data_->map_veh_pose.getTranslation().y() << ","
            << ins_loc_data_->map_veh_pose.getTranslation().z() << ","
            << ins_loc_data_->map_veh_pose.getSO3().getEulerYPR().z() *
                   mmath::kRadToDeg
            << ","
            << ins_loc_data_->map_veh_pose.getSO3().getEulerYPR().y() *
                   mmath::kRadToDeg
            << ","
            << ins_loc_data_->map_veh_pose.getSO3().getEulerYPR().x() *
                   mmath::kRadToDeg
...
bool MsfLocComponent::FixENUVel(const Eigen::Vector3d& velocity,
                                Eigen::Vector3d* velocity_fixed) {
  if (gamma_.has_value()) {
    Eigen::Matrix3d Rz;
    Rz = Eigen::AngleAxisd(gamma_.value(), Eigen::Vector3d::UnitZ());
    *velocity_fixed = Rz * velocity;
    return true;
  }
  AWARN << "gamma_ is not initialized, waiting for init...";
  return false;
...
bool MsfLocComponent::InitConfig() {
  ...
  if (config_node["use_gnss_zero_velocity_update"]) {
    config_.use_gnss_zero_velocity_update =
        config_node["use_gnss_zero_velocity_update"].as<bool>();
  }
  ...
  AINFO << "use_gnss_zero_velocity_update: "
        << config_.use_gnss_zero_velocity_update;
