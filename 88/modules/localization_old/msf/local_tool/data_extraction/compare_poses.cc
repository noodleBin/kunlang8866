...
int main(int argc, char **argv) {
  ...
  century::localization::msf::velodyne::LoadPosesAndStds(
      loc_file_a, &poses_a, &stds_a, &timestamps_a);
  if (poses_a.empty()) {
    return 0;
  }
  ...
  century::localization::msf::velodyne::LoadPosesAndStds(
      loc_file_b, &poses_b, &stds_b, &timestamps_b);
  if (poses_b.empty()) {
    return 0;
  }
  ...
  for (unsigned int idx = 0; idx < timestamps_a.size(); idx++) {
    ...
    if (pose_a_found_iter != out_poses_a.end()) {
      ...
      century::common::math::EulerAnglesZXY<double> euler_a(
      ...
      if (pose_b_found_iter != out_poses_b.end()) {
        ...
        century::common::math::EulerAnglesZXY<double> euler_b(
  ...
  std::ofstream csvfile(compare_file.c_str());
  csvfile << "id" << "," << "timestamp_s" << "," << "pos_diff" << std::endl;
  for (unsigned int i = 0; i < vec_idx.size(); ++i) {
    csvfile << vec_idx[i] << "," << std::setprecision(13) << vec_timestamp[i]
            << ","
            << std::sqrt(std::pow(vec_x_diff[i], 2) +
                         std::pow(vec_y_diff[i], 2))
            << std::endl;
  }
  csvfile.close();
}
