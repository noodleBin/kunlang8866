...
void LocalizationIntegProcess::GetResult(IntegState *state,
  ...
  *state = integ_state_;
  ...
  century::common::Header *headerpb_loc = localization->mutable_header();
  century::localization::Pose *posepb_loc = localization->mutable_pose();

  localization->set_measurement_time(ins_pva_.time);
  headerpb_loc->set_timestamp_sec(century::cyber::Clock::NowInSeconds());

  century::common::PointENU *position_loc = posepb_loc->mutable_position();
  century::common::Quaternion *quaternion = posepb_loc->mutable_orientation();
  UTMCoor utm_xy;
  ...
  century::common::Point3D *velocitylinear =
  ...
  century::common::Point3D *eulerangles = posepb_loc->mutable_euler_angles();
  ...
  century::common::Point3D *linear_velocity_std_dev =
