...
void LocalizationIntegImpl::ImuProcessImpl(const ImuData& imu_data) {
  ...
  century::localization::Pose* posepb_loc = integ_localization.mutable_pose();

  if (imu_altitude_from_lidar_localization_available_) {
    century::common::PointENU* position = posepb_loc->mutable_position();
  ...
  Eigen::Vector3d orig_acceleration(corrected_imu.fb[0], corrected_imu.fb[1],
                                    corrected_imu.fb[2]);
  const century::common::Quaternion& orientation =
  ...
  century::common::Point3D* linear_acceleration =
  ...
  century::common::Point3D* linear_acceleration_vrf =
  ...
  century::common::Point3D* angular_velocity =
  ...
  century::common::Point3D* angular_velocity_vrf =
...
void LocalizationIntegImpl::TransferGnssMeasureToLocalization(
  ...
  century::common::Header* headerpb = localization->mutable_header();
  century::localization::Pose* posepb = localization->mutable_pose();
  ...
  century::common::PointENU* position = posepb->mutable_position();
  position->set_x(utm_xy.x);
  position->set_y(utm_xy.y);
  position->set_z(measure.gnss_pos.height);

  century::common::Quaternion* quaternion = posepb->mutable_orientation();
  quaternion->set_qx(0.0);
  quaternion->set_qy(0.0);
  quaternion->set_qz(0.0);
  quaternion->set_qw(1.0);

  century::localization::Uncertainty* uncertainty =
      localization->mutable_uncertainty();

  century::common::Point3D* position_std_dev =
  ...
  century::common::Point3D* orientation_std_dev =
