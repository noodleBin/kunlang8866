...
bool LidarLocComponent::InitReaderWriter() {
  if (config_.is_sim) {
    left_lidar_listener_ = node_->CreateReader<drivers::PointXYZIRTCloud>(
        config_.left_lidar_topic,
        std::bind(&LidarLocComponent::LeftLidarCallback, this,
                  std::placeholders::_1));
    right_lidar_listener_ = node_->CreateReader<drivers::PointXYZIRTCloud>(
        config_.right_lidar_topic,
...
void LidarLocComponent::FusionPoseCallback(
  ...
  // Tx_Mp_imu.getTranslation().transpose()
  //       << ", ypr:"
  //       << Tx_Mp_imu.getSO3().getEulerYPR().transpose() * mmath::kRadToDeg;
