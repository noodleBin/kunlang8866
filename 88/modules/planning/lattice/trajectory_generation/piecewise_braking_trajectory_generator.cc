...
std::shared_ptr<Curve1d> PiecewiseBrakingTrajectoryGenerator::Generate(
  ...
  if (v_curr > v_target) {
    CHECK_NE(v_target, 0.0);
    CHECK_NE(d_comfort, 0.0);
...
double PiecewiseBrakingTrajectoryGenerator::CheckZero(const double dist) {
  double s_dist = dist;
  if (common::util::IsFloatEqual(s_dist, 0.0)) {
    AERROR << "the s_dist is zero!";
    s_dist = FLAGS_numerical_epsilon;
  }
  return s_dist;
}
