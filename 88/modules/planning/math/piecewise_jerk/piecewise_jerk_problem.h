...
class PiecewiseJerkProblem {
 public:
  ...
  void set_scale_factor(const std::array<double, 3>& scale_factor) {
    CHECK_GT(scale_factor[0], 0.0);
    CHECK_GT(scale_factor[1], 0.0);
    CHECK_GT(scale_factor[2], 0.0);
  ...
  virtual common::Status Optimize(const int max_iter = 4000);
 ...
 protected:
  ...
  /**
   * @brief construct affine matirx for constraint, is matrix A actually.
   * @param A_indices csc matrix: The row where valid digits are stored
   * @param A_A_indptr csc matrix: The valid value is at the position of
   * A_indices
   * @param A_data csc matrix: the valid value are stored here.
   */
  ...
  void ConstraintContinuty(
      const int n, int* constraint_index_in,
      std::vector<std::vector<std::pair<c_int, c_float>>>* variables,
      std::vector<c_float>* lower_bounds, std::vector<c_float>* upper_bounds);
  void ConstraintXInit(
      const int& n, int* constraint_index_in,
      std::vector<std::vector<std::pair<c_int, c_float>>>* variables,
      std::vector<c_float>* lower_bounds, std::vector<c_float>* upper_bounds);
  ...
  T* CopyData(const std::vector<T>& vec) {
    ...
    memcpy_s(data, sizeof(T) * vec.size(), vec.data(), sizeof(T) * vec.size());
    return data;
  }
