...
bool DiscretePointsMath::ComputePathProfile(
  ...
  for (std::size_t i = 0; i < points_size; ++i) {
    ...
    double yds = 0.0;
    if (i == 0) {
      double delta = std::fmax(FLAGS_numerical_epsilon,
                               (*accumulated_s)[i + 1] - (*accumulated_s)[i]);
      xds = (xy_points[i + 1].first - xy_points[i].first) / delta;
      yds = (xy_points[i + 1].second - xy_points[i].second) / delta;
    } else if (i == points_size - 1) {
      double delta = std::fmax(FLAGS_numerical_epsilon,
                               (*accumulated_s)[i] - (*accumulated_s)[i - 1]);
      xds = (xy_points[i].first - xy_points[i - 1].first) / delta;
      yds = (xy_points[i].second - xy_points[i - 1].second) / delta;
    } else {
      double delta =
          std::fmax(FLAGS_numerical_epsilon,
                    (*accumulated_s)[i + 1] - (*accumulated_s)[i - 1]);
      xds = (xy_points[i + 1].first - xy_points[i - 1].first) / delta;
      yds = (xy_points[i + 1].second - xy_points[i - 1].second) / delta;
    }
