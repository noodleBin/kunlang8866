...
// yield decision no expand upper.
...
STBoundary STBoundary::ExpandBySForOvertake(const double s) const {
  if (lower_points_.empty()) {
    return STBoundary();
  }
  std::vector<std::pair<STPoint, STPoint>> point_pairs;
...
STBoundary STBoundary::ExpandByT(const double t) const {
  ...
  // never be zero, see the function STBoundary::IsValid
  point_pairs.emplace_back(
...
double STBoundary::obs_v() const { return obs_v_; }

void STBoundary::set_obs_v(double obs_v) { obs_v_ = obs_v; }
