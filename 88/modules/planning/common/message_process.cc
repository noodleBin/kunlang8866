...
using century::dreamview::HMIStatus;
using century::hdmap::ClearAreaInfoConstPtr;
...
using century::hdmap::PNCJunctionInfoConstPtr;
using century::hdmap::SignalInfoConstPtr;
using century::hdmap::StopSignInfoConstPtr;
using century::hdmap::YieldSignInfoConstPtr;
...
using century::storytelling::CloseToJunction;
...
void MessageProcess::OnChassis(const century::canbus::Chassis& chassis) {
...
void MessageProcess::OnHMIStatus(century::dreamview::HMIStatus hmi_status) {
  const std::string& current_map = hmi_status.current_map();
  if (map_m_.count(current_map) > 0) {
    map_name_ = map_m_[current_map];
    const std::string& map_base_folder = "/century/modules/map/data/";
...
void MessageProcess::OnStoryTelling(
    const century::storytelling::Stories& stories) {
  // clear area
...
bool MessageProcess::GetADCCurrentRoutingIndex(int* adc_road_index,
  ...
  std::vector<std::shared_ptr<const century::hdmap::LaneInfo>> lanes;
  century::hdmap::HDMapUtil::BaseMapPtr()->GetLanes(pose.position(), kRadius,
                                                  &lanes);
...
century::hdmap::LaneInfoConstPtr MessageProcess::GetCurrentLane(
    const century::common::PointENU& position) {
  constexpr double kRadiusUnit = 0.1;
  std::vector<std::shared_ptr<const century::hdmap::LaneInfo>> lanes;
  for (int i = 1; i <= 10; ++i) {
    century::hdmap::HDMapUtil::BaseMapPtr()->GetLanes(position, i * kRadiusUnit,
                                                    &lanes);
    if (lanes.size() > 0) {
      break;
    }
...
void MessageProcess::CalculateRoadLength(
...
void MessageProcess::GetLocalRoutingEndInfo(
    const int adc_road_index, const double adc_passage_s,
    const std::vector<std::pair<std::string, double>>& road_lengths,
...
void MessageProcess::GetLocalRoutingRoadPassageSegmentInfo(
      ...
      for (int k = 0; k < passage.segment_size(); ++k) {
        ...
        if (i == adc_road_index) {
          ...
          if (j == adc_passage_index) {
            local_routing_lane_ids->emplace_back(lane_segment.id());
...
bool MessageProcess::GenerateLocalRouting(
  ...
  std::vector<std::pair<std::string, double>> road_lengths;
  CalculateRoadLength(&road_lengths);
  ...
  double local_routing_start_road_s = 0.0;
  GetLocalRoutingStartInfo(adc_road_index, adc_passage_s, road_lengths,
                           &local_routing_start_road_index,
  ...
  GetLocalRoutingEndInfo(adc_road_index, adc_passage_s, road_lengths,
  ...
  GetLocalRoutingRoadPassageSegmentInfo(
      local_routing_start_road_index, local_routing_start_road_s,
      local_routing_end_road_index, local_routing_end_road_s, adc_road_index,
      adc_passage_index, local_routing, local_routing_lane_ids);
...
void MessageProcess::GenerateADCTrajectoryPlanningTagFirst(
...
void MessageProcess::GenerateADCTrajectoryPlanningTagSecond(
    const int trajectory_point_index,
    const std::vector<ADCTrajectoryPoint>& adc_trajectory_points,
    const century::common::PointENU& cur_point, std::string* clear_area_id,
    double* clear_area_distance, std::string* crosswalk_id,
  ...
  if (FLAGS_planning_offline_learning) {
    ...
    if (HDMapUtil::BaseMap().GetSignals(hdmap_point, kSearchRadius, &signals) ==
            0 &&
        signals.size() > 0) {
...
void MessageProcess::GenerateADCTrajectoryPlanningTag(
    const int trajectory_point_index,
    const std::vector<ADCTrajectoryPoint>& adc_trajectory_points,
    const century::common::PointENU& cur_point, std::string* clear_area_id,
    double* clear_area_distance, std::string* crosswalk_id,
    ...
    double* stop_sign_distance, std::string* yield_sign_id,
    double* yield_sign_distance, PlanningTag* planning_tag) {
  GenerateADCTrajectoryPlanningTagFirst(
      trajectory_point_index, adc_trajectory_points, cur_point, clear_area_id,
      clear_area_distance, crosswalk_id, crosswalk_distance, pnc_junction_id,
      pnc_junction_distance, signal_id, signal_distance, stop_sign_id,
      stop_sign_distance, yield_sign_id, yield_sign_distance, planning_tag);
  GenerateADCTrajectoryPlanningTagSecond(
      trajectory_point_index, adc_trajectory_points, cur_point, clear_area_id,
      clear_area_distance, crosswalk_id, crosswalk_distance, pnc_junction_id,
      pnc_junction_distance, signal_id, signal_distance, stop_sign_id,
      stop_sign_distance, yield_sign_id, yield_sign_distance, planning_tag);
...
void MessageProcess::GenerateADCTrajectoryPoints(
  ...
  for (const auto& localization_sample : localization_samples) {
    ...
    // lane_turn
    century::hdmap::Lane::LaneTurn lane_turn = century::hdmap::Lane::NO_TURN;
    if (lane != nullptr) {
    ...
    GenerateADCTrajectoryPlanningTag(
        trajectory_point_index, adc_trajectory_points, cur_point,
        &clear_area_id, &clear_area_distance, &crosswalk_id,
        &crosswalk_distance, &pnc_junction_id, &pnc_junction_distance,
        &signal_id, &signal_distance, &stop_sign_id, &stop_sign_distance,
        &yield_sign_id, &yield_sign_distance, planning_tag);

    adc_trajectory_points.emplace_back(adc_trajectory_point);
