...
constexpr double kIgnoreTime = 60.0;
constexpr double kMinYieldDistance = 0.5;
constexpr double kMinStopDistanceForPedestrian = 1.5;
...
constexpr double kLDistanceStartTimer = 20.0;
...
bool SpeedDecider::CheckKeepClearCrossable(
  ...
  if (last_speed_point.has_v()) {
    ...
    if (len > 1) {
      ...
      if (!common::util::IsFloatEqual(last_speed_point.t(),
                                      last_2nd_speed_point.t())) {
        AERROR << "last_speed_point.t() and last_2nd_speed_point.t() equals!";
...
bool SpeedDecider::IsFollowTooClose(const Obstacle& obstacle) const {
  ...
  const double obs_speed = obstacle.LongitudinalSpeed();
  ...
  if (FLAGS_enable_use_radical_decision &&
    ...
    const double obstacle_reverse_speed =
        common::math::Vec2d::CreateUnitVec2d(
            injector_->vehicle_state()->heading())
            .InnerProd(Vec2d(velocity.x(), velocity.y()));
    AINFO << "reverse_speed = " << obstacle_reverse_speed;
    double obstacle_break_distance;
    if (obstacle_reverse_speed > 0.0) {
      obstacle_break_distance =
          obstacle_reverse_speed * obstacle_reverse_speed * 0.5 / kMaxAcc;
    } else {
      obstacle_break_distance =
          -obstacle_reverse_speed * obstacle_reverse_speed * 0.5 / kMaxAcc;
    }
    double min_stop_distance_obstacle =
        min_avoid_distance_ - obstacle_break_distance;
    distance = obstacle.path_st_boundary().min_s() - min_stop_distance_obstacle;
    if (obs_speed > FLAGS_ignore_obstacle_speed_coeff_radical * ego_speed) {
      AINFO << "IS TO CLOOSE ? ";
      if (distance > 0.0) {
        return false;
      }
...
Status SpeedDecider::MakeObjectDecision(
  ...
  // get min slow down distance to 0.0.
  double min_avoid_distance_ =
      init_point_.v() * init_point_.v() * 0.5 / half_adc_decel_;
  ADEBUG << "min_avoid_distance = " << min_avoid_distance_;
  ...
  for (const auto* obstacle : path_decision->obstacles().Items()) {
    ...
    if (CheckStopForPedestrian(*mutable_obstacle)) {
      MakeStopDecisionForPedestrian(mutable_obstacle);
      continue;
    }
    ...
    switch (location) {
      case BELOW:
        MakeDecisionForBelowLocation(mutable_obstacle);
        break;
      case ABOVE:
        MakeDecisionForAboveLocation(mutable_obstacle);
        break;
      case CROSS:
        if (!MakeDecisionForCrossLocation(mutable_obstacle)) {
          ...
          if (mutable_obstacle->IsSlowBreakingObstacle() &&
...
void SpeedDecider::MakeDecisionForBelowLocation(
    Obstacle* mutable_obstacle) const {
  const auto& boundary = mutable_obstacle->path_st_boundary();
  if (boundary.boundary_type() == STBoundary::BoundaryType::KEEP_CLEAR) {
    ObjectDecisionType stop_decision;
    const std::string& stop_tag = "dp_st_graph/keep_clear";
    if (CreateStopDecision(*mutable_obstacle, stop_tag, &stop_decision, 0.0)) {
      mutable_obstacle->AddLongitudinalDecision(stop_tag, stop_decision);
    }
  } else if (CheckIsFollow(*mutable_obstacle, boundary)) {
    // stop for low_speed decelerating
    if (IsFollowTooClose(*mutable_obstacle)) {
      ObjectDecisionType stop_decision;
      double min_stop_distance_obstacle = FLAGS_min_stop_distance_obstacle;
      bool is_perception_obstacle = (mutable_obstacle->Perception().type() ==
                                     PerceptionObstacle::PEDESTRIAN);
      if (reference_line_info_->IsObstacleInGateArea(*mutable_obstacle) &&
          injector_->vehicle_state()->linear_velocity() <
              FLAGS_lower_speed_in_public_road) {
        min_stop_distance_obstacle = FLAGS_stop_distance_in_common_junction;
      }
      if (reference_line_info_->GetLaneType() == hdmap::Lane::PLAY_STREET &&
          FLAGS_enable_shrink_stop_distance_for_pedestrian &&
          is_perception_obstacle) {
        min_stop_distance_obstacle = kMinStopDistanceForPedestrian;
      }
      if (FLAGS_enable_use_radical_decision &&
          injector_->is_can_enter_mixed_flow_) {
        ...
        const double obstacle_reverse_speed =
            common::math::Vec2d::CreateUnitVec2d(
                injector_->vehicle_state()->heading())
                .InnerProd(Vec2d(velocity.x(), velocity.y()));
        ADEBUG << "reverse_speed = " << obstacle_reverse_speed;
        double obstacle_break_distance;
        if (obstacle_reverse_speed > 0.0) {
          obstacle_break_distance =
              obstacle_reverse_speed * obstacle_reverse_speed * 0.5 / kMaxAcc;
        ...
        ADEBUG << "obstacle_break_distance = " << obstacle_break_distance;
        min_stop_distance_obstacle =
            min_avoid_distance_ - obstacle_break_distance;
        ADEBUG << "min_stop_distance_obstacle = " << min_stop_distance_obstacle;
      }
      const std::string& stop_tag = "dp_st_graph/too_close";
      if (CreateStopDecision(*mutable_obstacle, stop_tag, &stop_decision,
                             -min_stop_distance_obstacle)) {
        mutable_obstacle->AddLongitudinalDecision(stop_tag, stop_decision);
      }
      ...
      const std::string& follow_tag = "dp_st_graph/follow";
      if (CreateFollowDecision(*mutable_obstacle, follow_tag,
                               &follow_decision)) {
        mutable_obstacle->AddLongitudinalDecision(follow_tag, follow_decision);
      }
    }
  } else {
    // YIELD decision
    ObjectDecisionType yield_decision;
    ADEBUG << "add yield obstacle = " << mutable_obstacle->Id();
    const std::string& yield_tag = "dp_st_graph/yield";
    if (CreateYieldDecision(*mutable_obstacle, yield_tag, &yield_decision)) {
      mutable_obstacle->AddLongitudinalDecision(yield_tag, yield_decision);
    }
  }
}

void SpeedDecider::MakeDecisionForAboveLocation(
    Obstacle* mutable_obstacle) const {
  const auto& boundary = mutable_obstacle->path_st_boundary();
  if (boundary.boundary_type() == STBoundary::BoundaryType::KEEP_CLEAR) {
    ...
    // OVERTAKE decision
    const std::string& overtake_tag = "dp_st_graph/overtake";
    ObjectDecisionType overtake_decision;
    if (CreateOvertakeDecision(*mutable_obstacle, overtake_tag,
                               &overtake_decision)) {
      mutable_obstacle->AddLongitudinalDecision(overtake_tag,
                                                overtake_decision);
    }
  }
}

bool SpeedDecider::MakeDecisionForCrossLocation(
    Obstacle* mutable_obstacle) const {
  double min_stop_decel = kMinDecel;
  double min_stop_s = 0.0;
  ...
  if (mutable_obstacle->IsBlockingObstacle() &&
      !(mutable_obstacle->IsSlowBreakingObstacle() && is_need_slow_breaking)) {
    ObjectDecisionType stop_decision;
    double min_stop_distance_obstacle = FLAGS_min_stop_distance_obstacle;
    bool is_perception_obstacle = (mutable_obstacle->Perception().type() ==
                                   PerceptionObstacle::PEDESTRIAN);
    ...
    if (FLAGS_enable_use_radical_decision &&
      ...
      const double obstacle_reverse_speed =
          common::math::Vec2d::CreateUnitVec2d(
              ...
              .InnerProd(Vec2d(velocity.x(), velocity.y()));
      ADEBUG << "reverse_speed = " << obstacle_reverse_speed;
      double obstacle_break_distance = 2.0;
      if (obstacle_reverse_speed > 0.0) {
        obstacle_break_distance =
            obstacle_reverse_speed * obstacle_reverse_speed * 0.5 / kMaxAcc;
      } else {
    ...
    const std::string& tag = "dp_st_graph/cross";
    if (CreateStopDecision(*mutable_obstacle, tag, &stop_decision,
                           -min_stop_distance_obstacle)) {
      mutable_obstacle->AddLongitudinalDecision(tag, stop_decision);
    }
    return false;
  }
  return true;
...
void SpeedDecider::AppendIgnoreDecision(Obstacle* obstacle) const {
  ...
  if (!obstacle->HasLongitudinalDecision()) {
    obstacle->AddLongitudinalDecision("dp_st_graph/longitude", ignore_decision);
  }
  if (!obstacle->HasLateralDecision()) {
    obstacle->AddLateralDecision("dp_st_graph/lateral", ignore_decision);
  }
}
...
bool SpeedDecider::CreateStopDecision(const Obstacle& obstacle,
  ...
  stop->set_reason_code(StopReasonCode::STOP_REASON_SPEED);
  stop->set_tag(stop_tag);
...
bool SpeedDecider::CreateFollowDecision(
    const Obstacle& obstacle, const std::string& follow_tag,
    ObjectDecisionType* const follow_decision) const {
  const double follow_speed = init_point_.v();
  double follow_distance_s = -StGapEstimator::EstimateProperFollowingGap(
      follow_speed, obstacle.speed());
  ADEBUG << "follow_distance_s_estimate = " << follow_distance_s;
  const auto& perception_obstacle = obstacle.Perception();
  if (reference_line_info_->GetLaneType() == hdmap::Lane::PLAY_STREET &&
      FLAGS_enable_shrink_stop_distance_for_pedestrian &&
      perception_obstacle.type() == PerceptionObstacle::PEDESTRIAN) {
    follow_distance_s = -kMinStopDistanceForPedestrian;
  }
  const auto& boundary = obstacle.path_st_boundary();
...
bool SpeedDecider::CreateYieldDecision(
    const Obstacle& obstacle, const std::string& yield_tag,
  ...
  double yield_distance = StGapEstimator::EstimateProperYieldingGap(
      init_point_.v(), obstacle_speed);
  const auto& perception_obstacle = obstacle.Perception();
  if (reference_line_info_->GetLaneType() == hdmap::Lane::PLAY_STREET &&
      FLAGS_enable_shrink_stop_distance_for_pedestrian &&
      perception_obstacle.type() == PerceptionObstacle::PEDESTRIAN) {
    yield_distance = kMinStopDistanceForPedestrian;
  }

  // When approaching the intersection, reduce the interference of obstacles
  // behind the stop line on the normal stopping of the vehicle to the stop line
  bool is_traffilc_light_green =
      (frame_->signal_color_ == perception::TrafficLight::GREEN);
  double stop_line_s = frame_->stop_line_s_;
  if (!is_traffilc_light_green &&
      obstacle.PerceptionSLBoundary().start_s() > stop_line_s &&
      FLAGS_enable_shrink_yield_distance && injector_->near_traffic_line_) {
    yield_distance = kMinYieldDistance;
    ADEBUG << "out stop line yield_distance when in red :" << yield_distance;
  }
  ...
  // radical yield decision can't consider adc_center behind.
  if (FLAGS_enable_use_radical_decision &&
      injector_->is_can_enter_mixed_flow_) {
    ...
    double adc_start_l = reference_line_info_->AdcSlBoundary().start_l();
    double adc_end_l = reference_line_info_->AdcSlBoundary().end_l();
    ADEBUG << "obs_end_s = " << obs_end_s
           << "          (adc_start_s + adc_end_s) * 0.5 = "
           << (adc_start_s + adc_end_s) * 0.5;
    ADEBUG << "obs_end_l = " << obs_end_l << "    obs_start_l = " << obs_start_l
           << "   adc_start_l = " << adc_start_l
           << "    adc_end_l=" << adc_end_l;
    if ((obs_end_s < (adc_start_s + adc_end_s) * 0.5) ||
        (!(obs_end_l > adc_start_l - kLatBuffer &&
           obs_start_l < adc_end_l + kLatBuffer) &&
         obs_end_s < adc_end_s)) {
      ADEBUG << "SKIP obstacle";
      return false;
    }
  }
...
bool SpeedDecider::CreateOvertakeDecision(
    const Obstacle& obstacle, const std::string& overtake_tag,
    ObjectDecisionType* const overtake_decision) const {
  const auto& velocity = obstacle.Perception().velocity();
  const double obstacle_speed =
      common::math::Vec2d::CreateUnitVec2d(init_point_.path_point().theta())
          .InnerProd(Vec2d(velocity.x(), velocity.y()));
  // TODO(zongxingguo): use using component velocity?
  double overtake_distance_s = StGapEstimator::EstimateProperOvertakingGap(
      obstacle_speed, init_point_.v());
  ADEBUG << "overtake_distance_s = " << overtake_distance_s;
  // reverse obs and lonoverlap ,prefer overtake.
  if (obstacle_speed < 0.0 &&
      adc_sl_boundary_.end_s() > obstacle.PerceptionSLBoundary().start_s()) {
    overtake_distance_s = 0.0;
  }
  const auto& boundary = obstacle.path_st_boundary();
  // TODO(zongxingguo): use min_s?
  const double reference_line_fence_s =
...
bool SpeedDecider::CheckIsFollow(const Obstacle& obstacle,
  ...
  const double obstacle_longitudinal_speed =
      common::math::Vec2d::CreateUnitVec2d(path_point.theta())
          .InnerProd(Vec2d(velocity.x(), velocity.y()));
  // right is negative,left is positive,using projection points
  const double obstacle_lateral_speed =
      common::math::Vec2d::CreateUnitVec2d(path_point.theta())
          .CrossProd(Vec2d(velocity.x(), velocity.y()));
  ADEBUG << "obstacle_lateral_speed = " << obstacle_lateral_speed
         << "           obstacle_longitudinal_speed = "
         << obstacle_longitudinal_speed;

  double occupied_part = 0.0;
  if (std::fabs(obstacle.speed()) > kEpison) {
    occupied_part = std::fabs(obstacle_lateral_speed / obstacle.speed());
  }
  // no follow to static obs.
  if (std::fabs(obstacle.speed()) < kMinSpeed) {
    return false;
  }

  bool is_cross_obs = occupied_part > 0.5;
  ADEBUG << "is_cross_obs = " << is_cross_obs;
  ADEBUG << "obstacle_id = " << obstacle.Id();
  ADEBUG << "obstacle_lateral_speed = " << obstacle_lateral_speed;
  ADEBUG << "obstacle_longitudinal_speed = " << obstacle_longitudinal_speed;
...
bool SpeedDecider::CheckStopForPedestrian(const Obstacle& obstacle) const {
  ...
  double distance_start_timer = kSDistanceStartTimer;
  static constexpr double kMaxStopSpeed = 0.3;
  static constexpr double kPedestrianStopTimeout = 4.0;
  if (FLAGS_enable_high_speed) {
    distance_start_timer = kLDistanceStartTimer;
  }
  bool result = true;
  if (obstacle.path_st_boundary().min_s() < distance_start_timer) {
    const auto obstacle_speed = obstacle.speed();
  ...
  for (const auto& stop_time : stop_time_map) {
    // remove old data
    if (Clock::NowInSeconds() - stop_time.second > kIgnoreTime) {
      continue;
    }
...
void SpeedDecider::MakeStopDecisionForPedestrian(
    Obstacle* mutable_obstacle) const {
  // in playstreet
  double min_stop_distance_obstacle_for_pedestrian =
      FLAGS_min_stop_distance_obstacle;
  if (reference_line_info_->IsObstacleInGateArea(*mutable_obstacle) &&
      injector_->vehicle_state()->linear_velocity() <
          FLAGS_lower_speed_in_public_road) {
    min_stop_distance_obstacle_for_pedestrian =
        FLAGS_stop_distance_in_common_junction;
  }
  if (reference_line_info_->GetLaneType() == hdmap::Lane::PLAY_STREET &&
      FLAGS_enable_shrink_stop_distance_for_pedestrian) {
    min_stop_distance_obstacle_for_pedestrian = kMinStopDistanceForPedestrian;
  }
  ObjectDecisionType stop_decision;
  const std::string& tag = "dp_st_graph/pedestrian";
  if (CreateStopDecision(*mutable_obstacle, tag, &stop_decision,
                         -min_stop_distance_obstacle_for_pedestrian)) {
    mutable_obstacle->AddLongitudinalDecision(tag, stop_decision);
  }
