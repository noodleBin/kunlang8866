...
void PathBoundsDecider::GetCandidateRegularPathBounds(
  ...
  for (const auto& lane_borrow_info : lane_borrow_info_list) {
    ...
    for (size_t i = 0; i < regular_path_bound.size(); ++i) {
      double s = regular_path_bound[i].s;
      if ((lane_borrow_info == LaneBorrowInfo::LEFT_BORROW ||
           lane_borrow_info == LaneBorrowInfo::RIGHT_BORROW) &&
          (s > adc_sl.start_s() && s < adc_sl.end_s())) {
        if (adc_frenet_l_ < regular_path_bound[i].l_min) {
          // AINFO<<"BEFORE = "<<regular_path_bound[i].l_min;
          regular_path_bound[i].l_min = adc_frenet_l_;

          // AINFO<<"AFTER = "<<regular_path_bound[i].l_min;
          // AINFO << "CHANGE";
        }
        if (adc_frenet_l_ > regular_path_bound[i].l_max) {
          regular_path_bound[i].l_max = adc_frenet_l_;
          // AINFO << "CHANGE";
        }
      }
