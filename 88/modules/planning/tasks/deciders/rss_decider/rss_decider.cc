...
century::common::Status RssDecider::Execute(
...
Status RssDecider::Process(Frame *frame,
  ...
  bool no_obstacle = FindNearestFrontObstacle(
      frame, reference_line_info, &front_obstacle_distance, &nearest_obs_s_start,
      &nearest_obs_s_end, &nearest_obs_l_start, &nearest_obs_l_end,
      &nearest_obs_speed);
  ...
  BuildRssWorldModel(frame, reference_line_info, adc_velocity, nearest_obs_s_start,
                     nearest_obs_s_end, nearest_obs_l_start, nearest_obs_l_end,
                     &worldModel);

  return PerformRssCheckAndSetResult(worldModel, reference_line_info);
}

bool RssDecider::FindNearestFrontObstacle(
    Frame *frame, ReferenceLineInfo *reference_line_info,
    double *front_obstacle_distance, double *nearest_obs_s_start,
    double *nearest_obs_s_end, double *nearest_obs_l_start,
    double *nearest_obs_l_end, double *nearest_obs_speed) {
  ...
  *front_obstacle_distance = FLAGS_rss_max_front_obstacle_distance;
  *nearest_obs_s_start = 0.0;
  *nearest_obs_s_end = 0.0;
  *nearest_obs_l_start = 0.0;
  *nearest_obs_l_end = 0.0;
  *nearest_obs_speed = 0.0;
  ...
  for (const auto *obstacle : path_decision->obstacles().Items()) {
    ...
    if (perception::PerceptionObstacle::UNKNOWN == obs_type ||
      ...
      is_on_road = reference_line_info->reference_line().HasOverlap(
...
void RssDecider::BuildRssWorldModel(
    Frame *frame, ReferenceLineInfo *reference_line_info,
    double adc_velocity, double nearest_obs_s_start, double nearest_obs_s_end,
    double nearest_obs_l_start, double nearest_obs_l_end,
    ::ad_rss::world::WorldModel *worldModel) {
  const double ego_v_s_start = reference_line_info->AdcSlBoundary().start_s();
  const double ego_v_s_end = reference_line_info->AdcSlBoundary().end_s();
  double ego_v_l_start = reference_line_info->AdcSlBoundary().start_l();
  double ego_v_l_end = reference_line_info->AdcSlBoundary().end_l();
  ...
  if (common::util::IsFloatEqual(lane_width, 0.0)) {
    AERROR << "the lane_width is zero!";
    lane_width = FLAGS_numerical_epsilon;
  }
  if (common::util::IsFloatEqual(lane_length, 0.0)) {
    AERROR << "the lane_length is zero!";
    lane_length = FLAGS_numerical_epsilon;
  }
  ...
  rss_create_other_object(&leadingObject, rss_world_info.obs_speed, 0);
  ...
  leadingObject.occupiedRegions.emplace_back(occupiedRegion_leading);
  ...
  followingObject.occupiedRegions.emplace_back(occupiedRegion_following);
  ...
  roadSegment.emplace_back(laneSegment);
  roadArea.emplace_back(roadSegment);
  ...
  worldModel->scenes.emplace_back(scene);
  worldModel->timeIndex = frame->SequenceNum();
}

Status RssDecider::PerformRssCheckAndSetResult(
    const ::ad_rss::world::WorldModel &worldModel,
    ReferenceLineInfo *reference_line_info) {
