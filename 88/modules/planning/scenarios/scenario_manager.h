...
class ScenarioManager final {
 ...
 private:
  ...
  ScenarioConfig::ScenarioType SelectRescueScenario();
  ...
  void SetTebCommonStatus();

  bool JudgeReachTargetPoint(const common::VehicleState& car_position,
                             const common::PointENU& target_point);

  void CheckOverLap(const Frame& frame,
                    hdmap::PathOverlap** const traffic_sign_overlap,
                    hdmap::PathOverlap** const pnc_junction_overlap,
                    ReferenceLineInfo::OverlapType* const overlap_type);

  void CheckCurrentScenario(const Frame& frame,
                            ScenarioConfig::ScenarioType* const scenario_type);

  ScenarioConfig::ScenarioType GetCurrentScenario(
