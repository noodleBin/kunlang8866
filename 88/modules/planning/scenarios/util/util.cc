...
constexpr double kCloseL = 1.0;
constexpr double kEp = 0.01;
}  // namespace
...
bool CheckADCReadyToCruise(
  ...
  ADEBUG << "adc_heading: " << adc_heading * 180 / M_PI
         << ", path_point_amngle: " << path_point.theta() * 180 / M_PI
         << ", angle_difference: " << angle_difference
         << ", ref_heading_buffer: " << ref_heading_buffer;

  bool heading_align_w_reference_line =
      std::fabs(angle_difference) < ref_heading_buffer;
  bool is_near_front_obstacle = CheckADCSurroundObstacles(
      adc_position, path_point.theta(), frame, front_obstacle_buffer);
  bool is_still_rescue_state =
      frame->open_space_info().is_on_open_space_trajectory();
  ADEBUG << "linear_velocity: "
         << vehicle_status->vehicle_state().linear_velocity()
         << ", is_near_front_obstacle: " << is_near_front_obstacle
         << ", heading_align_w_reference_line: "
         << heading_align_w_reference_line
         << ", is_still_rescue_state: " << is_still_rescue_state;
...
bool GetEarlyExitRescueState(
  ...
  if (!rescue_config.enable_early_finish_rescue()) {
    AERROR << "disable early exit rescue state!!!";
    return false;
  }
  common::math::Vec2d ego_position = {vehicle_status->x(), vehicle_status->y()};
  double ego_heading = vehicle_status->heading();
  common::SLPoint ego_position_sl;
  auto& end_pose = frame->open_space_info().open_space_end_pose();
  const auto& rotate_angle = frame->open_space_info().origin_heading();
  const auto& rotate_origin = frame->open_space_info().origin_point();
  common::math::Vec2d world_end_pose;
  double world_end_pose_heading = 0;

  ADEBUG << "end_pose.size: " << end_pose.size();
  if (end_pose.size() > 3) {
    ADEBUG << "x: " << end_pose.at(0) << ", y: " << end_pose.at(1)
           << ", angle: " << end_pose.at(2) * 180 / M_PI;
  }
  if (end_pose.size() < 3) {
    AERROR << "get openspace end pose size < 3, return false.";
    return false;
  }
  common::math::Vec2d end_pt(end_pose.at(0), end_pose.at(1));
  common::util::Origin2World(rotate_origin, rotate_angle, end_pt,
                             end_pose.at(2), world_end_pose,
                             world_end_pose_heading);
  ...
  const auto reference_line_info = std::min_element(
      ...
      [&](const ReferenceLineInfo& ref_a, const ReferenceLineInfo& ref_b) {
        common::SLPoint adc_position_sl_a;
        common::SLPoint adc_position_sl_b;
        ref_a.reference_line().XYToSL(ego_position, &adc_position_sl_a);
        ref_b.reference_line().XYToSL(ego_position, &adc_position_sl_b);
        return std::fabs(adc_position_sl_a.l()) <
               std::fabs(adc_position_sl_b.l());
      });
  reference_line_info->reference_line().XYToSL(ego_position, &ego_position_sl);
  const auto& reference_line = reference_line_info->reference_line();
  const auto reference_point =
      reference_line.GetReferencePoint(ego_position_sl.s());
  const auto path_point = reference_point.ToPathPoint(ego_position_sl.s());
  double heading_diff =
      common::math::NormalizeAngle(ego_heading - path_point.theta());

  bool near_rescue_end_point =
      (ego_position.DistanceTo(world_end_pose) <
           rescue_config.early_cruise_dis_threshold() &&
       (reason == OpenspaceReason::OPERATION
            ? (ego_position.DistanceTo(world_end_pose) <
               rescue_config.operation_remain_dis_threshold())
            : true));
  ADEBUG << "ego_dis_to_end: " << ego_position.DistanceTo(world_end_pose);

  bool is_still_rescue_state =
      frame->open_space_info().is_on_open_space_trajectory();
  ADEBUG << "heading_diff_angle: " << heading_diff * 180 / M_PI
         << ", ego_position_sl.l: " << ego_position_sl.l()
         << ", near_rescue_end_point: " << near_rescue_end_point
         << ", is_still_rescue_state: " << is_still_rescue_state
         << ", openspace_reason: " << static_cast<int>(reason);

  if (std::fabs(heading_diff) * 180 / M_PI <
          rescue_config.early_cruise_heading_threshold() &&
      std::fabs(ego_position_sl.l()) <
          rescue_config.early_cruise_lat_threshold() &&
      near_rescue_end_point && is_still_rescue_state) {
    AINFO << "early from rescue to cruise. openspace_reason: "
...
bool CheckRefReadyToCruise(
  ...
  bool heading_align_w_reference_line =
      std::fabs(angle_difference) < ref_heading_buffer;
  bool is_near_front_obstacle = false;

  const double adc_length = common::VehicleConfigHelper::Instance()
                                ->GetConfig()
                                .vehicle_param()
                                .length();

  for (double i = 0.0; i < front_obstacle_buffer + kEp; i += adc_length) {
    if (i + adc_length > front_obstacle_buffer) {
      i = front_obstacle_buffer;
    }
    const auto ref_point =
        reference_line.GetReferencePoint(adc_position_sl.s() + i);
    const auto path_point = ref_point.ToPathPoint(adc_position_sl.s() + i);
    if (CheckADCSurroundObstacles(Vec2d{path_point.x(), path_point.y()},
                                  path_point.theta(), frame, 0)) {
      is_near_front_obstacle = true;
      break;
    }
  }

  AINFO << "is_ref_front_obstacle: " << is_near_front_obstacle
        << "heading_align_w_reference_line: " << heading_align_w_reference_line
        << "path_point.x() " << path_point.x();

  return (((vehicle_status->gear() == canbus::Chassis::GEAR_DRIVE ||
            std::fabs(vehicle_status->vehicle_state().linear_velocity()) <
                kMinSpeed) ||
           !rescue_status->is_select_back_pose()) &&
          !is_near_front_obstacle && heading_align_w_reference_line);
}
...
bool CheckADCSurroundObstacles(const common::math::Vec2d adc_position,
  ...
  // ADC box
  // lwt: added lat buffer; and use reference line heading, not adc heading
  Box2d adc_box(adc_position, adc_heading, adc_length + front_obstacle_buffer,
                adc_width + 2 * FLAGS_astar_first_lat_buffer);
  double shift_distance = (adc_length + front_obstacle_buffer) * 0.5 -
...
bool CheckADCHeading(const common::math::Vec2d adc_position,
  ...
  // const double kReducedHeadingBuffer = 0.2;  // (rad) TODO(Shu) move to
  // config
  const auto& reference_line = reference_line_info.reference_line();
  common::SLPoint adc_position_sl;
  reference_line.XYToSL(adc_position, &adc_position_sl);
  ...
  if (std::fabs(angle_difference) < heading_diff_to_reference_line) {
    return true;
  }
  return false;
}
...
bool CheckADCReadyToCruise(
  ...
  double ref_heading = reference_line_info->reference_line()
                           .GetReferencePoint(adc_position_sl.s())
                           .heading();

  // check whether has obstacle in front of vehicle
  bool is_near_front_obstacle =
      CheckADCSurroundObstacles(adc_position, ref_heading, frame,
  ...
  ADEBUG << "heading_align_w_reference_line: " << heading_align_w_reference_line
         << "ref_heading " << ref_heading << "adc_heading " << adc_heading;

  // check gear, linear_velocity, obstacle, beading, l
  return ((vehicle_status->gear() == canbus::Chassis::GEAR_DRIVE ||
           std::fabs(vehicle_status->vehicle_state().linear_velocity()) <
               common::VehicleConfigHelper::Instance()->GetConfig()
                   .vehicle_param().max_abs_speed_when_stopped()) &&
          !is_near_front_obstacle && heading_align_w_reference_line &&
          std::fabs(adc_position_sl.l()) < kCloseL);
}
