...
using century::common::math::AngleDiff;
using century::common::math::Vec2d;
using century::cyber::Clock;
using century::hdmap::HDMapUtil;
using century::hdmap::LaneWaypoint;
using century::hdmap::MapPathPoint;
using century::hdmap::PncMap;
using century::hdmap::RouteSegments;
...
bool ReferenceLineProvider::CanCreateReferenceLineForRouting(
    const routing::RoutingResponse& routing,
  ...
  if (nullptr == pnc_map_) {
    AERROR << "pnc_map is nullptr when checking routing applicability.";
    return false;
  }

  hdmap::PncMap pnc_map(pnc_map_->hdmap());
  if (!pnc_map.UpdateRoutingResponse(routing)) {
    AERROR << "Failed to update routing in temporary pnc_map.";
    return false;
  }

  std::list<hdmap::RouteSegments> route_segments;
  constexpr int kDefaultOvertakeStatus = 0;
  if (!pnc_map.GetRouteSegments(vehicle_state, kDefaultOvertakeStatus,
                                std::string(), std::string(),
                                &route_segments)) {
    AERROR << "Failed to extract route segments from temporary pnc_map.";
    return false;
  }
...
void ReferenceLineProvider::UpdateOvertakeStatus(
    const OvertakeStatus::Status &overtake_status) {
  std::lock_guard<std::mutex> lock(overtake_status_mutex_);
  overtake_status_ = overtake_status;
}

void ReferenceLineProvider::UpdateLaneBorrowLaneId(
    const std::string &lane_borrow_lane_id) {
  std::lock_guard<std::mutex> lock(lane_borrow_mutex_);
  lane_borrow_lane_id_ = lane_borrow_lane_id;
}

void ReferenceLineProvider::UpdateLaneChangeLaneId(
...
bool ReferenceLineProvider::IsStop() { return is_stop_; }

void ReferenceLineProvider::SetPlanningContext(PlanningContext *context) {
  context_ = context;
}
...
void ReferenceLineProvider::PrioritzeChangeLane(
  ...
  auto *prev_status =
      context_->mutable_planning_status()->mutable_change_lane();
  bool in_change_lane =
      ChangeLaneStatus::IN_CHANGE_LANE == prev_status->status();
  auto current_path_id = GetCurrentPathId(*route_segments);
  const auto &previous_path_id = prev_status->path_id();
  bool need_erase = false;
  auto iter = route_segments->begin();
  while (iter != route_segments->end()) {
    if (!iter->IsOnSegment()) {
      if (in_change_lane && iter->Id() == previous_path_id &&
          current_path_id != previous_path_id) {
        need_erase = true;
        break;
      } else {
  ...
  if (need_erase) {
    route_segments->erase(iter);
  }
}

bool ReferenceLineProvider::GetCurrentLane(
    const hdmap::HDMap &hdmap, std::string *adc_lane_id,
  ...
  *adc_lane_id = adc_lane_way_point->lane->id().id();
  auto *adc_navigation_path = century::common::util::FindOrNull(
  ...
  *adc_lane_priority = adc_navigation_path->path_priority();
...
void ReferenceLineProvider::GetNeighborLane(
    bool is_lane_change_needed,
...
void ReferenceLineProvider::GetSegmentForLaneChange(
    bool is_lane_change_needed, const std::string &lane_id,
    const std::string &adc_lane_id,
    const routing::ChangeLaneType &lane_change_type,
    const std::string &nearest_neighbor_lane_id, RouteSegments *segment) {
  if (is_lane_change_needed) {
...
bool ReferenceLineProvider::GetReferenceLinesFromRelativeMap(
  ...
  // // 1.get adc current lane info ,such as lane_id,lane_priority,neighbor
  // lanes std::unordered_set<std::string> navigation_lane_ids; for (const auto
  // &path_pair : relative_map_->navigation_path()) {
  //   const auto lane_id = path_pair.first;
  //   navigation_lane_ids.insert(lane_id);
  // }
  // if (navigation_lane_ids.empty()) {
  ...
  // vehicle_state_provider_->vehicle_state(); hdmap::LaneWaypoint
  // adc_lane_way_point; if
  // (!GetNearestWayPointFromNavigationPath(vehicle_state, navigation_lane_ids,
  //                                           &adc_lane_way_point)) {
  //   return false;
  // }
  ...
  std::string adc_lane_id;
  uint32_t adc_lane_priority;
  std::vector<std::string> left_neighbor_lane_ids;
  std::vector<std::string> right_neighbor_lane_ids;
  hdmap::LaneWaypoint adc_lane_way_point;
  bool get_current_lane = GetCurrentLane(
      *hdmap, &adc_lane_id, &adc_lane_priority, &left_neighbor_lane_ids,
      &right_neighbor_lane_ids, &adc_lane_way_point, reference_lines, segments);
  ...
  std::string nearest_neighbor_lane_id;
  GetNeighborLane(is_lane_change_needed, left_neighbor_lane_ids,
                  right_neighbor_lane_ids, target_lane_pair, adc_lane_way_point,
                  &lane_change_type, &nearest_neighbor_lane_id);
  // if (is_lane_change_needed) {
  //   // target on the left of adc
  //   if (left_neighbor_lane_ids.end() !=
  ...
  for (const auto &path_pair : relative_map_->navigation_path()) {
    ...
    GetSegmentForLaneChange(is_lane_change_needed, lane_id, adc_lane_id,
                            lane_change_type, nearest_neighbor_lane_id,
                            &segment);
    // if (is_lane_change_needed) {
    //   if (lane_id == nearest_neighbor_lane_id) {
...
bool ReferenceLineProvider::GetNearestWayPointFromNavigationPath(
  ...
  const int status = hdmap->GetLanesWithHeading(
      point, kMaxDistance, state.heading(), M_PI * 0.5, &lanes);
  if (status < 0) {
...
bool ReferenceLineProvider::CreateRouteSegments(
    const common::VehicleState &vehicle_state,
    std::list<hdmap::RouteSegments> *segments) {
  int overtake_status = static_cast<int>(OvertakeStatus::DEFAULT);
  {
    std::lock_guard<std::mutex> lock(overtake_status_mutex_);
    overtake_status = static_cast<int>(overtake_status_);
  }
  {
    std::lock_guard<std::mutex> lock(pnc_map_mutex_);
    if (!pnc_map_->GetRouteSegments(vehicle_state, overtake_status,
                                    lane_borrow_lane_id_, lane_change_lane_id_,
                                    segments)) {
...
bool ReferenceLineProvider::ExtendReferenceLine(const VehicleState &state,
  ...
  bool is_prepare =
      SmoothPreparation(segment_properties, &shifted_segments, *prev_ref,
                        *prev_segment, reference_line, segments);
  if (!is_prepare) {
    return false;
  }
...
bool ReferenceLineProvider::SmoothPreparation(
    const RouteSegments &segment_properties, RouteSegments *shifted_segments,
    const ReferenceLine &prev_ref, const RouteSegments &prev_segment,
...
bool ReferenceLineProvider::Shrink(const common::SLPoint &sl,
                                   ReferenceLine *reference_line,
                                   RouteSegments *segments) {
  static constexpr double kLookBackDisRatio = 1.5;
  ...
  if (sl.s() > FLAGS_look_backward_distance * kLookBackDisRatio) {
  ...
  if (FLAGS_enable_shrink_reference_line) {
    double max_heading_diff =
        M_PI * smoother_config_.shrink_reference_line_angle() / 180.0;
    // check heading
    ...
    if (last_index != ref_points.size() - 1) {
      need_shrink = true;
      ADEBUG << "SHRINK FOR U TURN";
      common::SLPoint forward_sl;
      reference_line->XYToSL(ref_points[last_index], &forward_sl);
      new_forward_distance = forward_sl.s() - sl.s();
    }
    ADEBUG << "last_index = " << last_index;
    auto index_current = reference_line->GetNearestReferenceIndex(sl.s());
    ADEBUG << "index_current = " << index_current;
    auto first_index = index_current;
    first_index--;
    while (first_index < index_current && first_index < last_index &&
           std::fabs(AngleDiff(ref_points[last_index].heading(),
                               ref_points[first_index].heading())) <
               max_heading_diff &&
           first_index > 0) {
      first_index--;
    }
    ADEBUG << "first_index = " << first_index;
    if (0 != first_index) {
      AINFO << "HAS BACK SHRINK";
      need_shrink = true;
      common::SLPoint backward_sl;
      reference_line->XYToSL(ref_points[first_index], &backward_sl);

      new_backward_distance = sl.s() - backward_sl.s();
    }
    ADEBUG << "new_backward_distance = " << new_backward_distance;
  }
  if (need_shrink) {
...
AnchorPoint ReferenceLineProvider::GetAnchorPoint(
  ...
  const Vec2d left_vec =
      Vec2d::CreateUnitVec2d(ref_point.heading() + M_PI * 0.5);
...
std::string ReferenceLineProvider::GetCurrentPathId(
