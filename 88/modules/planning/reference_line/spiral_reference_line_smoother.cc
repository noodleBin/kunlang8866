...
bool SpiralReferenceLineSmoother::SmoothWithoutAnchorPoints(
    const ReferenceLine& raw_reference_line, std::vector<double>* ptr_theta,
  ...
  return Smooth(raw_point2d, ptr_theta, ptr_kappa, ptr_dkappa, ptr_s, ptr_x,
                ptr_y);
}

bool SpiralReferenceLineSmoother::SmoothWithAnchorPoints(
    std::vector<double>* ptr_theta, std::vector<double>* ptr_kappa,
    std::vector<double>* ptr_dkappa, std::vector<double>* ptr_s,
    std::vector<double>* ptr_x, std::vector<double>* ptr_y) {
  bool smooth_succeed = false;
  ...
  if (start_index == 0) {
    ...
    smooth_succeed = Smooth(raw_point2d, ptr_theta, ptr_kappa, ptr_dkappa,
                            ptr_s, ptr_x, ptr_y);
    std::for_each(ptr_x->begin(), ptr_x->end(),
                  [this](double& x) { x += zero_x_; });

    std::for_each(ptr_y->begin(), ptr_y->end(),
    ...
    for (size_t i = 0; i + 1 < start_index; ++i) {
      ...
      overhead_s.emplace_back(p1.path_point.s() - p0.path_point.s());
    }

    std::vector<double> overhead_theta, overhead_kappa, overhead_dkappa;
    std::vector<double> overhead_x, overhead_y;
    for (size_t i = 0; i < anchor_points_.size(); ++i) {
      const auto& p = anchor_points_[i];
      if (i + 1 < start_index) {
        overhead_theta.emplace_back(p.path_point.theta());
        overhead_kappa.emplace_back(p.path_point.kappa());
        overhead_dkappa.emplace_back(p.path_point.dkappa());
        overhead_x.emplace_back(p.path_point.x());
        overhead_y.emplace_back(p.path_point.y());
      } else {
    ...
    ptr_theta->insert(ptr_theta->begin(), overhead_theta.begin(),
                      overhead_theta.end());
    ptr_kappa->insert(ptr_kappa->begin(), overhead_kappa.begin(),
                      overhead_kappa.end());
    ptr_dkappa->insert(ptr_dkappa->begin(), overhead_dkappa.begin(),
                       overhead_dkappa.end());
    ptr_s->insert(ptr_s->begin(), overhead_s.begin(), overhead_s.end());
    ptr_x->insert(ptr_x->begin(), overhead_x.begin(), overhead_x.end());
    ptr_y->insert(ptr_y->begin(), overhead_y.begin(), overhead_y.end());
...
bool SpiralReferenceLineSmoother::Smooth(
  ...
  if (anchor_points_.empty()) {
    SmoothWithoutAnchorPoints(raw_reference_line, &opt_theta, &opt_kappa,
                              &opt_dkappa, &opt_s, &opt_x, &opt_y);
  } else {
    SmoothWithAnchorPoints(&opt_theta, &opt_kappa, &opt_dkappa, &opt_s, &opt_x,
                           &opt_y);
  }
...
int SpiralReferenceLineSmoother::SmoothStandAlone(
    const std::vector<Eigen::Vector2d>& point2d, std::vector<double>* ptr_theta,
    std::vector<double>* ptr_kappa, std::vector<double>* ptr_dkappa,
    std::vector<double>* ptr_s, std::vector<double>* ptr_x,
    std::vector<double>* ptr_y) const {
...
std::vector<common::PathPoint> SpiralReferenceLineSmoother::Interpolate(
  ...
  common::PathPoint first_point =
      to_path_point(x.front(), y.front(), start_s, theta.front(), kappa.front(),
                    dkappa.front());
  smoothed_point2d.emplace_back(first_point);
