...
class SmootherUtil {
 public:
  ...
  void GetReferencePoints(const std::vector<common::math::Vec2d>& raw_points,
                          size_t* i, std::vector<ReferencePoint>* ref_points) {
    double s = 0.0;
    for (; s < FLAGS_smooth_length && *i < raw_points.size(); ++(*i)) {
      LineSegment2d segment(raw_points[*i - 1], raw_points[*i]);
      ref_points->emplace_back(MapPathPoint(raw_points[*i], segment.heading()),
                               0.0, 0.0);
      s += segment.length();
    }
  }
  ...
  bool Smooth() {
    ...
    size_t i = 1;
    {
      ...
      GetReferencePoints(raw_points_, &i, &ref_points);
    ...
    for (; i < raw_points_.size(); ++i) {
      ...
      while (j > 0 && s < FLAGS_smooth_length * 0.5) {
      ...
      while (j + 1 < raw_points_.size() && s < FLAGS_smooth_length * 0.5) {
 ...
 private:
  std::vector<AnchorPoint> CreateAnchorPoints(const ReferencePoint& init_point,
    ...
    for (const double s : anchor_s) {
      if (s + config_.max_constraint_interval() * 0.5 < sl.s()) {
        continue;
      }
...
int main(int argc, char* argv[]) {
  ...
  century::planning::SmootherUtil smoother_util(FLAGS_input_file);
