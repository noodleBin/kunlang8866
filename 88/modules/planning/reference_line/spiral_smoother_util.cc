...
int main(int argc, char* argv[]) {
  ...
  std::vector<Eigen::Vector2d> raw_points =
      century::planning::SpiralSmootherUtil::ReadCoordinatesFromFile(
          FLAGS_input_file);

  std::vector<century::common::PathPoint> smooth_points;
  auto res =
      century::planning::SpiralSmootherUtil::Smooth(raw_points, &smooth_points);
  if (!res) {
  ...
  century::planning::SpiralSmootherUtil::Export(FLAGS_output_file,
                                               smooth_points);
  return 0;
}
