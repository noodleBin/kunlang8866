...
century::common::Status NaviPathDecider::Process(
  ...
  if (!GetBasicPathData(reference_line, &path_points)) {
    ...
    return Status(century::common::ErrorCode::PLANNING_ERROR,
...
bool NaviPathDecider::GetBasicPathData(
  ...
  // get min path plan length s = v0 * t + 1 * 0.5 * a * t^2
  double path_len =
      start_plan_v_ * config_.min_look_forward_time() +
      start_plan_a_ * pow(config_.min_look_forward_time(), 2) * 0.5;
