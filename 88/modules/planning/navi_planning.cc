...
bool NaviPlanning::CheckSanity(const LocalView& local_view,
  ...
  DCHECK_GE(start_timestamp, vehicle_state_.timestamp());
  if (start_timestamp - vehicle_state_.timestamp() <
      FLAGS_message_latency_threshold) {
    auto future_xy = injector_->vehicle_state()->EstimateFuturePosition(
        start_timestamp - vehicle_state_.timestamp());
    vehicle_state_.set_x(future_xy.x());
    vehicle_state_.set_y(future_xy.y());
    vehicle_state_.set_timestamp(start_timestamp);
  }
...
bool NaviPlanning::FrameCheck(
    ADCTrajectory* const trajectory_pb,
    std::vector<TrajectoryPoint>* stitching_trajectory,
    const double& start_timestamp) {
  if (nullptr == stitching_trajectory) {
    return false;
  }
  ...
  TrajectoryStitcher::ComputeStitchingTrajectory(
      vehicle_state_, start_timestamp, planning_cycle_time,
  ...
  auto status =
      InitFrame(frame_num, stitching_trajectory->back(), vehicle_state_);
  auto* not_ready = trajectory_pb->mutable_decision()
                        ->mutable_main_decision()
                        ->mutable_not_ready();
  if (!frame_) {
  ...
  injector_->ego_info()->Update(stitching_trajectory->back(), vehicle_state_);
...
void NaviPlanning::RunOnce(const LocalView& local_view,
  ...
  if (CheckSanity(local_view, trajectory_pb, start_timestamp)) {
    return;
  }
  std::vector<TrajectoryPoint> stitching_trajectory;
  if (FrameCheck(trajectory_pb, &stitching_trajectory, start_timestamp)) {
    return;
  }
  ...
  Status status = Plan(start_timestamp, stitching_trajectory, trajectory_pb);
