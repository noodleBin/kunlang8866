...
GridSearch::GridSearch(const PlannerOpenSpaceConfig& open_space_conf) {
  ...
  node_radius_ = open_space_conf.warm_start_config().grid_a_star_node_radius();
}
...
void GridSearch::LoadGridAStarResult(GridAStartResult* result) {
  ...
  while (current_node->GetPreNode() != nullptr) {
    grid_a_x.emplace_back(current_node->GetGridX() * xy_grid_resolution_ +
                          XYbounds_[0]);
    grid_a_y.emplace_back(current_node->GetGridY() * xy_grid_resolution_ +
                          XYbounds_[2]);
