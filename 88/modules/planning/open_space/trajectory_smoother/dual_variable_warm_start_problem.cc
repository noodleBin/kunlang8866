...
void DualVariableWarmStartProblem::OSQPProblemSolve(
    const size_t horizon, const double ts, const Eigen::MatrixXd& ego,
    const Eigen::MatrixXi& obstacles_edges_num, size_t obstacles_num,
    const Eigen::MatrixXd& obstacles_a, const Eigen::MatrixXd& obstacles_b,
    const Eigen::MatrixXd& xWS, Eigen::MatrixXd* l_warm_up,
    Eigen::MatrixXd* n_warm_up, Eigen::MatrixXd* s_warm_up, bool& solve_flag) {
...
bool DualVariableWarmStartProblem::NormalOSQPProblemSolve(
    const size_t horizon, const double ts, const Eigen::MatrixXd& ego,
    const Eigen::MatrixXi& obstacles_edges_num, size_t obstacles_num,
    const Eigen::MatrixXd& obstacles_a, const Eigen::MatrixXd& obstacles_b,
    const Eigen::MatrixXd& xWS, Eigen::MatrixXd* l_warm_up,
...
void DualVariableWarmStartProblem::SlackQPProblemSolve(
    const size_t horizon, const double ts, const Eigen::MatrixXd& ego,
    const Eigen::MatrixXi& obstacles_edges_num, size_t obstacles_num,
    const Eigen::MatrixXd& obstacles_a, const Eigen::MatrixXd& obstacles_b,
    const Eigen::MatrixXd& xWS, Eigen::MatrixXd* l_warm_up,
...
bool DualVariableWarmStartProblem::IPOPTQPProblemSolve(
    const size_t horizon, const double ts, const Eigen::MatrixXd& ego,
    const Eigen::MatrixXi& obstacles_edges_num, size_t obstacles_num,
    const Eigen::MatrixXd& obstacles_a, const Eigen::MatrixXd& obstacles_b,
    const Eigen::MatrixXd& xWS, Eigen::MatrixXd* l_warm_up,
  ...
  solve_flag = (status == Ipopt::Solve_Succeeded ||
                 status == Ipopt::Solved_To_Acceptable_Level);
  return true;
}

bool DualVariableWarmStartProblem::IPOPTProblemSolve(
    const size_t horizon, const double ts, const Eigen::MatrixXd& ego,
    const Eigen::MatrixXi& obstacles_edges_num, size_t obstacles_num,
    const Eigen::MatrixXd& obstacles_a, const Eigen::MatrixXd& obstacles_b,
    const Eigen::MatrixXd& xWS, Eigen::MatrixXd* l_warm_up,
...
bool DualVariableWarmStartProblem::Solve(
  ...
  if (planner_open_space_config_.dual_variable_warm_start_config()
          .qp_format() == OSQP) {
    OSQPProblemSolve(horizon, ts, ego, obstacles_edges_num, obstacles_num,
                     obstacles_a, obstacles_b, xWS, l_warm_up, n_warm_up,
                     s_warm_up, solver_flag);
  } else if (planner_open_space_config_.dual_variable_warm_start_config()
                 .qp_format() == SLACKQP) {
    SlackQPProblemSolve(horizon, ts, ego, obstacles_edges_num, obstacles_num,
                        obstacles_a, obstacles_b, xWS, l_warm_up, n_warm_up,
                        s_warm_up, solver_flag);
  } else if (planner_open_space_config_.dual_variable_warm_start_config()
                 .qp_format() == IPOPTQP) {
    if (!IPOPTQPProblemSolve(horizon, ts, ego, obstacles_edges_num,
                             obstacles_num, obstacles_a, obstacles_b, xWS,
                             l_warm_up, n_warm_up, s_warm_up, solver_flag)) {
      return false;
    }
  } else if (planner_open_space_config_.dual_variable_warm_start_config()
                 .qp_format() == IPOPT) {
    if (!IPOPTProblemSolve(horizon, ts, ego, obstacles_edges_num, obstacles_num,
                           obstacles_a, obstacles_b, xWS, l_warm_up, n_warm_up,
                           s_warm_up, solver_flag)) {
      return false;
    }
  } else {  // debug mode
    if (NormalOSQPProblemSolve(horizon, ts, ego, obstacles_edges_num,
                               obstacles_num, obstacles_a, obstacles_b, xWS,
                               l_warm_up, n_warm_up, s_warm_up, solver_flag)) {
      return true;
    }
