...
DistanceApproachIPOPTFixedDualInterface::
  ...
  if (FLAGS_enable_use_minimum_turning_radius_to_get_kappa) {
    max_steer_angle_ = std::min(
        max_steer_angle_, std::atan2(1.0, vehicle_param_.min_turn_radius() /
...
void DistanceApproachIPOPTFixedDualInterface::eval_obj(int n, const T* x,
  ...
  CHECK(ts_ != 0) << "ts in distance_approach_ is 0";
  // TODO(ALL): It is necessary to check the values in x to prevent division by
  // zero during subsequent calculations.
  int control_index = control_start_index_;
  int time_index = time_start_index_;
  int state_index = state_start_index_;
...
void DistanceApproachIPOPTFixedDualInterface::eval_constraints(int n,
  ...
  /**
    ...
    CHECK(ts_ != 0) << "ts in distance_approach_ is 0";
    // TODO(ALL): It is necessary to check the values in x to prevent division
  by
    // zero during subsequent calculations.
    // state start index
    int state_index = state_start_index_;
