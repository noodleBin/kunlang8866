 ...
 */
inline Eigen::Vector2d ClosestPointOnLineSegment2D(
 ...
 */
inline double DistancePointToSegment2D(
    const Eigen::Ref<const Eigen::Vector2d>& point,
    const Eigen::Ref<const Eigen::Vector2d>& line_start,
    const Eigen::Ref<const Eigen::Vector2d>& line_end) {
  return (point - ClosestPointOnLineSegment2D(point, line_start, line_end))
      .norm();
}
 ...
 */
inline bool CheckLineSegmentsIntersection2D(
 ...
 */
inline double DistanceSegmentToSegment2D(
    const Eigen::Ref<const Eigen::Vector2d>& line1_start,
    const Eigen::Ref<const Eigen::Vector2d>& line1_end,
    const Eigen::Ref<const Eigen::Vector2d>& line2_start,
    const Eigen::Ref<const Eigen::Vector2d>& line2_end) {
  // TODO(all) more efficient implementation

  // check if segments intersect
  if (CheckLineSegmentsIntersection2D(line1_start, line1_end, line2_start,
                                      line2_end))
    return 0;
 ...
 */
inline double DistancePointToPolygon2D(const Eigen::Vector2d& point,
  ...
  for (size_t i = 0; i < vertices.size() - 1; ++i) {
    double new_dist =
        DistancePointToSegment2D(point, vertices.at(i), vertices.at(i + 1));
    //       double new_dist = CalcDistancePointToSegment( position,
...
inline double DistanceSegmentToPolygon2D(const Eigen::Vector2d& line_start,
  ...
  // check each polygon edge
  for (size_t i = 0; i < vertices.size() - 1; ++i) {
    double new_dist = DistanceSegmentToSegment2D(
  ...
  if (vertices.size() > 2) {
    // check last edge
    double new_dist = DistanceSegmentToSegment2D(
        line_start, line_end, vertices.back(), vertices.front());
    if (new_dist < dist) return new_dist;
  }
 ...
 * @return smallest distance between point and polygon
 */
inline double DistancePolygonToPolygon2D(const Point2dContainer& vertices1,
  ...
  for (size_t i = 0; i < vertices1.size() - 1; ++i) {
    double new_dist =
        DistanceSegmentToPolygon2D(vertices1[i], vertices1[i + 1], vertices2);
    if (new_dist < dist) dist = new_dist;
  }
  ...
  if (vertices1.size() > 2) {
    // check last edge
    double new_dist = DistanceSegmentToPolygon2D(vertices1.back(),
                                                 vertices1.front(), vertices2);
    if (new_dist < dist) return new_dist;
  }
...
inline double CalcDistanceLineToLine3D(
...
inline double CalcDistanceSegmentToSegment3D(
    const Eigen::Ref<const Eigen::Vector3d>& line1_start,
    const Eigen::Ref<const Eigen::Vector3d>& line1_end,
    const Eigen::Ref<const Eigen::Vector3d>& line2_start,
    const Eigen::Ref<const Eigen::Vector3d>& line2_end) {
...
double CalcClosestPointToApproachTime(const VectorType& x1,
...
template <typename VectorType>
double CalcClosestPointToApproachDistance(const VectorType& x1,
                                          const VectorType& vel1,
                                          const VectorType& x2,
                                          const VectorType& vel2,
                                          double bound_cpa_time = 0) {
  double ctime = CalcClosestPointToApproachTime<VectorType>(x1, vel1, x2, vel2);
...
template <typename VectorType>
double CalcDistancePointToLine(const VectorType& point,
...
template <typename VectorType>
double CalcDistancePointToSegment(const VectorType& point,
