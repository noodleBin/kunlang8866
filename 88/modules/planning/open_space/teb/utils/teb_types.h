...
struct Header {
  ...
  Header(const Header& h) : seq(h.seq), stamp(h.stamp), frame_id(h.frame_id) {}
  uint32_t seq;
  int stamp;
  int frame_id;
...
struct Twist {
  ...
  Twist(Eigen::Vector3f _linear, Eigen::Vector3f _angular)
      : linear(std::move(_linear)), angular(std::move(_angular)) {}
...
struct TwistWithCovariance {
  TwistWithCovariance() : twist(), covariance() { covariance.assign(0.0); }
  TwistWithCovariance(Twist _twist, boost::array<double, 36> _covariance)
      : twist(_twist), covariance(_covariance) {}
...
struct QuaternionStamped {
  ...
  QuaternionStamped(const QuaternionStamped& q)
      : header(q.header), quaternion(q.quaternion) {}
  Header header;
...
struct Point32 {
  ...
  Point32(const Point32& p) : x(p.x), y(p.y), z(p.z) {}
  float x;
  float y;
  float z;
...
struct Point {
  ...
  // Point(const Point& p) : x(0.0), y(0.0), z(0.0) {}
...
struct PoseStamped {
  ...
  PoseStamped(const PoseStamped& p) : header(p.header), pose(p.pose) {}
  Header header;
  Pose pose;
