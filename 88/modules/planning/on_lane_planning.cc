...
void OnLanePlanning::GenerateReversePathData(
  ...
  if (FLAGS_enable_reverse_trajectory) {
    ...
    while (i * kPathResolution < *distance_to_stop_point) {
      ...
      frenet_point.set_s(adc_sl_boundary.start_s() + i * kPathResolution +
                         common::VehicleConfigHelper::GetConfig()
                             .vehicle_param()
                             .back_edge_to_center() -
                         *distance_to_stop_point);
      frenet_point.set_l(center_l);
...
void OnLanePlanning::CalcuHonkingStats(ADCTrajectory* const ptr_trajectory_pb) {
  ...
  // If it is to sound the horn at the station, execute the immediate arrival at
  // the station.
  if (HonkingScenario::REACHE_DESTINATION ==
      ptr_trajectory_pb->honking_status().scenario()) {
    injector_->planning_context()
        ->mutable_planning_status()
        ->mutable_destination()
        ->set_arrived_station_immediately(true);
    JudgeArrivedStationImmediately(ptr_trajectory_pb);
  }
}
