...
century::common::PointENU SLToXYZ(const std::string& lane_id, const double s,
                                 const double l) {
...
void XYZToSL(const century::common::PointENU& point, std::string* lane_id,
             double* s, double* l) {
  ACHECK(lane_id);
  ACHECK(s);
  ACHECK(l);
...
double XYZDistance(const century::common::PointENU& p1,
...
void RefreshDefaultEndPoint() {
  century::routing::POI old_poi;
  ACHECK(cyber::common::GetProtoFromASCIIFile(EndWayPointFile(), &old_poi));

  century::routing::POI new_poi;
  for (const auto& old_landmark : old_poi.landmark()) {
    century::routing::Landmark* new_landmark = new_poi.add_landmark();
    ...
    for (const auto& old_end_point : old_landmark.waypoint()) {
      century::common::PointENU old_xyz;
      ...
      century::routing::LaneWaypoint new_end_point;
...
int main(int argc, char* argv[]) {
  ...
  century::hdmap::RefreshDefaultEndPoint();
  return 0;
}
