...
Status LanesXmlParser::ParseLane(const tinyxml2::XMLElement& xml_node,
  ...
  auto ret = ParseLaneBoundaryTypes(sub_node, lane);
  if (ret != Status::OK()) {
    return ret;
  }
  ...
  ret = ParseLaneProperties(xml_node, sub_node, lane);
  if (ret != Status::OK()) {
    return ret;
  }
...
Status LanesXmlParser::ParseLaneProperties(const tinyxml2::XMLElement& xml_node,
                                           const tinyxml2::XMLElement* sub_node,
                                           PbLane* lane) {
  // turn type
  Status success;
...
Status LanesXmlParser::ParseLaneBoundaryTypes(
    const tinyxml2::XMLElement* sub_node, PbLane* lane) {
  Status success;
  int checker;
  if (sub_node) {
