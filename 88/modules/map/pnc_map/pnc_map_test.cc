...
TEST_F(PncMapTest, GetNeighborPassages) {
  const auto& road0 = routing_.road(0);
  {
    ...
    pnc_map_->GetNeighborPassages(road0, 0, &result);
  ...
  {
    std::vector<int> result;
    pnc_map_->GetNeighborPassages(road0, 1, &result);
  ...
  {
    std::vector<int> result;
    pnc_map_->GetNeighborPassages(road0, 2, &result);
    EXPECT_EQ(1, result.size());
  }
  {
    std::vector<int> result;
    pnc_map_->GetNeighborPassages(road0, 3, &result);
    EXPECT_EQ(1, result.size());
