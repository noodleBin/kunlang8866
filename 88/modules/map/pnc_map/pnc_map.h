...
class PncMap {
 ...
 public:
  ...
  bool UpdateRoutingResponse(routing::RoutingResponse routing);
 ...
 private:
  bool UpdateRouteSegments(const std::vector<int> &drive_passages,
                           const routing::RoadSegment &road,
                           const int passage_index,
                           const double backward_length,
                           const double forward_length,
                           std::list<RouteSegments> *const route_segments);
  ...
  bool LongFunWithGetNearestPointFromRouting(
   ...
   * @param drive_passages the result of the neighboring passages
   * @return all the indices of the neighboring passages, including
   * start_passage.
   */
  bool GetNeighborPassages(const routing::RoadSegment &road,
                           const int &overtake_status, int start_passage,
                           std::vector<int> *drive_passages) const;

  bool GetNeighborPassagesForOvertake(
  ...
  OvertakeDirection ConvertOvertakeDirection(const int &overtake_status) const;

  bool TurnAroundChangeGetNearestPointFromRouting(
      const common::VehicleState &point, LaneWaypoint *waypoint) const;
 ...
 private:
  ...
  std::unordered_set<std::string> current_route_segment_lane_ids_;
