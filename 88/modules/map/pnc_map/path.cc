...
using century::common::math::kMathEpsilon;
using century::common::math::LineSegment2d;
using century::common::math::Sqr;
using century::common::math::Vec2d;
using century::common::util::DebugStringFormatter;
using std::placeholders::_1;

namespace {

constexpr double kSampleDistance = 0.25;
constexpr double kLateralBuffer = 0.1;
...
void Path::InitWidth() {
  ...
  for (int i = 0; i < num_sample_points_; ++i) {
    ...
    if (point.lane_waypoints().empty()) {
      lane_left_width_.push_back(FLAGS_default_lane_width * 0.5);
      lane_right_width_.push_back(FLAGS_default_lane_width * 0.5);

      road_left_width_.push_back(FLAGS_default_lane_width * 0.5);
      road_right_width_.push_back(FLAGS_default_lane_width * 0.5);
...
bool Path::GetProjectionUseOneProjection(const common::math::Vec2d& point,
                                         const double adc_s,
                                         double* accumulate_s,
                                         double* lateral) const {
  double distance = 0.0;
  return GetProjectionUseOneProjection(point, adc_s, accumulate_s, lateral,
                                       &distance);
}
...
// only projection in front of adc.
bool Path::GetProjectionUseOneProjection(const Vec2d& point, const double adc_s,
  ...
  *min_distance = std::numeric_limits<double>::infinity();
  ...
  for (int i = 0; i < num_segments_; ++i) {
    ADEBUG << "accumulated_s_[" << i << "]  " << accumulated_s_[i];
    // projection in front of ADC.
    if (accumulated_s_[i] < adc_s) {
      continue;
    }
    const double distance = segments_[i].DistanceSquareTo(point);
    if (distance < *min_distance) {
  ...
  *min_distance = std::sqrt(*min_distance);
  ...
  } else if (min_index == num_segments_ - 1) {  // last point
    *accumulate_s = accumulated_s_[min_index] + std::max(0.0, proj);
    if (proj > 0) {
      *lateral = prod;
    } else {
...
// check whether the bounding box is on path
bool Path::IsOnPath(const Box2d& box) const {
  for (auto point : box.GetAllCorners()) {
    if (!IsOnPath(point)) {
      return false;
    }
  }
...
// check whether the point is on path
...
bool Path::OverlapWith(const common::math::Box2d& box, double width) const {
  ...
  const double radius_sqr = Sqr(box.diagonal() * 0.5 + width) + kMathEpsilon;
...
bool Path::OverlapWith(const common::math::Box2d& box,
                       const common::math::Polygon2d& polygon2d,
                       double width) const {
  // TODO(wanglei): add use path approximation case
  ...
  for (const auto& segment : segments_) {
    ...
    if (polygon2d.DistanceTo(segment) <= width + kLateralBuffer) {
      return true;
    }
  }
  return false;
...
void PathApproximation::GetProjectionSegmentsMinprj(int *first_segment_idx,
                                    int last_segment_idx, int i,
                                    const double min_projection) const {
...
bool PathApproximation::GetProjectionSegments(const common::math::Vec2d& point,
                                          double *min_distance_sqr_with_error,
                                          const auto& original_segments,
                                          double* min_distance, int i,
                                          int *nearest_segment_idx) const {
    ...
    GetProjectionSegmentsMinprj(&first_segment_idx, i,
                                last_segment_idx, min_projection);
    }
    ...
    double proj = x0 * ux + y0 * uy, distance = 0.0;
      if (proj < 0.0) {
...
bool PathApproximation::GetProjection(const Path& path,
  ...
  *min_distance = std::numeric_limits<double>::infinity();
  ...
  for (size_t i = 0; i < segments_.size(); ++i) {
    ...
    if (GetProjectionSegments(point, &min_distance_sqr_with_error,
        original_segments, min_distance, i, &nearest_segment_idx) == false) {
      continue;
    }
  }
...
bool PathApproximation::OverlapWith(const Path& path, const Box2d& box,
  ...
  const Vec2d center = box.center();
  const double radius = box.diagonal() * 0.5 + width;
