...
using Point = century::common::PointENU;
using AABox2d = century::common::math::AABox2d;
using Vec2d = century::common::math::Vec2d;
...
TEST(TestSuite, hdmap_circle_path) {
  ...
  EXPECT_NEAR(accumulate_s, total_length * 0.5, 1e-6);
  ...
  for (int case_id = -10; case_id <= 80; ++case_id) {
    ...
    const double length = kRadius * sin(angle * 0.5) * 2.0;
    if (s <= 0.0) {
