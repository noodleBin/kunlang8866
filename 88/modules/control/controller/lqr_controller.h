...
/**
 * @file
 * @brief Defines the LQRController class.
 */
...
class LQRController : public Controller {
 public:
  ...
  common::Status ComputeControlCommand(
      ...
      ControlCommand *const cmd) override;
 ...
 protected:
  bool UpdateState(const size_t traj_size, SimpleLateralDebug *debug,
  ...
  void UpdateMatrixQ(SimpleLateralDebug *debug);
  void MRACControl(const canbus::Chassis *chassis,
                   const double steering_position, const double steer_limit,
                   const double steer_diff_with_max_rate,
                   SimpleLateralDebug *debug, double *steer_angle);

  void SteeringAngleCalculate(const canbus::Chassis *chassis,
                              SimpleLateralDebug *debug,
                              ControlCommand *const cmd);
  double ComputeFeedForward(double ref_curvature) const;

  bool ComputeLateralErrors(const size_t traj_size, const double x,
                            ...
                            SimpleLateralDebug *debug,
                            ControlCommand *const cmd);
  void ComputeHeadingData(const double linear_v, const double heading_error,
                          const double lateral_error, const double linear_a,
  ...
  void CloseLogFile();
  void NavigationModePositionUpdate();
  void SolveMatrixK();

  void CalculateErrorSubfunction(
      const double x, const double y, const double theta, const double linear_v,
      const double linear_a, const double angular_v,
      const TrajectoryAnalyzer &trajectory_analyzer,
      const century::common::TrajectoryPoint &target_point,
      const century::common::TrajectoryPoint &adc_point,
      const century::common::TrajectoryPoint &curvature_preview_point,
      SimpleLateralDebug *debug, ControlCommand *const cmd);

  bool TrajectoryAnomalyDetection(const size_t traj_size,
  ...
  bool InputAnomalyDetection(
      ...
      const century::common::TrajectoryPoint &adc_point);
  void ResetData();
  bool CalculateReachEndS(
      const size_t traj_size,
      const google::protobuf::RepeatedPtrField<century::common::PathPoint> path_data,
      ControlCommand *const cmd);
  bool DiagonalModeCompute(
      const canbus::Chassis *chassis,
  ...
  common::MeanFilter real_lateral_error_filter_;
  common::MeanFilter l_rate_filter_;
  common::MeanFilter heading_error_filter_;
  common::MeanFilter curve_filter_;
  common::MeanFilter steering_straightaway_filter_;
  common::MeanFilter steering_curve_filter_;
  // Lead/Lag controller
  ...
  double pre_steer_angle_1axis_ = 0.0;

  double pre_steer_angle_4axis_ = 0.0;
  ...
  double lateral_error_threshold_ = 0.3;

  double steer_limit_velocity_threshold_ = 3.0;

  double max_centripetal_acceleration_rate_ = 0.35;

  // add
  double min_q0_ = 0.0;

  double q0_offset_ = 0.0;
  std::shared_ptr<DependencyInjector> injector_;
  bool lanefollow_flag_ = true;
  double lateral_error_straightaway_ = 0.0;
  planning::ADCTrajectory target_tracking_trajectory_;
  const localization::LocalizationEstimate *localization_ = nullptr;
  const canbus::Chassis *chassis_ = nullptr;
  bool enable_use_path_data_ = false;
  bool need_forward_control_logic_ = true;
  size_t path_data_size_ = 0;
  double steering_compensation_ = 0.0;
  double heading_compensation_ = 0.0;
};
