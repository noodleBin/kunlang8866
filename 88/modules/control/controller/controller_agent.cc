  ...
  for (auto active_controller : control_conf->active_controllers()) {
    switch (active_controller) {
      ...
      case ControlConf::LON_CONTROLLER:
        ...
        break;
      case ControlConf::PP_CONTROLLER:
        controller_factory_.Register(
            ControlConf::PP_CONTROLLER,
            []() -> Controller * { return new PPController(); });
        break;
      case ControlConf::LQR_CONTROLLER:
        controller_factory_.Register(
            ControlConf::LQR_CONTROLLER,
            []() -> Controller * { return new LQRController(); });
        break;
      default:
...
Status ControllerAgent::ComputeControlCommand(
    ...
    Status t_control_status = Status::OK();
  for (auto &controller : controller_list_) {
    ADEBUG << "controller:" << controller->Name() << " processing ...";
    double start_timestamp = Clock::NowInSeconds();
    t_control_status = controller->ComputeControlCommand(localization, chassis, trajectory,aeb_result, cmd);
    ...
    if(!t_control_status.ok())
    {
      return t_control_status;
    }
  }
  return t_control_status;
}
