...
## Input
  * Point cloud data from LiDAR sensor (ROS topic `/century/sensor/velodyne64/compensator/PointCloud2`)
  * GNSS observation and ephemeris data from GNSS sensor (ROS topic `/century/sensor/gnss/rtk_obs` and `/century/sensor/gnss/rtk_eph`)
  * GNSS best pose from GNSS sensor (ROS topic is `/century/sensor/gnss/best_pose`)
  * Imu data from IMU sensor (ROS topic `/century/sensor/gnss/imu`)
...
## Output
  * Localization result defined by Protobuf message `LocalizationEstimate`, which can be found in file `localization/proto/localization.proto`. (ROS topic `/century/localization/pose`)
  * Localization status defined by Protobuf message `LocalizationStatus`, which can be found in file `localization/proto/localization.proto`. (ROS topic `/century/localization/msf_status`)
  * We also provide intermediate results: Gnss localization result, LiDAR localization result. (ROS topic `/century/localization/msf_gnss`, `/century/localization/msf_lidar`)
...
### GNSS Localization Setting
  This module works in two modes. One is directly to use GNSS best pose (`/century/sensor/gnss/best_pose`) from INS system, such as NovAtel system. In the second mode, the initial gnss observations and ephemeris information (`/century/sensor/gnss/rtk_obs` and `/century/sensor/gnss/rtk_eph`) are used to calculate GNSS localization result.

  You can set `gnss_mode` in `century/localization/conf/localization.conf` to decide which mode you want to use. The default mode is gnss best pose.
...
## Generate Localization Map
  Localization map is used for LiDAR-based localization, which is a grid-cell representation of the environment. Each cell stores the statistics of laser reflection intensity and altitude. The map is organized as a group of map nodes. For more information, please refer to `century/modules/localization/msf/local_map`.

  We provide a script (`century/scripts/msf_local_map_creator.sh`) to generate localization map. Usage: `bash msf_local_map_creator.sh [pcd_folder] [pose_file] [zone_id] [map_folder]`.   
