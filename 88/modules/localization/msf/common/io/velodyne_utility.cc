...
void LoadPcdPoses(const std::string& file_path,
  ...
  LoadPcdPosesFromCsv(file_path, poses, timestamps);
}
  ...
  if (file.is_open()) {
    ...
    while (std::getline(file, line)) {
      ...
      if (std::getline(iss, cell, ',')) {
        pose.timestamp = std::stod(cell);
      }
      if (std::getline(iss, cell, ',')) {
        pose.x = std::stof(cell);
      }
      if (std::getline(iss, cell, ',')) {
        pose.y = std::stof(cell);
      }
      if (std::getline(iss, cell, ',')) {
        pose.z = std::stof(cell);
      }
      if (std::getline(iss, cell, ',')) {
        pose.roll = std::stof(cell);
      }
      if (std::getline(iss, cell, ',')) {
        pose.pitch = std::stof(cell);
      }
      if (std::getline(iss, cell, ',')) {
        pose.yaw = std::stof(cell);
      }

      poses->emplace_back(pcl::getTransformation(pose.x + UTM_offset_x, pose.y + UTM_offset_y, pose.z + UTM_offset_z, pose.roll, pose.pitch, pose.yaw).cast<double>());
      timestamps->emplace_back(pose.timestamp);
      AINFO << "timestamp: " << pose.timestamp << ", x: " << pose.x << ", y: " << pose.y << ", z: "
                  << pose.z << ", roll: " << pose.roll << ", pitch: " << pose.pitch << ", yaw: " << pose.yaw;
    }
    file.close();
  }
}
