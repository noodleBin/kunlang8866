...
TEST(MapNdtTestSuite, matrix) {
  const std::string pcd_folder =
      "/century/modules/localization/msf/local_map/test_data/ndt_map/pcds";
  EXPECT_TRUE(century::cyber::common::EnsureDirectory(pcd_folder));
  const std::string pose_files = pcd_folder + "/poses.txt";

  const std::string map_base_folder =
      "/century/modules/localization/msf/local_map/test_data/ndt_map/map_data";
  EXPECT_TRUE(century::cyber::common::EnsureDirectory(map_base_folder));

  ::century::common::EigenAffine3dVec pcd_poses;
