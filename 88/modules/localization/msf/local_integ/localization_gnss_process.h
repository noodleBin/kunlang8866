...
struct EphKey {
  ...
  EphKey(const century::drivers::gnss::GnssType type, const unsigned int prn,
         double toe) {
    gnss_type = type;
    sat_prn = prn;
    eph_toe = toe;
  ...
  EphKey() {
    gnss_type = century::drivers::gnss::SYS_UNKNOWN;
    sat_prn = 0;
    eph_toe = -0.1;
  }
...
class LocalizationGnssProcess {
 public:
  ...
  ~LocalizationGnssProcess();
  century::common::Status Init(const LocalizationIntegParam &param);
