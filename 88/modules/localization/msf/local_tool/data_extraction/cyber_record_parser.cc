...
using century::localization::msf::CyberRecordReader;
using century::localization::msf::LocationExporter;
using century::localization::msf::PCDExporter;

bool get_data_filenames(const std::string &bag_folder,
                        std::vector<std::string> &data_filenames) {  // NOLINT
  data_filenames.clear();

  if (!boost::filesystem::exists(bag_folder) &&
      !boost::filesystem::is_directory(bag_folder)) {
    AERROR << bag_folder << " not exist!";
    return false;
  }

  boost::filesystem::path inputFilePath(bag_folder);
  boost::filesystem::recursive_directory_iterator it(inputFilePath);
  boost::filesystem::recursive_directory_iterator endit;

  while (it != endit) {
    if (boost::filesystem::is_regular_file(*it) &&
        it->path().extension() == ".record") {
      std::string file_name = it->path().string();
      data_filenames.push_back(file_name);
    } else {
      AINFO << it->path().extension() << ", not  .record! ";
    }
    it++;
  }

  if (data_filenames.size() > 0) return true;
  return false;
}
...
int main(int argc, char **argv) {
  ...
  boost_desc.add_options()("help", "produce help message")(
      "bag_folder", boost::program_options::value<std::string>(),
      "provide the bag folder")("out_folder",
  ...
  if (boost_args.count("help") || !boost_args.count("bag_folder") ||
      !boost_args.count("out_folder")) {
    AERROR << boost_desc;
    return 0;
  }

  const std::string bag_folder = boost_args["bag_folder"].as<std::string>();
  ...
  const std::string ins_loc_topic =
      boost_args["ins_loc_topic"].as<std::string>();
  ...
  reader.Subscribe(ins_loc_topic, [&](const std::string &msg) {
    loc_exporter->InsLocCallback(msg);
  });
  ...
  if (!get_data_filenames(bag_folder, data_files)) {
    AERROR << "Not found record files, please check the file path!";
    return 0;
  }
  reader.Read(data_files);
  AINFO << " saving trajectory ...... ";
  loc_exporter->WritePCD(pcd_folder);
  AINFO << " saving trajectory OK! ";
