...
int main(int argc, char** argv) {
  ...
  std::cout << "single_resolution_map: " << single_resolution_map
            << std::endl;
  ...
  for (std::size_t i = 0; i < pose_files.size(); ++i) {
    century::localization::msf::velodyne::LoadPcdPoses(
        pose_files[i], &pcd_poses[i], &time_stamps[i] /*, &pcd_indices[i]*/);
  }

  if (!century::localization::msf::system::IsExists(map_base_folder)) {
    if (century::localization::msf::system::CreateDirectory(map_base_folder)) {
      std::cerr << "Create map directory failed." << std::endl;
    }
  }

  // Output Config file
  century::localization::msf::pyramid_map::NdtMapConfig ndt_map_config(
      "map_ndt_v01");
  if (strcasecmp(resolution_type.c_str(), "single") == 0) {
  ...
  century::localization::msf::pyramid_map::NdtMap ndt_map(&ndt_map_config);
  unsigned int thread_size = 4;
  century::localization::msf::pyramid_map::NdtMapNodePool ndt_map_node_pool(
      pool_size + 1, thread_size);
  ...
  // Plane extractor
  century::localization::msf::FeatureXYPlane plane_extractor;

  // std::ofstream file("/century/data/test_data/scan.txt");
  for (unsigned int i = 0; i < pcd_folder_paths.size(); ++i) {
    ...
    for (unsigned int frame_idx = 0; frame_idx < pcd_poses_i.size();
         ++frame_idx) {
      century::localization::msf::velodyne::VelodyneFrame velodyne_frame;
      std::string pcd_file_path = pcd_folder_paths[i] + "/" +
                                  std::to_string(time_stamps[i][frame_idx]) +
                                  ".pcd";
      AINFO << "pcd_file_path: " << pcd_file_path;
      Eigen::Affine3d pcd_pose = pcd_poses_i[frame_idx];
      // Load pcd
      century::localization::msf::velodyne::LoadPcds(pcd_file_path, frame_idx, pcd_pose,
                                                     &velodyne_frame, false);

      AINFO << "Loaded " << velodyne_frame.pt3ds.size()
            << ", 3D Points at Trial: " << i
            << ", Frame: " << std::to_string(time_stamps[i][frame_idx]) << ".";

      // Process the point cloud
      for (size_t k = 0; k < velodyne_frame.pt3ds.size(); ++k) {
        Eigen::Vector3d& pt3d_local = velodyne_frame.pt3ds[k];
        unsigned char intensity = /*150 * */velodyne_frame.intensities[k];
        Eigen::Vector3d pt3d_global = velodyne_frame.pose * pt3d_local;
        // AINFO << "intensity: " << float(intensity);


        pt3d_global.x() = pt3d_local.x() + 250932.851957;
        pt3d_global.y() = pt3d_local.y() + 3987498.593868;
        pt3d_global.z() = pt3d_local.z();

        // file << pt3d_local[0]+2868.149567 << " " << pt3d_local[1]-2132.023845 << " " << pt3d_local[2]-10.308940 << std::endl;

        for (size_t res = 0; res < ndt_map_config.map_resolutions_.size();
             ++res) {
          century::localization::msf::pyramid_map::MapNodeIndex index =
              century::localization::msf::pyramid_map::MapNodeIndex::
                  GetMapNodeIndex(ndt_map_config, pt3d_global,
                                  static_cast<unsigned int>(res), zone_id);

          century::localization::msf::pyramid_map::NdtMapNode* ndt_map_node =
              static_cast<century::localization::msf::pyramid_map::NdtMapNode*>(
                  ndt_map.GetMapNodeSafe(index));
          century::localization::msf::pyramid_map::NdtMapMatrix&
              ndt_map_matrix = static_cast<
                  century::localization::msf::pyramid_map::NdtMapMatrix&>(
          ...
          century::localization::msf::pyramid_map::NdtMapCells& ndt_map_cell =
