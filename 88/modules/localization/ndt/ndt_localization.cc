...
void NDTLocalization::Init() {
  tf_buffer_ = century::transform::Buffer::Instance();
  tf_buffer_->Init();
...
bool NDTLocalization::QueryPoseFromTF(double time, Eigen::Affine3d* pose) {
  ...
  century::transform::TransformStamped query_transform;
  try {
...
void NDTLocalization::ComposeLocalizationStatus(
  ...
  century::common::Header* header = localization_status->mutable_header();
  double timestamp = century::cyber::Clock::NowInSeconds();
  header->set_timestamp_sec(timestamp);
