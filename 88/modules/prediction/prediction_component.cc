...
using century::common::adapter::AdapterConfig;
...
bool IsTrajectoryPointOnMap(const common::TrajectoryPoint& trajectory_point) {
  if (!trajectory_point.has_path_point()) {
    return false;
  }

  const auto& path_point = trajectory_point.path_point();
  const Eigen::Vector2d position(path_point.x(), path_point.y());

  if (PredictionMap::InJunction(position, FLAGS_junction_search_radius)) {
    return true;
  }

  century::common::PointENU map_point;
  map_point.set_x(position.x());
  map_point.set_y(position.y());
  map_point.set_z(0.0);

  const auto nearby_lanes =
      PredictionMap::GetNearbyLanes(map_point, FLAGS_lane_search_radius);
  for (const auto& lane_info : nearby_lanes) {
    if (lane_info != nullptr &&
        lane_info->IsOnLane({position.x(), position.y()})) {
      return true;
    }
  }
  return false;
...
void TruncatePredictionObstaclesToMap(PredictionObstacles* obstacles) {
  if (obstacles == nullptr) {
    return;
  }

  auto* mutable_prediction_obstacles = obstacles->mutable_prediction_obstacle();
  for (int obstacle_idx = mutable_prediction_obstacles->size() - 1;
       obstacle_idx >= 0; --obstacle_idx) {
    auto* prediction_obstacle =
        mutable_prediction_obstacles->Mutable(obstacle_idx);
    auto* trajectories = prediction_obstacle->mutable_trajectory();

    for (int traj_idx = trajectories->size() - 1; traj_idx >= 0; --traj_idx) {
      auto* trajectory = trajectories->Mutable(traj_idx);
      auto* points = trajectory->mutable_trajectory_point();

      int invalid_point_idx = -1;
      for (int point_idx = 0; point_idx < points->size(); ++point_idx) {
        if (!IsTrajectoryPointOnMap(points->Get(point_idx))) {
          invalid_point_idx = point_idx;
          break;
        }
      }

      if (invalid_point_idx >= 0) {
        points->DeleteSubrange(invalid_point_idx,
                               points->size() - invalid_point_idx);
      }

      if (points->empty()) {
        trajectories->DeleteSubrange(traj_idx, 1);
      }
    }

    if (prediction_obstacle->trajectory().empty()) {
      mutable_prediction_obstacles->DeleteSubrange(obstacle_idx, 1);
    }
  }
}
...
void PredictionComponent::OfflineProcessFeatureProtoFile(
  ...
  Features features;
  century::cyber::common::GetProtoFromBinaryFile(features_proto_file_name,
                                                &features);
...
bool PredictionComponent::PredictionEndToEndProc(
  ...
  TruncatePredictionObstaclesToMap(&prediction_obstacles);
