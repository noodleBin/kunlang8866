...
void ObstaclesContainer::Insert(const ::google::protobuf::Message& message) {
  ...
  if (std::fabs(timestamp - timestamp_) > FLAGS_replay_timestamp_gap ||
    ...
    // Backward or large-gap timestamp: clear obstacle history (bag loop replay)
    ptr_obstacles_.Clear();
    ADEBUG << "Replay mode is enabled, clearing obstacle history.";
  } else if (timestamp == timestamp_) {
    AWARN << "Duplicate timestamp [" << std::fixed << std::setprecision(6)
          << timestamp << "], skipping frame.";
    return;
  }
...
SubmoduleOutput ObstaclesContainer::GetSubmoduleOutput(
    const size_t history_size, const century::cyber::Time& frame_start_time) {
