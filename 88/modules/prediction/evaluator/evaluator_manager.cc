...
void EvaluatorManager::RegisterEvaluators() {
  const std::array<ObstacleConf::EvaluatorType, 9> evaluator_types = {{
      ...
      interaction_evaluator_,
  }};

  for (const auto type : evaluator_types) {
...
std::unique_ptr<Evaluator> EvaluatorManager::CreateEvaluator(
  ...
  switch (type) {
    ...
    case ObstacleConf::VECTORNET_TRT_EVALUATOR: {
      evaluator_ptr.reset(new VectornetTrtEvaluator());
      break;
    }
    default: {
      break;
