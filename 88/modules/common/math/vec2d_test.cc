...
TEST(Vec2dTest, NomralCases) {
  ...
  EXPECT_NEAR(unit_pt.x(), std::sqrt(2.0) * 0.5, 1e-5);
  EXPECT_NEAR(unit_pt.y(), std::sqrt(2.0) * 0.5, 1e-5);
  EXPECT_NEAR(unit_pt.Angle(), M_PI_4, 1e-5);
}

TEST(Vec2dTest, rotate) {
  Vec2d pt(4, 0);
  auto p1 = pt.rotate(M_PI * 0.5);
