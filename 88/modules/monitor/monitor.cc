...
bool Monitor::Init() {
  ...
  node_ = century::cyber::CreateNode(node_name);
...
// #endif
  ...
  runners_.emplace_back(new DaemonMonitor());

  return true;
}

bool Monitor::Proc() {
  const double current_time = century::cyber::Clock::NowInSeconds();
  if (!MonitorManager::Instance()->StartFrame(current_time)) {
    runners_.back()->Tick(current_time);
    return false;
  }
