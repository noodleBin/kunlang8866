...
DECLARE_string(camera_right_rear_topic);
DECLARE_string(camera_rear_topic);
DECLARE_string(camera_left_front_topic);
DECLARE_string(camera_left_rear_topic);
DECLARE_string(camera_right_front_topic);
DECLARE_string(camera_front_topic);

DECLARE_string(lidar_helios_front_left_topic);
DECLARE_string(lidar_helios_rear_right_topic);
DECLARE_string(lidar_bp_front_left_topic);
DECLARE_string(lidar_bp_rear_right_topic);
DECLARE_string(lidar_bp_rear_left_topic);
DECLARE_string(lidar_bp_front_right_topic);
...
// Returns true when running on the slave Orin (sensor perception board).
// Mirrors the DomainController check in launch_century_system.sh.
inline bool IsSlaveOrin() {
  std::ifstream f("/usr/autowin/version");
  if (!f.is_open()) {
    return false;
  }
  std::string line;
  while (std::getline(f, line)) {
    if (line.find("DomainController:") != std::string::npos) {
...
class LidarPackedReader : public CyberReader<drivers::PointCloudPacked> {
 public:
  using MsgType = drivers::PointCloudPacked;
  using MsgPtr = std::shared_ptr<MsgType>;
  using CyberReader<MsgType>::CyberReader;

  ~LidarPackedReader() = default;

 protected:
  void OnReceivedMsg(const MsgPtr& msg) noexcept override {
    CheckMessageFrequency(msg, channel_name_, module_type::E_LIDAR,
                          E_EMERGENCY_STOP, E_PULL_OVER);
  }
};

class CameraImageReader : public CyberReader<drivers::Image> {
 public:
  using MsgType = drivers::Image;
  using MsgPtr = std::shared_ptr<MsgType>;
  using CyberReader<MsgType>::CyberReader;

  ~CameraImageReader() = default;

 protected:
  void OnReceivedMsg(const MsgPtr& msg) noexcept override {
    CheckMessageFrequency(msg, channel_name_, module_type::E_CAMERA,
                          E_EMERGENCY_STOP, E_PULL_OVER);
  }
};
...
class DaemonReaderManager {
 public:
  ...
  void CreateReader() noexcept {
    // all_readers_ = {
    if (IsSlaveOrin()) {
      // Camera image frequency monitoring (from camera_video.pb.txt).
      const std::vector<std::string> camera_topics = {
          FLAGS_camera_right_rear_topic,
          FLAGS_camera_rear_topic,
          FLAGS_camera_left_front_topic,
          FLAGS_camera_left_rear_topic,
          FLAGS_camera_right_front_topic,
          FLAGS_camera_front_topic,
      };
      for (const auto& topic : camera_topics) {
        DaemonMonitorData::Instance()->channel_to_switch_flag_.emplace(topic, true);
        all_readers_.emplace_back(
            CreateReaderByTopic<CameraImageReader>(topic, daemon_monitor_node_));
      }

      // Lidar PointCloudPacked frequency monitoring (default set + RSM1 set).
      const std::vector<std::string> lidar_topics = {
          FLAGS_lidar_helios_front_left_topic,
          FLAGS_lidar_helios_rear_right_topic,
          FLAGS_lidar_bp_front_left_topic,
          FLAGS_lidar_bp_rear_right_topic,
          FLAGS_lidar_bp_rear_left_topic,
          FLAGS_lidar_bp_front_right_topic,
      };
      for (const auto& topic : lidar_topics) {
        DaemonMonitorData::Instance()->channel_to_switch_flag_.emplace(topic, true);
        all_readers_.emplace_back(
            CreateReaderByTopic<LidarPackedReader>(topic, daemon_monitor_node_));
      }
    } else {
    ...
    // Camera and lidar frequency monitoring only runs on the slave Orin,
    // which is the board that actually launches these sensor drivers.
  }
