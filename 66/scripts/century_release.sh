...
export OPT_CENTURY="$(dirname "${CENTURY_SYSROOT_DIR}")"
export PREFIX_DIR=/century/output
...
SHORTHAND_TARGETS=
BAZEL_OPTS="--config=opt --config=gpu --copt=-fPIC --host_copt=-fPIC"


function _usage() {
    info "Usage: $0 <module>(Can be empty) [Options]"
    info "Options:"
    info "${TAB} -p, --prefix <DIR> Use absolute path <DIR> as install prefix instead of '/century/output'"
...
function determine_release_targets() {
    ...
    for component in $@; do
        ...
        elif [[ -d "${CENTURY_ROOT_DIR}/modules/${component}" ]]; then
...
function retrieve_so_deps() {
    ldd $1 | awk '/=>/ {print $3}' | sort -u | \
        grep -Ev "^(${OPT_CENTURY}|/usr/local|${PREFIX_DIR})/"
}
export -f retrieve_so_deps

function generate_solibs() {
    listing="$1"
    find ${PREFIX_DIR}/bin ${CENTURY_SYSROOT_DIR}/bin -executable -type f  \
...
function resolve_directory_path() {
    pushd ${PREFIX_DIR} > /dev/null
    info "Resolve directory path for Century binary distribution..."
    mkdir -p bazel-bin
    ln -s ../cyber bazel-bin/cyber
    ln -s ../modules bazel-bin/modules
    cp /century/*.sh /century/output/
    cp /century/output/bin//dreamview /century/output/modules/dreamview/
    cp /century/output/bin/sample_lidar_tracking /century/output/modules/perception/lidar_tracking/

tar -zcvf /century/lidar_models_$(date +%Y%m%d).tgz --transform='s|^century/modules/perception/production/data/perception/lidar/models|models/lidar|' /century/modules/perception/production/data/perception/lidar/models
    rm -rf /century/output/modules/perception/data/params
    rm -rf /century/output/modules/perception/production/data/perception/lidar/models
    rm -rf /century/output/modules/common/data/global_flagfile.txt
    rm -rf /century/output/modules/map/data
    rm -rf /century/output/modules/mcloud/conf/mcloud.conf

    ln -sf /home/nvidia/workspace/vehicle_config /century/output/vehicle_config
    ln -sf /home/nvidia/workspace/vehicle_config/map /century/output/modules/map/data
    ln -sf /home/nvidia/workspace/vehicle_config/perception/models/lidar /century/output/modules/perception/production/data/perception/lidar/models
    ln -sf /home/nvidia/workspace/vehicle_config/perception/params /century/output/modules/perception/data/params
    ln -s /home/nvidia/workspace/vehicle_config/global_flagfile.txt /century/output/modules/common/data/global_flagfile.txt
    ln -s /home/nvidia/workspace/vehicle_config/mcloud.conf /century/output/modules/mcloud/conf/mcloud.conf

    rm -rf /century/output/modules/perception/production/data/perception/camera
    rm -rf /century/output/lib/third_party/local_config_*

    tar -zcvf /century/output_$(date +%Y%m%d).tgz -C /century output
