...
CENTURY_ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)"
CENTURY_IN_DOCKER=false

# If inside docker container
if [ -f /.dockerenv ]; then
  CENTURY_IN_DOCKER=true
  CENTURY_ROOT_DIR="/century"
fi

export CENTURY_ROOT_DIR="${CENTURY_ROOT_DIR}"
export CENTURY_IN_DOCKER="${CENTURY_IN_DOCKER}"
export CENTURY_CACHE_DIR="${CENTURY_ROOT_DIR}/.cache"
export CENTURY_SYSROOT_DIR="/opt/apollo/sysroot"

export TAB="    " # 4 spaces

source ${CENTURY_ROOT_DIR}/scripts/common.bashrc
...
function run() {
  if [ "${VERBOSE}" = yes ]; then
    ...
    local errfile="${CENTURY_ROOT_DIR}/.errors.log"
    echo "${@}" >"${errfile}"
    if ! "${@}" >>"${errfile}" 2>&1; then
      local exitcode=$?
      cat "${errfile}" 1>&2
