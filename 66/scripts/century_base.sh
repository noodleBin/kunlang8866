...
function set_lib_path() {
  local CYBER_SETUP="${CENTURY_ROOT_DIR}/cyber/setup.bash"
  [ -e "${CYBER_SETUP}" ] && . "${CYBER_SETUP}"
  pathprepend ${CENTURY_ROOT_DIR}/modules/tools PYTHONPATH
  pathprepend ${CENTURY_ROOT_DIR}/modules/teleop/common PYTHONPATH
  pathprepend /century/modules/teleop/common/scripts
}

function create_data_dir() {
  local DATA_DIR="${CENTURY_ROOT_DIR}/data"
...
function determine_bin_prefix() {
  CENTURY_BIN_PREFIX=$CENTURY_ROOT_DIR
  if [ -e "${CENTURY_ROOT_DIR}/bazel-bin" ]; then
    CENTURY_BIN_PREFIX="${CENTURY_ROOT_DIR}/bazel-bin"
  fi
  export CENTURY_BIN_PREFIX
}
...
function decide_task_dir() {
  ...
  if [ -z "${DISK}" ]; then
    info "Cannot find portable disk. Fallback to century data dir."
    DISK="/century"
  fi
...
function start_customized_path() {
  ...
  if [ $? -eq 1 ]; then
    eval "nohup cyber_launch start ${CENTURY_ROOT_DIR}/modules/${MODULE_PATH}/launch/${MODULE}.launch &"
    sleep 0.5
    is_stopped_customized_path "${MODULE_PATH}" "${MODULE}"
    if [ $? -eq 0 ]; then
...
function start_prof_customized_path() {
  ...
  info "Make sure you have built with 'bash century.sh build_prof'"
  LOG="${CENTURY_ROOT_DIR}/data/log/${MODULE}.out"
  is_stopped_customized_path "${MODULE_PATH}" "${MODULE}"
  if [ $? -eq 1 ]; then
    PROF_FILE="/tmp/$MODULE.prof"
    rm -rf $PROF_FILE
    BINARY=${CENTURY_BIN_PREFIX}/modules/${MODULE_PATH}/${MODULE}
    eval "CPUPROFILE=$PROF_FILE $BINARY \
        --flagfile=modules/${MODULE_PATH}/conf/${MODULE}.conf \
        --log_dir=${CENTURY_ROOT_DIR}/data/log $@ </dev/null >${LOG} 2>&1 &"
    sleep 0.5
    is_stopped_customized_path "${MODULE_PATH}" "${MODULE}"
    if [ $? -eq 0 ]; then
...
function start_fe_customized_path() {
  ...
  is_stopped_customized_path "${MODULE_PATH}" "${MODULE}"
  if [ $? -eq 1 ]; then
    eval "cyber_launch start ${CENTURY_ROOT_DIR}/modules/${MODULE_PATH}/launch/${MODULE}.launch"
  else
    info "Module ${MODULE} is already running - skipping."
    return 2
  fi
...
function start_gdb_customized_path() {
  ...
  eval "gdb --args ${CENTURY_BIN_PREFIX}/modules/${MODULE_PATH}/${MODULE} \
      --flagfile=modules/${MODULE_PATH}/conf/${MODULE}.conf \
      --log_dir=${CENTURY_ROOT_DIR}/data/log $@"
}
...
function stop_customized_path() {
  ...
  cyber_launch stop "${CENTURY_ROOT_DIR}/modules/${MODULE_PATH}/launch/${MODULE}.launch"
  if [ $? -eq 0 ]; then
...
function help() {
  cat << EOF
Invoke ". scripts/century_base.sh" within docker to add the following commands to the environment:
...
if [ $CENTURY_IN_DOCKER = "true" ]; then
  create_data_dir
  set_lib_path $1
  if [ -z $CENTURY_BASE_SOURCED ]; then
    determine_bin_prefix
    export CENTURY_BASE_SOURCED=1
  fi
fi
