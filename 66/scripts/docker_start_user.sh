...
function setup_century_directories() {
  local century_dir="/opt/apollo"
  [[ -d "${century_dir}" ]] || mkdir -p "${century_dir}"
  # chown -R "${uid}:${gid}" "${century_dir}"
}

# FIXME(infra): This will change core pattern on the host also,
# where the `/century` directory may not exist.
function setup_core_pattern() {
  if [[ -w /proc/sys/kernel/core_pattern ]]; then
    echo "/century/data/core/core_%e.%p" > /proc/sys/kernel/core_pattern
  fi
}
...
function main() {
  ...
  setup_century_directories "${uid}" "${gid}"
