...
source "${APOLLO_ROOT_DIR}/scripts/century.bashrc"
...
function determine_dev_image() {
    ...
    if [ "${TARGET_ARCH}" = "x86_64" ]; then
        ...
        error "Logic can't reach here! Please file an issue to Century GitHub."
        exit 3
    fi
...
function check_host_environment() {
    ...
    if [ "${kernel}" != "Linux" ]; then
        warning "Running Century dev container on ${kernel} is UNTESTED, exiting..."
        exit 1
    fi
}
...
function setup_devices_and_mount_local_volumes() {
    ...
    source "${APOLLO_ROOT_DIR}/scripts/century_base.sh"
    setup_device

    local volumes="-v $APOLLO_ROOT_DIR:/century"
...
function main() {
    ...
    ${DOCKER_RUN} -i --rm \
        ...
        "${APOLLO_DEV_IMAGE}" \
        /bin/bash /century/scripts/century_ci.sh
