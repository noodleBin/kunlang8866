...
Cloud::Cloud() : buffer_(buffer_size), node_(cyber::CreateNode("cloud")) {
  ...
  routing_request_reader_ = node_->CreateReader<century::routing::RoutingRequest>(
      ...
      [this](const std::shared_ptr<century::routing::RoutingRequest>& result) {
        local_routing_request_ = std::make_shared<century::routing::RoutingRequest>(*result);
      });
  ...
  while (true) {
    ...
    if (msg) {
      ...
      if (msg->has_reached_station()) {
        if (msg->has_routing_header() && 
            msg->routing_header().module_name() != "cloud") {
          // Do nothing: ignore reached_station for routing requests not from cloud
        } else {
  ...
  for (int i = 1; i <= count; i++) {
    ...
    if (0x01 == data[10 + i]) {
      ...
      if (planning_msg && planning_msg->has_background_music_enable()) {
        data_unit.emplace_back(planning_msg->background_music_enable() ? 0x02 : 0x01);
      } else {
        data_unit.emplace_back(0xFF);
      }
    }
...
void Cloud::ReceTask(const VehicleMessage& message) {
  ...
  if (0x01 == commandID) {
    if (local_routing_request_ &&
        local_routing_request_->task_type() ==
            century::routing::TaskType::TEMPORARY_PARKING) {
      AINFO << "Rejecting task: last task_type is TEMPORARY_PARKING";
...
void Cloud::SendSystemInfo() {
  ...
  system_info_.work_status = 0x01;
  // work status
  data_unit.emplace_back(system_info_.work_status);
  system_info_.task_status = 0x01;
  // task status
  data_unit.emplace_back(system_info_.task_status);
  // system_info_.software_version = "v0000.0.1";
  ...
  // cpu use
  append_data(&system_info_.cpu, sizeof(system_info_.cpu));
  double memory = 2.2;
  system_info_.memory = memory * 10;
  // memory use
  append_data(&system_info_.memory, sizeof(system_info_.memory));

  system_info_.temprate = 0xff;
  // temperature
  append_data(&system_info_.temprate, sizeof(system_info_.temprate));

  system_info_.tbox_status = box_status_;
  // tbox status
  append_data(&system_info_.tbox_status, sizeof(system_info_.tbox_status));

  system_info_.map_status = 0x02;
  // map status
  append_data(&system_info_.map_status, sizeof(system_info_.map_status));

  // box load status
  append_data(&system_info_.box_status, sizeof(system_info_.box_status));
  append_data(&system_info_.box_status_back,
              sizeof(system_info_.box_status_back));
  ...
  uint8_t temporary_parking_status = (0x02 == temporary_parking_) ? 0x01 : 0x00;
  system_info_.fas_aeb_status |= (temporary_parking_status << 1);


  uint8_t stop = immediately_stop_ ? 0x01 : 0x00;
  ...
  if (chassis_msg) {
    if (is_remote_status &&
      canbus::Chassis::DrivingMode::Chassis_DrivingMode_COMPLETE_AUTO_DRIVE == chassis_msg->driving_mode()) {
  ...
  if (planning_msg) {
    if (planning_msg->has_pass_stacker_request() &&
      ...
      bool borrowFlag = msg.request_for_pass_stacker();
      if (borrowFlag) AINFO << "borrowFlag - pass_stacker : " << borrowFlag;

      uint16_t borrowBits = (borrowFlag ? 1 : 0) << 0;
      uint16_t idBits = 0;
      system_info_.borrow_status = idBits | borrowBits;

    } else if (planning_msg->has_borrow_request() &&
               planning_msg->borrow_request()) {
      if (planning_msg->has_block_obs_id()) borrow_id = "0";
      bool borrowFlag = planning_msg->borrow_request();
      if (borrowFlag) AINFO << "borrowFlag - block_obs : " << borrowFlag;
  ...
  append_data(&system_info_.borrow_status, sizeof(system_info_.borrow_status));
  ...
  localization_reader_->Observe();
  auto p = localization_reader_->GetLatestObserved();

  if (p) {
    double lat = 0.0;
    double lon = 0.0;
    double utm_x = offset_x_ + p->pose().position().x();
    double utm_y = offset_y_ + p->pose().position().y();
    // char zone[20] = "51S";
  ...
  }
  {
    if (!multi_responses_.empty()) {
      ...
      std::unique_lock<std::mutex> lock(responses_mutex_);
      for (auto& data_tt : multi_responses_) {
        data_unit.emplace_back(data_tt);
      }
    }
  }
