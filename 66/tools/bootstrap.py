...
_CENTURY_ROOT_DIR = ''
_CENTURY_BAZELRC = '.century.bazelrc'

_CENTURY_CURRENT_BAZEL_VERSION = None
_CENTURY_MIN_BAZEL_VERSION = '2.0.0'
_CENTURY_INSIDE_DOCKER = True
_CENTURY_DOCKER_STAGE = "dev"
...
def docker_stage():
    default_century_stage = "dev"
    if not inside_docker():
        return default_century_stage
    stage_conf = "/etc/century.conf"
    if not os.path.exists(stage_conf) or not os.path.isfile(stage_conf):
        return default_century_stage

    with open("/etc/century.conf") as f:
        for line in f:
            line = line.strip()
...
def write_to_bazelrc(line):
    with open(_CENTURY_BAZELRC, 'a') as f:
        f.write(line + '\n')
...
def get_python_path(environ_cp, python_bin_path):
    ...
    for path in all_paths:
        if os.path.isdir(path) and not path.startswith(
                os.path.join(_CENTURY_ROOT_DIR, "bazel-bin")):
            paths.append(path)
    return paths
...
def setup_common_dirs(environ_cp):
    ...
    cache_dir = os.path.join(_CENTURY_ROOT_DIR, '.cache')
    dist_dir = os.path.join(_CENTURY_ROOT_DIR, '.cache/distdir')

    # if cyber/setup.bash not sourced
    if 'CENTURY_CACHE_DIR' in environ_cp:
        cache_dir = environ_cp['CENTURY_CACHE_DIR']

    if 'CENTURY_BAZEL_DIST_DIR' in environ_cp:
        dist_dir = environ_cp['CENTURY_BAZEL_DIST_DIR']
...
def reset_century_bazelrc():
    """Reset file that contains customized config settings."""
    open(_CENTURY_BAZELRC, 'w').close()
...
def main():
    ...
    global _CENTURY_ROOT_DIR
    global _CENTURY_BAZELRC
    global _CENTURY_CURRENT_BAZEL_VERSION
    global _CENTURY_INSIDE_DOCKER
    global _INTERACTIVE_MODE
    global _CENTURY_DOCKER_STAGE

    parser = argparse.ArgumentParser()
    parser.add_argument(
        '--output_file',
        type=str,
        default='.century.bazelrc',
        help='Path of the bazelrc file to write to (relative to CENTURY_ROOT_DIR)')
    ...
    _CENTURY_ROOT_DIR = default_root_dir()
    _CENTURY_BAZELRC = os.path.join(_CENTURY_ROOT_DIR, args.output_file)
    _CENTURY_INSIDE_DOCKER = inside_docker()
    _CENTURY_DOCKER_STAGE = docker_stage()
    ...
    try:
        current_bazel_version = check_bazel_version(_CENTURY_MIN_BAZEL_VERSION)
    except subprocess.CalledProcessError as e:
        print('Error checking bazel version: ',
              e.output.decode('UTF-8').strip())
        raise e

    _CENTURY_CURRENT_BAZEL_VERSION = convert_version_to_int(
        current_bazel_version)

    reset_century_bazelrc()
    ...
    if not _CENTURY_INSIDE_DOCKER and 'LD_LIBRARY_PATH' in environ_cp:
