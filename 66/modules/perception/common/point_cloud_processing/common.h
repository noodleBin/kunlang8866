...
void TransformPoint(const InPointT &point_in, const Eigen::Affine3d &pose,
  OutPointT *point_out) {
  Eigen::Vector3d point3d(point_in.x, point_in.y, point_in.z);
  point3d = pose * point3d;
  point_out->x = static_cast<typename OutPointT::Type>(point3d.x());
  point_out->y = static_cast<typename OutPointT::Type>(point3d.y());
  point_out->z = static_cast<typename OutPointT::Type>(point3d.z());
}

// @brief transform a point cloud
// old name: transform_point_cloud
template <typename InPointT, typename OutPointT>
void TransformPointCloud(const base::PointCloud<InPointT> &cloud_in,
                         const Eigen::Affine3d &pose,
                         base::PointCloud<OutPointT> *cloud_out) {
  if (cloud_out->size() < cloud_in.size()) {
    cloud_out->resize(cloud_in.size());
  }
  for (size_t i = 0; i < cloud_in.size(); ++i) {
    TransformPoint<InPointT, OutPointT>(cloud_in.at(i), pose, &(cloud_out->at(i)));
  }
}
