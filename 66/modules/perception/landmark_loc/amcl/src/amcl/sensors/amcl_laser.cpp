...
namespace {
  const std::unordered_map<int, double> layer_weights = {
    {0, 0.3},  //number
    {1, 0.1},   // slotline
    {2, 0.3},   //boundary
    {3, 0.3},   // sign
...
//std::vector<pf_vector_t> test_points;
////////////////////////////////////////////////////////////////////////////////
// Default constructor
...
AMCLLaser::AMCLLaser(size_t max_beam, const std::array<map_t*, MapLayerSize> map_layers): AMCLSensor(), map_layers_(map_layers),max_beams(max_beam)
{
  return;
}
...
double AMCLLaser::LikelihoodFieldModel(AMCLLaserData *data, pf_sample_set_t* set)
{
 ...
 // test_points.clear();
  ...
  for (j = 0; j < set->sample_count; j++)
  {
   ...
   double dx = sample->pose.v[0] - data->sensor->pose.v[0];
   double dy = sample->pose.v[1] - data->sensor->pose.v[1];
   if(sqrt(dx*dx + dy*dy) > data->to_odom_distance) {
     sample->weight = 0;
     continue;
   }
    ...
    for (i = 0; i < data->range_count; ++i)
    {
     ...
     // LOG(INFO) << "hit: " << hit.v[0] << " " << hit.v[1] <<" from " << pose.v[0] << " " << pose.v[1] << " bearing " << obs_bearing << " range " << obs_range;
    
    
      // Convert to map grid coords.
      auto actual_label = (data->labels[i] - 9) > 0 ? (data->labels[i] - 9) : 0;
      auto* target_map = self->map_layers_.at(actual_label);
      int mi, mj;
    //  mi = MAP_GXWX(target_map, hit.v[0]);
    //  mj = MAP_GYWY(target_map, hit.v[1]);
       mi = hit.v[0]*10;
       mj = hit.v[1]*10;
      // Part 1: Get distance from the hit to closest obstacle.
      // Off-map penalized as max distance
      /*
      if(!MAP_VALID(target_map, mi, mj))
        z = target_map->max_occ_dist;
      else
        z = target_map->cells[MAP_INDEX(target_map,mi,mj)].occ_dist;
      */
     if(!MAP_VALID(target_map, mi, mj))
      //  continue;
        z = target_map->max_occ_dist;
      else
        z = target_map->cells[mi + mj * target_map->size_x].occ_dist;
      // Gaussian model
    ...
    //  pz += self->z_rand * z_rand_mult;
     ...
     if(z<2) {
      sample->semantic_count++;
     }
    
    double semantic_weight = 0.1;
    if(layer_weights.find(actual_label) != layer_weights.end()) {
       semantic_weight = layer_weights.at(actual_label);
    }
...
// 计算 pz 后：
      pz = (self->z_hit * exp(-(z*z)/(2*self->sigma_hit*self->sigma_hit))
            + self->z_rand * (1.0 / data->range_max)) * semantic_weight;
      ...
      // works well, though...
      p += pz*pz*pz;
     // LOG(INFO) << "layer " << data->labels[i] << " z " << z << " pz " << pz <<" p "<< p;

    }

    //enhance the weight of the number layer
    sample->weight *= p;

    //LOG(INFO) << pose.v[0] <<" "<<pose.v[1]<<" weight is "<< sample->weight;
    total_weight += sample->weight;
  }
