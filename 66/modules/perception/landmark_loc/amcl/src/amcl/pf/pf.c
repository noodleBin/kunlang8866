...
void pf_cluster_stats(pf_t *pf, pf_sample_set_t *set)
{
  ...
  for (i = 0; i < set->sample_count; i++)
  {
    ...
    cluster->mean_semantic_count += sample->semantic_count;
  ...
  for (i = 0; i < set->cluster_count; i++)
  {
    ...
    cluster->cov = pf_matrix_zero();
    cluster->mean_semantic_count =  cluster->mean_semantic_count/cluster->count;
...
int pf_get_cluster_stats(pf_t *pf, int clabel, double *weight,
                         pf_vector_t *mean, pf_matrix_t *cov, int* semantic_count)
{
  pf_sample_set_t *set;
  pf_cluster_t *cluster;
  ...
  *mean = cluster->mean;
  *cov = cluster->cov;
  *semantic_count = cluster->mean_semantic_count;
