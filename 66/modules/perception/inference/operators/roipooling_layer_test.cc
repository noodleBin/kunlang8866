...
TEST(ROIPoolFloorTest, test) {
  ...
  auto roi_layer = new century::perception::inference::ROIPoolingLayer<float>(
  ...
  std::shared_ptr<century::perception::base::Blob<float>> feat_b;
  feat_b.reset(new century::perception::base::Blob<float>(feat_shape));
  std::shared_ptr<century::perception::base::Blob<float>> rois_b;
  rois_b.reset(new century::perception::base::Blob<float>(rois_shape));
  std::shared_ptr<century::perception::base::Blob<float>> top;
  top.reset(new century::perception::base::Blob<float>(top_shape));
  for (int b = 0; b < 2; b++) {
  ...
  rois_data[18] = 500.6f;
  rois_data[19] = 500.7f;
  std::vector<std::shared_ptr<century::perception::base::Blob<float>>> bottoms;
  bottoms.push_back(feat_b);
  bottoms.push_back(rois_b);
  std::vector<std::shared_ptr<century::perception::base::Blob<float>>> tops;
  tops.push_back(top);
