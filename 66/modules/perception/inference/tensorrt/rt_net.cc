...
void RTNet::init_blob(std::vector<std::string> *names) {
  ...
  for (auto name : *names) {
    ...
    nvinfer1::Dims3 dims = static_cast<nvinfer1::Dims3 &&>(
        engine->getBindingDimensions(bindingIndex));
    int count = dims.d[1] * dims.d[2] * dims.d[3] * max_batch_size_;
    ...
    blob.reset(new century::perception::base::Blob<float>(shape));
...
std::shared_ptr<century::perception::base::Blob<float>> RTNet::get_blob(
