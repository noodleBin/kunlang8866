...
int main(int argc, char **argv) {
  ...
  century::perception::inference::Inference *rt_net;
  std::vector<std::string> outputs{"loc_pred", "obj_pred", "cls_pred",
                                   "ori_pred", "dim_pred", "lof_pred",
                                   "lor_pred", "conv4_3"};
  std::vector<std::string> inputs{"data"};
  century::perception::inference::load_data<std::string>(FLAGS_names_file,
                                                        &outputs);

  std::shared_ptr<nvinfer1::Int8EntropyCalibrator> calibrator = nullptr;
  if (FLAGS_int8) {
    century::perception::inference::BatchStream stream(2, 100, "./batches/");
    calibrator.reset(
        new nvinfer1::Int8EntropyCalibrator(stream, 0, true, "./"));
    rt_net = new century::perception::inference::RTNet(
        proto_file, weight_file, outputs, inputs, calibrator.get());
  } else {
    rt_net = new century::perception::inference::RTNet(proto_file, weight_file,
                                                      outputs, inputs);
  }
  ...
  century::perception::inference::load_data<std::string>(FLAGS_test_list,
                                                        &image_lists);
  ...
  century::perception::inference::write_result(FLAGS_res_dir, output_data_vec);
  delete rt_net;
  return 0;
}
