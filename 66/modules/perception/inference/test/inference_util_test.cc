...
TEST(loadTest, test) {
  ACHECK(!century::perception::inference::load_binary_data("unknow.txt"));
}
...
TEST(UtilTest, test_resize_gpu) {
  ...
  std::shared_ptr<century::perception::base::Blob<uint8_t>> src_blob(
      new century::perception::base::Blob<uint8_t>);
  NppiSize roi;
...

  {
    std::vector<int> shape = {1, img.rows * 0.5, img.cols * 0.5,
                              img.channels()};
    std::shared_ptr<century::perception::base::Blob<float>> dst_blob(
        new century::perception::base::Blob<float>(shape));
