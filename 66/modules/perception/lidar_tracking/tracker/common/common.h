...
bool ConvertObjectToPb(const base::ObjectPtr &object_ptr,
  ...
  constexpr double kMinVelocity = 0.3;
  auto temp_velocity = Eigen::Vector2d(obj_velocity->x(), obj_velocity->y());
  if ((temp_velocity.norm() < kMinVelocity) &&
      (object_ptr->type == base::ObjectType::UNKNOWN)) {
  ...
  constexpr double kMinHeight = 2.5;
  if ((object_ptr->size(2) < kMinHeight) &&
      (object_ptr->type == base::ObjectType::UNKNOWN)) {
  ...
  if (object_ptr->fusion_supplement.on_use) {
    for (const auto &measurement : object_ptr->fusion_supplement.measurements) {
      ...
      century::common::Point3D *pb_position = pb_measurement->mutable_position();
      ...
      century::common::Point3D *pb_velocity = pb_measurement->mutable_velocity();
