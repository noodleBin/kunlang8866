...
double MlfTrackData::CalcuateThetaRate() const {
  if(history_objects_.size() < 2) {
    return 0.0;
  }
  auto it = history_objects_.rbegin();
  auto last_theta = it->second->object_ptr->theta;
  auto last_time = it->first;
  ++it;
  auto second_last_theta = it->second->object_ptr->theta;
  auto second_last_time = it->first;
  auto yaw_rate = (last_theta - second_last_theta) / (last_time - second_last_time);
  return yaw_rate;
}
...
bool MlfTrackData::ToObject(const Eigen::Vector3d& local_to_global_offset,
  ...
  //update yaw rate
  object->acceleration(2) = CalcuateThetaRate();
  return true;
}
