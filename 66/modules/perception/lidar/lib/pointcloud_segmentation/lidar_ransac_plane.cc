...
// template int RansacPlane::DoRansacPlaneNoAVX(
//     const std::shared_ptr<PointCloudType>& cloud, const std::vector<int>&
//     indices, Eigen::Vector4f& vec);
...
template Eigen::Vector4f RansacPlane::PlaneFitting(
    const std::shared_ptr<PointXYZIRTFCloud>& cloud,
