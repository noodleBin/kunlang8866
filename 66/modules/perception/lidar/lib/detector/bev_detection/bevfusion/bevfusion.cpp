...
class CoreImplement : public Core {
 public:
  ...
  bool init(const CoreParameter& param) {
    ...
    AINFO << "<<<<<<<<<<<<<<<<<<<<<<<<<<<Start to create_transfusion";
    ...
    AINFO << "<<<<<<<<<<<<<<<<<<<<<<<<<<<Start to create_transbbox";
  ...
  std::vector<head::transbbox::BoundingBox> forward_timer(const void* camera_images, const nvtype::half* lidar_points,
    ...
    timer_.start(_stream);
    const nvtype::half* fusion_feature = this->transfusion_->forward(nullptr, lidar_feature, stream);
  ...
  virtual void print() override {
    // AERROR << "************************print camera backbone************************";
    // camera_backbone_->print();
    // AERROR << "************************print camera_vtransform_************************";

    // camera_vtransform_->print();
    AINFO << "************************print transfusion_************************";
    transfusion_->print();
    AINFO << "************************print transbbox_************************";
    transbbox_->print();
  }
