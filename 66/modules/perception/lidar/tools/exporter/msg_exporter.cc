...
MsgExporter::MsgExporter(std::shared_ptr<century::cyber::Node> node,
...
void MsgExporter::PointCloudMessageHandler(
  ...
  if (cloud_msg->point_size() > 0) {
    ...
    for (int i = 0; i < cloud_msg->point_size(); ++i) {
      const century::drivers::PointXYZIT& pt = cloud_msg->point(i);
      ...
      cloud[i].timestamp = static_cast<double>(pt.timestamp());
    }
  }
  Eigen::Matrix4d pose;
  if (QuerySensorToWorldPose(timestamp, child_frame_id, &pose)) {
