...
bool IsOriginInsideBBox(const std::shared_ptr<base::Object>& box) {
  float dx = -box->center[0];
  float dy = -box->center[1];

  float cos_yaw = std::cos(-box->theta);
  float sin_yaw = std::sin(-box->theta);

  float local_x = dx * cos_yaw - dy * sin_yaw;
  float local_y = dx * sin_yaw + dy * cos_yaw;

  float hx = box->size[0] * 0.5f;
  float hy = box->size[1] * 0.5f;

  return (std::fabs(local_x) <= hx &&
          std::fabs(local_y) <= hy);
}
