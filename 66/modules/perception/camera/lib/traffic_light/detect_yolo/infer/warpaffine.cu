...
__device__ float3 operator+(const float3& value0, const float3& value1) {
    float3 result;
...
void AffineTransform::updateMatrix(int src_width, int src_height, int dst_width, int dst_height) {
    if (src_width == last_src_width_ && src_height == last_src_height_) {
        return;
    }
    last_src_width_ = src_width;
    last_src_height_ = src_height;
    double scale_x = static_cast<double>(dst_width) / src_width;
    double scale_y = static_cast<double>(dst_height) / src_height;
    double inv_scale_x = 1.0 / scale_x;
    double inv_scale_y = 1.0 / scale_y;
    matrix[0] = make_float3(inv_scale_x, 0.0, 0.0);
    matrix[1] = make_float3(0.0, inv_scale_y, 0.0);
    dst_offset_x = 0;
    dst_offset_y = 0;
}
