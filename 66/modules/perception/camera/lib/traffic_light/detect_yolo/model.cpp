...
std::unique_ptr<BaseModel<ResultType>> BaseModel<ResultType>::clone() const {
    ...
    clone_model->backend_ = backend_->clone();
...
ClassifyRes BaseModel<ClassifyRes>::postProcess(int idx) {
    ...
    for (int i = 0; i < result.num; ++i) {
        result.scores.emplace_back(topk[i * tensor_info.shape.d[2]]);
        result.classes.emplace_back(topk[i * tensor_info.shape.d[2] + 1]);
    }
    return result;
}
...
DetectRes BaseModel<DetectRes>::postProcess(int idx) {
    ...
    auto& affine_transform = !backend_->option.input_shape.empty()
...
static void MyNMSBoxes(const std::vector<cv::Rect>& boxes, const std::vector<float>& scores,
    ...
    for (size_t i = 0; i < candidates.size(); ++i) {
        int idx_i = candidates[i].second;
        if (is_suppressed[idx_i]) {
            continue;
        }
        indices.emplace_back(idx_i);
        for (size_t j = i + 1; j < candidates.size(); ++j) {
            int idx_j = candidates[j].second;
            if (is_suppressed[idx_j]) {
                continue;
            }
            if (CalculateIoU(boxes[idx_i], boxes[idx_j]) > nms_thresh) {
...
PoseRes BaseModel<PoseRes>::postProcess(int idx) {
    ...
    for (int i = 0; i < num; ++i) {
        ...
        for (int j = 0; j < nkpt; ++j) {
            ...
            keypoints.emplace_back((2 == ndim) ? KeyPoint(x, y)
                                               : KeyPoint(x, y, kpts[i * nkpt * ndim + j * ndim + 2]));
        }
