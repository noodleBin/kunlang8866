...
using TLCamID = century::perception::TrafficLightDetection::CameraID;
using century::cyber::Clock;
using century::cyber::common::GetAbsolutePath;
using century::perception::common::SensorManager;
...
static int GetGpuId(
    const century::perception::camera::CameraPerceptionInitOptions& options) {
  century::perception::camera::app::TrafficLightParam trafficlight_param;
  std::string work_root = century::perception::camera::GetCyberWorkRoot();
...
bool TrafficLightsPerceptionComponent::Init() {
  ...
  writer_ = node_->CreateWriter<century::perception::TrafficLightDetection>(
...
int TrafficLightsPerceptionComponent::InitConfig() {
  century::perception::onboard::TrafficLight traffic_light_param;
...
int TrafficLightsPerceptionComponent::InitCameraListeners() {
  ...
  for (size_t i = 0; i < camera_names_.size(); ++i) {
    ...
    typedef const std::shared_ptr<century::drivers::Image> ImageMsgType;
...
int TrafficLightsPerceptionComponent::InitV2XListener() {
  typedef const std::shared_ptr<century::v2x::IntersectionTrafficLightData>
...
void TrafficLightsPerceptionComponent::OnReceiveV2XMsg(
    const std::shared_ptr<century::v2x::IntersectionTrafficLightData> v2x_msg) {
  std::lock_guard<std::mutex> lck(mutex_);
  v2x_msg_buffer_.push_back(*v2x_msg);
}

void TrafficLightsPerceptionComponent::GenerateTrafficLights(
    const std::vector<century::hdmap::Signal>& signals,
...
bool TrafficLightsPerceptionComponent::GetPoseFromTF(
  ...
  PERF_FUNCTION();
  century::cyber::Time query_time(timestamp);
...
bool TrafficLightsPerceptionComponent::TransformOutputMessage(
  ...
  if (max_light_id >= 0) {
    for (size_t i = 0; i < lights.size(); i++) {
      century::perception::TrafficLight* light_result =
      ...
      light_result->set_color(
          static_cast<century::perception::TrafficLight_Color>(
...
void TrafficLightsPerceptionComponent::TransRect2Box(
    const base::RectI& rect, century::perception::TrafficLightBox* box) {
  box->set_x(rect.x);
  box->set_y(rect.y);
...
double TrafficLightsPerceptionComponent::stopline_distance(
  ...
  const century::hdmap::Curve& stopline = stoplines_.Get(0);
...
bool TrafficLightsPerceptionComponent::TransformDebugMessage(
    ...
    std::shared_ptr<century::perception::TrafficLightDetection>* out_msg) {
  PERF_FUNCTION();
...
void TrafficLightsPerceptionComponent::Visualize(
  ...
  // cv::resize(output_image, output_image, cv::Size(), 2, 2);
  cv::imwrite(absl::StrCat("/century/modules/perception/camera/lib/traffic_light/detect_yolo/yolo_img_output/",
      ...
      if (std::fabs(camera_frame_timestamp - v2x_timestamp) <
        ...
        for (int i = 0; i < v2x_lights_num; ++i) {
          ...
          switch (v2x_light.color()) {
            default:
            case century::v2x::SingleTrafficLight::UNKNOWN:
              v2x_color = base::TLColor::TL_UNKNOWN_COLOR;
              break;
            case century::v2x::SingleTrafficLight::RED:
              v2x_color = base::TLColor::TL_RED;
              break;
            case century::v2x::SingleTrafficLight::YELLOW:
              v2x_color = base::TLColor::TL_YELLOW;
              break;
            case century::v2x::SingleTrafficLight::GREEN:
              v2x_color = base::TLColor::TL_GREEN;
              break;
            case century::v2x::SingleTrafficLight::BLACK:
              v2x_color = base::TLColor::TL_BLACK;
              break;
            case century::v2x::SingleTrafficLight::FLASH_GREEN:
              v2x_color = base::TLColor::TL_GREEN;
              blink = true;
              break;
          }
