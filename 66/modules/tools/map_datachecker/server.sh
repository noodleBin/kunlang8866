...
function set_global_var() {
  ...
  CENTURY_ROOT_PATH="${SCRIPT_PATH}/../../.."
  MAP_DATACHECKER_SERVER=${CENTURY_ROOT_PATH}/bazel-bin/modules/map/tools/map_datachecker/server/map_datachecker_server
  CONF=${CENTURY_ROOT_PATH}/modules/map/tools/map_datachecker/server/conf/map-datachecker.conf
  LOG_DIR=${SCRIPT_PATH}/log
}

function start_server() {
  if [ ! -e ${MAP_DATACHECKER_SERVER} ];then
    echo "/century/century.sh build should be executed before run this script"
    exit -1
  fi
  if [ ! -e "${LOG_DIR}" ];then
    mkdir -p ${LOG_DIR}
