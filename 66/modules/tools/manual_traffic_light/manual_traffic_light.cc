...
using century::common::color::ANSI_GREEN;
using century::common::color::ANSI_RED;
using century::common::color::ANSI_RESET;
...
class ManualTrafficLight final : public century::cyber::TimerComponent {
 public:
  bool Init() {
 ...
 private:
  bool GetAllTrafficLights(std::vector<SignalInfoConstPtr> *traffic_lights) {
    static auto map_filename = century::hdmap::BaseMapFile();
    static century::hdmap::Map map_proto;
    ...
    if (map_proto.lane().empty() && map_traffic_lights.empty()) {
      ...
      if (absl::EndsWith(map_filename, ".xml")) {
        if (!century::hdmap::adapter::OpendriveAdapter::LoadData(map_filename,
                                                                &map_proto)) {
          return false;
        }
      } else if (!century::cyber::common::GetProtoFromFile(map_filename,
                                                          &map_proto)) {
        return false;
      }
  ...
  bool CreateTrafficLightDetection(
    ...
    century::common::util::FillHeader("manual_traffic_light", detection);
    return true;
  }
 ...
 private:
  ...
  std::shared_ptr<century::cyber::Writer<TrafficLightDetection>>
