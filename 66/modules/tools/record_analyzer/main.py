...
def process(control_analyzer, planning_analyzer, lidar_endtoend_analyzer,
    ...
    for msg in reader.read_messages():
        if msg.topic == "/century/canbus/chassis":
        ...
        if msg.topic == "/century/sensor/velodyne64/compensator/PointCloud2" or \
                msg.topic == "/century/sensor/lidar128/compensator/PointCloud2":
        ...
        if msg.topic == "/century/perception/obstacles":
            if ((not is_auto_drive) and (not all_data)) or is_simulation or \
                    plot_planning_path or plot_planning_refpath:
                continue
        ...
        if msg.topic == "/century/prediction":
            if ((not is_auto_drive) and (not all_data)) or is_simulation or \
                    plot_planning_path or plot_planning_refpath:
                continue
