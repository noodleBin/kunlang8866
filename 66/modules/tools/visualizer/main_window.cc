...
const std::string CompressedImageType("century.drivers.CompressedImage");
...
struct MainWindow::VideoImgProxy {
  ...
  union {
    CyberChannReader<century::drivers::Image>* image_reader_;
    CyberChannReader<century::drivers::CompressedImage>*
...
struct MainWindow::RadarData {
  ...
  CyberChannReader<century::drivers::RadarObstacles>* channel_reader_;
  ...
  if (b) {
    ...
    if (!radarProxy->channel_reader_) {
      radarProxy->channel_reader_ =
          new CyberChannReader<century::drivers::RadarObstacles>();
      ...
      auto radarcallback =
          [this, radarProxy](
              const std::shared_ptr<century::drivers::RadarObstacles>& pdata) {
...
void MainWindow::RadarRenderCallback(
    const std::shared_ptr<const century::drivers::RadarObstacles>& rawData,
    RadarData* radar) {
...
void MainWindow::PointCloudReaderCallback(
    const std::shared_ptr<const century::drivers::PointCloud>& pdata) {
  pointcloud_reader_mutex_.lock();
  pointcloud_reader_mutex_.unlock();
  ...
  if (b) {
    ...
    if (!pointcloud_channel_Reader_) {
      pointcloud_channel_Reader_ =
          new CyberChannReader<century::drivers::PointCloud>();
      ...
      auto pointCallback =
          [this](const std::shared_ptr<century::drivers::PointCloud>& pdata) {
...
void MainWindow::ImageReaderCallback(
    const std::shared_ptr<const century::drivers::Image>& imgData,
  ...
  if (theVideoImgProxy->dynamic_texture_ != nullptr && imgData != nullptr) {
    ...
    std::cerr
        << "----Dynamic Texture is nullptr or century.drivers.Image is nullptr"
        << std::endl;
  }
...
void MainWindow::ImageReaderCallback(
    const std::shared_ptr<const century::drivers::CompressedImage>& imgData,
    VideoImgProxy* theVideoImgProxy) {
  theVideoImgProxy->reader_mutex_.lock();
  if (theVideoImgProxy->dynamic_texture_ != nullptr && imgData != nullptr) {
    QImage img;
    ...
    if (!theVideoImg->image_reader_) {
      if (theVideoImg->isCompressedImage_) {
        theVideoImg->compressed_image_reader_ =
            new CyberChannReader<century::drivers::CompressedImage>();
      } else {
        theVideoImg->image_reader_ =
            new CyberChannReader<century::drivers::Image>();
      }
      ...
      if (theVideoImg->isCompressedImage_) {
        auto videoCallback =
            [this, theVideoImg](
                const std::shared_ptr<century::drivers::CompressedImage>&
        ...
        auto videoCallback =
            [this, theVideoImg](
                const std::shared_ptr<century::drivers::Image>& pdata) {
...
void MainWindow::TopologyChanged(
    const century::cyber::proto::ChangeMsg& changeMsg) {
  if (century::cyber::proto::ChangeType::CHANGE_CHANNEL ==
          changeMsg.change_type() &&
      century::cyber::proto::RoleType::ROLE_WRITER == changeMsg.role_type() &&
      century::cyber::proto::OperateType::OPT_JOIN == changeMsg.operate_type()) {
    AddNewWriter(changeMsg.role_attr());
  }
}

void MainWindow::AddNewWriter(
    const century::cyber::proto::RoleAttributes& role) {
  ...
  if (msgType.contains("PointXYZIRTCloud", Qt::CaseInsensitive)) {
