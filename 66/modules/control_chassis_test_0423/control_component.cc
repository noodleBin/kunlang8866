...
bool ControlComponent::Init() {
  ...
  std::cout << control_conf_.DebugString() << std::endl;
...
bool ControlComponent::Proc() {
  ...
  if(control_conf_.test_mode_conf().enable_test_mode()) {
    ChassisTestMode(&control_command);
    control_command.set_aeb_enable(false);
...
void ControlComponent::ChassisTestMode(ControlCommand *const control_command) {
  bool test_mode = control_conf_.test_mode_conf().enable_test_mode();
  auto gear_request = control_conf_.test_mode_conf().lon_speed_test().request_gear();
  static bool init_zero_ = true;
  static int step_peak_flag = 1;
  // static double pre_time = Clock::NowInSeconds();
  control_command->mutable_header()->set_timestamp_sec(Clock::NowInSeconds());
  if (century::control::D == gear_request) {
    control_command->set_gear_location(canbus::Chassis::GEAR_DRIVE);
  } else if (century::control::R == gear_request) {
    control_command->set_gear_location(canbus::Chassis::GEAR_REVERSE);
  } else if (century::control::N == gear_request) {
    control_command->set_gear_location(canbus::Chassis::GEAR_NEUTRAL);
  }
  chassis_reader_->Observe();
  const auto &chassis_msg = chassis_reader_->GetLatestObserved();
  localization_reader_->Observe();
  const auto &localization_msg = localization_reader_->GetLatestObserved();
  bool auto_drive_mode = static_cast<bool>(chassis_msg->driving_mode());
  if (test_mode && auto_drive_mode) {
    // std::cout << "auto_drive_mode OK!" << std::endl;
    auto lat_signal_type = control_conf_.test_mode_conf().sig_type();
    static double start_time = Clock::NowInSeconds();
    static double test_time = Clock::NowInSeconds();
    static double target_angle_ = control_conf_.test_mode_conf().step_signal_init_value();
    // double velocity = control_conf_.test_mode_conf().velocity();
    if (init_zero_) {
      if (Clock::NowInSeconds() - start_time < 5) {
        control_command->set_target_torque_2axis(0.0);
        control_command->set_target_torque_3axis(0.0);
        control_command->set_target_steering_angle_1axis(target_angle_);
        control_command->set_target_steering_angle_4axis(target_angle_);
        return;
      } else {
        init_zero_ = false;
      }
    }
    switch (lat_signal_type) {
      case century::control::STEP_SIG: // 阶跃信号
        {
          if (abs(target_angle_) + control_conf_.test_mode_conf().step_signal_delt_value() >
             control_conf_.test_mode_conf().max_abs_angle()) {
             step_peak_flag = target_angle_ > 0 ? -1 : 1;
          }
          if (Clock::NowInSeconds() - test_time >=
              control_conf_.test_mode_conf().step_signal_cycle()) {
            target_angle_ += step_peak_flag * control_conf_.test_mode_conf().step_signal_delt_value();
            test_time = Clock::NowInSeconds();
          }
        }
        break;
      case century::control::WAVE_SIG: // 正弦信号
        {
          static double test_start_time = Clock::NowInSeconds();
          double t = Clock::NowInSeconds() - test_start_time;
          double w = 2 * M_PI /  control_conf_.test_mode_conf().sinewave_signal_cycle();
          double a = control_conf_.test_mode_conf().sinewave_signal_amplitude();
          double init_phase = control_conf_.test_mode_conf().sinewave_signal_init_phase();
          target_angle_ = a * std::sin(w * t + init_phase);
        }
        break;
      case century::control::FIXED_SIG:
        {
          target_angle_ = 0;
        }
        break;
    ...
    if (target_angle_ > control_conf_.test_mode_conf().max_abs_angle()) {
      target_angle_ = control_conf_.test_mode_conf().max_abs_angle();
    }
    if (target_angle_ < -control_conf_.test_mode_conf().max_abs_angle()) {
      target_angle_ = -control_conf_.test_mode_conf().max_abs_angle();
    }
    if (century::control::EIGHT == control_conf_.test_mode_conf().turnmode()) {
      control_command->set_guide1_zhuanxiang_mode(STEERINGMODE_FIGURE_EIGHT);
      control_command->set_target_steering_angle_1axis(target_angle_);
      control_command->set_target_steering_angle_4axis(-target_angle_);
    } else if(century::control::DIAGONAL == control_conf_.test_mode_conf().turnmode()) {
      control_command->set_guide1_zhuanxiang_mode(STEERINGMODE_DIAGONAL);
      control_command->set_target_steering_angle_1axis(target_angle_);
      control_command->set_target_steering_angle_4axis(target_angle_);
    } else if (century::control::FRONT_HALF_EIGHT == control_conf_.test_mode_conf().turnmode()) {
      control_command->set_guide1_zhuanxiang_mode(STEERINGMODE_FRONT_HALF_FIGURE_EIGHT);
      control_command->set_target_steering_angle_1axis(target_angle_);
      control_command->set_target_steering_angle_4axis(0);
    } else if (century::control::REAR_HALF_EIGHT == control_conf_.test_mode_conf().turnmode()) {
      control_command->set_guide1_zhuanxiang_mode(STEERINGMODE_REAR_HALF_FIGURE_EIGHT);
      control_command->set_target_steering_angle_1axis(0);
      control_command->set_target_steering_angle_4axis(target_angle_);
    }
    //以上为横向测试用例
    double acc = control_conf_.test_mode_conf().lon_speed_test().acceleration();
    double dec = control_conf_.test_mode_conf().lon_speed_test().deceleration();
    double max_speed = control_conf_.test_mode_conf().lon_speed_test().max_speed();
    double max_speed_still_time = control_conf_.test_mode_conf().lon_speed_test().max_speed_still_time();
    auto decel_method = control_conf_.test_mode_conf().lon_speed_test().decel_method();
    double brake = control_conf_.test_mode_conf().lon_speed_test().brake();
    static double velocity = 0;
    static double init_time = Clock::NowInSeconds();
    double delt_t = Clock::NowInSeconds() - init_time;
    double acc_time = max_speed / acc;
    double brake_cmd = 0;
    // double dec_time = max_speed / dec;
    if (delt_t > acc_time + max_speed_still_time) {
      if (century::control::MotorDecel == decel_method) {
        velocity += dec * 0.02;
        brake_cmd = 0;
      } else if (century::control::BrakeDecel == decel_method) {
        velocity = 0.0;
        brake_cmd = brake;
        PoseSave(*localization_msg);
      }
    } else if (delt_t > acc_time) {
      velocity = max_speed;
    } else {
      velocity += acc * 0.02;
    }    
    velocity = std::clamp(velocity,0.0,max_speed);
    control_command->set_target_torque_2axis(velocity);
    control_command->set_target_torque_3axis(velocity);
    control_command->set_guide1_brake(brake_cmd);
    // std::cout << " target_angle_: " << target_angle_ << " velocity: " << velocity  << " delt_t: " << delt_t << std::endl;
  }
  
}
bool ControlComponent::PoseSave(const century::localization::LocalizationEstimate localization) {
  if (!localization.has_pose()) {
    return false;
  }
  double speed = std::fabs(localization.pose().linear_velocity().x());
  if (speed > 0.02) {
    static double s = 0;
    static auto pre_location = localization.pose().position();
    static double brake_start_time = Clock::NowInSeconds();
    double length = 0;
    double dx = localization.pose().position().x() - pre_location.x();
    double dy = localization.pose().position().y() - pre_location.y();
    ...
    if (length > 0.005) {
      ...
      if (!filewrite) {
        ...
        time_str = std::to_string(p->tm_year + 1900) + "-" +
                   std::to_string(p->tm_mon + 1) + "-" +
                   std::to_string(p->tm_mday) + "-" +
                   std::to_string(p->tm_hour) + "-" +
                   std::to_string(p->tm_min) + "-" +
                   std::to_string(p->tm_sec);
        filewrite = true;
      }
      std::ofstream imuFile;
      imuFile.open("./brake:" + time_str + ".csv", std::ios::app);
      imuFile << std::setprecision(16)
      << " x: " << localization.pose().position().x()
      << " y: " << localization.pose().position().y()
      << " v: " << speed
      << " t: " << Clock::NowInSeconds() - brake_start_time
      << " s: " << s
      << std::endl;
      pre_location = localization.pose().position();
      imuFile.close();
    }
    return true;
  }
