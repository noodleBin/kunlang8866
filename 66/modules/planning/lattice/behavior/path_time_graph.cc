...
using century::common::math::lerp;
...
std::vector<std::pair<double, double>> PathTimeGraph::GetLateralBounds(
  ...
  for (size_t i = 0; i < num_bound; ++i) {
    ...
    ptr_reference_line_info_->reference_line().GetLaneWidth(s_curr, &left_width,
                                                            &right_width);
    double ego_d_lower = init_d_[0] - ego_width * 0.5;
    double ego_d_upper = init_d_[0] + ego_width * 0.5;
    bounds.emplace_back(
  ...
  for (size_t i = 0; i < bounds.size(); ++i) {
    bounds[i].first += ego_width * 0.5;
    bounds[i].second -= ego_width * 0.5;
