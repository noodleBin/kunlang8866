...
bool Stage::ExecuteTaskOnOpenSpace(Frame* frame) {
  ...
  for (auto* task : task_list_) {
    ...
    RecordDebugInfo(task->Name(), time_diff_ms,
  ...
  AINFO << "fallback_flag: " << frame->open_space_info().fallback_flag();
  if (frame->open_space_info().fallback_flag()) {
    ...
    *(frame->mutable_open_space_info()->mutable_publishable_trajectory_data()) =
    ...
    //   AINFO << "publish trajectory s= " << point.path_point().s()
    //         << "  v=" << point.v() << "  a=" << point.a()
    //         << " relative_time=" << point.relative_time()
    //         << " x=" << std::setprecision(9) << point.path_point().x()
    //         << " y=" << std::setprecision(9) << point.path_point().y();
    // }
    auto& gear =
...
void Stage::RecordDebugInfo(const std::string& name, const double time_diff_ms,
                            OpenSpaceInfo* open_space_info) {
  if (!FLAGS_enable_record_debug) {
    ADEBUG << "Skip record debug info.";
    return;
  }
  if (open_space_info == nullptr) {
    AERROR << "Open space info is null.";
    return;
  }
