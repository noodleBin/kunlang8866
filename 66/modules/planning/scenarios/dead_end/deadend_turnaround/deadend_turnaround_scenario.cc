...
namespace {
constexpr double kMinDeadEndDistance = 1.0;  // m
}  // namespace
...
bool DeadEndTurnAroundScenario::IsTransferable(
  ...
  if (!CheckDistanceToDeadEnd(vehicle_state, nearby_path, dead_end_start_range,
                              &junction)) {
...
bool DeadEndTurnAroundScenario::CheckDistanceToDeadEnd(
  ...
  ADEBUG << "dead_end_start_range is: " << dead_end_start_range;
  ADEBUG << "car to junction dis is: "
         << std::abs(junction_polygon.DistanceTo(car_position));

  bool is_not_too_far = std::abs(junction_polygon.DistanceTo(car_position)) <
                        dead_end_start_range;
  bool is_not_too_near =
      std::abs(junction_polygon.DistanceTo(car_position)) > kMinDeadEndDistance;

  ADEBUG << "is_not_too_far " << is_not_too_far;
  ADEBUG << "is_not_too_near " << is_not_too_near;

  return is_not_too_far && is_not_too_near;
}
