...
const double kWatchVehiclePassStopLineBuff = 30.0;
...
using century::hdmap::OverlapInfoConstPtr;
...
Stage::StageStatus StopSignUnprotectedStageStop::Process(
  ...
  bool is_newest_super_traffic_light = frame->IsNewestSuperTrafficLight();
  if (!is_newest_super_traffic_light) {
    // Request remote takeover traffic light
    auto& mutable_reference_line_info =
        frame->mutable_reference_line_info()->front();
    mutable_reference_line_info.SetTrafficLightRequest(true);
    return Stage::RUNNING;
  }
...
int StopSignUnprotectedStageStop::RemoveWatchVehicle(
  ...
  bool in_stop_sign_junction = false;
  ...
  HDMapUtil::BaseMap().GetJunctions(vehicle_state_point, kRadius, &junctions);
  for (auto& vehicle : *watch_vehicles) {
    ...
    for (const auto& perception_obstacle_id : vehicles) {
      ...
      for (size_t i = 0; i < junctions.size(); ++i) {
        ADEBUG << "type:[" << junctions.at(i)->junction().type() << "],i" << i
               << "].";
        if (Junction::STOP_SIGN == junctions.at(i)->junction().type()) {
          Vec2d point(perception_obstacle->position().x(),
                      perception_obstacle->position().y());
          in_stop_sign_junction = junctions.at(i)->polygon().IsPointIn(point);
        }
      }
      // TODO(all): move 10.0 to conf
      if (distance > kWatchVehiclePassStopLineBuff && !in_stop_sign_junction) {
        ADEBUG << "mark ERASE obstacle_id[" << perception_obstacle_id << "]";
        remove_vehicles.emplace_back(perception_obstacle_id);
      }
    }
