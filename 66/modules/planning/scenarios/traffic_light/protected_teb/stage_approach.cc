...
constexpr double KHeadingErrorThreshold = 18.0 * M_PI / 180;  // rad
constexpr double KCheckDist = 20;                             // m
constexpr double KDistanceBuffer = 2.0;                       // m
}  // namespace
...
Stage::StageStatus TrafficLightProtectedStageApproach::Process(
  ...
  double ref_heading1 =
      reference_line_info.reference_line()
          .GetReferencePoint(current_traffic_light_overlap->start_s)
          .heading();
  ...
  double heading_error =
      std::fabs(common::math::NormalizeAngle(ref_heading2 - ref_heading1));
  ...
  if (current_traffic_light_overlap->end_s + KDistanceBuffer <=
      adc_back_edge_s) {
    return FinishStage();
  }
  // pose for adjust
  if ((frame->GetSignalCenterFromId(traffic_id, traffic_light_pose)) &&
      std::fabs(injector_->vehicle_state()->vehicle_state().linear_velocity()) <
          KStopSpeedThreshold &&
      heading_error > (FLAGS_tl_headingerror_limit * M_PI / 180)) {
    // test position
...
void TrafficLightProtectedStageApproach::SetAdjustStageInfo() {
  next_stage_ = ScenarioConfig::TRAFFIC_LIGHT_PROTECTED_ADJUST_TEB;
}
