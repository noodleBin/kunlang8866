...
#include "modules/planning/scenarios/traffic_light/protected_teb/stage_intersection_cruise.h"
...
void TrafficLightProtectedTebScenario::Init() {
  if (init_) {
    return;
  }
...
century::common::util::Factory<
    ...
    TrafficLightProtectedTebScenario::s_stage_factory_;

void TrafficLightProtectedTebScenario::RegisterStages() {
  ...
  s_stage_factory_.Register(
      ScenarioConfig::TRAFFIC_LIGHT_PROTECTED_ADJUST_TEB,
      [](const ScenarioConfig::StageConfig& config,
         const std::shared_ptr<DependencyInjector>& injector) -> Stage* {
        return new TrafficLightProtectedStageAdjustTeb(config, injector);
      });
  s_stage_factory_.Register(
...
std::unique_ptr<Stage> TrafficLightProtectedTebScenario::CreateStage(
 ...
 */
bool TrafficLightProtectedTebScenario::GetScenarioConfig() {
