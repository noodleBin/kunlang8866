...
static const char ROADMAP_IMG_PATH[] =
    "/century/modules/planning/data/semantic_map/sunnyvale_with_two_offices.png";
static const char SPEEDLIMITMAP_IMG_PATH[] =
    "/century/modules/planning/data/semantic_map/"
...
bool BirdviewImgFeatureRenderer::Init(const PlanningSemanticMapConfig& config) {
  ...
  century::hdmap::HDMapUtil::BaseMap();
...
bool BirdviewImgFeatureRenderer::RenderTrafficLight(
  ...
  for (const auto& traffic_light_status :
    ...
    const auto& traffic_light =
        century::hdmap::HDMapUtil::BaseMap().GetSignalById(
    ...
    for (const auto& overlap_id : traffic_light->signal().overlap_id()) {
      const auto& overlap =
          century::hdmap::HDMapUtil::BaseMap().GetOverlapById(overlap_id);
      if (overlap == nullptr) {
      ...
      for (const auto& overlap_object : overlap->overlap().object()) {
        ...
        const auto& lane = century::hdmap::HDMapUtil::BaseMap().GetLaneById(
            overlap_object.id());
        if (lane == nullptr) {
...
bool BirdviewImgFeatureRenderer::RenderRouting(
  ...
  for (int i = 0; i < routing_lanes_size; ++i) {
    ...
    const auto& lane =
        century::hdmap::HDMapUtil::BaseMap().GetLaneById(hdmap::MakeMapId(
