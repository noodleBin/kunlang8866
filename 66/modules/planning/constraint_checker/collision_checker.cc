...
bool CollisionChecker::InCollision(
  ...
  for (size_t i = 0; i < ego_trajectory.NumOfPoints(); ++i) {
    ...
    double shift_distance = ego_length * 0.5 - ego_back_edge_to_center;
    ...
    for (const auto obstacle : obstacles) {
      ...
      if (perception::PerceptionObstacle::UNKNOWN ==
          ...
          perception::PerceptionObstacle::UNKNOWN_UNMOVABLE ==
              obstacle->Perception().type()) {
        Polygon2d obstacle_polygon = obstacle->GetPolygon(obtacle_point);
...
bool CollisionChecker::InCollision(
  ...
  for (size_t i = 0; i < discretized_trajectory.NumOfPoints(); ++i) {
    ...
    double shift_distance =
        ego_length * 0.5 - vehicle_config.vehicle_param().back_edge_to_center();
    Vec2d shift_vec{shift_distance * std::cos(ego_theta),
                    shift_distance * std::sin(ego_theta)};
    ego_box.Shift(shift_vec);
