...
TEST(Obstacle, IsValidPerceptionObstacle) {
  ...
  std::vector<double> p_x = {1.0, 1.0, -1.0, -1.0};
  std::vector<double> p_y = {1.0, -1.0, -1.0, 1.0};
  EXPECT_TRUE(p_x.size() == p_y.size());
  for (size_t i = 0; i < p_x.size(); ++i) {
    perception_obstacle.add_polygon_point();
    perception_obstacle.mutable_polygon_point(i)->set_x(p_x[i]);
    perception_obstacle.mutable_polygon_point(i)->set_y(p_y[i]);
    perception_obstacle.mutable_polygon_point(i)->set_z(1);
  }
  EXPECT_TRUE(Obstacle::IsValidPerceptionObstacle(perception_obstacle));

  // Valid points are less than 3
  p_x.clear();
  p_y.clear();
  p_x = {1.0, 1.0, -1.0};
  p_y = {1.0, 1.0, -1.0};
  EXPECT_TRUE(p_x.size() == p_y.size());
  perception_obstacle.clear_polygon_point();
  for (size_t i = 0; i < p_x.size(); ++i) {
    perception_obstacle.add_polygon_point();
    perception_obstacle.mutable_polygon_point(i)->set_x(p_x[i]);
    perception_obstacle.mutable_polygon_point(i)->set_y(p_y[i]);
  ...
  // The polygon of footprint is non-convex
  p_x.clear();
  p_y.clear();
  p_x = {1.0, -1.0, 0.0, 0.0};
  p_y = {1.0, 1.0, 0.0, -1.0};
  ...
  // The footprint area is too small
  p_x.clear();
  p_y.clear();
  p_x = {0.0, 1e-8, 1e-8, 0};
  p_y = {0.0, 0, 1e-8, 1e-8};
 ...
 public:
  virtual void SetUp() {
    ...
    ASSERT_TRUE(cyber::common::GetProtoFromFile(
        "/century/modules/planning/testdata/common/sample_prediction.pb.txt",
...
TEST_F(ObstacleTest, CreateObstacles) {
  ...
  EXPECT_TRUE(indexed_obstacles_.Find("2162"));
}
...
TEST_F(ObstacleTest, PerceptionBoundingBox) {
  ...
  EXPECT_DOUBLE_EQ(prediction_heading, box.heading());
}
...
TEST_F(ObstacleTest, GetPolygon) {
  const auto* obstacle = indexed_obstacles_.Find("2162");
  ASSERT_TRUE(obstacle);
  century::common::TrajectoryPoint point;
  auto* path_point = point.mutable_path_point();
  path_point->set_x(5.0);
  path_point->set_y(6.0);
  path_point->set_z(1.0);
  path_point->set_theta(0.78539815);
  const auto& polygon = obstacle->GetPolygon(point);
  std::vector<common::math::Vec2d> vertices;
  polygon.GetAllVertices(&vertices);
  EXPECT_EQ(4, vertices.size());
  EXPECT_DOUBLE_EQ(2.8786770496331089, vertices.at(0).x());
  EXPECT_DOUBLE_EQ(5.2929010392832492, vertices.at(0).y());
  EXPECT_DOUBLE_EQ(4.292885398353266, vertices.at(1).x());
  EXPECT_DOUBLE_EQ(3.8786822632764366, vertices.at(1).y());
  EXPECT_DOUBLE_EQ(7.1213229503668911, vertices.at(2).x());
  EXPECT_DOUBLE_EQ(6.7070989607167508, vertices.at(2).y());
  EXPECT_DOUBLE_EQ(5.707114601646734, vertices.at(3).x());
  EXPECT_DOUBLE_EQ(8.1213177367235634, vertices.at(3).y());
}
