...
void CopyTrajectory(const DiscretizedTrajectory trajectory_src,
                    century::common::Trajectory* trajectory_tgt_ptr) {
...
void OpenSpaceInfo::RecordDebug(century::planning_internal::Debug* ptr_debug) {
  ...
  *ptr_debug = debug_instance_;
  ...
  open_space->mutable_hybrid_search_info()->mutable_origin_point()->set_x(
      origin_point_.x());
  open_space->mutable_hybrid_search_info()->mutable_origin_point()->set_y(
      origin_point_.y());
  open_space->mutable_hybrid_search_info()->mutable_origin_point()->set_z(
      0.0);

  open_space->mutable_hybrid_search_info()->set_origin_heading_rad(
      origin_heading_);
}
