...
class ReferenceLineInfo {
 public:
  ...
  void OverrideTurnFlagToFalseInRange(double start_s, double end_s) {
    for (auto& entry : path_point_in_turn_vector_) {
      if (entry.first >= start_s && entry.first <= end_s) {
        entry.second = false;
      }
    }
  }
  void SetFenceSafetyRange(double start_s, double end_s) {
    fence_safety_range_start_s_ = start_s;
    fence_safety_range_end_s_ = end_s;
  }
  bool IsInFenceSafetyRange(double s) const {
    return fence_safety_range_end_s_ > fence_safety_range_start_s_ &&
           s >= fence_safety_range_start_s_ &&
           s <= fence_safety_range_end_s_;
  }
bool FindClosestPointInTurn(double target);
 ...
 private:
  ...
  double fence_safety_range_start_s_ = 0.0;
  double fence_safety_range_end_s_ = -1.0;
