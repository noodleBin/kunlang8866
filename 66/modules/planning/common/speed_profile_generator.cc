...
SpeedData SpeedProfileGenerator::GenerateFallbackSpeed(
  ...
  // if already stopped
  if (init_v < kEpison && init_a < kEpison) {
  ...
  // Solve the problem
  bool is_low_speed = ego_info->vehicle_state().linear_velocity() <
                      FLAGS_lower_speed_for_speed_fallback;
  bool is_low_kappa = std::abs(ego_info->start_point().path_point().kappa()) <
                      FLAGS_lower_kappa_for_speed_fallback;
  // if DP or QP faild stop use -6m/s^2.
  if (FLAGS_enable_emergency_speed_fallback && is_low_speed && is_low_kappa) {
    AERROR << "emergency speed fallback.";
...
void SpeedProfileGenerator::FillEnoughSpeedPoints(SpeedData* const speed_data) {
  ...
  double last_point_s = last_point.s();
...
SpeedData SpeedProfileGenerator::GenerateStopProfile(const double init_speed,
  ...
  if (FLAGS_enable_emergency_speed_fallback) {
    const auto& veh_param =
        common::VehicleConfigHelper::GetConfig().vehicle_param();
    acc = veh_param.max_deceleration();
  }
