...
int Destination::MakeDecisions(Frame* frame,
  ...
  if (!routing->routing_request().parking_info().parking_space_id().empty()) {
    AINFO << "parking space scenario";
    return 0;
  }
  ...
  reference_line_info->set_routing_end(routing_end);
  ...
  // build stop decision
  const double front_to_base_link = common::VehicleConfigHelper::GetConfig()
                                        .vehicle_param()
                                        .front_edge_to_center();
  if (reference_line.IsOnLane(dest_sl)) {
    // const double dest_lane_s =
    //     std::fmax(0.0, routing_end.s() + front_to_base_link);
    ...
    const double dest_s = std::fmax(0.0, dest_sl.s() + front_to_base_link);
    util::BuildStopDecision(
        stop_wall_id, dest_s, config_.destination().stop_distance(),
        StopReasonCode::STOP_REASON_DESTINATION, wait_for_obstacle_ids,
        TrafficRuleConfig::RuleId_Name(config_.rule_id()), frame,
        reference_line_info);
  } else if (util::IsOvertake(injector_->planning_context()) &&
             reference_line.IsOnRoad(dest_sl)) {
    const double dest_s = std::fmax(0.0, dest_sl.s() + front_to_base_link);
    util::BuildStopDecision(
        stop_wall_id, dest_s, config_.destination().stop_distance(),
        StopReasonCode::STOP_REASON_DESTINATION, wait_for_obstacle_ids,
    ...
    // Not need to build destination stop wall.
  }
