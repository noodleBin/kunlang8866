...
bool Rerouting::ChangeLaneFailRerouting() {
  ...
  if (!reference_line_info_->GetRoutePassageEndPoint(
          injector_->planning_context(), PassageType::SELF, &routing_end)) {
    return true;
  }
  const auto& reference_line = reference_line_info_->reference_line();
  common::SLPoint sl_point;
  if (!reference_line.XYToSL(routing_end, &sl_point)) {
    AERROR << "Failed to project point: " << routing_end.ShortDebugString();
    return false;
  }
