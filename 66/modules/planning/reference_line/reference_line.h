...
struct CornerPoint {
  ...
  CornerPoint() = default;
  CornerPoint(double _s, double _l, double _x, double _y) {
    sl_point.set_s(_s);
    sl_point.set_l(_l);
    xy_point.set_x(_x);
    xy_point.set_y(_y);
...
class ReferenceLine {
 public:
  ...
  bool GetSLBoundary(const hdmap::Polygon& polygon,
                     SLBoundary* const sl_boundary) const;
  bool GetSLBoundaryUseOneProjection(const common::TrajectoryPoint adc_point,
  ...
  bool TrimOutsideRoadPartSL(const hdmap::Polygon& polygon,
                             SLBoundary* const sl_boundary) const;

  bool GetCornerPointsInfo(
      const hdmap::Polygon& polygon,
      std::vector<CornerPoint>* const corner_points,
      std::vector<std::pair<double, double>>* const road_width,
      SLBoundary* const sl_boundary) const;

  double GetTrimOutsideStartS(
      const std::vector<CornerPoint>& corner_points,
      const std::vector<std::pair<double, double>>& road_width) const;

  double GetTrimOutsideEndS(
      const std::vector<CornerPoint>& corner_points,
      const std::vector<std::pair<double, double>>& road_width) const;

  void GetCrossPointSValueWithRoad(const common::math::Vec2d& point_a,
                                   const common::math::Vec2d& point_b,
                                   const double check_l, double* const s) const;

  bool HasInRoadStartPoint(
      const std::vector<CornerPoint>& corner_points,
  ...
  bool XYToSLUseOneProjection(const common::math::Vec2d& xy_point,
  ...
  template <class XYPoint>
  bool XYToSLUseOneProjection(const XYPoint& xy,
                              common::SLPoint* const sl_point,
                              const double adc_s) const {
    return XYToSLUseOneProjection(common::math::Vec2d(xy.x(), xy.y()), sl_point,
                                  adc_s);
  }
  bool XYToSLProjectionWithHueristicParams(const common::math::Vec2d& xy,
                                           common::SLPoint* const sl_point,
                                           const double ref_s,
                                           const double obs_length) const;

  template <class XYPoint>
  bool XYToSLProjectionWithHueristicParams(const XYPoint& xy,
                                           common::SLPoint* const sl_point,
                                           const double ref_s,
                                           const double obs_length) const {
    return XYToSLProjectionWithHueristicParams(
        common::math::Vec2d(xy.x(), xy.y()), sl_point, ref_s, obs_length);
  }
  ...
  double GetLeftDrivingWidth(const SLBoundary& sl_boundary) const;
  /**
  ...
  bool IsOnADCLane(const SLBoundary& adc_sl_boundary,
                   const SLBoundary& obs_sl_boundary) const;

  bool IsCrossLane(const SLBoundary& sl_boundary) const;
  /**
  ...
  /**
   * @brief Check if a polygon is blocking the road surface. The criteria is to
   * check whether the remaining space on the road surface is larger than the
   * provided gap space.
   * @param boxed the provided polygon
   * @param gap check the gap of the space
   * @return true if the polygon blocks the road.
   */
  bool IsBlockRoad(const common::math::Box2d& box2d,
                   const common::math::Polygon2d& polygon2d, double gap) const;
  ...
  /**
   * @brief check if any part of the polygon has overlap with the road.
   */
  bool HasOverlap(const common::math::Polygon2d& polygon) const;
