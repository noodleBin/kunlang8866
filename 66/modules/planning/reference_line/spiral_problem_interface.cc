...
void SpiralProblemInterface::get_variables_info(double* x_l, double* x_u) {
  for (int i = 0; i < num_of_points_; ++i) {
    int index = i * 5;

    double theta_lower = 0.0, theta_upper = 0.0, kappa_lower = 0.0,
           kappa_upper = 0.0, dkappa_lower = 0.0, dkappa_upper = 0.0,
           x_lower = 0.0, x_upper = 0.0, y_lower = 0.0, y_upper = 0.0;
...
bool SpiralProblemInterface::get_starting_point(int n, bool init_x, double* x,
  ...
  for (int i = 0; i + 1 < num_of_points_; ++i) {
    ...
    if (common::util::IsFloatEqual(point_distances_[i], 0.0)) {
      AERROR << "the point_distances_[i] is zero!";
      point_distances_[i] = FLAGS_numerical_epsilon;
    }
    if (0.0 != std::cos(0.5 * delta_theta)) {
      x[variable_offset + i] =
...
bool SpiralProblemInterface::eval_f(int n, const double* x, bool new_x,
  ...
  for (int i = 0; i + 1 < num_of_points_; ++i) {
    ...
    CHECK_GT(num_of_internal_points_, 0);
...
void SpiralProblemInterface::set_values(int* iRow, int* jCol, int nele_jac) {
  int nz_index = 0, variable_offset = num_of_points_ * 5;
  for (int i = 0; i + 1 < num_of_points_; ++i) {
    int variable_index = i * 5;
  ...
  set_constraint_offset_values(iRow, jCol, nele_jac, nz_index);
  // int constraint_offset = 2 * (num_of_points_ - 1);
  // for (int i = 0; i < num_of_points_; ++i) {
...
void SpiralProblemInterface::set_constraint_offset_values(int* iRow, int* jCol,
                                                          int nele_jac,
                                                          int nz_index) {
  int constraint_offset = 2 * (num_of_points_ - 1);
  for (int i = 0; i < num_of_points_; ++i) {
...
bool SpiralProblemInterface::eval_jac_g(int n, const double* x, bool new_x,
  ...
  if (values == nullptr) {
    ...
    for (int i = 0; i + 1 < num_of_points_; ++i) {
      ...
      // for x coordinate
      set_x_values(x_diff, y_diff, pos_theta0, pos_kappa0, pos_dkappa0,
                   pos_theta1, pos_kappa1, pos_dkappa1, pos_delta_s, &nz_index,
                   values);
...
void SpiralProblemInterface::set_x_values(
    const double x_diff, const double y_diff,
    const std::pair<double, double>& pos_theta0,
    const std::pair<double, double>& pos_kappa0,
    const std::pair<double, double>& pos_dkappa0,
    const std::pair<double, double>& pos_theta1,
    const std::pair<double, double>& pos_kappa1,
    const std::pair<double, double>& pos_dkappa1,
    const std::pair<double, double>& pos_delta_s, int* nz_index,
    double* values) {
