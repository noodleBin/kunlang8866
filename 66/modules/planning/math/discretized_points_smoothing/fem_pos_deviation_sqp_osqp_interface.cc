...
bool FemPosDeviationSqpOsqpInterface::SanityCheck() {
...
void FemPosDeviationSqpOsqpInterface::CalculateOptimalStates() {
...
void FemPosDeviationSqpOsqpInterface::OsqpSolverSetting(
    OSQPSettings* const settings) {
  if (nullptr == settings) {
    return;
  }
...
bool FemPosDeviationSqpOsqpInterface::Solve() {
  ...
  // Sequential solution
  bool ret = SequentialSolution(&primal_warm_start, &q, &A_data, &lower_bounds,
                                &upper_bounds, &A_indptr, &A_indices, work);
  clear_work();
  if (!ret) {
    return false;
  }
...
bool FemPosDeviationSqpOsqpInterface::SequentialSolution(
    std::vector<c_float>* primal_warm_start, std::vector<c_float>* q,
    std::vector<c_float>* A_data, std::vector<c_float>* lower_bounds,
    std::vector<c_float>* upper_bounds, std::vector<c_int>* A_indptr,
    std::vector<c_int>* A_indices, OSQPWorkspace* work) {
  int pen_itr = 0;
  double ctol = 0.0;
  ...
  while (pen_itr < sqp_pen_max_iter_) {
    ...
    while (sub_itr < sqp_sub_max_iter_) {
      ...
      if (common::util::IsFloatEqual(last_fvalue, 0.0)) {
        AERROR << "the last_fvalue is zero!";
        last_fvalue = FLAGS_numerical_epsilon;
      }
