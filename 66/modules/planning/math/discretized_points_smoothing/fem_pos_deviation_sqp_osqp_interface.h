...
class FemPosDeviationSqpOsqpInterface {
 ...
 private:
  bool SanityCheck();
  void OsqpSolverSetting(OSQPSettings* const settings);
  void CalculateOptimalStates();
  bool SequentialSolution(std::vector<c_float>* primal_warm_start,
                          std::vector<c_float>* q, std::vector<c_float>* A_data,
