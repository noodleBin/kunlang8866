...
Status NaviSpeedDecider::Execute(Frame* frame,
  ...
  if (enable_accel_auto_compensation_) {
    if (prev_v_ > 0.0 && current_v > 0.0 && prev_v_ > expected_v) {
      if (common::util::IsFloatEqual(prev_v_, current_v)) {
        AERROR << "prev_v_ and current_v equals!";
        prev_v_ += FLAGS_numerical_epsilon;
      }
      auto raw_ratio = (prev_v_ - expected_v) / (prev_v_ - current_v);
      raw_ratio = Clamp(raw_ratio, 0.0, kDecelCompensationLimit);
      decel_compensation_ratio_ = (decel_compensation_ratio_ + raw_ratio) * 0.5;
...
Status NaviSpeedDecider::MakeSpeedDecision(
  ...
  GenerateSpeedPoint(start_s, &ts_points, speed_data);
...
Status NaviSpeedDecider::AddCentricAccelerationConstraints(
  ...
  CalculateAccelerationConstraints(path_points, bs, &max_kappa,
        &max_kappa_v, &preffered_kappa_v, &max_kappa_s);
...
void NaviSpeedDecider::GenerateSpeedPoint(
      const double start_s, std::vector<NaviSpeedTsPoint>* const ts_points,
      SpeedData* const speed_data) {
  speed_data->clear();
  ...
  if (1 == speed_data->size()) {
    const auto& prev = speed_data->back();
    speed_data->AppendSpeedPoint(prev.s(), prev.t() + 1.0, 0.0, 0.0, 0.0);
  }
}

void NaviSpeedDecider::CalculateAccelerationConstraints(
      const std::vector<common::PathPoint>& path_points,
      const double bs, double* const max_kappa, double* const max_kappa_v,
      double* const preffered_kappa_v, double* const max_kappa_s) {
  struct CLimit {
    double s;
    double v_max;
    double v_preffered;
  ...
  for (size_t i = 1; i < path_points.size(); ++i) {
    ...
    auto kappa = std::abs((start_k + end_k) * 0.5);
    if (std::abs(kappa) < kappa_threshold_) {
      kappa /= kKappaAdjustRatio;
    }
    if (common::util::IsFloatEqual(kappa, 0.0)) {
      AERROR << "the kappa is zero!";
      kappa = FLAGS_numerical_epsilon;
    }
