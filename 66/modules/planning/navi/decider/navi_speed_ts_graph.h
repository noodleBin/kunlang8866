...
class NaviSpeedTsGraph {
 public:
   ...
   */
  century::common::Status Solve(std::vector<NaviSpeedTsPoint>* points);

 private:
  void CalculateConstraints(const double v1, const double vm,
                            const double v0, const double od,
                            const double v, double* const s);
  void SolvePreProcess();
  std::string SolveProcess(std::vector<NaviSpeedTsPoint>* const output);
  void SolvePostProcess(std::vector<NaviSpeedTsPoint>* const points);
