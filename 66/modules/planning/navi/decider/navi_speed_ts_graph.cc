...
void NaviSpeedTsGraph::UpdateObstacleConstraints(double distance,
  ...
  auto v0 = start_v_;
  auto v1 = v;
  if (common::util::IsFloatEqual(distance + safe_distance, 0.0)) {
    distance += FLAGS_numerical_epsilon;
  }
  if (common::util::IsFloatEqual(od, 0.0)) {
    AERROR << "the od is zero!";
    od = FLAGS_numerical_epsilon;
  }
  auto r0 = (distance - safe_distance) / (distance + safe_distance);
  auto vm = std::max(cruise_speed * r0, 0.0) +
            (1.0 + following_accel_ratio * r0) * v1;
  if (common::util::IsFloatEqual(v1, vm)) {
    AERROR << "v1 and vm equals!";
    vm += FLAGS_numerical_epsilon;
  }

  CalculateConstraints(v1, vm, v0, od, v, &s);
}
...
Status NaviSpeedTsGraph::Solve(std::vector<NaviSpeedTsPoint>* output) {
  ...
  std::string msg = SolveProcess(output);
  if (!msg.empty()) {
    AERROR << msg;
    return Status(ErrorCode::PLANNING_ERROR, msg);
  }

  SolvePostProcess(output);

  return Status::OK();
}

void NaviSpeedTsGraph::SolvePreProcess() {
...
std::string NaviSpeedTsGraph::SolveProcess(
      std::vector<NaviSpeedTsPoint>* const output) {
  auto& points = *output;
  ...
  for (size_t i = 1; i < points.size(); ++i) {
    ...
    if (common::util::IsFloatEqual(prev.v + v_max, 0.0)) {
      AERROR << "the prev.v + v_max is zero!";
      t_min = std::max(prev.t + 2.0 * s_step_ / FLAGS_numerical_epsilon, t_min);
    } else {
    ...
    if (r0 > 0.0) {
      if (common::util::IsFloatEqual(b_max, 0.0)) {
        AERROR << "the b_max is zero!";
        b_max = FLAGS_numerical_epsilon;
      }
      t_max = prev.t + (prev.v - std::sqrt(r0)) / b_max;
    }
    // if t_max < t_min
    if (t_max < t_min) {
      msg = "failure to satisfy the constraints.";
      return msg;
    }

    // compute t_preffered base on v_preffered
    auto v_preffered = constraints.v_preffered;
    double v_sum = (prev.v + v_preffered);
    if (common::util::IsFloatEqual(v_sum, 0.0)) {
      AERROR << "the v_sum is zero!";
      v_sum = FLAGS_numerical_epsilon;
    }
    auto t_preffered = prev.t + 2 * s_step_ / v_sum;

    cur.s = prev.s + s_step_;
    cur.t = Clamp(t_preffered, t_min, t_max);
    auto dt = cur.t - prev.t;
    if (common::util::IsFloatEqual(dt, 0.0)) {
      AERROR << "the dt is zero!";
      dt = FLAGS_numerical_epsilon;
    }
...
void NaviSpeedTsGraph::SolvePostProcess(
      std::vector<NaviSpeedTsPoint>* const output) {
  auto& points = *output;
  ...
  if (common::util::IsFloatEqual(second.t, first.t)) {
    AERROR << "second.t and first.t equals!";
    first.a = (second.v - first.v) / FLAGS_numerical_epsilon;
  } else {
...
void NaviSpeedTsGraph::CalculateConstraints(const double v1, const double vm,
    const double v0, const double od, const double v, double* const s) {
  ...
  for (auto& pc : constraints_) {
    ...
    if (first) {
      ...
      *s = 0.0;
      ...
      if (common::util::IsFloatEqual(a, 0.0)) {
        AERROR << "the a is zero!";
        a = FLAGS_numerical_epsilon;
      }
      ...
      *s += s_step_;
      ...
      auto cur_v = v;
      if (common::util::IsFloatEqual(prev_v + cur_v, 0.0)) {
        AERROR << "the prev_v + cur_v is zero!";
        t += 2.0 * s_step_ / FLAGS_numerical_epsilon;
      } else {
