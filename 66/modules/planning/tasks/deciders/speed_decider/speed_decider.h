...
class SpeedDecider : public Task {
 ...
 private:
  ...
  bool CreateStopDecision(const Obstacle& obstacle, const std::string& stop_tag,
  ...
  void MakeStopDecisionForPedestrian(Obstacle* mutable_obstacle) const;

  common::Status MakeObjectDecision(const SpeedData& speed_profile,
                                    PathDecision* const path_decision) const;
  void MakeDecisionForBelowLocation(Obstacle* mutable_obstacle) const;

  void MakeDecisionForAboveLocation(Obstacle* mutable_obstacle) const;

  bool MakeDecisionForCrossLocation(Obstacle* mutable_obstacle) const;
 ...
 private:
  ...
  double min_avoid_distance_ = 6.0;
  double half_adc_decel_ = 3.0;
};
