...
constexpr double kRoutingEndLateralTolerance = 5.0;
...
void RuleBasedStopDecider::AddPathEndStop(
  ...
  if (path_data.path_label().empty() ||
      path_data.frenet_frame_path().empty()) {
    return;
  }
  const double path_start_s = path_data.frenet_frame_path().front().s();
  const double path_end_s = path_data.frenet_frame_path().back().s();
  const double path_length = path_end_s - path_start_s;
  if (path_length >= FLAGS_short_path_length_threshold) {
    return;
  }
  ...
  if (routing != nullptr && routing->routing_request().waypoint_size() >= 2 &&
    ...
    const double distance_to_routing_end = routing_end_sl.s() - adc_center_s;
    const double routing_end_to_path_end = routing_end_sl.s() - path_end_s;
    const bool path_end_reaches_routing_end =
        routing_end_to_path_end < FLAGS_short_path_length_threshold;
    const bool is_near_routing_end =
        std::fabs(routing_end_sl.l()) < kRoutingEndLateralTolerance &&
        distance_to_routing_end > 0.0 &&
        distance_to_routing_end < FLAGS_short_path_length_threshold &&
        path_end_reaches_routing_end;
    if (is_near_routing_end) {
      return;
    }
  }
  ...
  util::BuildStopDecision(
      stop_wall_id,
      path_end_s - rule_based_stop_decider_config_.stop_wall_distance(), 0.0,
      StopReasonCode::STOP_REASON_REFERENCE_END, wait_for_obstacles,
