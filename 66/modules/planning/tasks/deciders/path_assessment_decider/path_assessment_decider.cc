...
void PathAssessmentDecider::PickOptimalPath(
  ...
  if (valid_path_data.front().path_label().find("right") != std::string::npos) {
    ...
    injector_->last_using_lateral_ = injector_->right_laneborrow_endstate;
    auto* mutable_laneborrow = injector_->planning_context()
                                   ->mutable_planning_status()
                                   ->mutable_lane_borrow();
  ...
  const bool is_lane_borrow_scenario =
      injector_->planning_context()
          ->planning_status()
          .path_decider()
          .is_in_path_lane_borrow_scenario();
  const bool should_commit_last_lateral =
  ...
  if (should_commit_last_lateral) {
    injector_->planning_context()
        ->mutable_planning_status()
        ->set_last_lateral_distance(injector_->last_using_lateral_);
    AINFO << "commit last_lateral_distance="
          << injector_->last_using_lateral_
          << ", is_lane_borrow_scenario=" << is_lane_borrow_scenario;
  }
