...
enum PathType { STRAIGHT_PATH = 0, LEFT_PATH, RIGHT_PATH };
class PathAssessmentDecider : public Decider {
 ...
 private:
  ...
  void RemoveInvalidPath(ReferenceLineInfo* const reference_line_info,
                         const std::vector<PathData>& candidate_path_data,
                         std::vector<PathData>* const valid_path_data);

  void AnalyzePath(ReferenceLineInfo* const reference_line_info,
                   const Obstacle* blocking_obstacle_on_selflane,
                   std::vector<PathData>* const valid_path_data);

  void PickOptimalPath(ReferenceLineInfo* const reference_line_info,
                       const Obstacle* blocking_obstacle_on_selflane,
                       const std::vector<PathData>& valid_path_data);

  void UpdateNecessaryInfoForLaneBorrow(
      ReferenceLineInfo* const reference_line_info);

  void UpdateBorrowObstacles(const PathType& path_typ,
                             ReferenceLineInfo* const reference_line_info,
                             const FrenetFramePath& frenet_path,
                             const double& path_typ_end_s);

  bool IgnoreUnstableObstacle(const Obstacle& obstacle);

  bool LeftPathCanBorrow(ReferenceLineInfo* const reference_line_info,
                         const FrenetFramePath& frenet_path,
                         const Obstacle* obstacle,
                         const double& path_typ_end_s);
  bool RightPathCanBorrow(ReferenceLineInfo* const reference_line_info,
                          const FrenetFramePath& frenet_path,
                          const Obstacle* obstacle,
                          const double& path_typ_end_s);
};
