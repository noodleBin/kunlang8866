...
Status PathReferenceDecider::Process(Frame *frame,
  ...
  if (FLAGS_skip_path_reference_in_change_lane &&
    ...
    SkipPathReference(err_msg, reference_line_info);
    return Status::OK();
  }
...
void PathReferenceDecider::ConvertTrajectoryToPath(
  ...
  for (auto trajectory_point : trajectory_points) {
    if (trajectory_point.has_path_point()) {
      path_points->emplace_back(trajectory_point.path_point());
    }
  }
}
...
void PathReferenceDecider::SkipPathReference(
    const std::string &err_msg, ReferenceLineInfo *const reference_line_info) {
