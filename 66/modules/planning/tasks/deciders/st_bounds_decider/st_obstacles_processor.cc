...
namespace {
constexpr double kHighVelocity = 7.0;
constexpr double kLowVelocity = 0.0;
constexpr double kMaxLateralBuffer = 1.5;
constexpr double kSecondaryLateralBuffer = 1.0;
...
Status STObstaclesProcessor::InputCheck(PathDecision* const path_decision) {
  ...
  obs_id_to_st_boundary_.clear();
  adc_velocity_ = path_decision->linear_velocity();
  return Status::OK();
}

Status STObstaclesProcessor::MapObstaclesToSTGraph(
    PathDecision* const path_decision,
  ...
  for (const auto* obs_item_ptr : path_decision->obstacles().Items()) {
    ...
    if (obs_ptr->Trajectory().trajectory_point().empty()) {
      ...
      if (boundary.bottom_left_point().s() - adc_path_init_s_ <
        ...
        if (!FLAGS_enable_check_back_side_obstacle) {
          continue;
        }
        // be sure obstacles that are behind.
        if (obs_ptr->PerceptionSLBoundary().end_s() <
                path_data_.frenet_frame_path().front().s() &&
            FLAGS_enable_check_back_side_obstacle) {
          continue;
        }
      }
...
void STObstaclesProcessor::AddDecision(
    PathDecision* const path_decision,
    const std::unordered_set<std::string>& non_ignore_obstacles) {
  for (const auto* obs_item_ptr : path_decision->obstacles().Items()) {
    Obstacle* obs_ptr = path_decision->Find(obs_item_ptr->Id());
...
Status STObstaclesProcessor::MapObstaclesToSTBoundaries(
    PathDecision* const path_decision) {
  // Sanity checks.
  const auto& status = InputCheck(path_decision);
  if (!status.ok()) {
    return status;
  }
  ...
  std::unordered_set<std::string> non_ignore_obstacles;
  std::tuple<std::string, STBoundary, Obstacle*> closest_stop_obstacle;
  const auto& status_map = MapObstaclesToSTGraph(
      path_decision, &non_ignore_obstacles, &closest_stop_obstacle);
  if (!status_map.ok()) {
    return status_map;
  }
  ...
  if (std::get<0>(closest_stop_obstacle) != "NULL") {
    ...
    // ADEBUG << "Adding " << closest_stop_obs_ptr->Id() << " into the
    // ST-graph."; ADEBUG << "min_s = " << closest_stop_obs_boundary.min_s();
  }

  // Set IGNORE decision for those that are not in ST-graph:
  AddDecision(path_decision, non_ignore_obstacles);
...
bool STObstaclesProcessor::GetSBoundsFromDecisions(
  ...
  SetNewBoundaries(s_min, s_max, new_t_edges, &ambiguous_t_edges);
  // For ambiguous ones, enumerate all decisions and corresponding bounds.
  bool is_get_decision =
      GetAvailableDecisions(s_min, s_max, ambiguous_t_edges, available_s_bounds,
                            available_obs_decisions);

  return is_get_decision;
}

void STObstaclesProcessor::SetNewBoundaries(
    double s_min, double s_max, const std::vector<ObsTEdge>& new_t_edges,
    std::vector<ObsTEdge>* ambiguous_t_edges) {
...
bool STObstaclesProcessor::GetAvailableDecisions(
    double s_min, double s_max, std::vector<ObsTEdge> ambiguous_t_edges,
...
bool STObstaclesProcessor::ComputeObstacleSTBoundary(
  ...
  *is_caution_obstacle = false;
  ...
  if (obs_trajectory.trajectory_point().empty()) {
    ...
    //   if (GetOverlappingS(adc_path_points, obs_polygon, kADCSafetyLBuffer,
    //                       &overlapping_s)) {
    ...
    *obs_caution_end_t = planning_time_;
    ...
    for (const auto& obs_traj_pt : obs_trajectory.trajectory_point()) {
      ...
      if (perception::PerceptionObstacle::UNKNOWN == obs_type ||
        ...
        const Polygon2d& obs_polygon = obstacle.GetPolygon(obs_traj_pt);
        rst = GetOverlappingS(adc_path_points, obs_polygon, kADCSafetyLBuffer,
                              &overlapping_s);
      } else {
        const Box2d& obs_box = obstacle.GetBoundingBox(obs_traj_pt);
        ADEBUG << obs_box.DebugString();
        rst = GetOverlappingS(adc_path_points, obs_box, kADCSafetyLBuffer,
                              &overlapping_s);
      }
      if (rst) {
...
bool STObstaclesProcessor::GetOverlappingS(
    const std::vector<PathPoint>& adc_path_points,
    const Polygon2d& obstacle_instance, const double adc_l_buffer,
    std::pair<double, double>* const overlapping_s) {
  // Locate the possible range to search in details.
  int pt_before_idx = GetSBoundingPathPointIndex(
      adc_path_points, obstacle_instance, vehicle_param_.front_edge_to_center(),
...
int STObstaclesProcessor::GetSBoundingPathPointIndex(
    const std::vector<PathPoint>& adc_path_points,
    const Polygon2d& obstacle_instance, const double s_thresh,
...
bool STObstaclesProcessor::IsPathPointAwayFromObstacle(
    const PathPoint& path_point, const PathPoint& direction_point,
    const Polygon2d& obs_polygon, const double s_thresh, const bool is_before) {
  Vec2d path_pt(path_point.x(), path_point.y());
  Vec2d dir_pt(direction_point.x(), direction_point.y());
  LineSegment2d path_dir_lineseg(path_pt, dir_pt);
  LineSegment2d normal_line_seg(path_pt, path_dir_lineseg.rotate(M_PI_2));

  auto corner_points = obs_polygon.GetAllVertices();
  for (const auto& corner_pt : corner_points) {
    Vec2d normal_line_ft_pt;
    normal_line_seg.GetPerpendicularFoot(corner_pt, &normal_line_ft_pt);
    Vec2d path_dir_unit_vec = path_dir_lineseg.unit_direction();
...
bool STObstaclesProcessor::IsADCOverlappingWithObstacle(
  ...
  // just for box
  double collision_buffer = l_buffer * 2;
  double adc_velocity = adc_velocity_;
  adc_velocity = std::fabs(adc_velocity);
  if (FLAGS_enable_add_adc_buffer) {
    collision_buffer =
        common::math::lerp(kMinLateralBuffer * 2, kLowVelocity,
                           kMaxLateralBuffer * 2, kHighVelocity, adc_velocity);
  }
  Box2d adc_box(ego_center_map_frame, adc_path_point.theta(),
                vehicle_param_.length(),
                vehicle_param_.width() + collision_buffer);
...
bool STObstaclesProcessor::IsADCOverlappingWithObstacle(
    const PathPoint& adc_path_point, const Polygon2d& obs_polygon,
    const double l_buffer) const {
  // Convert reference point from center of rear axis to center of ADC.
  Vec2d ego_center_map_frame((vehicle_param_.front_edge_to_center() -
                              vehicle_param_.back_edge_to_center()) *
  ...
  ADEBUG << "    ADC box is: " << adc_box.DebugString();
  ADEBUG << "    Obs polygon is: " << obs_polygon.DebugString();
