...
void STBoundsDecider::InitSTBoundsDecider(
  ...
  path_decision->set_linear_velocity(
...
Status STBoundsDecider::GenerateFallbackSTBound(STBound* const st_bound,
  ...
  InitSTBoundary(st_bound, vt_bound);
  ...
  for (size_t i = 0; i < st_bound->size(); ++i) {
    ...
    if (!st_obstacles_processor_.GetSBoundsFromDecisions(
      ...
      AERROR << "Failed to find a proper boundary.";
      return Status(ErrorCode::PLANNING_ERROR,
                    "Failed to find a proper boundary.");
    }
    ...
    if (!available_choices.empty()) {
      ...
      AERROR << "No valid st-boundary exists.";
      return Status(ErrorCode::PLANNING_ERROR, "No valid st-boundary exists.");
    }
...
void STBoundsDecider::InitSTBoundary(STBound* const st_bound,
                                     STBound* const vt_bound) {
