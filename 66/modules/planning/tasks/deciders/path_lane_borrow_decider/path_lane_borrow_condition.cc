...
constexpr double kFinishStoppedLateralError = 0.35;
}  // namespace
...
bool LaneBorrowFsmCondition::Finish2Default(
  ...
  const bool is_stop =
      reference_line_info->vehicle_state().linear_velocity() < 0.1;
  const bool is_stopped_near_ref_line =
      is_stop && std::fabs(center_l) < kFinishStoppedLateralError &&
      diff_heading < FLAGS_same_heading;
  bool can_change_state = is_in_ref_line || is_stopped_near_ref_line;
