...
constexpr double kDistanceToTargetPointInLateral = 16.0;
...
void PathDecider::GetPathPointInTurnLaneRange(const PathData& path_data) {
  ...
  OverrideTurnFlagForFenceSafety(path_data);
}

void PathDecider::OverrideTurnFlagForFenceSafety(const PathData& path_data) {
  ...
  double last_collision_s = -1.0;
  for (const auto& path_point : path_data.discretized_path()) {
    if (path_point.s() > kFenceLookaheadDistance) {
      break;
    }
    if (IsCollisionWithElectricFence(path_point.x(), path_point.y(),
                                     path_point.theta(),
                                     /*is_path_point=*/false)) {
      last_collision_s = path_point.s();
    }
  }
  if (last_collision_s < 0.0) {
    return;
  }

  double safe_local_s = -1.0;
  for (const auto& path_point : path_data.discretized_path()) {
    if (path_point.s() <= last_collision_s) {
      continue;
    }
    if (!IsCollisionWithElectricFence(path_point.x(), path_point.y(),
                                      path_point.theta(),
                                      /*is_path_point=*/false)) {
      safe_local_s = path_point.s();
      break;
    }
  }
  if (safe_local_s < 0.0) {
    AERROR << "figure-8 turn collides with fence along entire path; keep "
              "original turn flags.";
    return;
  }
  reference_line_info_->SetNeedDiagonal(true);
  reference_line_info_->OverrideTurnFlagToFalseInRange(
      center_s, center_s + safe_local_s);
  reference_line_info_->SetFenceSafetyRange(center_s,
                                            center_s + safe_local_s);
}
...
void PathDecider::MakeObstacleLateralDecision(const PathData &path_data,
  ...
  if (IsCollisionWithObstacle(path_data, obstacle,
    ...
    if (is_in_borrow && obstacle.IsIgv()) {
      auto planning_status =
          injector_->planning_context()->mutable_planning_status();
      const auto *top_bull =
          (planning_status != nullptr && planning_status->has_top_bull())
              ? &planning_status->top_bull()
              : nullptr;

      bool is_blocking_igv =
          reference_line_info_->reference_line().IsOnLane(sl_boundary) &&
          (sl_boundary.end_s() > reference_line_info_->AdcSlBoundary().end_s());
      ...
      if (injector_->borrow_response().has_response() &&
        ...
        is_blocking_igv = false;
      }

      if (is_blocking_igv && top_bull && top_bull->is_in_top_bull() &&
          obstacle.Id() == top_bull->blocking_igv_id()) {
        is_blocking_igv = false;
      }

      if (is_blocking_igv) {
        const std::string stop_tag = "PathDecider/borrow-igv-stop";
        AINFO << "path will collision with igv in borrow: " << obstacle.Id()
              << ", stop distance: "
              << GetObstacleStopDistance(obstacle, stop_distance);
        util::GenerateObjectStopDecision(
            obstacle, -GetObstacleStopDistance(obstacle, stop_distance),
            stop_tag, *reference_line_info_, object_decision.mutable_stop());
        if (path_decision->MergeWithMainStop(
                object_decision.stop(), obstacle.Id(),
                reference_line_info_->reference_line(),
...
void PathDecider::MakePathDecision(const PathData &path_data,
  ...
  HandleTopBullBorrow(reference_line_info, path_decision);
  ...
  MakeBarrierBlockDecision(path_data, reference_line_info, path_decision);
...
void PathDecider::HandleTopBullBorrow(ReferenceLineInfo *reference_line_info,
                                      PathDecision *const path_decision) {
  auto *planning_status = injector_->planning_context()->mutable_planning_status();
  ...
  if (action_type != planning::TopBullStatus::BORROW) {
    AINFO << "TopBull action type is not BORROW, action_type=" << action_type;
    return;
  }
  ...
  if (!borrow_response.has_response() ||
      ...
      borrow_response.block_obs_id().empty()) {
    AINFO << "TopBull borrow response is not valid, response_type="
          << borrow_response.response_type()
          << " block_obs_id=" << borrow_response.block_obs_id();
    return;
  }
  if (borrow_response.block_obs_id() != top_bull.blocking_igv_id()) {
    return;
  }
  
  bool is_block_igv_same_direction = false;
  auto v2x_info = injector_->v2x_info();
  for (const auto& vehicle_info : v2x_info.vehicle_info()) {
    std::string vehicle_id = vehicle_info.id();
    if(top_bull.blocking_igv_id() != vehicle_id){
      continue;
    }
    common::math::Vec2d vehicle_point =
        Vec2d(vehicle_info.x(), vehicle_info.y());
    double vehicle_heading = vehicle_info.heading();
    if (canbus::Chassis::GEAR_REVERSE == vehicle_info.gear()) {
      vehicle_heading =
          common::math::NormalizeAngle(vehicle_info.heading() + M_PI);
    }
    const auto& reference_point =
        reference_line_info->reference_line().GetNearestReferencePoint(vehicle_point);
    double ref_heading = reference_point.heading();
    double diff_heading = common::math::NormalizeAngle(vehicle_heading - ref_heading);
    double diff_angle = std::fabs(diff_heading);
    if (diff_angle < kStaticSameDirectionAngle) {
      is_block_igv_same_direction = true;
    }
  }
  if(is_block_igv_same_direction){
    return;
  }

  const std::string &obs_id = borrow_response.block_obs_id();
  double obs_end_s = 0.0;
  bool has_obs_end_s = false;

  const auto igv_boundary_map = reference_line_info->GetIgvBoundaryMap();
  auto obs_sl_it = igv_boundary_map.find(obs_id);
  if (obs_sl_it != igv_boundary_map.end()) {
    obs_end_s = obs_sl_it->second.end_s();
    has_obs_end_s = true;
  } else if (path_decision != nullptr) {
    const auto *blocking_obs = path_decision->Find(obs_id);
    if (blocking_obs != nullptr) {
      obs_end_s = blocking_obs->PerceptionSLBoundary().end_s();
      has_obs_end_s = true;
    }
  }
  if (!has_obs_end_s) {
    AWARN << "TopBull: cannot find borrow obstacle boundary, obs_id="
          << obs_id;
    return;
  }

  const double adc_start_s = reference_line_info->AdcSlBoundary().start_s();
  if (adc_start_s > obs_end_s + kTopBullBorrowCompleteBuffer) {
    planning_status->mutable_top_bull()->set_ego_complete_action(true);
    AINFO << "TopBull: set ego_complete_action true for borrow obstacle "
          << obs_id << " obs_end_s=" << obs_end_s
          << " adc_start_s=" << adc_start_s
          << " complete_buffer=" << kTopBullBorrowCompleteBuffer;
  }
}
...
void PathDecider::MakeBarrierBlockDecision(
  ...
  if (reference_line_info->IsWestIn()) {
    ...
    pass_point_x = kEastOutStopPointX;
    pass_point_y = kEastOutStopPointY;
    is_gate_route = true;
  ...
  if (!reference_line.XYToSL(pass_point, &pass_point_sl)) {
    return;
  }
    if (is_east_out_stop) {
  ...
  const double gate_start_s = std::min(stop_point_sl.s(), pass_point_sl.s());
  const double gate_end_s = std::max(stop_point_sl.s(), pass_point_sl.s());
  if (adc_center_s > gate_start_s && adc_center_s < gate_end_s) {
    reference_line_info->SetIsPassingGate(true);
  }
}
...
void PathDecider::MakeWheelCraneObjectDecision(
  ...
  double adc_center_s = (adc_start_s + adc_end_s) * 0.5;
  ...
  for (const auto *obstacle : path_decision->obstacles().Items()) {
    ...
    double dist_center = std::sqrt(
        (obs_center_s - adc_center_s) * (obs_center_s - adc_center_s) +
        (obs_center_l - adc_center_l) * (obs_center_l - adc_center_l));
    has_wheel_crane = true;
    if (dist_center < min_wheel_crane_start_s) {
      min_wheel_crane_start_s = dist_center;
...
bool PathDecider::IsInDrivableArea(const PathData &path_data,
  ...
  for (const auto &path_point : path_data.discretized_path()) {
    ...
    if ((!reference_line_info_->NeedDiagonal() || is_begin_lane_trun ||
        ...
        !reference_line_info_->IsInFenceSafetyRange(path_point.s() +
                                                    center_s)) {
      heading = path_point.theta();
    }
