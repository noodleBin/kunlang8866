...
void TaskFactory::InitDecidersPartOne(
  ...
  task_factory_.Register(
      TaskConfig::LANE_OVERTAKE_DECIDER,
      [](const TaskConfig& config,
         const std::shared_ptr<DependencyInjector>& injector) -> Task* {
        return new LaneOvertakeDecider(config, injector);
      });
  task_factory_.Register(
...
void TaskFactory::InitDecidersPartTwo(
    const PlanningConfig& config,
    const std::shared_ptr<DependencyInjector>& injector) {
  task_factory_.Register(
      TaskConfig::RISK_SHIFT_DECIDER,
      [](const TaskConfig& config,
         const std::shared_ptr<DependencyInjector>& injector) -> Task* {
        return new RiskShiftDecider(config, injector);
      });
  task_factory_.Register(
  ...
  task_factory_.Register(
      TaskConfig::TOP_BULL_DECIDER,
      [](const TaskConfig& config,
         const std::shared_ptr<DependencyInjector>& injector) -> Task* {
        return new TopBullDecider(config, injector);
      });
  task_factory_.Register(
  ...
  task_factory_.Register(
      TaskConfig::TEB_PRE_OBSERVATION_DECIDER,
      [](const TaskConfig& config,
         const std::shared_ptr<DependencyInjector>& injector) -> Task* {
        return new TEBPreObservationDecider(config, injector);
      });
  task_factory_.Register(
      TaskConfig::TEB_PLANNER_DECIDER,
      [](const TaskConfig& config,
         const std::shared_ptr<DependencyInjector>& injector) -> Task* {
        return new TEBPlannerDecider(config, injector);
      });
  task_factory_.Register(
      TaskConfig::TEB_FALLBACK_DECIDER,
      [](const TaskConfig& config,
         const std::shared_ptr<DependencyInjector>& injector) -> Task* {
        return new TEBFallbackDecider(config, injector);
      });
}

void TaskFactory::InitOptimizers(
    const PlanningConfig& config,
    const std::shared_ptr<DependencyInjector>& injector) {
  task_factory_.Register(
      TaskConfig::TEB_TRAJECTORY_PROVIDER,
      [](const TaskConfig& config,
         const std::shared_ptr<DependencyInjector>& injector) -> Task* {
        return new TEBTrajectoryProvider(config, injector);
      });
  task_factory_.Register(
      TaskConfig::TEB_TRAJECTORY_PARTITION,
      [](const TaskConfig& config,
         const std::shared_ptr<DependencyInjector>& injector) -> Task* {
        return new TEBTrajectoryPartition(config, injector);
      });
  task_factory_.Register(
  ...
  task_factory_.Register(
      ...
      [](const TaskConfig& config,
         const std::shared_ptr<DependencyInjector>& injector) -> Task* {
        return new PiecewiseJerkSpeedNonlinearOptimizer(config, injector);
      });
  task_factory_.Register(
  ...
  task_factory_.Register(
      ...
      [](const TaskConfig& config,
         const std::shared_ptr<DependencyInjector>& injector) -> Task* {
        return new PiecewiseJerkSpeedOptimizer(config, injector);
      });
  task_factory_.Register(
      TaskConfig::SPEED_HEURISTIC_OPTIMIZER,
      [](const TaskConfig& config,
         const std::shared_ptr<DependencyInjector>& injector) -> Task* {
        return new PathTimeHeuristicOptimizer(config, injector);
      });
...
void TaskFactory::Init(const PlanningConfig& config,
  ...
  ///////////////////////////
  // deciders
  InitDecidersPartOne(config, injector);
  InitDecidersPartTwo(config, injector);

  ///////////////////////////
  // optimizers
  InitOptimizers(config, injector);
}
