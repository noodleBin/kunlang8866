...
ComparableCost TrajectoryCost::CalculatePathCost(
  ...
  if (curr_level == total_level) {
    ...
    path_cost +=
        std::sqrt(end_l - init_sl_point_.l() * 0.5) * config_.path_end_l_cost();
  }
...
bool TrajectoryCost::IsOffRoad(const double ref_s, const double l,
  ...
  if (rear_center.y() + r + buffer * 0.5 > left_bound ||
      rear_center.y() - r - buffer * 0.5 < right_bound) {
    return true;
  }
  if (front_center.y() + r + buffer * 0.5 > left_bound ||
      front_center.y() - r - buffer * 0.5 < right_bound) {
    return true;
  }
