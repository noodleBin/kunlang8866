 ...
 public:
  virtual void SetUp() {
    ...
    FLAGS_scenario_lane_follow_config_file =
        "/century/modules/planning/conf/scenario/lane_follow_config.pb.txt";
...
TEST_F(DpStGraphTest, simple) {
  ...
  path_decision.AddObstacle(obstacle_list_.back());
  ...
  GriddedPathTimeGraph dp_st_graph(st_graph_data_, dp_config_, path_decision,
                                   init_point_, injector);

  SpeedData speed_data;
  auto ret = dp_st_graph.Search(false, &speed_data);
  EXPECT_TRUE(ret.ok());
}
