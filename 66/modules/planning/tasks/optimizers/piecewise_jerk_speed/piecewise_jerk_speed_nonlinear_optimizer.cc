...
Status PiecewiseJerkSpeedNonlinearOptimizer::CheckData(
...
Status PiecewiseJerkSpeedNonlinearOptimizer::Process(
  ...
  const auto& check_data_status = CheckData(path_data, speed_data);
  if (!check_data_status.ok()) {
...
Status PiecewiseJerkSpeedNonlinearOptimizer::SetUpStatesAndBounds(
  ...
  if (FLAGS_use_soft_bound_in_nonlinear_speed_opt) {
    const auto& status = UseSoftBound(speed_data);
    if (!status.ok()) {
      return status;
    }
  } else {
...
Status PiecewiseJerkSpeedNonlinearOptimizer::SmoothSpeedLimit() {
  ...
  Status ret = piecewise_jerk_problem.Optimize(4000);
  if (!ret.ok()) {
...
Status PiecewiseJerkSpeedNonlinearOptimizer::SmoothPathCurvature(
  ...
  Status ret = piecewise_jerk_problem.Optimize(1000);
  if (!ret.ok()) {
...
Status PiecewiseJerkSpeedNonlinearOptimizer::OptimizeByQP(
  ...
  if (FLAGS_enable_acc_start) {
    ...
    if (reference_line.XYToSL(injector_->last_signal_overlap_end_xy_,
      ...
      if (s_dot_init_ < kMaxVelocitySatrt && s_ddot_init_ > 0.0 &&
        ...
        ADEBUG << "high start for signal overlap .";
        piecewise_jerk_problem.set_weight_ddx(
            config.acc_weight_for_trafficlight());
        piecewise_jerk_problem.set_weight_dddx(
            config.jerk_weight_for_trafficlight());
      }
    }
  }
  ...
  // Solve the problem
  Status ret = piecewise_jerk_problem.Optimize();
  if (!ret.ok()) {
    const std::string msg =
...
Status PiecewiseJerkSpeedNonlinearOptimizer::GetWarmStart(
...
Status PiecewiseJerkSpeedNonlinearOptimizer::OptimizeByNLP(
  ...
  if (config.use_warm_start()) {
    std::vector<std::vector<double>>* warm_start;
    GetWarmStart(distance, velocity, acceleration, warm_start);
  ...
  if (status == Ipopt::Solve_Succeeded ||
    ...
    AERROR << "Piecewise jerk speed nonlinear optimizer failed";
...
Status PiecewiseJerkSpeedNonlinearOptimizer::UseSoftBound(
