...
OpenSpaceTrajectoryOptimizer::OpenSpaceTrajectoryOptimizer(
  ...
  warm_start_.reset(
      new HybridAStarBidirectional(config.planner_open_space_config()));
  ...
  planned_end_pose_.clear();
}

// wentao liu : if you want add different param for astar plan and optimizer,
// you can add a proto or struct instead "****bool use_pure_astar")
Status OpenSpaceTrajectoryOptimizer::Plan(
    ...
    double* time_latency, bool use_pure_astar,
  ...
  if (XYbounds.empty() || end_pose.empty() || obstacles_edges_num.cols() == 0 ||
    ...
    AERROR << "OpenSpaceTrajectoryOptimizer input data not ready";
  ...
  if (IsInitPointNearDestination(stitching_trajectory.back(), end_pose,
    ...
    AERROR << "Plannint init point is close to destination, skip now";
  ...
  // (init_x, init_y, init_phi)  world ---> origin
  ...
  if (warm_start_->BidirectionalPlan(
          world_obstacles_vertices_vec, init_x, init_y, init_phi, end_pose[0],
          end_pose[1], end_pose[2], rotate_angle, translate_origin, XYbounds,
          obs, obstacles_vertices_vec, &result, nearby_path, use_pure_astar)) {
...

  {
    for (size_t i = 0; i < result.x.size(); ++i) {
      // origin ---> world
      Vec2d point = Vec2d(result.x.at(i), result.y.at(i));
      point.SelfRotate(rotate_angle);
      point += translate_origin;
      double point_heading = NormalizeAngle(result.phi.at(i) + rotate_angle_);
      ADEBUG << "warm_start, i: " << i << ", x: " << point.x()
            << ", y: " << point.y()
            << ", angle: " << point_heading * 180 / M_PI;
    }
  }

  // Containers for distance approach trajectory smoothing problem
  Eigen::MatrixXd xWS;  Eigen::MatrixXd uWS;  Eigen::MatrixXd state_result_ds;
  Eigen::MatrixXd control_result_ds;  Eigen::MatrixXd time_result_ds;
  Eigen::MatrixXd l_warm_up;  Eigen::MatrixXd n_warm_up;
  Eigen::MatrixXd dual_l_result_ds;  Eigen::MatrixXd dual_n_result_ds;

  // TODO(zhiqiang.ding): multiple threads
  FLAGS_enable_parallel_trajectory_smoothing = true;
  if (FLAGS_enable_parallel_trajectory_smoothing) {
    // deal with each part.
    auto status = TrajectorySmoother(
        result, trajectory_stitching_point, obstacles_vertices_vec,
        obstacles_edges_num, obstacles_A, obstacles_b, XYbounds, &xWS, &uWS,
        &state_result_ds, &control_result_ds, &time_result_ds, &l_warm_up,
        &n_warm_up, &dual_l_result_ds, &dual_n_result_ds);
    AINFO << "Trajectory smoothing result: " << status;
    if (Status::OK() != status) {
      return status;
    }
  } else {
  ...
  // get optimized_trajectory_
  LoadTrajectory(state_result_ds, control_result_ds, time_result_ds);

  // update planned_end_pose_
  if (nullptr == nearby_path || optimized_trajectory_.empty()) {
    planned_end_pose_ = end_pose;
  } else {
    // world ---> origin
    Vec2d point = common::util::PointFactory::ToVec2d(
        optimized_trajectory_.back().path_point());
    point -= translate_origin;
    point.SelfRotate(-rotate_angle);
    planned_end_pose_.emplace_back(point.x());
    planned_end_pose_.emplace_back(point.y());
    planned_end_pose_.emplace_back(common::math::NormalizeAngle(
        optimized_trajectory_.back().path_point().theta() - rotate_angle));
    planned_end_pose_.emplace_back(optimized_trajectory_.back().v());
  }

  const auto end_timestamp = std::chrono::system_clock::now();
  std::chrono::duration<double> diff = end_timestamp - start_timestamp;
  AINFO << "open space trajectory smoother total time: "
...
common::Status OpenSpaceTrajectoryOptimizer::TrajectorySmoother(
  ...
  if (!warm_start_->TrajectoryPartition(result, &partition_trajectories)) {
    AERROR << "smoother partition failed, return error.";
    return Status(ErrorCode::PLANNING_ERROR, "Hybrid Astar partition failed");
  }
  size_t size = partition_trajectories.size();
  std::vector<Eigen::MatrixXd> xWS_vec(size);
  std::vector<Eigen::MatrixXd> uWS_vec(size);
  std::vector<Eigen::MatrixXd> state_result_ds_vec(size);
  std::vector<Eigen::MatrixXd> control_result_ds_vec(size);
  std::vector<Eigen::MatrixXd> time_result_ds_vec(size);
  std::vector<Eigen::MatrixXd> l_warm_up_vec(size);
  std::vector<Eigen::MatrixXd> n_warm_up_vec(size);
  std::vector<Eigen::MatrixXd> dual_l_result_ds_vec(size);
  std::vector<Eigen::MatrixXd> dual_n_result_ds_vec(size);

  ADEBUG << "Trajectories size in smoother is " << size;
  for (size_t i = 0; i < size; ++i) {
    ADEBUG << "trajectory segment " << i;
    // HybridAStartResult ---> std::vector<Eigen::MatrixXd>
    LoadHybridAstarResultInEigen(&partition_trajectories[i], &xWS_vec[i],
                                 &uWS_vec[i], &time_result_ds_vec[i]);
    Eigen::MatrixXd last_time_u(2, 1);
    double init_v = 0.0;
    ...
    FLAGS_use_iterative_anchoring_smoother = true;
  ...
  CombineTrajectories(
      xWS_vec, uWS_vec, state_result_ds_vec, control_result_ds_vec,
...
void OpenSpaceTrajectoryOptimizer::RecordDebugInfo(
  ...
  a_star_search_info_ptr->mutable_trajectory_stitching_point()->CopyFrom(
  ...
  for (size_t i = 0; i < horizon; ++i) {
    for (size_t j = 0; j < l_warm_up_rows; j++) {
      a_star_optimization_info_ptr->add_warm_start_dual_lambda(
          l_warm_up(j, i));
    }
  }
  ...
  for (size_t i = 0; i < horizon; ++i) {
    for (size_t j = 0; j < n_warm_up_rows; j++) {
      a_star_optimization_info_ptr->add_warm_start_dual_miu(
          n_warm_up(j, i));
    }
  }
  ...
  for (size_t i = 0; i < horizon; ++i) {
    for (size_t j = 0; j < dual_l_result_ds_rows; j++) {
      a_star_optimization_info_ptr->add_optimized_dual_lambda(
          dual_l_result_ds(j, i));
    }
  }
  size_t dual_n_result_ds_rows = dual_n_result_ds.rows();
  for (size_t i = 0; i < horizon; ++i) {
    for (size_t j = 0; j < dual_n_result_ds_rows; j++) {
      a_star_optimization_info_ptr->add_optimized_dual_miu(
          dual_n_result_ds(j, i));
    }
  }
...
// check whether the planning_init_point is close to the end_pose
...
void OpenSpaceTrajectoryOptimizer::LoadTrajectory(
  ...
  for (size_t i = 0; i < states_size; ++i) {
    ...
    if (i == controls_size) {
      ...
      point.set_a((std::fabs(state_result(3, i + 1)) - std::fabs(state_result(3, i))) /
                  time_result(0, i));
      double delta_s = std::hypot(state_result(0, i + 1) - state_result(0, i),
                                  state_result(1, i + 1) - state_result(1, i));
      double delta_theta = common::math::NormalizeAngle(
          state_result(2, i + 1) - state_result(2, i)) + common::math::kMathEpsilon;
      point.mutable_path_point()->set_kappa(delta_s / delta_theta);
    }
...
void OpenSpaceTrajectoryOptimizer::UseWarmStartAsResult(
  ...
  // set state_result_ds, control_result_ds
  *state_result_ds = xWS;
  *control_result_ds = uWS;
  // set dual_l_result_ds, dual_n_result_ds
...
bool OpenSpaceTrajectoryOptimizer::GenerateDistanceApproachTraj(
  ...
  const auto dual_start_timestamp = std::chrono::system_clock::now();
  ...
  const auto dual_end_timestamp = std::chrono::system_clock::now();
  std::chrono::duration<double> smoother_diff =
      dual_end_timestamp - dual_start_timestamp;
  ...
  if (distance_approach_->Solve(
    ...
    if (FLAGS_enable_smoother_failsafe) {
      // state_result_ds = xWS   control_resuld_ds = uWS, time_result_ds
...
void OpenSpaceTrajectoryOptimizer::LoadHybridAstarResultInEigen(
  ...
  size_t horizon = result->x.size() - 1;    // result max index
  xWS->resize(4, horizon + 1);
  uWS->resize(2, horizon);
  time_result_ds->resize(1, horizon + 1);
  ...
  Eigen::VectorXd t = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(
      result->t.data(), horizon + 1);
  ...
  time_result_ds->row(0) = std::move(t);
...
bool OpenSpaceTrajectoryOptimizer::GenerateDecoupledTraj(
  ...
  if (!iterative_anchoring_smoother_->Smooth(
    ...
    AERROR << "optimize failed, return false.";
    return false;
  }

  // DiscretizedTrajectory ---> Eigen::MatrixXd
