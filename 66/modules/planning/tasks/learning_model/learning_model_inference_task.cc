...
Status LearningModelInferenceTask::Process(Frame* frame) {
  ...
  std::string msg = EvaluateTrajectory(&learning_data_frame);
  if (msg.empty()) {
    return Status::OK();
  } else {
    AERROR << msg;
...
void LearningModelInferenceTask::ConvertADCFutureTrajectory(
  ...
  for (const auto& trajectory_point_feature : trajectory) {
    ...
    century::common::TrajectoryPoint trajectory_point;
    ...
    adc_future_trajectory->emplace_back(trajectory_point);
  }
  ...
  SetADCFutureTrajectoryLinearAccAndJerk(adc_future_trajectory);
  SetADCFutureTrajectoryAngularAccAndJerk(adc_future_trajectory);

}

void LearningModelInferenceTask::SetADCFutureTrajectoryLinearAccAndJerk(
      std::vector<common::TrajectoryPoint> *adc_future_trajectory) {
  for (size_t i = 0; i + 1 < adc_future_trajectory->size(); ++i) {
    ...
    CHECK_NE(next_relative_time, cur_relative_time);
    const double cur_a =
...
void LearningModelInferenceTask::SetADCFutureTrajectoryAngularAccAndJerk(
      std::vector<common::TrajectoryPoint> *adc_future_trajectory) {
  for (size_t i = 0; i + 1 < adc_future_trajectory->size(); ++i) {
    auto* cur_path_point = (*adc_future_trajectory)[i].mutable_path_point();
    const auto& next_path_point = (*adc_future_trajectory)[i + 1].path_point();
    const double cur_kappa = std::numeric_limits<double>::max();
    if (common::util::IsFloatEqual(next_path_point.s(), cur_path_point->s())) {
      AERROR << "next_path_point.s() and cur_path_point->s() equals!";
      cur_kappa = century::common::math::NormalizeAngle(next_path_point.theta() -
...
std::string LearningModelInferenceTask::EvaluateTrajectory(
  ...
  for (const auto& tp :
    ...
    future_trajectory.emplace_back(tp);
  }
