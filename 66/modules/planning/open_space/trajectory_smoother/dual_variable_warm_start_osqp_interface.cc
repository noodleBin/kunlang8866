...
void DualVariableWarmStartOSQPInterface::assemble_lb_ub(const int num,
                                                        c_float* lb,
                                                        c_float* ub) {
  // assemble lb & ub
  for (int i = 0; i < num; ++i) {
...
void DualVariableWarmStartOSQPInterface::SolverSetting(
...
bool DualVariableWarmStartOSQPInterface::optimize() {
  ...
  assemble_lb_ub(kNumConst, lb, ub);

  // Problem settings
  OSQPSettings* settings =
      reinterpret_cast<OSQPSettings*>(c_malloc(sizeof(OSQPSettings)));
  SolverSetting(settings);
...
int DualVariableWarmStartOSQPInterface::assemble_constraint_lambda_variables(
  ...
  return first_row_location;
}

void DualVariableWarmStartOSQPInterface::assemble_constraint_miu_variables(
    int first_row_location, std::vector<c_float>* A_data,
...
void DualVariableWarmStartOSQPInterface::assemble_constraint(
  ...
  int first_row_location = 0;
  first_row_location =
      assemble_constraint_lambda_variables(A_data, A_indices, A_indptr);
  assemble_constraint_miu_variables(first_row_location, A_data, A_indices,
                                    A_indptr);
