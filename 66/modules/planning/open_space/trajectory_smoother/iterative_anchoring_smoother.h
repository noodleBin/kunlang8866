...
class IterativeAnchoringSmoother {
 ...
 private:
  void ConvertObstacleToLineSegments(
      const std::vector<std::vector<common::math::Vec2d>>&
          obstacles_vertices_vec);
  ...
  // check trajectory gear position, forward is true, backward is false
  ...
  void GenerateSimpleSpeed(const double init_a, const double init_v,
