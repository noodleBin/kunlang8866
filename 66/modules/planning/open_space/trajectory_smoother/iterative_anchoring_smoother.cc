...
namespace {
constexpr double kInterpLength = 0.3;
constexpr int kMinSize = 4;
constexpr int kSixSize = 6;
...
IterativeAnchoringSmoother::IterativeAnchoringSmoother(
  ...
  center_shift_distance_ =
      ego_length_ * 0.5 - vehicle_param.back_edge_to_center();
  planner_open_space_config_ = planner_open_space_config;
}

// std::vector<std::vector<common::math::Vec2d>> --->
// std::vector<std::vector<common::math::LineSegment2d>>
void IterativeAnchoringSmoother::ConvertObstacleToLineSegments(
...
// interpolate
void IterativeAnchoringSmoother::InterpolateTrajectory(
    const Eigen::MatrixXd& xWS,
    std::vector<std::pair<double, double>>* const point2ds) {
  ...
  // Eigen::MatrixXd ---> DiscretizedPath
  ...
  if (common::util::IsFloatEqual(path_length, 0.0)) {
    AERROR << "the path_length is zero!";
    path_length = kEpsilon;
  }
  // step size
  double interpolated_delta_s =
      (path_length > kMinSize * kInterpLength)
          ? kInterpLength
  ...
  for (double s = 0; s < path_length; s += delta_s) {
    ...
    point2ds->emplace_back(point2d.x(), point2d.y());
  }
}

// smooth trajectory segment
bool IterativeAnchoringSmoother::Smooth(
  ...
  // trajectory is less than 2 points, return false
  if (xWS.cols() < 2) {
  ...
  // Set obstacle in form of linesegments
  ConvertObstacleToLineSegments(obstacles_vertices_vec);

  // Interpolate the traj
  std::vector<std::pair<double, double>> interpolated_warm_start_point2ds;
  InterpolateTrajectory(xWS, &interpolated_warm_start_point2ds);

  // trajectory points size
  const size_t interpolated_size = interpolated_warm_start_point2ds.size();
  AINFO << "interpolated_size " << interpolated_size;
  if (interpolated_size < kMinSize) {
    AERROR << "interpolated_warm_start_path smaller than 4, can't enforce "
              "heading continuity";
    return false;
  } else if (interpolated_size < kSixSize) {
    ADEBUG << "interpolated_warm_start_path smaller than 6, can't enforce "
              "initial zero kappa";
  ...
  // std::vector<std::pair<double, double>> ---> DiscretizedPath
  ...
  if (!SmoothPath(interpolated_warm_start_path, bounds,
    ...
    AERROR << "SmoothPath failed , use raw traj";
    std::vector<std::pair<double, double>> raw_point2d;

    for (const auto& path_point : interpolated_warm_start_path) {
      raw_point2d.emplace_back(path_point.x(), path_point.y());
    }

    if (!SetPathProfile(raw_point2d, &smoothed_path_points)) {
      AERROR << "Set raw path profile fails";
      return false;
    }
    use_raw_traj_ = true;
  } else {
  ...
  if (!SmoothSpeed(init_a, init_v, smoothed_path_points.Length(),
                   &smoothed_speeds)) {
    GenerateSimpleSpeed(init_a, init_v, smoothed_path_points.Length(),
                        &smoothed_speeds);
  }
...
bool IterativeAnchoringSmoother::SetPathProfile(
  ...
  // compute headings, accumulated_s, kappas, dkappas
...
bool IterativeAnchoringSmoother::CombinePathAndSpeed(
  ...
  common::SpeedPoint speed_point;
  speed_points.EvaluateByTime(speed_points.TotalTime(), &speed_point);
  if (speed_point.s() < path_points.Length() && !discretized_trajectory->empty()) {
    common::PathPoint path_point = path_points.Evaluate(path_points.Length());
    common::TrajectoryPoint trajectory_point;
    trajectory_point.mutable_path_point()->CopyFrom(path_point);
    trajectory_point.set_v(0.0);
    trajectory_point.set_a(0.0);
    double relative_time = std::fmin(std::fabs(discretized_trajectory->back().v() /
                                               discretized_trajectory->back().a()), 0.1);
    trajectory_point.set_relative_time(
        discretized_trajectory->back().relative_time() + relative_time);
    discretized_trajectory->AppendTrajectoryPoint(trajectory_point);
  }
...
bool IterativeAnchoringSmoother::GenerateStopProfileFromPolynomial(
  ...
  for (double t = 3.0; t <= kMaxT; t += kUnitT) {
    ...
    for (double curve_t = 0.0; curve_t <= t; curve_t += kUnitT) {
      ...
      AINFO << "curve_s " << curve_s << " curve_v " << curve_v << "curve_a"
            << curve_a;
...
// generate speed and acceleration
void IterativeAnchoringSmoother::GenerateSimpleSpeed(
    const double init_a, const double init_v,
    const double traj_length, SpeedData* smoothed_speeds) {

  double target_v = gear_ ?
      planner_open_space_config_.iterative_anchoring_smoother_config()
          .max_forward_v() :
      planner_open_space_config_.iterative_anchoring_smoother_config()
          .max_reverse_v();
  double target_a = gear_ ?
  ...
  // init_v --> target_v  s
  double acc_s = (target_v * target_v - init_v * init_v) / (2.0 * target_a);
  // init_v --> 0  s
  double dec_s = init_v * init_v / (2.0 * target_a);
  // target_v --> 0  s
  double reverse_s = target_v * target_v / (2.0 * target_a);

  if (traj_length < dec_s) {        // dec
    double a = init_v * init_v / (2.0 * traj_length);
    smoothed_speeds->AppendSpeedPoint(0.0, 0.0, init_v, -a, 0.0);
  } else {
    if (acc_s + reverse_s < traj_length) {   // acc, uniform, dec
      double acc_t = (target_v - init_v) / target_a;
      smoothed_speeds->AppendSpeedPoint(0.0, 0.0, init_v, target_a, 0.0);
      double uniform_t = (traj_length - acc_s - reverse_s) / target_v;
      smoothed_speeds->AppendSpeedPoint(acc_s, acc_t, target_v, 0.0, 0.0);
      smoothed_speeds->AppendSpeedPoint(traj_length - reverse_s, acc_t + uniform_t,
                                        target_v, -target_a, 0.0);
    } else {                                 // acc, dec
      double middle_v = sqrt((2.0 * traj_length * target_a + init_v * init_v) / 2.0);
      if (middle_v > init_v) {               // dec
        double acc_t = (middle_v - init_v) / target_a;
        smoothed_speeds->AppendSpeedPoint(0.0, 0.0, init_v, target_a, 0.0);
        double acc_s = (middle_v * middle_v - init_v * init_v) / (2.0 * target_a);
        smoothed_speeds->AppendSpeedPoint(acc_s, acc_t, middle_v, -target_a, 0.0);
      } else {                               // acc, dec
        double a = init_v * init_v / (2.0 * traj_length);
        smoothed_speeds->AppendSpeedPoint(0.0, 0.0, init_v, -a, 0.0);
      }
