...
class DualVariableWarmStartOSQPInterface {
 ...
 private:
  ...
  int assemble_constraint_lambda_variables(std::vector<c_float>* A_data,
                                           std::vector<c_int>* A_indices,
                                           std::vector<c_int>* A_indptr);

  void assemble_constraint_miu_variables(int first_row_location,
  ...
  void assemble_lb_ub(const int num, c_float* lb, c_float* ub);

  void SolverSetting(OSQPSettings* const settings);
