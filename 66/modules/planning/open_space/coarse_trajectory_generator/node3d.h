...
class Node3d {
 public:
  ...
  static century::common::math::Box2d GetBoundingBox(
      const common::VehicleParam& vehicle_param_, const double x,
      const double y, const double phi);
  static century::common::math::Box2d GetBoundingBoxWithBuffer(
      const common::VehicleParam& vehicle_param_, const double x,
      const double y, const double phi, const double extra_length,
      const double extra_width);
  static century::common::math::Box2d GetObsCostBoxWithBuffer(
      const common::VehicleParam& vehicle_param_, const double x,
      const double y, const double phi, const double extra_length,
      const double extra_width);
  double DistanceTo(const Node3d& other) const;
  double DistanceSquareTo(const Node3d& other) const;
  ...
  uint64_t GetIntIndex() const { return int_index_; }
  ...
  void SetTraversed(const std::vector<double>& xs,
 ...
 private:
  static uint64_t CalculateIndex(int x_grid, int y_grid, int phi_grid);
