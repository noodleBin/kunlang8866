...
namespace routing {
namespace {
constexpr double kFixLaneChangeCost = 50.0;
...
double AStarStrategy::GetCostToNeighbor(const TopoEdge* edge) {
  if (is_need_lanechange_first_) {
    return (edge->ToNode()->Cost());
  }
  return (edge->Cost() + edge->ToNode()->Cost());
  // return (kFixLaneChangeCost + edge->ToNode()->Cost());
}
...
void AStarStrategy::InitializeSearch(
...
void AStarStrategy::ProcessNode(
...
void AStarStrategy::UpdateNode(
    const TopoNode* from_node, const TopoNode* to_node, const TopoEdge* edge,
    double tentative_g_score, const TopoNode* dest_node,
    std::priority_queue<SearchNode>* open_set_detail) {
  double f = tentative_g_score + HeuristicCost(to_node, dest_node);
  // double f = tentative_g_score;
  // AINFO<<"HeuristicCost(to_node, dest_node) = "<<HeuristicCost(to_node,
  // dest_node);
  // AINFO<<"f = "<<f;
...
bool AStarStrategy::Search(const TopoGraph* graph,
  ...
  is_need_lanechange_first_ = is_need_lanechange_first;
  std::priority_queue<SearchNode> open_set_detail;
  InitializeSearch(src_node, dest_node, &open_set_detail);

  SearchNode current_node;
  std::unordered_set<const TopoEdge*> next_edge_set;
  std::unordered_set<const TopoEdge*> sub_edge_set;
  while (!open_set_detail.empty()) {
    ProcessNode(&current_node, &open_set_detail);
    ...
    for (const auto* edge : next_edge_set) {
      ...
      // AINFO << "g_score_[current_node.topo_node]:"
      //       << g_score_[current_node.topo_node]
      //       << ",g_score_[current_node.topo_node]"
      //       << g_score_[current_node.topo_node]
      //       << ",is_need_lanechange_first_:" << is_need_lanechange_first_
      //       << "edge->FromNode()->Cost():" << edge->FromNode()->Cost()
      //       << ",edge->ToNode()->Cost():" << edge->ToNode()->Cost();
      // AINFO << "current_node:" << current_node.topo_node->LaneId()
      //       << "-->to_node:" << to_node->LaneId()
      //       << ",FromNode Cost:" << edge->FromNode()->Cost()
      //       << ",ToNode Cost:" << edge->ToNode()->Cost()
      //       << ",Cost():" << edge->Cost() << ",Type():" << edge->Type()
      //       << ",tentative_g_score = " << tentative_g_score;
      UpdateNode(from_node, to_node, edge, tentative_g_score, dest_node,
                 &open_set_detail);
    }
  }
