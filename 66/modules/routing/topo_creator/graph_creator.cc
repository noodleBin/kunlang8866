...
bool GraphCreator::LongFunWithCreate() {
...
bool GraphCreator::Create() {
  ...
  for (const auto& lane : pbmap_.lane()) {
    ...
    if (lane.turn() == hdmap::Lane::U_TURN &&
      ...
      // ADEBUG << "The lane radius is too small for the vehicle to turn";
      continue;
    }
...
void GraphCreator::InitForbiddenLanes() {
  for (const auto& lane : pbmap_.lane()) {
    if (hdmap::Lane::CITY_DRIVING != lane.type() &&
        hdmap::Lane::PLAY_STREET != lane.type()) {
      forbidden_lane_id_set_.insert(lane.id().id());
    }

    if (lane.has_is_blacklist() && lane.is_blacklist()) {
      forbidden_lane_id_set_.insert(lane.id().id());
    }
  }
}
