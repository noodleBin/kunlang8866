...
void LatencyRecorder::AppendLatencyRecord(const uint64_t message_id,
  ...
  const century::cyber::Duration kPublishInterval(3.0);
...
std::shared_ptr<century::cyber::Writer<LatencyRecordMap>>
LatencyRecorder::CreateWriter() {
  ...
  if (node_ == nullptr) {
    ...
    node_ = century::cyber::CreateNode(absl::StrCat(
...
void LatencyRecorder::PublishLatencyRecords(
  ...
  century::common::util::FillHeader("LatencyRecorderMap", records_.get());
