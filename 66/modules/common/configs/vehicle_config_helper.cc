...
common::math::Box2d VehicleConfigHelper::GetBoundingBox(
    const common::PathPoint &path_point, const double lat_buffer) {
  ...
  return common::math::Box2d(true_center, path_point.theta(),
                             ...
                             vehicle_param.width() + lat_buffer * 2.0);
}

common::math::Box2d VehicleConfigHelper::GetBoundingBox(
    const double x, const double y, const double heading,
    const double long_buffer, const double lat_buffer) {
  const auto &vehicle_param = vehicle_config_.vehicle_param();
  double diff_truecenter_and_pointX = (vehicle_param.front_edge_to_center() -
                                       vehicle_param.back_edge_to_center()) /
                                      2.0;
  common::math::Vec2d true_center(
      x + diff_truecenter_and_pointX * std::cos(heading),
      y + diff_truecenter_and_pointX * std::sin(heading));

  return common::math::Box2d(true_center, heading,
                             vehicle_param.length() + long_buffer * 2.0,
                             vehicle_param.width() + lat_buffer * 2.0);
}

common::math::Box2d VehicleConfigHelper::GetBoundingBox(
    const double x, const double y, const double heading,
    const double long_buffer, const double long_offset,
    const double lat_buffer) {
  const auto &vehicle_param = vehicle_config_.vehicle_param();

  common::math::Vec2d true_center(
      x + long_offset * std::cos(heading),
      y + long_offset * std::sin(heading));
