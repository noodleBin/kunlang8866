...
// Area < kMathEpsilon will cause core dump,
// then need filter these polygon
// return is invalid obj
...
void Polygon2d::InsertPoint(const Vec2d point) { points_.push_back(point); }

const std::vector<Vec2d> Polygon2d::GetAllPoints() const {
  std::vector<Vec2d> points;

  // Obtain the minimum bounding rectangle of the polygon to reduce the number
  // of points that need to be checked.
  const Box2d &box = MinAreaBoundingBox();
  double xmin = std::ceil(box.min_x());
  double xmax = std::floor(box.max_x());
  double ymin = std::ceil(box.min_y());
  double ymax = std::floor(box.max_y());

  for (double x = xmin; x <= xmax; x += 0.3) {
    for (double y = ymin; y <= ymax; y += 0.3) {
      Vec2d point(x, y);
      // If the point is within the polygon, add it to the result set.
      if (IsPointIn(point)) {
...
bool Polygon2d::ComputeConvexHull(const std::vector<Vec2d> &points,
  ...
  // we need filter little object
  if (CheckTinyArea(result_points)) {
    return false;
  }
...
Polygon2d Polygon2d::ExpandByDistance(const double distance,
                                      Vec2d center_pos) const {
  std::vector<Vec2d> points;
  for (const auto &point : points_) {
    double theta =
        std::atan2(point.y() - center_pos.y(), point.x() - center_pos.x());
    points.emplace_back(point + Vec2d::CreateUnitVec2d(theta) * distance);
  }
  Polygon2d new_polygon;
  ACHECK(ComputeConvexHull(points, &new_polygon));
  return new_polygon;
