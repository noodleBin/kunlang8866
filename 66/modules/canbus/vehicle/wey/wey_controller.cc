...
using ::century::drivers::canbus::ProtocolData;
...
ErrorCode WeyController::Init(
    ...
    CanSender<::century::canbus::ChassisDetail>* const can_sender,
    MessageManager<::century::canbus::ChassisDetail>* const message_manager) {
  if (is_initialized_) {
  ...
  if (nullptr == can_sender) {
    AERROR << "Canbus sender is null.";
    return ErrorCode::CANBUS_ERROR;
  }
  can_sender_ = can_sender;

  if (nullptr == message_manager) {
...
Chassis WeyController::chassis() {
  ...
  if (Chassis::EMERGENCY_MODE == driving_mode()) {
  ...
  if (wey.has_fbs4_235() && wey.fbs4_235().has_steerwheelangle() &&
    ...
    if (Status_310::STEERWHEELANGLESIGN_LEFT_POSITIVE ==
        wey.status_310().steerwheelanglesign()) {
    ...
    } else if (Status_310::STEERWHEELANGLESIGN_RIGHT_NEGATIVE ==
               wey.status_310().steerwheelanglesign()) {
      chassis_.set_steering_percentage(
  ...
  if (wey.has_status_310() && wey.status_310().has_epbsts()) {
    chassis_.set_parking_brake(Status_310::EPBSTS_CLOSED ==
                               wey.status_310().epbsts());
  } else {
    chassis_.set_parking_brake(false);
  }
  // 14, 15
  if (wey.has_status_310() && wey.status_310().has_lowbeamsts() &&
      Status_310::LOWBEAMSTS_ON == wey.status_310().lowbeamsts()) {
  ...
  if (wey.has_status_310()) {
    if (wey.status_310().has_leftturnlampsts() &&
        Status_310::LEFTTURNLAMPSTS_ON == wey.status_310().leftturnlampsts()) {
...
void WeyController::SetWheelSpeed(const Wey& wey) {
  if (wey.has_fbs2_240() && wey.fbs2_240().has_vehiclespd() &&
    ...
    if (Fbs2_240::RRWHEELDIRECTION_FORWARD == fbs2_240.rrwheeldirection()) {
      chassis_.mutable_wheel_speed()->set_wheel_direction_rr(
          WheelSpeed::FORWARD);
    } else if (Fbs2_240::RRWHEELDIRECTION_BACKWARD ==
               fbs2_240.rrwheeldirection()) {
      chassis_.mutable_wheel_speed()->set_wheel_direction_rr(
          WheelSpeed::BACKWARD);
    } else if (Fbs2_240::RRWHEELDIRECTION_STOP == fbs2_240.rrwheeldirection()) {
      chassis_.mutable_wheel_speed()->set_wheel_direction_rr(
    ...
    if (Fbs2_240::RLWHEELDRIVEDIRECTION_FORWARD ==
        fbs2_240.rlwheeldrivedirection()) {
      chassis_.mutable_wheel_speed()->set_wheel_direction_rl(
          WheelSpeed::FORWARD);
    } else if (Fbs2_240::RLWHEELDRIVEDIRECTION_BACKWARD ==
               fbs2_240.rlwheeldrivedirection()) {
      chassis_.mutable_wheel_speed()->set_wheel_direction_rl(
          WheelSpeed::BACKWARD);
    } else if (Fbs2_240::RLWHEELDRIVEDIRECTION_STOP ==
               fbs2_240.rlwheeldrivedirection()) {
      chassis_.mutable_wheel_speed()->set_wheel_direction_rl(
          WheelSpeed::STANDSTILL);
    } else {
    ...
    if (Fbs1_243::FRWHEELDIRECTION_FORWARD == fbs1_243.frwheeldirection()) {
      chassis_.mutable_wheel_speed()->set_wheel_direction_fr(
          WheelSpeed::FORWARD);
    } else if (Fbs1_243::FRWHEELDIRECTION_BACKWARD ==
               fbs1_243.frwheeldirection()) {
      chassis_.mutable_wheel_speed()->set_wheel_direction_fr(
          WheelSpeed::BACKWARD);
    } else if (Fbs1_243::FRWHEELDIRECTION_STOP == fbs1_243.frwheeldirection()) {
      chassis_.mutable_wheel_speed()->set_wheel_direction_fr(
          WheelSpeed::STANDSTILL);
    } else {
      chassis_.mutable_wheel_speed()->set_wheel_direction_fr(
    ...
    if (Fbs2_240::FLWHEELDIRECTION_FORWARD == fbs2_240.flwheeldirection()) {
      chassis_.mutable_wheel_speed()->set_wheel_direction_fl(
          WheelSpeed::FORWARD);
    } else if (Fbs2_240::FLWHEELDIRECTION_BACKWARD ==
               fbs2_240.flwheeldirection()) {
      chassis_.mutable_wheel_speed()->set_wheel_direction_fl(
          WheelSpeed::BACKWARD);
    } else if (Fbs2_240::FLWHEELDIRECTION_STOP == fbs2_240.flwheeldirection()) {
      chassis_.mutable_wheel_speed()->set_wheel_direction_fl(
          WheelSpeed::STANDSTILL);
    } else {
      chassis_.mutable_wheel_speed()->set_wheel_direction_fl(
...
void WeyController::SetVin(const Wey& wey) {
...
ErrorCode WeyController::EnableAutoMode() {
  if (Chassis::COMPLETE_AUTO_DRIVE == driving_mode()) {
...
ErrorCode WeyController::EnableSteeringOnlyMode() {
  if (Chassis::COMPLETE_AUTO_DRIVE == driving_mode() ||
      Chassis::AUTO_STEER_ONLY == driving_mode()) {
...
ErrorCode WeyController::EnableSpeedOnlyMode() {
  if (Chassis::COMPLETE_AUTO_DRIVE == driving_mode() ||
      Chassis::AUTO_SPEED_ONLY == driving_mode()) {
...
void WeyController::SetTurningSignal(const ControlCommand& command) {
  ...
  if (common::VehicleSignal::TURN_LEFT == signal) {
    ads3_38e_->set_turnllighton(Ads3_38e::TURNLLIGHTON_TURN_LEFT_ON);
  } else if (common::VehicleSignal::TURN_RIGHT == signal) {
...
bool WeyController::CheckChassisError() {
  ...
  // check steer error
  if (wey.has_fail_241()) {
    if (Fail_241::EPSFAIL_FAULT == wey.fail_241().epsfail()) {
      return true;
    }
  }
  // check ems error
  if (wey.has_fail_241()) {
    if (Fail_241::ENGFAIL_FAIL == wey.fail_241().engfail()) {
      return true;
    }
  }
  // check braking error
  if (wey.has_fail_241()) {
    if (Fail_241::ESPFAIL_FAILURE == wey.fail_241().espfail()) {
      return true;
    }
  }
  // check gear error question
  if (wey.has_fail_241()) {
    if (Fail_241::SHIFTFAIL_TRANSMISSION_P_ENGAGEMENT_FAULT ==
        wey.fail_241().shiftfail()) {
      return true;
    }
  }
  // check parking error
  if (wey.has_fail_241()) {
    if (Fail_241::EPBFAIL_ERROR == wey.fail_241().epbfail()) {
      return true;
    }
  }
  return false;
...
void WeyController::SecurityDogThreadFunc() {
  ...
  int64_t end = 0;
  while (can_sender_->IsRunning()) {
    start = ::century::cyber::Time::Now().ToMicrosecond();
    const Chassis::DrivingMode mode = driving_mode();
    bool emergency_mode = false;

    // 1. Horizontal control check
    if ((Chassis::COMPLETE_AUTO_DRIVE == mode ||
         Chassis::AUTO_STEER_ONLY == mode) &&
    ...
    end = ::century::cyber::Time::Now().ToMicrosecond();
