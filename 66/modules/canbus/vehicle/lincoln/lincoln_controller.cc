...
Chassis LincolnController::chassis() {
  ...
  if (chassis_detail.has_eps() &&
    ...
    chassis_.set_parking_brake(Epb::PBRAKE_ON ==
                               chassis_detail.epb().parking_brake_status());
  } else {
  ...
  if (chassis_detail.has_light() &&
    ...
    chassis_.mutable_signal()->set_high_beam(
        Light::BEAM_HIGH == chassis_detail.light().lincoln_lamp_type());
  } else {
  ...
  if (chassis_detail.has_light() &&
    ...
    if (Light::TURN_LEFT_ON == chassis_detail.light().turn_light_type()) {
      chassis_.mutable_signal()->set_turn_signal(
          common::VehicleSignal::TURN_LEFT);
    } else if (Light::TURN_RIGHT_ON ==
               chassis_detail.light().turn_light_type()) {
      chassis_.mutable_signal()->set_turn_signal(
  ...
  if (chassis_detail.vehicle_id().has_vin()) {
    ...
    if (!received_vin_) {
      century::common::KVDB::Put("century:canbus:vin",
  ...
  if (chassis_error_mask_ ||
      common::util::IsZero(chassis_.throttle_percentage()) ||
      common::util::IsZero(chassis_.brake_percentage())) {
...
void LincolnController::SetWheelSpeed(
    const canbus::ChassisDetail& chassis_detail) {
...
void LincolnController::SetGps(const canbus::ChassisDetail& chassis_detail) {
...
void LincolnController::Emergency() {
  ...
  ResetProtocol();
  if (Chassis::NO_ERROR == chassis_error_code()) {
...
void LincolnController::Gear(Chassis::GearPosition ref_gear_position) {
  ...
  } else if (Chassis::GEAR_NEUTRAL == current_gear_position) {
...
void LincolnController::Steer(double angle, double angle_spd) {
  ...
  const double real_angle_spd =
      ProtocolData<::century::canbus::ChassisDetail>::BoundedValue(
...
bool LincolnController::CheckSafetyError(
    const ::century::canbus::ChassisDetail &chassis_detail) {
  bool safety_error =
