...
using century::prediction::math_util::ComputePolynomial;
using century::prediction::math_util::EvaluateCubicPolynomial;
...
void JunctionMLPEvaluator::SetEgoVehicleFeatureValues(
  ...
  *feature_values = std::vector<double>(4, 0.0);
  ...
  century::common::math::Vec2d ego_relative_position(
      ego_position.x() - obstacle_feature.position().x(),
      ego_position.y() - obstacle_feature.position().y());
  century::common::math::Vec2d ego_relative_velocity(ego_velocity.x(),
                                                    ego_velocity.y());
...
void JunctionMLPEvaluator::SetJunctionFeatureValues(
  ...
  for (int i = 0; i < num_junction_exit; ++i) {
    ...
    double diff_heading =
        century::common::math::AngleDiff(heading, junction_exit.exit_heading());
