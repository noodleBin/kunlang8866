...
bool VectorNet::offline_query(const double obstacle_x,
  ...
  century::prediction::VectorNetFeature vector_net_pb_;
...
void VectorNet::GetRoads(const common::PointENU& center_point,
                         ...
                         FeatureVector* const feature_ptr,
                         PidVector* const p_id_ptr) {
  std::vector<century::hdmap::RoadInfoConstPtr> roads;
  century::hdmap::HDMapUtil::BaseMap().GetRoads(center_point,
...
void VectorNet::GetLaneQueue(
  ...
  while (!lane_set.empty()) {
    std::deque<century::hdmap::LaneInfoConstPtr> one_lane_deque;
    ...
    while (cur_lane->lane().successor_id_size() > 0) {
      ...
      cur_lane = century::hdmap::HDMapUtil::BaseMap().GetLaneById(id);
...
void VectorNet::GetLanes(const common::PointENU& center_point,
                         ...
                         FeatureVector* const feature_ptr,
                         PidVector* const p_id_ptr) {
  std::vector<century::hdmap::LaneInfoConstPtr> lanes;
  century::hdmap::HDMapUtil::BaseMap().GetLanes(center_point,
                                               FLAGS_road_distance, &lanes);

  std::vector<std::deque<century::hdmap::LaneInfoConstPtr>> lane_deque_vector;
...
void VectorNet::GetJunctions(const common::PointENU& center_point,
                             ...
                             FeatureVector* const feature_ptr,
                             PidVector* const p_id_ptr) {
  std::vector<century::hdmap::JunctionInfoConstPtr> junctions;
  century::hdmap::HDMapUtil::BaseMap().GetJunctions(
...
void VectorNet::GetCrosswalks(const common::PointENU& center_point,
                              ...
                              FeatureVector* const feature_ptr,
                              PidVector* const p_id_ptr) {
  std::vector<century::hdmap::CrosswalkInfoConstPtr> crosswalks;
  century::hdmap::HDMapUtil::BaseMap().GetCrosswalks(
