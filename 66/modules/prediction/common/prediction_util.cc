...
void GenerateFreeMoveTrajectoryPoints(
  ...
  const Eigen::Matrix<double, 8, 8>& transition,  const double start_time, 
  const double total_time, const double period,
  const double yaw_rate_active_perception, std::vector<TrajectoryPoint>* points) {

const double omega_active_time = total_time * 0.2;  // 前20%时间活跃
const size_t total_steps = static_cast<size_t>(total_time / period);
const size_t omega_active_steps = static_cast<size_t>(omega_active_time / period);
const size_t decay_start_step = omega_active_steps / 2 ;

double x = (*state)(0);
double y = (*state)(1);
double v_x = (*state)(2);
double v_y = (*state)(3);
double acc_x = (*state)(4);
double acc_y = (*state)(5);
double theta = (*state)(6);
double omega = (*state)(7);
...
for (std::size_t i = 0; i < total_steps; ++i) {
  ...
  (*state)(4) = acc_x;
  (*state)(5) = acc_y;
  (*state)(6) = std::remainder(theta, 2 * M_PI);  
  (*state)(7) = omega;  
  if (i >= decay_start_step ) {
    (*state)(7) *= 0.9;  
  } else if (i >= omega_active_steps) {  
    (*state)(7) = 0.0;
  }  
  ...
  const double current_theta = (*state)(6);
  const double cos_theta = std::cos(current_theta);
  const double sin_theta = std::sin(current_theta);
  
  // 全局速度和加速度
  v_x = (*state)(2) * cos_theta - (*state)(3) * sin_theta;
  v_y = (*state)(2) * sin_theta + (*state)(3) * cos_theta;
  acc_x = (*state)(4) * cos_theta - (*state)(5) * sin_theta;
  acc_y = (*state)(4) * sin_theta + (*state)(5) * cos_theta;
  x += v_x * period + 0.5 * acc_x * period * period;
  y += v_y * period + 0.5 * acc_y * period * period;
}
}
...
double AdjustSpeedByCurvature(const double speed, const double curvature) {
  ...
  return century::common::math::lerp(
