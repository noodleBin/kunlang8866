...
bool PredictorSubmodule::Proc(
  ...
  const century::common::Header& perception_header =
      perception_obstacles->header();
  const century::common::ErrorCode& perception_error_code =
      perception_obstacles->error_code();
  const century::cyber::Time& frame_start_time =
  ...
  const century::cyber::Time& end_time = Clock::Now();
