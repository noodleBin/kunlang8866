...
class MCloudTestServer:
    ...
    def _send(self, pkt: bytes, name: str) -> None:
        ...
        bcc = pkt[-1]
        self._log(
            f"[send] {name} cmd=0x{cmd:02X} rsp=0x{rsp:02X} enc=0x{enc:02X} "
            f"len={data_len} uid={unique} bcc=0x{bcc:02X}"
        )
        self._log(f"[send-full-hex] {pkt.hex()}")
        self._log(f"[send-data-hex] {data.hex()}")
