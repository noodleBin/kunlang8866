...
class MqttClient {
 ...
 private:
  ...
  // Watchdog: runs in a dedicated thread, monitors connected_ and triggers
  // mosquitto_reconnect() with exponential back-off whenever the link is down.
  void WatchdogLoop();
 ...
 private:
  ...
  // Shared by HandleConnect, HandleDisconnect, Disconnect and WatchdogLoop
  // to signal connection-state changes without busy-waiting.
