...
#include "modules/mcloud/proto/road_event_code_config.pb.h"
...
struct RoadEventCodeMaps {
  std::unordered_map<std::string, RoadEvent> monitor;
  std::unordered_map<int, RoadEvent> planning;
  std::unordered_map<int, RoadEvent> decision;
};

RoadEventCodeMaps g_road_event_code_maps;
...
uint64_t RoadEventKey(const RoadEvent& event) {
  return (static_cast<uint64_t>(event.level) << 32) | event.error_code;
}

bool ParseRoadEventUint32(const std::string& text, uint32_t* value) {
  if (nullptr == value || text.empty()) {
    return false;
  }
  ...
  try {
    raw_value = std::stoull(text, &parsed_size, 0);
  } catch (const std::exception&) {
    return false;
  }
  if (parsed_size != text.size() ||
      raw_value > std::numeric_limits<uint32_t>::max()) {
    return false;
  }
...
bool ParseRoadEventLevel(const std::string& level_text, uint8_t* level) {
  uint32_t raw_level = 0;
  if (nullptr == level||
      !ParseRoadEventUint32(level_text, &raw_level) ||
      raw_level > std::numeric_limits<uint8_t>::max()) {
    return false;
  }
  *level = static_cast<uint8_t>(raw_level);
  return true;
}

bool BuildRoadEventFromConfigItem(const RoadEventCodeItem& item,
                                  RoadEvent* event) {
  if (nullptr == event|| !item.has_code() || !item.has_level() ||
      !ParseRoadEventUint32(item.code(), &event->error_code) ||
      !ParseRoadEventLevel(item.level(), &event->level)) {
    return false;
  }
  return true;
}

void LoadMonitorRoadEventCodes(const RoadEventCodeConfig& config,
                               RoadEventCodeMaps* maps) {
  for (const auto& item : config.monitor()) {
    RoadEvent event;
    if (!item.has_name() || !BuildRoadEventFromConfigItem(item, &event)) {
      AERROR << "Invalid monitor road event config: " << item.DebugString();
      continue;
    }
    maps->monitor[item.name()] = event;
  }
}

void LoadPlanningRoadEventCodes(const RoadEventCodeConfig& config,
                                RoadEventCodeMaps* maps) {
  for (const auto& item : config.planning()) {
    century::common::ErrorCode status_code = century::common::OK;
    RoadEvent event;
    if (!item.has_name() || !item.has_code() ||
        !century::common::ErrorCode_Parse(item.name(), &status_code) ||
        !BuildRoadEventFromConfigItem(item, &event)) {
      AERROR << "Invalid planning road event config: " << item.DebugString();
      continue;
    }
    maps->planning[static_cast<int>(status_code)] = event;
  }
}

void LoadDecisionRoadEventCodes(const RoadEventCodeConfig& config,
                                RoadEventCodeMaps* maps) {
  for (const auto& item : config.decision()) {
    century::planning::StopReasonCode reason_code =
        century::planning::STOP_REASON_HEAD_VEHICLE;
    RoadEvent event;
    if (!item.has_name() || !item.has_code() ||
        !century::planning::StopReasonCode_Parse(item.name(), &reason_code) ||
        !BuildRoadEventFromConfigItem(item, &event)) {
      AERROR << "Invalid decision road event config: " << item.DebugString();
      continue;
    }
    maps->decision[static_cast<int>(reason_code)] = event;
  }
}

bool LoadRoadEventCodeConfig(const std::string& config_path) {
  RoadEventCodeConfig config;
  if (!cyber::common::GetProtoFromFile(config_path, &config)) {
    AERROR << "Failed to load road event code config: " << config_path;
    return false;
  }

  RoadEventCodeMaps maps;
  LoadMonitorRoadEventCodes(config, &maps);
  LoadPlanningRoadEventCodes(config, &maps);
  LoadDecisionRoadEventCodes(config, &maps);

  g_road_event_code_maps = std::move(maps);
  AINFO << "Loaded road event code config: " << config_path
        << ", monitor=" << g_road_event_code_maps.monitor.size()
        << ", planning=" << g_road_event_code_maps.planning.size()
        << ", decision=" << g_road_event_code_maps.decision.size();
  return true;
}
...
bool BuildMonitorRoadEvent(const century::monitor::FaultData& fault,
  ...
  const auto iter = g_road_event_code_maps.monitor.find(fault.code_msg());
  if (iter != g_road_event_code_maps.monitor.end()) {
    *event = iter->second;
    return true;
  }
...
bool BuildPlanningRoadEvent(const century::common::StatusPb& status,
                            RoadEvent* event) {
  if (nullptr == event||century::common::OK == status.error_code()) {
    return false;
  }

  const auto iter = g_road_event_code_maps.planning.find(
      static_cast<int>(status.error_code()));
  if (iter != g_road_event_code_maps.planning.end()) {
    *event = iter->second;
    return true;
  }
...
bool BuildDecisionStopRoadEvent(
  ...
  if (nullptr == event|| !stop_decision.has_reason_code()) {
    return false;
  }

  const auto iter = g_road_event_code_maps.decision.find(
      static_cast<int>(stop_decision.reason_code()));
  if (iter != g_road_event_code_maps.decision.end()) {
    *event = iter->second;
    return true;
  }
...
Cloud::Cloud()
    : buffer_(static_cast<size_t>(FLAGS_cloud_buffer_block_max_size) *
              static_cast<size_t>(FLAGS_cloud_circular_buffer_block_count)),
      node_(cyber::CreateNode("cloud")) {
  stop_flag_ = false;

  gflags::ReadFromFlagsFile(FLAGS_cloud_server_conf, "cloud", true);
  LoadRoadEventCodeConfig(FLAGS_road_event_code_config);
...
void Cloud::Report() {
  while (true) {
    std::this_thread::sleep_for(
        std::chrono::milliseconds(FLAGS_cloud_report_interval_ms));
    if (!client_->IsConnect()) {
      continue;
    }
    // get planning info
...
void Cloud::HandleDataTaskStatusCommand(const VehicleMessage& message,
  ...
  const bool need_stop = (0x01 == data[11]);
  const uint8_t area = data[23];

  century::planning::BlockingAreaType area_type;
  if (0x31 == area) {
    area_type = century::planning::BLOCKING_AREA_J1;
  } else if (0x32 == area || 0x33 == area) {
    area_type = century::planning::BLOCKING_AREA_J2J3;
  } else if (0x34 == area) {
    area_type = century::planning::BLOCKING_AREA_J4;
  } else {
    AWARN << "HandleDataTaskStatusCommand: unknown area 0x" << std::hex
          << static_cast<int>(area);
    return;
  }

  century::planning::BlockingAreaResponse merged;
  {
    std::lock_guard<std::mutex> lock(blocking_areas_mutex_);
    if (!need_stop) {
      accumulated_blocking_areas_.erase(area);
    } else {
      accumulated_blocking_areas_[area] = area_type;
    }

    const bool any_blocking = !accumulated_blocking_areas_.empty();
    merged.set_need_stop(any_blocking);
    for (const auto& kv : accumulated_blocking_areas_) {
      merged.add_blocking_areas()->set_blocking_area_response_type(kv.second);
    }
  }

  common::util::FillHeader("cloud", &merged);
  blocking_area_writer_->Write(merged);
}
...
std::vector<uint8_t> Cloud::GetTime() {
  ...
  std::tm now;
  if (nullptr == localtime_r(&current_time, &now)) {
...
void Cloud::AppendRoadEventSection(std::vector<uint8_t>& data_unit) {
  ...
  data_unit.emplace_back(0x00);
  std::unordered_set<uint64_t> appended_events;

  auto append_unique_event = [&data_unit, count_pos, &appended_events](
                                 const bool event_built,
                                 const RoadEvent& event) {
    if (!event_built || !appended_events.emplace(RoadEventKey(event)).second) {
      return false;
    }
    return AppendRoadEvent(data_unit, count_pos, event);
  };

  auto append_faults = [&data_unit, count_pos,
                        &append_unique_event](const auto& reader) {
    if (!reader || 0xFF == data_unit[count_pos]) {
      return;
    }
    reader->Observe();
    ...
    for (const auto& fault : monitor_msg->fault_data()) {
      ...
      RoadEvent event;
      append_unique_event(BuildMonitorRoadEvent(fault, &event), event);
    }
  };
  ...
  auto planning_msg = GetLatestPlanningMessage();
  if (!planning_msg || 0xFF == data_unit[count_pos]) {
    return;
  }
  ...
  if (planning_msg->has_header() && planning_msg->header().has_status()) {
    ...
    RoadEvent event;
    append_unique_event(BuildPlanningRoadEvent(status, &event), event);
  }

  if (0xFF == data_unit[count_pos] || !planning_msg->has_decision() ||
  ...
  if (main_decision.has_stop()) {
    RoadEvent event;
    append_unique_event(BuildDecisionStopRoadEvent(main_decision.stop(), &event),
                        event);
  }
}
