...
constexpr const char* kBarrierGateDataTopicWildcard = "barrier_gate/data/+";
...
CloudMqtt::CloudMqtt()
  ...
  blocking_area_writer_ =
      node_->CreateWriter<century::planning::BlockingAreaResponse>(
...
bool CloudMqtt::RegisterInternalTopicsLocked() {
  ...
  registered_topics_[kBoomBarrierTopicWildcard] =
      TopicEntry{[this](const std::string& recv_topic,
                 ...
                 MqttQos::kAtLeastOnce};
  registered_topics_[kBarrierGateDataTopicWildcard] =
      TopicEntry{[this](const std::string& recv_topic,
                        const nlohmann::json& payload) {
                   HandleBarrierGateData(recv_topic, payload);
                 },
                 MqttQos::kAtLeastOnce};
  return true;
}
...
void CloudMqtt::HandleBarrierGateData(const std::string& topic,
                                      const nlohmann::json& json_data) {
  if (!json_data.is_object()) {
    AWARN << "[CloudMqtt] barrier_gate/data payload must be JSON object, topic="
          << topic;
    return;
  }
  if (!json_data.contains("arm_status")) {
    AWARN << "[CloudMqtt] barrier_gate/data payload missing arm_status, topic="
          << topic;
    return;
  }

  const int arm_status = json_data.value("arm_status", 0);
  const bool forced_open = (1 == arm_status);

  {
    std::lock_guard<std::mutex> lock(barrier_mutex_);
    forced_passage_active_ = forced_open;
    forced_passage_last_seen_ = std::chrono::steady_clock::now();
    RecomputeBarrierStateLocked(forced_passage_last_seen_);
  }

  ADEBUG << "[CloudMqtt] barrier_gate/data topic=" << topic
         << " arm_status=" << arm_status << " forced_open=" << forced_open;
}

void CloudMqtt::WriteJ1BlockingAreaLocked(bool j1_e_open, bool j1_w_open) {
  if (j1_e_open == prev_j1_e_open_ && j1_w_open == prev_j1_w_open_) {
    return;
  }
  prev_j1_e_open_ = j1_e_open;
  prev_j1_w_open_ = j1_w_open;

  century::planning::BlockingAreaResponse base;
  if (blocking_area_reader_) {
    blocking_area_reader_->Observe();
    auto latest = blocking_area_reader_->GetLatestObserved();
    if (latest) {
      base = *latest;
    }
  }

  const bool j1_barrier_open = j1_e_open && j1_w_open;
  century::planning::BlockingAreaResponse new_message;

  for (const auto& area : base.blocking_areas()) {
    if (area.blocking_area_response_type() !=
        century::planning::BLOCKING_AREA_J1) {
      new_message.add_blocking_areas()->CopyFrom(area);
    }
  }

  if (!j1_barrier_open) {
    new_message.add_blocking_areas()->set_blocking_area_response_type(
        century::planning::BLOCKING_AREA_J1);
  }

  new_message.set_need_stop(new_message.blocking_areas_size() > 0);
  century::common::util::FillHeader("cloud_mqtt", &new_message);
  if (blocking_area_writer_) {
    blocking_area_writer_->Write(new_message);
  }
}
...
void CloudMqtt::RecomputeBarrierStateLocked(
  ...
  if (barrier_cmd && barrier_cmd->enabled() && barrier_cmd->has_header()) {
    ...
    if (elapsed <= 5.0) {
      ...
      WriteJ1BlockingAreaLocked(forced_open, forced_open);
      return;
    }
  }

  // barrier_gate/data forced passage override
  const auto forced_elapsed_sec =
      std::chrono::duration_cast<std::chrono::seconds>(
          now - forced_passage_last_seen_)
          .count();
  if (forced_elapsed_sec <= kBarrierOfflineTimeoutSec) {
    barrier_state_.set_east_south(forced_passage_active_);
    barrier_state_.set_west_south(forced_passage_active_);
    barrier_state_.set_east_north(forced_passage_active_);
    barrier_state_.set_west_north(forced_passage_active_);
    WriteJ1BlockingAreaLocked(forced_passage_active_, forced_passage_active_);
    return;
  }
  ...
  WriteJ1BlockingAreaLocked(j1_e_open, j1_w_open);
