...
class CloudMqtt final {
 ...
 private:
  ...
  void HandleBoomBarrierStatus(const std::string& topic,
                               const nlohmann::json& payload);
  void HandleBarrierGateData(const std::string& topic,
                             const nlohmann::json& payload);
  ...
  void WriteJ1BlockingAreaLocked(bool j1_e_open, bool j1_w_open);
  ...
  void OnBlockingAreaResponse(
      const std::shared_ptr<century::planning::BlockingAreaResponse>& msg);
  void MaybePublishCorrectedBlockingArea();
 ...
 private:
  ...
  std::shared_ptr<cyber::Writer<century::planning::BlockingAreaResponse>>
  ...
  std::shared_ptr<cyber::Reader<century::planning::BlockingAreaResponse>>
      blocking_area_reader_;
  ...
  mutable std::mutex cached_blocking_area_mutex_;
  century::planning::BlockingAreaResponse cached_blocking_area_response_;
  bool has_cached_blocking_area_ = false;
  std::mutex barrier_mutex_;
  century::planning::Barrier barrier_state_;
  bool j1_prev_initialized_ = false;
  bool prev_j1_e_open_ = false;
  bool prev_j1_w_open_ = false;
  std::unordered_map<std::string, bool> barrier_status_cache_;
  std::unordered_map<std::string, std::chrono::steady_clock::time_point>
      barrier_last_seen_;
  bool forced_passage_active_ = false;
  std::chrono::steady_clock::time_point forced_passage_last_seen_;
};
