...
std::shared_ptr<CyberRecordInfo> ChannelVerify::GetRecordInfo(
  ...
  std::shared_ptr<century::cyber::record::RecordReader> sp_reader =
      std::make_shared<century::cyber::record::RecordReader>(record_path);
  if (sp_reader == nullptr || !sp_reader->IsValid()) {
    AINFO << "open record [" << record_path << "] failed";
    return nullptr;
  }
  std::shared_ptr<century::cyber::record::RecordViewer> sp_viewer(
      new century::cyber::record::RecordViewer(sp_reader));
