...
std::ostream &operator<<(
    std::ostream &os,
    const ::google::protobuf::RepeatedPtrField<century::hdmap::Id> &ids) {
 ...
 public:
  ...
  int SLToPoint(LaneInfoConstPtr lane_ptr, const double s, const double l,
    ...
    *heading = lane_ptr->Heading(s);
    auto normal_vec =
        common::math::Vec2d::CreateUnitVec2d(*heading + M_PI * 0.5) * l;
  ...
  int LaneProjection(const century::common::math::Vec2d &vec2d,
...
int main(int argc, char *argv[]) {
  ...
  const std::string map_file = century::hdmap::BaseMapFile();
  bool valid_arg = false;

  century::hdmap::MapUtil map_util;
  ...
  if (!FLAGS_dump_txt_map.empty()) {
    century::hdmap::Map map;
    ACHECK(century::cyber::common::GetProtoFromFile(map_file, &map));
    ACHECK(century::cyber::common::SetProtoToASCIIFile(map, FLAGS_dump_txt_map));
    valid_arg = true;
  }
  if (!FLAGS_dump_bin_map.empty()) {
    ...
    ACHECK(century::cyber::common::GetProtoFromFile(map_file, &map));
    ACHECK(
        century::cyber::common::SetProtoToBinaryFile(map, FLAGS_dump_bin_map));
    valid_arg = true;
  }
  if (!valid_arg) {
