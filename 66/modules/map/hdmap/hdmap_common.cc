...
century::common::math::Polygon2d ConvertToPolygon2d(const Polygon &polygon) {
  ...
  while (points.size() >= 2 && points[0].DistanceTo(points.back()) <=
                                   century::common::math::kMathEpsilon) {
    points.pop_back();
  }
  return century::common::math::Polygon2d(points);
}

void SegmentsFromCurve(
    const Curve &curve,
    std::vector<century::common::math::LineSegment2d> *segments) {
...
double LaneInfo::Heading(const double s) const {
  ...
  size_t index =
      static_cast<size_t>(std::distance(accumulated_s_.begin(), iter));
  if (index == headings_.size()) {
    return headings_.back();
  }
  if (0UL == index || *iter - s <= common::math::kMathEpsilon) {
    return headings_[index];
  }
  return common::math::slerp(headings_[index - 1UL],
                             accumulated_s_[index - 1UL], headings_[index],
...
double LaneInfo::Curvature(const double s) const {
  ...
  return ((accumulated_s_[index] - accumulated_s_[index - 1] + kEpsilon) < 1e-9)
             ? 0
             : (headings_[index] - headings_[index - 1]) /
...
double LaneInfo::GetWidthFromSample(
  ...
  const double ratio =
      ((sample2.first - sample1.first) < 1e-9)
          ? 0
          : (sample2.first - s) / (sample2.first - sample1.first);
...
bool LaneInfo::IsOnLane(const century::common::math::Box2d &box) const {
...
PointENU LaneInfo::GetSmoothPoint(double s) const {
  ...
  if (delta_s < century::common::math::kMathEpsilon) {
...
void LaneInfo::CreateKDTree() {
  century::common::math::AABoxKDTreeParams params;
  ...
  for (size_t id = 0; id < segments_.size(); ++id) {
    ...
    segment_box_list_.emplace_back(
        century::common::math::AABox2d(segment.start(), segment.end()), this,
        &segment, id);
  }
  ...
  for (const auto &overlap_id : overlap_ids_) {
    ...
    for (const auto &object : overlap_ptr->overlap().object()) {
      ...
      if (object.has_signal_overlap_info()) {
        overlap_signal_ids_.push_back(object.id());
      }
    }
  }
}

ElectricFenceInfo::ElectricFenceInfo(const ElectricFence &electric_fence)
    : electric_fence_(electric_fence) {
  Init();
}

void ElectricFenceInfo::Init() {
  polygon_ = ConvertToPolygon2d(electric_fence_.polygon());
  CHECK_GT(polygon_.num_points(), 2);
}
...
ParkingLotInfo::ParkingLotInfo(const ParkingLot &parking_lot)
    : parking_lot_(parking_lot) {
  Init();
}

void ParkingLotInfo::Init() {
  polygon_ = ConvertToPolygon2d(parking_lot_.polygon());
  CHECK_GT(polygon_.num_points(), 2);
}
...
ObstacleInfo::ObstacleInfo(const Obstacle &obstacle) : obstacle_(obstacle) {
  Init();
}

void ObstacleInfo::Init() {
  polygon_ = ConvertToPolygon2d(obstacle_.polygon());
  CHECK_GT(polygon_.num_points(), 2);
}
