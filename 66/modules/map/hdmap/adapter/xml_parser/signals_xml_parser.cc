...
Status SignalsXmlParser::ParseTrafficLights(
  ...
  while (signal_node) {
    ...
    if (checker != tinyxml2::XML_SUCCESS) {
      ...
      return Status(century::common::ErrorCode::HDMAP_DATA_ERROR, err_msg);
    }
    ...
    if (object_type == "trafficLight") {
      ...
      auto parse_result = ParseTrafficLightAndStopLine(
          signal_node, object_id, &traffic_light, traffic_lights);
      if (!parse_result.ok()) {
        return parse_result;
      }
    }
...
Status SignalsXmlParser::ParseTrafficLightAndStopLine(
    const tinyxml2::XMLElement* signal_node, const std::string& object_id,
    PbSignal* traffic_light,
    std::vector<TrafficLightInternal>* traffic_lights) {
  int checker;
      ...
      while (sub_node) {
        ...
        checker = UtilXmlParser::QueryStringAttribute(*sub_node, "type",
                                                      &sub_signal_xml_type);
    checker +=
        UtilXmlParser::QueryStringAttribute(*sub_node, "id", &sub_signal_id);
        if (checker != tinyxml2::XML_SUCCESS) {
