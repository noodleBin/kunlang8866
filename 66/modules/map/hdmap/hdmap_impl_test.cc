...
TEST_F(HDMapImplTestSuite, GetRoads) {
  ...
  const century::hdmap::RoadSection& section = roads[0]->sections()[0];
  ...
  for (int i = 0; i < section.lane_id_size(); ++i) {
    const century::hdmap::Id& lane_id = section.lane_id(i);
...
// TEST_F(HDMapImplTestSuite, GetObstacleAreas) {
//   std::vector<ObstacleInfoConstPtr> obstacles;
//   century::common::PointENU point;
//   point.set_x(278160.74);
//   point.set_y(3482670.41);
//   point.set_z(0.0);

//   EXPECT_EQ(0, hdmap_impl_.GetObstacleAreas(point, 2, &obstacles));
//   EXPECT_EQ(1, obstacles.size());
//   EXPECT_EQ("-710342", obstacles[0]->id().id());
//   EXPECT_EQ(true, hdmap_impl_.IsInRodArea(point));
// }
}  // namespace hdmap
}  // namespace century
