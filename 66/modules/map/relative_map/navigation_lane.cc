...
using century::common::util::operator+;
...
void NavigationLane::GeneratePathOnPerception(
    const century::perception::LaneMarkers &lane_marker) {
  ...
  double left_width =
      perceived_left_width_ > 0.0 ? perceived_left_width_ : default_left_width_;
...
void NavigationLane::CreateNavigationPathList() {
...
bool NavigationLane::GeneratePath() {
  ...
  if (config_.lane_source() == NavigationLaneConfig::OFFLINE_GENERATED &&
    ...
    if (navigation_path_list_.empty()) {
      GeneratePathOnPerception(lane_marker);
      return true;
    }
...
void NavigationLane::CalculatePathWidth() {
    ...
    for (auto iter = navigation_path_list_.begin();
      ...
      double width =
          lateral_distance_sum / static_cast<double>(average_point_size) * 0.5;
...
void NavigationLane::EnuToFlu(const double enu_x, const double enu_y,
...
void NavigationLane::GenNaviPathLoop(const int start, const int end,
                                     const double ref_s_base,
                                     const double max_length,
                                     const common::Path &navigation_path,
                                     common::Path *path) {
        CHECK_NOTNULL(path);
    ...
    EnuToFlu(point->x(), point->y(), point->theta(), &flu_x, &flu_y,
             &flu_theta);
...
bool NavigationLane::ConvertNavigationLineToPath(const int line_index,
  ...
  if (FLAGS_enable_cyclic_rerouting &&
    ...
    if (item_iter != stitch_index_map_.end()) {
      ...
      double length = navigation_path.path_point(stitch_start_index).s() -
                      navigation_path.path_point(current_project_index).s();
      GenNaviPathLoop(std::max(0, current_project_index - 3),
                      stitch_start_index + 1, 0.0, length, navigation_path,
                      path);
      if (length > config_.max_len_from_navigation_line()) {
        return true;
      }
      GenNaviPathLoop(stitch_end_index, navigation_path.path_point_size(),
                      length, config_.max_len_from_navigation_line(),
  ...
  GenNaviPathLoop(
      std::max(0, current_project_index - 3), navigation_path.path_point_size(),
      0.0, config_.max_len_from_navigation_line(), navigation_path, path);
  return true;
}
...
ProjIndexPair NavigationLane::UpdateProjectionIndex(const common::Path &path,
  ...
  auto check_distance_func = [this, &path, &path_size](
    ...
    if (std::fabs(flu_x) < config_.max_distance_to_navigation_line() * 0.5 &&
...
void NavigationLane::ConvertLaneMarkerToPath(
  ...
  double path_c0 = (left_lane.c0_position() + right_lane.c0_position()) * 0.5;

  double path_c1 =
      (left_lane.c1_heading_angle() + right_lane.c1_heading_angle()) * 0.5;

  double path_c2 = (left_lane.c2_curvature() + right_lane.c2_curvature()) * 0.5;
...
void NavigationLane::ProcessNavigationPaths(
    const MapGenerationParam &map_config, hdmap::Map *const hdmap,
    google::protobuf::Map<std::string, NavigationPath> *const navigation_path)
    const {
  int fail_num = 0;
...
bool NavigationLane::CreateMap(const MapGenerationParam &map_config,
  ...
  ProcessNavigationPaths(map_config, hdmap, navigation_path);
  ...
  left_edge->set_type(century::hdmap::BoundaryEdge::LEFT_BOUNDARY);
  left_edge->mutable_curve()->CopyFrom(hdmap->lane(0).left_boundary().curve());

  auto *right_edge = outer_polygon->add_edge();
  right_edge->set_type(century::hdmap::BoundaryEdge::RIGHT_BOUNDARY);
