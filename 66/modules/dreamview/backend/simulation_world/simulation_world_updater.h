 ...
 */
 ...
 #include "modules/dreamview/backend/perception_obstacles/obstacles.h"
 ...
 class SimulationWorldUpdater {
  public:
   ...
   void ModifyConfig(const nlohmann::json &json);
  ...
  private:
   ...
   bool ConstructParkingRoutingTask(
       const nlohmann::json &json,
       century::task_manager::ParkingRoutingTask *parking_routing_task);
   ...
   bool ConstructDeadJunctionRoutingTask(
       const nlohmann::json &json,
       century::task_manager::DeadEndRoutingTask
   ...
   nlohmann::json CheckVersion(const std::string &file_path);
  ...
  bool SendRoutingRequestFromJson(const nlohmann::json& json);

  void SetDestinationTriggerPoint(
      const century::routing::RoutingRequest& routing_request);
  ...
  century::routing::TaskType StringToTaskType(const std::string& task_type_str);
  void RegisterMessageHandlers();
  void RegisterConnectionReadyHandlers();
  void RegisterMapDataHandlers();
  void RegisterRoutingAndTaskHandlers();
  void RegisterRoutingBasicHandlers();
  void RegisterTemporaryMoveHandlers();
  void RegisterRoutingTaskRequestHandlers();
  void RegisterSimulationWorldHandlers();
  void RegisterRecordHandlers();
  void RegisterControlAndConfigHandlers();
  void RegisterCameraHandlers();
  void BroadcastBarrierCommandStatus(bool enabled);
  ...
  WebSocketHandler *websocket_ = nullptr;
   ...
   century::routing::POI poi_;
 
   // default routings
   century::routing::POI default_routings_;
   century::routing::Landmark *default_routing_;
   ...
   std::shared_ptr<century::dreamview::ObstacleTask> obstacle_task_;
   std::thread task_thread_;//([&task]() { task.Start(); });
   
  nlohmann::json virtual_obstacle_;
  nlohmann::json dynamic_confs_;

  std::shared_ptr<cyber::Node> record_node_;
  std::shared_ptr<cyber::Writer<RecordCommand>> record_cmd_writer_;
  std::shared_ptr<cyber::Reader<RecordStatus>> record_status_reader_;
  std::mutex record_status_mutex_;
  std::condition_variable record_status_cv_;
  bool record_status_valid_ = false;
  int record_status_value_ = 0;
  std::string record_status_error_;
  uint64_t record_status_seq_ = 0;

  century::routing::RoutingRequest temporary_move_original_routing_request_;
  bool has_temporary_move_original_routing_request_ = false;
  bool last_barrier_command_active_ = false;
 };
