...
class HMIWorker {
 public:
  ...
  explicit HMIWorker(const std::shared_ptr<century::cyber::Node>& node);
  void Start();
  void Stop();
 ...
 private:
  ...
  bool ChangeDrivingMode(const century::canbus::Chassis::DrivingMode mode);
  ...
  std::shared_ptr<cyber::Writer<century::control::PadMessage>> pad_writer_;
  std::shared_ptr<cyber::Writer<century::audio::AudioEvent>> audio_event_writer_;
  std::shared_ptr<cyber::Writer<century::common::DriveEvent>>
      drive_event_writer_;
  std::shared_ptr<cyber::Writer<MapChangeCmd>> map_change_cmd_writer_;
};
