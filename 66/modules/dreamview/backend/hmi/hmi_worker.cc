...
DEFINE_string(hmi_modes_config_path, "/century/modules/dreamview/conf/hmi_modes",
              "HMI modes config path.");

DEFINE_string(maps_data_path, "/century/modules/map/data", "Maps data path.");

DEFINE_string(vehicles_config_path, "/century/modules/calibration/data",
              "Vehicles config path.");

DEFINE_double(status_publish_interval, 5, "HMI Status publish interval.");

DEFINE_string(current_mode_db_key, "/century/hmi/status:current_mode",
...
using century::control::DrivingAction;
using century::cyber::Clock;
using century::cyber::Node;
using century::cyber::proto::DagConfig;
using century::localization::LocalizationEstimate;
using century::monitor::ComponentStatus;
using century::monitor::SystemStatus;
...
void HMIWorker::Start() {
  ...
  RegisterStatusUpdateHandler(
      [this](const bool status_changed, HMIStatus* status) {
        century::common::util::FillHeader("HMI", status);
...
void HMIWorker::InitReadersAndWriters() {
  ...
  map_change_cmd_writer_ =
      node_->CreateWriter<MapChangeCmd>("/century/map_change_cmd");
  ...
  node_->CreateReader<SystemStatus>(
      ...
      [this](const std::shared_ptr<SystemStatus>& system_status) {
        ...
        const size_t new_fingerprint =
            century::common::util::MessageFingerprint(status_);
...
void HMIWorker::SubmitAudioEvent(const uint64_t event_time_ms,
  ...
  century::common::util::FillHeader("HMI", audio_event.get());
  ...
  audio_event->set_audio_type(
      static_cast<century::audio::AudioType>(audio_type));
  audio_event->set_moving_result(
      static_cast<century::audio::MovingResult>(moving_result));
  audio_event->set_audio_direction(
      static_cast<century::audio::AudioDirection>(audio_direction));
...
void HMIWorker::SubmitDriveEvent(const uint64_t event_time_ms,
  ...
  century::common::util::FillHeader("HMI", drive_event.get());
...
void HMIWorker::SensorCalibrationPreprocess(const std::string& task_type) {
  std::string start_command = absl::StrCat(
      "nohup bash /century/modules/tools/sensor_calibration/extract_data.sh -t ",
      task_type, " &");
...
void HMIWorker::VehicleCalibrationPreprocess() {
  std::string start_command = absl::StrCat(
      "nohup bash /century/modules/tools/vehicle_calibration/preprocess.sh "
      "--vehicle_type=\"",
...
void HMIWorker::ChangeMap(const std::string& map_name) {
  ...
  auto map_change_cmd = std::make_shared<MapChangeCmd>();
  map_change_cmd->set_map_dir(*map_dir);
  map_change_cmd_writer_->Write(map_change_cmd);

  System("pkill -9 -f autoset.sh");
  System("pkill -9 -f planning.dag");
  System("pkill -9 -f control.dag");
  System("pkill -9 -f routing.dag");
  System("pkill -9 -f mcloud.dag");
  System("pkill -9 -f monitor.dag");
  System("pkill -9 -f fas_aeb_backend.dag");
#endif
}
...
void HMIWorker::RestartCentury() {
  System("bash /century/stop.sh");
  System("bash /century/launch_century_system.sh");
}
