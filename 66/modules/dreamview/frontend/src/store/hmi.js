...
export default class HMI {
  ...
  @observable FASAEBStatus = true;

  @observable bckGroundMusicStatus = true;

  @observable barrierCommandEnabled = false;
  ...
  @observable blockObsId = null;

  @observable vehicleConfig = null;

  @observable allVehicleConfig = null;

  @observable curCarPosition = {};

  @observable currentPointCloudChannel = '';

  @observable showAutoNotification = false;

  @observable showAebNotification = false;

  @observable recordInfoStatus = false;

  @action updateRecordStatusInfo(status) {
    if (!!status !== this.recordInfoStatus) {
      this.recordInfoStatus = status ? true : false;
    }
  }

  @action changePointSize(event) {
    this.pointSize = event.target.value;
  }

  @action setBlockObstacleId(id) {
    this.blockObsId = id;
  }
  ...
  updateCurCarPosition(car) {
    this.curCarPosition.x = car.positionX;
    this.curCarPosition.y = car.positionY;
    this.curCarPosition.z = 0;
  }

  updateFASAEBStatus(status) {
    if (status !== 'undefined' && status !== this.FASAEBStatus) {
      this.FASAEBStatus = status;
    }
  }

  updateShowAutoNotification(show) {
    if (show !== 'undefined' && show !== this.showAutoNotification) {
      this.showAutoNotification = show;
    }
  }

  updateAebNotification(aebAction) {
    if (aebAction !== undefined && aebAction !== this.showAebNotification) {
      this.showAebNotification = aebAction;
    }
  }

  updateBckGroundMusicStatus(status) {
    if (status !== 'undefined' && status === 'true' && !this.bckGroundMusicStatus) {
      this.bckGroundMusicStatus = true;
    } else if (status !== 'undefined' && status === 'false' && this.bckGroundMusicStatus) {
      this.bckGroundMusicStatus = false;
    }
  }

  @action updateBarrierCommandEnabled(status) {
    this.barrierCommandEnabled = !!status;
  }

  @action update(world) {
    this.enableStartAuto = world.engageAdvice === 'READY_TO_ENGAGE';
    this.updateCurCarPosition(world.autoDrivingCar);
    this.updateFASAEBStatus(world.fasAebSwitch);
    this.updateShowAutoNotification(world.enableAutoDrive);
    this.updateBckGroundMusicStatus(world.backgroundMusicSwitch);
    this.updateAebNotification(world.aebAction);
  }
  ...
  @computed get allMonitoredComponentSuccess() {
    return (
      ...
      _.every(this.componentStatus.keys(), (key) => {
        return (
          key === 'Recorder' ||
          _.get(this.componentStatus.get(key), 'status') === 'OK'
        );
      })
    );
  }
    ...
    Object.keys(data)
      ...
      .forEach((scenarioName) => {
        ...
        const categoryStatus =
          this.dataCollectionUpdateStatus.get(scenarioName);
  ...
  @action updateVehicleConfig(data) {
    this.allVehicleConfig = data;
    const pattern = /speed/;
    this.vehicleConfig = Object.keys(data)
      .filter((key) => {
        if (pattern.test(key)) {
          if (key === 'planning_upper_speed_limit') {
            return true;
          }
          return false;
        }
