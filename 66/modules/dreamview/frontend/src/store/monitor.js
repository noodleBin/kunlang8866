...
export default class Monitor {
    ...
    @observable centuryVersion = 'unknown';

    @observable cpuInfo = 0;

    @observable memoryInfo = 0;

    @observable diskInfo = 0;

    @observable statusInfo = null;

    @observable allChannelsInfo = [];
    ...
    updateProjectVersion(data) {
      this.centuryVersion = data.version ? data.version : 'unknown';
    }
    ...
    updateSystemInfo(systemInfo) {
      ...
      this.cpuInfo = (systemInfo.data.cpu * 100).toFixed(2) + '%';
      this.memoryInfo = (systemInfo.data.memory * 100).toFixed(2) + '%';
      this.diskInfo = (systemInfo.data.disk * 100).toFixed(2) + '%';
      this.statusInfo = systemInfo.data.status.toUpperCase();
    }

    updateAllChannelsInfo(allChannelsInfo) {
      if (!allChannelsInfo.data) {
        return;
      }

      this.allChannelsInfo = allChannelsInfo.data.channel;
    }
