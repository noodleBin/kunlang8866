...
import { DEFAULT_SPEED_COLOR } from 'renderer/obstacles.js';
...
export default class Text3D {
  constructor() {
    ...
    // this.charMeshes = {};
    this.defaultCharMeshes = {};
    this.speedCharMeshes = {};
  ...
  composeText(text, color, textAlign) {
    ...
    let offsetSum = 0;
    const charMeshesname = color === DEFAULT_SPEED_COLOR ? 'speedCharMeshes'
      : 'defaultCharMeshes';
    ...
    for (let j = 0; j < charIndices.length; j++) {
      ...
      if (this[charMeshesname][idx] === undefined) {
        this[charMeshesname][idx] = [];
      }
      let mesh = this[charMeshesname][idx][pIdx];
      if (mesh === undefined) {
        if (this[charMeshesname][idx].length > 0) {
          mesh = this[charMeshesname][idx][0].clone();
        } else {
          const { charMesh, charWidth } = this.drawChar3D(text[j], color);
          mesh = charMesh;
          this.charWidths[idx] = isFinite(charWidth) ? charWidth : 0.2;
        }
        this[charMeshesname][idx].push(mesh);
      }
  ...
  drawChar3D(char, color = 0xF5F5F5, font = fonts.gentilis_bold, size = 0.6, height = 0) {
