...
export default class Map {
  ...
  addPolygonId(polygon, coordinates, scene) {
    const text = this.textRender.drawText(polygon.id.id, scene, colorMapping.WHITE);
    const points = _.get(polygon, 'polygon.point');
        ...
        switch (kind) {
          ...
          case 'junction':
            ...
            this.data[kind].push(Object.assign(newData[kind][i], {
              ...
              text: this.addPolygonId(newData[kind][i], coordinates, scene),
            }));
            break;
          case 'pncJunction':
            this.data[kind].push(Object.assign(newData[kind][i], {
          ...
          case 'parkingLot':
            this.data[kind].push(Object.assign(newData[kind][i], {
              drewObjects: this.addBorder(
                newData[kind][i], colorMapping.DEEPPINK, coordinates, scene,
              ),
            }));
            break;
          case 'electricFence':
            this.data[kind].push(Object.assign(newData[kind][i], {
              drewObjects: this.addBorder(
                newData[kind][i], colorMapping.DARKKHAKI, coordinates, scene,
              ),
              text: this.addPolygonId(newData[kind][i], coordinates, scene),
            }));
            break;
  ...
  shouldDrawTextOfThisElementKind(kind) {
    ...
    return STORE.options.showMapLaneId && ['parkingSpace', 'lane', 'electricFence', 'junction'].includes(kind);
  }
  ...
  calcParkingSpaceExtraInfo(parkingSpace, coordinates) {
    ...
    const routingRes = this.getRoutingPoint(successor, projectionPointOnLane,
      STORE.scenarioEditingManager.parkingRoutingDistanceThreshold,
