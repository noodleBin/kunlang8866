...
using ::century::localization::CorrectedImu;
...
bool DataParser::Init() {
  ...
  insx_writer_ = node_->CreateWriter<Insx>(FLAGS_insx_topic);
...
void DataParser::CheckInsStatus(::century::drivers::gnss::Ins *ins) {
  ...
  if (ins_status_record_ != static_cast<uint32_t>(ins->type()) ||
    ...
    switch (ins->type()) {
      case century::drivers::gnss::Ins::GOOD:
        ins_status_.set_type(century::drivers::gnss::InsStatus::GOOD);
        break;

      case century::drivers::gnss::Ins::CONVERGING:
        ins_status_.set_type(century::drivers::gnss::InsStatus::CONVERGING);
        break;

      case century::drivers::gnss::Ins::INVALID:
      default:
        ins_status_.set_type(century::drivers::gnss::InsStatus::INVALID);
        break;
    }
...
void DataParser::CheckGnssStatus(::century::drivers::gnss::Gnss *gnss) {
...
void DataParser::DispatchMessage(Parser::MessageType type, MessagePtr message) {
  switch (type) {
    case Parser::MessageType::GNSS:
      CheckGnssStatus(As<::century::drivers::gnss::Gnss>(message));
      break;
    ...
    //   CheckInsStatus(As<::century::drivers::gnss::Ins>(message));
    //   PublishCorrimu(message);
    //   PublishOdometry(message);
    //   break;

    case Parser::MessageType::INSX:
      PublishCorrimuX(message);
      PublishInsx(message);
      break;
...
void DataParser::PublishInsx(const MessagePtr message) {
  auto insx = std::make_shared<Insx>(*As<Insx>(message));
  common::util::FillHeader("gnss", insx.get());
  insx_writer_->Write(insx);
}
...
void DataParser::PublishOdometry(const MessagePtr message) {
  ...
  double unix_sec = century::drivers::util::gps2unix(ins->measurement_time());
...
void DataParser::PublishCorrimu(const MessagePtr message) {
  ...
  // double unix_sec = century::drivers::util::gps2unix(ins->measurement_time());
  imu->mutable_header()->set_timestamp_sec(ins->measurement_time());
...
void DataParser::PublishCorrimuX(const MessagePtr message) {
  Insx *insx = As<Insx>(message);
  auto imu = std::make_shared<CorrectedImu>();
  // double unix_sec = century::drivers::util::gps2unix(insx->measurement_time());
  imu->mutable_header()->set_timestamp_sec(insx->measurement_time());

  auto *imu_msg = imu->mutable_imu();
  imu_msg->mutable_linear_acceleration()->set_x(
      -insx->linear_acceleration().y());
  imu_msg->mutable_linear_acceleration()->set_y(insx->linear_acceleration().x());
  imu_msg->mutable_linear_acceleration()->set_z(insx->linear_acceleration().z());

  imu_msg->mutable_angular_velocity()->set_x(-insx->angular_velocity().y());
  imu_msg->mutable_angular_velocity()->set_y(insx->angular_velocity().x());
  imu_msg->mutable_angular_velocity()->set_z(insx->angular_velocity().z());

  imu_msg->mutable_euler_angles()->set_x(insx->euler_angles().x());
  imu_msg->mutable_euler_angles()->set_y(-insx->euler_angles().y());
  imu_msg->mutable_euler_angles()->set_z(insx->euler_angles().z() -
                                         90 * DEG_TO_RAD_LOCAL);
