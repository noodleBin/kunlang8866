...
class ContiRadarMessageManager
    : public century::drivers::canbus::MessageManager<ContiRadar> {
 public:
  explicit ContiRadarMessageManager(
      const std::shared_ptr<century::cyber::Writer<ContiRadar>> &writer);
  virtual ~ContiRadarMessageManager() {}
  void set_radar_conf(RadarConf radar_conf);
  century::drivers::canbus::ProtocolData<ContiRadar> *GetMutableProtocolDataById(
      const uint32_t message_id);
  void Parse(const uint32_t message_id, const uint8_t *data, int32_t length);
  void set_can_client(
      std::shared_ptr<century::drivers::canbus::CanClient> can_client);

 private:
  bool is_configured_ = false;
  RadarConfig200 radar_config_;
  std::shared_ptr<century::drivers::canbus::CanClient> can_client_;
  std::shared_ptr<century::cyber::Writer<ContiRadar>> conti_radar_writer_;
};
