...
/**
 * @namespace century::drivers::lidar
 *
 */
...
class LidarDriverFactory
    : public century::common::util::Factory<
          LidarParameter::LidarBrand, LidarDriver,
          LidarDriver* (*)(const std::shared_ptr<::century::cyber::Node>& node,
                           const century::drivers::lidar::config& config)> {
 public:
  LidarDriverFactory(const century::drivers::lidar::config& config);
  /**
   * @brief Register the lidar driver of all brands. This function call the
   *        Function century::common::util::Factory::Register() for all of the
   *        lidar.
   */
  ...
  std::unique_ptr<LidarDriver> CreateLidarDriver(
      ...
      const century::drivers::lidar::config& parameter);
