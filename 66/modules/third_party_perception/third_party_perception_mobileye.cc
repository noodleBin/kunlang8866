...
using century::drivers::DelphiESR;
using century::drivers::Mobileye;
using century::perception::PerceptionObstacles;

ThirdPartyPerceptionMobileye::ThirdPartyPerceptionMobileye(
    century::cyber::Node* const node)
    : ThirdPartyPerception(node) {
  mobileye_reader_ = node_->CreateReader<century::drivers::Mobileye>(
      FLAGS_mobileye_topic,
      [this](const std::shared_ptr<century::drivers::Mobileye>& message) {
        OnMobileye(*message.get());
      });
  delphi_esr_reader_ = node_->CreateReader<century::drivers::DelphiESR>(
      FLAGS_delphi_esr_topic,
      [this](const std::shared_ptr<century::drivers::DelphiESR>& message) {
        OnDelphiESR(*message.get());
      });

  conti_radar_reader_ = node_->CreateReader<century::drivers::ContiRadar>(
      FLAGS_conti_radar_topic,
      [this](const std::shared_ptr<century::drivers::ContiRadar>& message) {
