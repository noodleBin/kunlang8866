...
const std::map<HonkingScenario, std::tuple<uint8_t, double, double>> kHonkingMap = {
  {HonkingScenario::NO_HONKING, {0, 0, 0}},
...
constexpr uint32_t kElectricFenceCount = 12;
}  // namespace
...
Status ControlComponent::ProduceControlCommand(
  ...
  if (estop_) {
    ...
    if (estop_reason_.find("estop from planning") != std::string::npos) {
      ++estop_from_planning_count_;
      if (estop_from_planning_count_ >= kElectricFenceCount) {
        m_AEB_enable_b = true;
      }
    } else {
      estop_from_planning_count_ = 0;
    }
...
void ControlComponent::VoiceBroadcastCommand(
  ...
  switch (local_view_.trajectory().display_type()) {
    ...
    case DisplayType::DANGER_STAY_AWAY: {
      control_command->set_voice_broadcast_scenario(
          VOICE_BROADCAST_DANGER_STAY_AWAY);
      break;
    }
