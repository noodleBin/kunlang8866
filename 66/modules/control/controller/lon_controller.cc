...
constexpr double kDecel1Threshold = -1.0;
constexpr double kDecel2Threshold = -2.0;
constexpr double kDeltPathLength = -10.0;
constexpr double kPatchActiveSpeedThreshold = 3.0;
constexpr double kStopRequestSpeed = 0.1;
...
Status LonController::ComputeControlCommand(
 ...
 /**************calculate station and speed PID***** priprotiy speed or station regulation **********************/
  ...
  m_brake_request_ = ComputeBrakeCommand(debug->preview_speed_error(), acceleration_cmd, debug, m_vehicle_mass_);
  ...
  if (lon_controller_conf.enable_traj_jump_decel()) {
    DriveBrakeCal(acceleration_cmd, m_brake_request_, t_speedcmd_d_);
  }
...
void LonController::DriveBrakeCal(const double acceleration_cmd, double &BrakeRequest, double &speed_cmd) {
  static double slope_speed = m_lon_control_input_.m_vehicle_speed_;
  static double decel = 0.0;
  double traj_end_speed = 0.0;
  static bool brake = false;
  static double pre_path_remain = path_remain_;
  if (std::fabs(m_lon_control_input_.m_vehicle_speed_) > kPatchActiveSpeedThreshold &&
      path_remain_ - pre_path_remain < kDeltPathLength &&
      !brake) {
    brake = true;
    size_t traj_size = trajectory_message_->trajectory_point().size();
    traj_end_speed = traj_size > 0 ? trajectory_message_->trajectory_point(traj_size - 1).v() : 0.1;
    traj_end_speed = traj_end_speed > std::fabs(m_lon_control_input_.m_vehicle_speed_) ?
      std::fabs(m_lon_control_input_.m_vehicle_speed_) / 2 : traj_end_speed;
    decel = (std::pow(traj_end_speed, 2) - std::pow(slope_speed, 2)) / path_remain_ / 2;
    decel = std::clamp(decel,kDecel2Threshold,kDecel1Threshold);
  }
  if (brake) {
    slope_speed += slope_speed * decel * KSysTime;
    slope_speed = slope_speed < 0 ? kStopRequestSpeed : slope_speed;
    speed_cmd = slope_speed;
    brake = std::fabs(std::fabs(m_lon_control_input_.m_vehicle_speed_) - traj_end_speed) < 0.27 ? false : brake;
  } else {
    slope_speed = std::fabs(m_lon_control_input_.m_vehicle_speed_);
  }
  pre_path_remain = path_remain_;
  AINFO << "brake: " << brake;
  AINFO << "decel: " << decel;
  AINFO << "m_vehicle_speed_: " << std::fabs(m_lon_control_input_.m_vehicle_speed_);
  AINFO << "traj_end_speed: " << traj_end_speed;
  AINFO << "slope_speed: " << slope_speed;  
}
