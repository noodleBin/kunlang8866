...
bool ControlTestBase::test_control() {
  ...
  if (!FLAGS_test_pad_file.empty()) {
    ...
    control_.OnPad(std::make_shared<century::control::PadMessage>(pad_message));
  }
  ...
  if (!FLAGS_test_planning_file.empty()) {
    ...
    control_.OnPlanning(
        std::make_shared<century::planning::ADCTrajectory>(trajectory));
  }
  ...
  if (!FLAGS_test_chassis_file.empty()) {
    ...
    control_.OnChassis(std::make_shared<century::canbus::Chassis>(chassis));
  }
...
void ControlTestBase::SetUpTestCase() {
  FLAGS_control_conf_file =
      "/century/modules/control/testdata/conf/control_conf.pb.txt";
