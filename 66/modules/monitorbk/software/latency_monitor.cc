...
using century::common::LatencyRecordMap;
using century::common::LatencyReport;
using century::common::LatencyStat;
using century::common::LatencyTrack;
...
void LatencyMonitor::UpdateStat(
  ...
  if (!records->latency_records().empty()) {
    ...
    if (end_time > begin_time) {
      double timeSecond =
          century::cyber::Time(end_time - begin_time).ToSecond();
      freq_map_[module_name] =
          (0.0 == timeSecond) ? 0.0
                              : records->latency_records().size() / timeSecond;
    }
  }
}
...
void LatencyMonitor::PublishLatencyReport() {
  ...
  century::common::util::FillHeader("LatencyReport", &latency_report_);
  AggregateLatency();
