...
using century::dreamview::HMIWorker;

MonitorManager::MonitorManager()
    : hmi_config_(HMIWorker::LoadConfig()),
      log_buffer_(century::common::monitor::MonitorMessageItem::MONITOR) {}

void MonitorManager::Init(const std::shared_ptr<century::cyber::Node>& node) {
  node_ = node;
...
bool MonitorManager::StartFrame(const double current_time) {
  ...
  static auto hmi_status_reader =
      CreateReader<century::dreamview::HMIStatus>(FLAGS_hmi_status_topic);
