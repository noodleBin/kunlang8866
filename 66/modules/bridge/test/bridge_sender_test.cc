...
bool send(const std::string &remote_ip, uint16_t remote_port, uint32_t count) {
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    auto pb_msg = std::make_shared<century::canbus::Chassis>();
    ...
    pb_msg->set_driving_mode(century::canbus::Chassis::COMPLETE_MANUAL);
    pb_msg->set_error_code(century::canbus::Chassis::NO_ERROR);
    pb_msg->set_gear_location(century::canbus::Chassis::GEAR_NEUTRAL);
    ...
    century::bridge::BridgeProtoSerializedBuf<century::canbus::Chassis> proto_buf;
