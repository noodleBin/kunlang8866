...
GrpcServerImpl::GrpcServerImpl()
    : node_(::century::cyber::CreateNode("v2x_grpc_server")) {
  CHECK(!!node_);
  ...
  }
  {
    ...
    if (obu_header->has_timestamp_sec()) {
      ...
      AINFO << "SVB Timestamp: " << std::setiosflags(std::ios::fixed)
            << std::setprecision(11) << ::century::cyber::Time::Now().ToSecond();
      obu_header->set_timestamp_sec(::century::cyber::Time::Now().ToSecond());
    }
    rsi_refresh_ = true;
  }
...
grpc::Status GrpcServerImpl::SendPerceptionObstacles(
    grpc::ServerContext *, const century::v2x::V2XObstacles *request,
    StatusResponse *response) {
...
grpc::Status GrpcServerImpl::SendObuAlarm(
    grpc::ServerContext *context, const ::century::v2x::ObuAlarm *request,
    StatusResponse *response) {
  ADEBUG << "Received SendOBUAlarm request from client!";
  if (request->error_num() != ::century::v2x::ErrorCode::OBUID) {
  } else {
...
void GrpcServerImpl::GetMsgFromGrpc(
    std::shared_ptr<::century::v2x::V2XObstacles> *ptr) {
  if (nullptr == ptr) {
    return;
  }
  ...
  obs_refresh_ = false;
  *ptr = std::make_shared<::century::v2x::V2XObstacles>();
  if (nullptr == *ptr) {
    return;
  }
