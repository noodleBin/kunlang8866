...
float GetSystemMemoryUsage() {
  ...
  return (0 == total_memory) ? 0.f : \
    100.f * (static_cast<float>(used_memory) / total_memory);
}
...
float GetSystemDiskload(const std::string& device_name) {
  ...
  if (0 == tmp_prev_disk_stats || 0 == FLAGS_resource_monitor_interval) {
    return 0.f;
  }
  return 100.f *
...
void ResourceMonitor::UpdateStatus(
    const century::dreamview::ResourceMonitorConfig& config,
