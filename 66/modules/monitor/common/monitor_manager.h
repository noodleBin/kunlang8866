...
/**
 * @namespace century::monitor
 * @brief century::monitor
 */
namespace century {
namespace monitor {
...
class MonitorManager {
 public:
  void Init(const std::shared_ptr<century::cyber::Node>& node);
  ...
  // Getters.
  const century::dreamview::HMIMode& GetHMIMode() const { return mode_config_; }
  bool IsInAutonomousMode() const { return in_autonomous_driving_; }
  SystemStatus* GetStatus() { return &status_; }
  century::common::monitor::MonitorLogBuffer& LogBuffer() {
    return log_buffer_;
  }
 ...
 private:
  ...
  const century::dreamview::HMIConfig hmi_config_;
  century::dreamview::HMIMode mode_config_;
  bool in_autonomous_driving_ = false;
  bool CheckAutonomousDriving(const double current_time);

  century::common::monitor::MonitorLogBuffer log_buffer_;
