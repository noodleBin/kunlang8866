...
class ShmTransmitter : public Transmitter<M> {
 public:
  ...
  bool TransmitSerialized(const std::string& payload,
                          const MessageInfo& msg_info);
  bool AcquireWritableBlock(std::size_t msg_size, WritableBlock* wb);
  void ReleaseWritableBlock(const WritableBlock& wb);
  bool CommitWritableBlock(const WritableBlock& wb, std::size_t msg_size,
                           const MessageInfo& msg_info);
...
template <typename M>
bool ShmTransmitter<M>::TransmitSerialized(const std::string& payload,
                                           const MessageInfo& msg_info) {
  if (!this->enabled_) {
    ADEBUG << "not enable.";
    return false;
  ...
  WritableBlock wb;
  const std::size_t msg_size = payload.size();
  if (!segment_->AcquireBlockToWrite(msg_size, &wb)) {
    AERROR << "acquire block failed.";
    return false;
  }

  std::memcpy(wb.buf, payload.data(), msg_size);
...
template <typename M>
bool ShmTransmitter<M>::AcquireWritableBlock(std::size_t msg_size,
                                             WritableBlock* wb) {
  if (!this->enabled_ || wb == nullptr) {
    return false;
  }
  return segment_->AcquireBlockToWrite(msg_size, wb);
}

template <typename M>
void ShmTransmitter<M>::ReleaseWritableBlock(const WritableBlock& wb) {
  if (!this->enabled_) {
    return;
  }
  segment_->ReleaseWrittenBlock(wb);
...
template <typename M>
bool ShmTransmitter<M>::CommitWritableBlock(const WritableBlock& wb,
                                            std::size_t msg_size,
                                            const MessageInfo& msg_info) {
  if (!this->enabled_) {
    return false;
