...
if [ "$1" == "rebuild" ] ; then
  ...
  docker exec -u $USER -t century_dev_$USER bazel clean --expunge || true
  docker exec -u $USER -t century_dev_$USER /century/century.sh release --clean --resolve
fi
...
# Expects that the Century was already built in century_dev_$USER
if ! [ -f output/syspkgs.txt ] ; then
  echo "ERROR: this directory doesn't have output/syspkgs.txt"
  echo "       make sure century was built with \"/century/century.sh release --resolve\""
