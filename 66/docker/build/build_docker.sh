...
CENTURY_REPO="centuryauto/century"
UBUNTU_LTS="18.04"
...
function check_experimental_docker() {
    ...
    if [ "${enabled}" != "true" ]; then
        echo "Experimental features should be enabled to run Century docker build."
...
function determine_images_in_out_x86_64() {
    ...
    local base_image="${CENTURY_REPO}:cuda${CUDA_LITE}-cudnn${cudnn_ver}-trt${trt_ver}-devel-${UBUNTU_LTS}-x86_64"
    if [[ "${stage}" == "base" ]]; then
        ...
        IMAGE_IN="${base_image}"
        if [[ "${dist}" == "stable" ]]; then
            IMAGE_OUT="${CENTURY_REPO}:cyber-x86_64-${UBUNTU_LTS}-${timestamp}"
        else
            IMAGE_OUT="${CENTURY_REPO}:cyber-x86_64-${UBUNTU_LTS}-testing-${timestamp}"
        fi
    elif [[ "${stage}" == "dev" ]]; then
        if [[ "${dist}" == "stable" ]]; then
            IMAGE_IN="${CENTURY_REPO}:cyber-x86_64-${UBUNTU_LTS}-${PREV_IMAGE_TIMESTAMP}"
            IMAGE_OUT="${CENTURY_REPO}:dev-x86_64-${UBUNTU_LTS}-${timestamp}"
        else
            IMAGE_IN="${CENTURY_REPO}:cyber-x86_64-${UBUNTU_LTS}-testing-${PREV_IMAGE_TIMESTAMP}"
            IMAGE_OUT="${CENTURY_REPO}:dev-x86_64-${UBUNTU_LTS}-testing-${timestamp}"
        fi
    elif [[ "${stage}" == "runtime" ]]; then
        if [[ "${dist}" == "stable" ]]; then
            IMAGE_IN="nvidia/cuda:${CUDA_LITE}-runtime-ubuntu${UBUNTU_LTS}"
            DEV_IMAGE_IN="${CENTURY_REPO}:dev-x86_64-${UBUNTU_LTS}-${PREV_IMAGE_TIMESTAMP}"
            IMAGE_OUT="${CENTURY_REPO}:runtime-x86_64-${UBUNTU_LTS}-${timestamp}"
        else
            echo "Runtime Docker for Century Testing not ready. Exiting..."
            exit 1
        fi
    else
...
function determine_images_in_out_aarch64() {
    ...
    if [[ "${stage}" == "base" ]]; then
        ...
        IMAGE_OUT="${CENTURY_REPO}:${BASE_FMT}-${timestamp}"
    elif [[ "${stage}" == "cyber" ]]; then
        IMAGE_IN="${CENTURY_REPO}:${BASE_FMT}-${PREV_IMAGE_TIMESTAMP}"
        IMAGE_OUT="${CENTURY_REPO}:${CYBER_FMT}-${timestamp}"
    elif [[ "${stage}" == "dev" ]]; then
        IMAGE_IN="${CENTURY_REPO}:${CYBER_FMT}-${PREV_IMAGE_TIMESTAMP}"
        IMAGE_OUT="${CENTURY_REPO}:dev-aarch64-${UBUNTU_LTS}-${timestamp}"
    elif [[ "${stage}" == "runtime" ]]; then
...
function check_arguments() {
    ...
    if [[ "${TARGET_DIST}" == "stable" ]]; then
        ...
        echo "Unknown CENTURY_DIST: ${TARGET_DIST}. Exit..."
        exit 1
    fi
...
function docker_build_preview() {
    ...
    echo "|  INSTALL_MODE=${INSTALL_MODE}, GEOLOC=${TARGET_GEOLOC}, CENTURY_DIST=${TARGET_DIST}"
    echo "=====.=====.=====.=====.=====.=====.=====.=====.=====.=====.=====.=====.====="
}
...
function docker_build_run() {
    ...
    if [[ "${TARGET_STAGE}" == "base" ]]; then
        ...
        build_args="${build_args} --build-arg CENTURY_DIST=${TARGET_DIST}"
