# Century Docker Image Build Process

## Introduction

As you may already know, Century runs inside Docker container. There are
basically two types of Century Docker images: CyberRT and Dev. CyberRT images
were for developers who want to play with the CyberRT framework only, while Dev
images were used to build and run the whole Century project.
...
## Build Century Dev Image

Run the following command to build Century Dev image:
...
Successfully tagged centuryauto/century:dev-x86_64-18.04-20200824_0339
Built new image centuryauto/century:dev-x86_64-18.04-20200824_0339
```

## Build Century Runtime Docker Image

Century Runtime Docker was used in combination with Release Build output for easy
deployment. You can run the following commands to build Century Runtime Docker
image on your own.

```
# Generate required APT packages
./century.sh release -c -r

cd docker/build
# Copy the generated package list for docker build
cp /century/output/syspkgs.txt .

cp runtime.x86_64.dockerfile.sample runtime.x86_64.dockerfile
bash build_docker.sh -f runtime.x86_64.dockerfile
```

> Note: Century Runtime Docker supports x86_64 only as Release Build was not
