...
CYBER_CONTAINER="century_cyber_${USER}"
...
OPTIONS:
    ...
    stop [-f|--force]      Stop all running Century containers. Use "-f" to force removal.
EOF
}
    ...
    while [[ $# -gt 0 ]]; do
        ...
        case "${opt}" in
            ...
            stop)
                ...
                stop_all_century_containers "${force}"
                exit 0
                ;;
            *)
...
# if [ ! -e /century ]; then
#    sudo ln -sf "${CENTURY_ROOT_DIR}" /century
# fi
# if [ -e /proc/sys/kernel ]; then
#    echo "/century/data/core/core_%e.%p" | sudo tee /proc/sys/kernel/core_pattern > /dev/null
# fi
