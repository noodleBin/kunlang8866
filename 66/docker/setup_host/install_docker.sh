...
function install_filesystem_support() {
  ...
  if [ "$kernel_version" == "4.4.32-century-2-RT" ]; then
    info "Century realtime kernel ${kernel_version} found."
...
function setup_docker_repo_and_install() {
  local issues_link="https://github.com/CenturyAuto/century/issues"
  local arch_alias=
  if [ "${ARCH}" == "x86_64" ]; then
    arch_alias="amd64"
